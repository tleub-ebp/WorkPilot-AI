import { Globe, MessageSquare, Monitor } from "lucide-react";
	...
	const integrations = [
		{
			icon: Globe,
			name: "Github",
			desc: "Enables automated workflows, code synchronization",
			enabled: github,
			toggle: setGithub,
			color: "#333",
		},
		{
			icon: MessageSquare,
			name: "Slack",
			desc: "Used as a main source of communication",
			enabled: slack,
			toggle: setSlack,
			color: "#E91E63",
		},
		{
			icon: Monitor,
			name: "Google meet",
			desc: "Used for all types of calls",
			enabled: meet,
			toggle: setMeet,
			color: "#00897B",
		},
				...
				{integrations.map((int) => (
					<div key={int.name} className="flex items-center gap-3">
						...
						<div className="flex-1 min-w-0">
							...
							<p className="text-body-small text-(--color-text-secondary) truncate">
								{int.desc}
							</p>
						</div>
