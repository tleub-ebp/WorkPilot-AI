name: ebp-business-comms
description: Ressources pour rédiger des communications business EBP (Business Planning) en utilisant les formats standards de l'entreprise. Utiliser pour rapports de projet, newsletters business, FAQs, rapports de statut, mises à jour leadership, etc.
license: Complete terms in LICENSE.txt
---

## Quand utiliser ce skill

Pour rédiger des communications business EBP, utiliser ce skill pour:
- Rapports de projet et statuts
- Newsletters business internes
- FAQs business et techniques
- Rapports de performance KPI
- Mises à jour leadership
- Communications de projet
- Rapports d'incident business
- Plans d'action business

## Comment utiliser ce skill

Pour rédiger toute communication business:

1. **Identifier le type de communication** depuis la demande
2. **Charger le fichier guideline approprié** depuis le répertoire `examples/`:
    - `examples/project-reports.md` - Pour rapports de projet et statuts
    - `examples/business-newsletter.md` - Pour newsletters business internes
    - `examples/business-faq.md` - Pour FAQs business et techniques
    - `examples/kpi-reports.md` - Pour rapports de performance et KPIs
    - `examples/leadership-updates.md` - Pour mises à jour leadership
    - `examples/general-business-comms.md` - Pour tout autre type de communication
3. **Suivre les instructions spécifiques** dans ce fichier pour le formatage, ton, et contenu

Si le type de communication ne correspond à aucune guideline existante, demander clarification ou plus de contexte sur le format désiré.

## Standards EBP

### Structure des Rapports

**Format Standard EBP:**
1. **En-tête**: Logo EBP, date, titre du rapport
2. **Résumé Exécutif**: Points clés en 3-4 lignes
3. **KPIs Principaux**: Métriques essentielles avec graphiques
4. **Analyse Détaillée**: Sections par domaine
5. **Plans d'Action**: Prochaines étapes et responsabilités
6. **Risques et Mitigations**: Identification et gestion des risques

### Ton et Style

- **Professionnel**: Langage formel mais accessible
- **Orienté action**: Focus sur solutions et prochaines étapes
- **Basé sur données**: Utilisation de métriques et faits concrets
- **Concis**: Information dense mais facile à digérer

### Templates Disponibles

#### Rapport de Projet Hebdomadaire
- Progression cette semaine
- Blocages et défis
- Plans semaine prochaine
- Besoins de support

#### Newsletter Business Mensuelle
- Highlights du mois
- Performance KPIs
- Initiatives en cours
- Annonces importantes

#### FAQ Technique Business
- Questions fréquentes utilisateurs
- Réponses détaillées avec exemples
- Liens vers documentation
- Contacts support

## Mots-clés
rapports EBP, communications business, newsletter business, KPIs, rapports de projet, mises à jour business, FAQs business, communications internes EBP
