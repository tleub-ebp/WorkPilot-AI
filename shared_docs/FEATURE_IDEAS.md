...
- `library.py` — `TemplateLibrary` : gestion des templates built-in + custom (persistance dans `.auto-claude/spec_templates.json`)
...
- `cache.py` — `DependencyGraphCache` : cache JSON dans `.auto-claude/dependency_graph.json` avec fingerprint basé sur les mtimes des fichiers source
