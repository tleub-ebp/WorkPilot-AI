...
from ..core.convention_integration import get_convention_integration, initialize_convention_system
...
@conventions.command()
@click.option('--project-root', default='.', help='Project root directory')
@click.option('--auto-apply', is_flag=True, help='Auto-apply high confidence evolutions')
    ...
    for i, evolution in enumerate(pending_evolutions, 1):
        ...
        if auto_apply and evolution.confidence_score >= 0.9:
            if learning_loop.apply_evolution(evolution.evolution_id):
                click.echo(f"   ✅ Auto-applied!")
            else:
                click.echo(f"   ❌ Failed to apply")
        elif not auto_apply:
            if click.confirm(f"   🔄 Apply this evolution?"):
                if learning_loop.apply_evolution(evolution.evolution_id):
                    click.echo(f"   ✅ Applied!")
                else:
                    click.echo(f"   ❌ Failed to apply")
            elif click.confirm(f"   🗑️  Reject this evolution?"):
                learning_loop.reject_evolution(evolution.evolution_id)
                click.echo(f"   🗑️  Rejected")
...
def patterns(project_root: str, output: str):
    ...
    if output == 'json':
        ...
        for ptype, patts in pattern_types.items():
            ...
            for pattern in sorted(patts, key=lambda p: p.confidence_score, reverse=True):
                confidence_emoji = "🔥" if pattern.confidence_score >= 0.9 else "⭐" if pattern.confidence_score >= 0.7 else "💡"
                click.echo(f"  {confidence_emoji} {pattern.description}")
                click.echo(f"     📊 Success: {pattern.success_rate:.1%} | 🔄 Frequency: {pattern.frequency} | 🎯 Confidence: {pattern.confidence_score:.1%}")
...
def check_file(project_root: str, file_path: str):
    ...
    if violations:
        ...
        for violation in violations:
            ...
            click.echo(f"  {emoji} {violation.rule_type}{line_info}: {violation.message}")
            if violation.suggestion:
                click.echo(f"    💡 {violation.suggestion}")
            if violation.auto_fixable:
                click.echo(f"    🔧 Auto-fixable")
    else:
        click.echo("✅ No convention violations found!")
