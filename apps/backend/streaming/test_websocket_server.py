...
class TestStreamingWebSocketServer:
    ...
    @pytest.mark.asyncio
    async def test_handle_client_with_session_in_path(self, websocket_server, mock_websocket):
        """Test handling a client connection."""
        # Mock the websocket to simulate message reception
        mock_websocket.__aiter__ = AsyncMock()
        mock_websocket.__aiter__.return_value = json.dumps({"type": "test", "data": "test"})
    ...
    async def test_handle_init_session_message(self, websocket_server, mock_websocket):
        ...
        # Start session first
        await websocket_server.streaming_manager.start_session(session_id, {"task": "test"})
        ...
        await websocket_server._handle_message(session_id, mock_websocket, json.dumps(init_message))
    ...
    async def test_handle_chat_message(self, websocket_server, mock_websocket):
        ...
        await websocket_server._handle_message(session_id, mock_websocket, json.dumps(chat_message))
        ...
        assert broadcast["data"]["message"] == "Hello, how are you implementing this feature?"
    ...
    async def test_handle_control_message_pause(self, websocket_server, mock_websocket):
        ...
        await websocket_server._handle_message(session_id, mock_websocket, json.dumps(control_message))
    ...
    async def test_handle_unknown_message_type(self, websocket_server, mock_websocket):
        ...
        await websocket_server._handle_message(session_id, mock_websocket, json.dumps(unknown_message))
...
class TestWebSocketIntegration:
    ...
    async def test_full_client_session(self):
        ...
        server = StreamingWebSocketServer(host="localhost", port=8766)  # Different port for testing
