...
class Build(Base):
    ...
    # Relationships
    phases = relationship("BuildPhase", back_populates="build", cascade="all, delete-orphan")
    token_usage = relationship("TokenUsage", back_populates="build", cascade="all, delete-orphan")
    qa_results = relationship("QAResult", back_populates="build", cascade="all, delete-orphan")
    errors = relationship("BuildError", back_populates="build", cascade="all, delete-orphan")
...
class AgentPerformance(Base):
    ...
    # Indexes
    __table_args__ = (
        Index("idx_agent_performance_type_period", "agent_type", "period_type", "period_start"),
        Index("idx_agent_performance_provider_model", "llm_provider", "llm_model"),
        UniqueConstraint("agent_type", "llm_provider", "llm_model", "period_start", "period_type", 
