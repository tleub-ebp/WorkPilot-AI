...
class AnalyticsCollector:
    ...
    @contextmanager
    def build_session(self, spec_id: str, spec_name: Optional[str] = None, 
    ...
    def end_phase(self, success: bool = True, error_message: Optional[str] = None):
        """
        ...
        try:
            # Update phase record
            phase = db.query(BuildPhase).filter(
    ...
    def record_token_usage(self, input_tokens: int, output_tokens: int, 
                          cost_usd: float, operation_type: str,
                          llm_provider: Optional[str] = None,
                          llm_model: Optional[str] = None):
        """
    ...
    def record_qa_result(self, iteration: int, tests_run: int, tests_passed: int,
                        tests_failed: int, coverage_percentage: float,
                        quality_score: float, security_issues_found: int,
                        security_issues_fixed: int, qa_type: str,
    ...
    def _finalize_build(self, db: Session, build_id: str):
        ...
        if build.completed_at and build.started_at:
            build.total_duration_seconds = (build.completed_at - build.started_at).total_seconds()
        ...
        if qa_results:
            build.qa_success_rate = sum(1 for qa in qa_results if qa.success) / len(qa_results) * 100
