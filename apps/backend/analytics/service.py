...
from .collector import get_analytics_collector, track_phase, track_build
...
class AnalyticsService:
    ...
    def get_build_statistics(self, build_id: str) -> Optional[Dict[str, Any]]:
        """
    ...
    def cleanup_old_data(self, days_to_keep: int = 90):
        ...
        try:
            ...
            for build in old_builds:
                ...
                db.query(TokenUsage).filter(TokenUsage.build_id == build.build_id).delete()
                db.query(QAResult).filter(QAResult.build_id == build.build_id).delete()
                db.query(BuildError).filter(BuildError.build_id == build.build_id).delete()
                db.query(BuildPhase).filter(BuildPhase.build_id == build.build_id).delete()
