...
def get_anthropic_usage_metrics() -> dict[str, Any]:
    ...
    try:
        ...
        usage_result = subprocess.run(
            ["claude", "usage"], capture_output=True, text=True, timeout=15
        )
            ...
            if parsed_usage:
                ...
                return {
                    ...
                    "usage": {
                        ...
                        "sessionResetTime": parsed_usage.get(
                            "session_reset_time", "N/A"
                        ),
                        ...
                        "needsReauthentication": parsed_usage.get(
                            "needs_reauthentication", False
                        ),
                        "profile_info": {
                            "email": auth_data.get("email", ""),
                            "subscription": auth_data.get(
...
def parse_claude_usage_output(output: str) -> dict[str, Any] | None:
    """
    ...
    try:
        ...
        for line in lines:
            ...
            session_match = re.search(
                r"Session usage:\s*(\d+)%\s*\(resets in\s*([^)]+)\)",
                line,
                re.IGNORECASE,
            )
            if session_match:
                usage_data["session_percent"] = int(session_match.group(1))
                usage_data["session_reset_time"] = (
                    f"resets in {session_match.group(2).strip()}"
                )
                continue

            # Parser weekly usage
            weekly_match = re.search(
                r"Weekly usage:\s*(\d+)%\s*\(resets in\s*([^)]+)\)", line, re.IGNORECASE
            )
            if weekly_match:
                usage_data["weekly_percent"] = int(weekly_match.group(1))
                usage_data["weekly_reset_time"] = (
                    f"resets in {weekly_match.group(2).strip()}"
                )
                continue
