...
class CompositeSkillExecutor:
    ...
    def execute(
        self, composite_skill: CompositeSkill, context: dict | None = None
    ) -> dict[str, Any]:
        ...
        try:
            ...
            if len(composite_skill.sub_skills) > 3:
                investigation_result = self._use_subagent(
                    "investigator", composite_skill, optimized_context
                )
            ...
            if composite_skill.composition_type == CompositionType.SEQUENTIAL:
                ...
                raise ValueError(
                    f"Unsupported composition type: {composite_skill.composition_type}"
                )
            ...
            execution_record = {
                ...
                "success": all(
                    r.status == ExecutionStatus.COMPLETED for r in results.values()
                ),
                "sub_skill_results": {
            ...
            self.performance_metrics["token_savings"] += (
                self.token_optimizer.metrics.saved_tokens
            )

            logger.info(
                f"Completed optimized composite skill {composite_skill.name} in {execution_time:.2f}s"
            )
            ...
            logger.error(
                f"Failed to execute composite skill {composite_skill.name}: {e}"
            )
            return {
                "success": False,
                "error": str(e),
    ...
    def _execute_sequential(
        self, composite_skill: CompositeSkill, context: dict
        ...
        for skill_name in ordered_skills:
            ...
            if result.status == ExecutionStatus.FAILED:
                logger.warning(
                    f"Skill {skill_name} failed, stopping sequential execution"
                )
                break
    ...
    def _execute_parallel(
        ...
        with ThreadPoolExecutor(
            max_workers=min(
                composite_skill.max_parallel_workers, len(independent_skills)
            )
        ) as executor:
            # Submit all independent skills
            future_to_skill = {
                executor.submit(
                    self._execute_single_skill, skill_name, context, composite_skill
                ): skill_name
        ...
        dependent_skills = [
            ...
            if skill not in independent_skills
        ]
        ...
        for skill_name in composite_skill.sub_skills:
            ...
            if should_execute:
                result = self._execute_single_skill(
                    skill_name, context, composite_skill
                )
                results[skill_name] = result
            else:
                results[skill_name] = SkillExecutionResult(
    ...
    def _execute_pipeline(
        ...
        for skill_name in composite_skill.sub_skills:
            ...
            skill_context["previous_results"] = {
                name: result.result for name, result in results.items() if result.result
            }

            result = self._execute_single_skill(
                skill_name, skill_context, composite_skill
            )
            results[skill_name] = result
                ...
                for rule in composite_skill.composition_rules:
                    if rule.name == f"transform_{skill_name}" and rule.transformer:
                        try:
                            ...
                            logger.warning(
                                f"Transformation failed for {skill_name}: {e}"
                            )
    ...
    def _execute_single_skill(
        self, skill_name: str, context: dict, composite_skill: CompositeSkill
        ...
        for attempt in range(composite_skill.retry_attempts + 1):
            try:
                ...
                # Execute skill
                result = (
                    skill.execute_script("main.py", context)
                    if hasattr(skill, "execute_script")
                    else None
                )
                ...
                if attempt < composite_skill.retry_attempts:
                    logger.warning(
                        f"Skill {skill_name} failed (attempt {attempt + 1}), retrying: {e}"
                    )
                    ...
                    for rule in composite_skill.composition_rules:
                        if rule.name == f"error_{skill_name}" and rule.error_handler:
                            try:
                                ...
                                logger.error(
                                    f"Error handler failed for {skill_name}: {handler_error}"
                                )
    ...
    def _combine_results(
        self, results: dict[str, SkillExecutionResult], composite_skill: CompositeSkill
    ) -> dict[str, Any]:
        ...
        combined = {
            ...
            "skipped": sum(
                1 for r in results.values() if r.status == ExecutionStatus.SKIPPED
            ),
            "results": {},
            "execution_summary": {},
        }
    ...
    def _use_subagent(
        self, agent_name: str, composite_skill: CompositeSkill, context: dict
    ) -> dict | None:
    ...
    def _investigate_skill(
        ...
        if len(composite_skill.sub_skills) > 5:
            investigation["optimization_suggestions"].append(
                "Consider splitting into smaller composite skills"
            )
        ...
        if composite_skill.composition_type == CompositionType.SEQUENTIAL:
            ...
            for skill in composite_skill.sub_skills:
                ...
                if not dependencies or not any(
                    dep in composite_skill.sub_skills for dep in dependencies
                ):
...
class CompositeSkillManager:
    ...
    def create_composite_skill(
        ...
        description: str,
        sub_skills: list[str],
        composition_type: str | CompositionType,
        **kwargs,
    ) -> CompositeSkill:
    ...
    def create_from_template(
        self, name: str, template_name: str, sub_skills: list[str], **kwargs
    ) -> CompositeSkill:
    ...
    def get_composite_skill(self, name: str) -> CompositeSkill | None:
    ...
    def export_composite_skill(self, name: str, filepath: str):
        ...
        export_data = {
            ...
            "composition_rules": [
                {"name": rule.name, "metadata": rule.metadata}
    ...
    def import_composite_skill(self, filepath: str) -> CompositeSkill:
        ...
        for rule_data in data.get("composition_rules", []):
            rule = CompositionRule(
                name=rule_data["name"], metadata=rule_data.get("metadata", {})
            )
