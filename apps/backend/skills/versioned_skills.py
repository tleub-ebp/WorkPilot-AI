...
class VersionMigration:
    ...
    def execute_migration(self, skill_path: Path) -> bool:
        ...
        if not self.migration_script:
            logger.warning(
                f"No migration script available for {self.from_version} -> {self.to_version}"
            )
            return False
...
class VersionedSkillManager:
    ...
    def __init__(self, skills_dir: str = "skills"):
        ...
        self.skill_versions: dict[
            str, dict[str, SkillVersion]
        ] = {}  # skill_name -> version -> SkillVersion
        self.active_versions: dict[str, str] = {}  # skill_name -> active_version
        self.migrations: dict[
            str, list[VersionMigration]
        ] = {}  # skill_name -> migrations
        ...
        logger.info(
            f"Versioned skill manager initialized with {len(self.skill_versions)} skills"
        )
    ...
    def _load_versioned_skills(self):
        ...
        for skill_dir in self.skills_dir.iterdir():
            ...
            if self.skill_versions[skill_name]:
                self.active_versions[skill_name] = self._select_active_version(
                    skill_name
                )
            ...
            if compatibility_file.exists():
                with open(compatibility_file) as f:
                    ...
                    for other_version, compat_level in compat_data.items():
                        compatibility_matrix[other_version] = VersionCompatibility(
                            compat_level
                        )
    ...
    def _select_active_version(self, skill_name: str) -> str:
        ...
        sorted_versions = sorted(
            versions.keys(), key=lambda v: versions[v].version_tuple, reverse=True
        )
    ...
    def get_latest_version(self, skill_name: str) -> str | None:
    ...
    def get_compatible_versions(
        self, skill_name: str, target_version: str
    ) -> list[str]:
    ...
    def add_skill_version(
        ...
        version: str,
        skill_path: Path,
        compatibility_matrix: dict[str, str] | None = None,
    ...
    def migrate_skill(
        self, skill_name: str, target_version: str, force: bool = False
    ) -> bool:
        ...
        migration_path = self._find_migration_path(
            skill_name, current_version, target_version
        )
        if not migration_path:
            logger.error(
                f"No migration path found for {skill_name} {current_version} -> {target_version}"
            )
            return False
        ...
        for migration in migration_path:
            if not migration.auto_migrate and not force:
                logger.warning(
                    f"Manual migration required for {skill_name} {migration.from_version} -> {migration.to_version}"
                )
                return False

            if not migration.execute_migration(
                self.skill_versions[skill_name][migration.from_version].skill_path
            ):
                logger.error(
                    f"Migration failed for {skill_name} {migration.from_version} -> {migration.to_version}"
                )
                return False
    ...
    def rollback_skill(self, skill_name: str, target_version: str) -> bool:
        ...
        if not target_skill.is_compatible_with(current_version):
            logger.warning(
                f"Rollback may cause compatibility issues: {target_version} not compatible with {current_version}"
            )
    ...
    def deprecate_version(self, skill_name: str, version: str, reason: str = ""):
        ...
        if (
            skill_name not in self.skill_versions
            or version not in self.skill_versions[skill_name]
        ):
        ...
        if self.active_versions.get(skill_name) == version:
            ...
            logger.info(
                f"Updated active version for {skill_name} to {new_active} (previous version deprecated)"
            )
    ...
    def export_version_info(self, filepath: str):
        ...
        try:
            export_data = {"export_timestamp": time.time(), "skills": {}}
    ...
    def check_compatibility(
        self, skill_name: str, version1: str, version2: str
        ...
        compatibility = skill1.compatibility_matrix.get(
            version2, VersionCompatibility.UNKNOWN
        )
    ...
    def cleanup_old_versions(self, skill_name: str, keep_count: int = 3):
        ...
        logger.info(
            f"Cleaned up {skill_name}: kept {len(versions_to_keep)} versions, removed {len(versions_to_remove)}"
        )
