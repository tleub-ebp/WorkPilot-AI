...
class ContextMetrics:
    ...
    def update_compression(self, original: int, compacted: int):
        ...
        self.compression_ratio = (
            (original - compacted) / original if original > 0 else 0.0
        )
...
class ContextCompactor:
    ...
    def __init__(
        self, max_context_size: int = 10000, compaction_threshold: float = 0.7
    ):
    ...
    def compact_context(
        self, context: dict[str, Any], instructions: str | None = None
    ) -> dict[str, Any]:
        ...
        compression_ratio = (
            (start_size - end_size) / start_size if start_size > 0 else 0.0
        )

        logger.info(
            f"Context compacted: {start_size} -> {end_size} bytes ({compression_ratio:.1%} reduction)"
        )
    ...
    def _compact_with_instructions(
        self, context: dict[str, Any], instructions: str
    ) -> dict[str, Any]:
    ...
    def _focus_on_api_changes(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        for key, value in context.items():
            if any(
                api_term in key.lower()
                for api_term in ["api", "endpoint", "route", "service"]
            ):
                focused[key] = value
            else:
    ...
    def _focus_on_modified_files(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        for key, value in context.items():
            if any(
                file_term in key.lower()
                for file_term in ["file", "modified", "changed", "path"]
            ):
                focused[key] = value
            else:
                focused[key] = self._summarize_value(value)
...
class ContextOptimizer:
    ...
    def __init__(
        self, max_context_size: int = 10000, checkpoint_dir: str = "context_checkpoints"
    ):
        self.max_context_size = max_context_size
    ...
    def cleanup_context(
        self, context: dict[str, Any], task_type: str | None = None
    ) -> dict[str, Any]:
    ...
    def _load_checkpoint(self, checkpoint_id: str) -> ContextCheckpoint | None:
    ...
    def list_checkpoints(self) -> list[dict[str, Any]]:
        ...
        return [
            ...
            for cp in sorted(
                self.checkpoints.values(), key=lambda x: x.timestamp, reverse=True
            )
        ]
