    ...
    def update(self, metrics: SkillExecutionMetrics):
        ...
        if self.total_executions == 1:
            ...
            self.avg_execution_time = (
                alpha * metrics.execution_time + (1 - alpha) * self.avg_execution_time
            )
        ...
        if self.total_executions == 1:
            ...
            self.avg_memory_usage = (
                alpha * metrics.memory_usage + (1 - alpha) * self.avg_memory_usage
            )

        # Update cache hit rate
        if metrics.cache_hit:
            self.cache_hit_rate = (
                self.cache_hit_rate * (self.total_executions - 1) + 1
...
class OptimizationSuggester:
    ...
    def _suggest_performance_optimization(
        self, stats: SkillPerformanceStats
    ) -> list[dict]:
...
class SkillMetrics:
    ...
    def track_execution(
        ...
        with self._lock:
            ...
            if skill_name not in self.skill_stats:
                self.skill_stats[skill_name] = SkillPerformanceStats(
                    skill_name=skill_name
                )
            ...
            logger.debug(
                f"Tracked execution for {skill_name}: {execution_time:.3f}s, {tokens_used} tokens"
            )

    def _is_cache_hit(self, skill_name: str, user_context: dict | None) -> bool:
    ...
    def get_skill_metrics(self, skill_name: str) -> dict | None:
        ...
        with self._lock:
            ...
            recent_executions = [
                ...
                and time.time() - m.timestamp < 3600  # Last hour
            ]
            ...
            return {
                ...
                "alerts": [
                    a
                    for a in self.alert_manager.active_alerts
    ...
    def get_global_metrics(self) -> dict:
        ...
        with self._lock:
            ...
            total_executions = sum(
                stats.total_executions for stats in self.skill_stats.values()
            )
            total_successful = sum(
                stats.successful_executions for stats in self.skill_stats.values()
            )
            ...
            if total_executions > 0:
                avg_execution_time = (
                    sum(
                        ...
                        for stats in self.skill_stats.values()
                    )
                    / total_executions
                )
                avg_tokens_used = (
                    sum(
                        stats.avg_tokens_used * stats.total_executions
                        for stats in self.skill_stats.values()
                    )
                    / total_executions
                )
            ...
            return {
                ...
                "global_success_rate": total_successful / total_executions
                if total_executions > 0
                else 0.0,
    ...
    def _get_performance_summary(self) -> dict:
        ...
        return {
            ...
            "avg_execution_time": sum(s.avg_execution_time for s in all_stats)
            / len(all_stats),
            "avg_tokens_per_execution": sum(s.tokens_per_execution for s in all_stats)
            / len(all_stats),
            "avg_cache_hit_rate": sum(s.cache_hit_rate for s in all_stats)
            / len(all_stats),
            "total_alerts": len(self.alert_manager.active_alerts),
            "skills_with_alerts": len(
                set(a["skill"] for a in self.alert_manager.active_alerts)
            ),
        }
    ...
    def export_metrics(self, filepath: str, format: str = "json") -> None:
        ...
        data = {
            ...
            "skill_metrics": {
                name: self.get_skill_metrics(name) for name in self.skill_stats.keys()
            },
        ...
        try:
            with open(filepath, "w") as f:
                if format == "json":
                    ...
                    f.write("=" * 50 + "\n\n")
                    f.write(
                        f"Export Time: {datetime.fromtimestamp(data['export_timestamp'])}\n"
                    )
                    f.write(f"Total Skills: {data['global_metrics']['total_skills']}\n")
                    f.write(
                        f"Total Executions: {data['global_metrics']['total_executions']}\n"
                    )
                    f.write(
                        f"Global Success Rate: {data['global_metrics']['global_success_rate']:.2%}\n"
                    )
            ...
            if Path(self.metrics_file).exists():
                ...
                if "skill_metrics" in data:
                    for skill_name, stats_data in data["skill_metrics"].items():
                        ...
                        stats.successful_executions = stats_data.get(
                            "successful_executions", 0
                        )
                        stats.failed_executions = stats_data.get("failed_executions", 0)
                        stats.avg_execution_time = stats_data.get(
                            "avg_execution_time", 0.0
                        )
    ...
    def _save_metrics(self):
        ...
        try:
            data = {
                "skill_stats": {
                    name: stats.to_dict() for name, stats in self.skill_stats.items()
                },
                "global_stats": self.global_stats,
