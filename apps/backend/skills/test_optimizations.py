...
def test_optimization_config():
    ...
    try:
        ...
        print(f"  Memory cleanup interval: {config.performance.memory_cleanup_interval}s")
        ...
        print(f"  Encrypted token validation: {config.claude_code.encrypted_token_validation}")
        ...
        print(f"✅ Configuration export: {'PASSED' if isinstance(config_dict, dict) else 'FAILED'}")
...
def test_constants():
    ...
    try:
        from optimization_config import (
            MAX_DESCRIPTION_LENGTH, MAX_TRIGGERS_COUNT, SAMPLING_THRESHOLD,
            MAX_CONTEXT_LIMIT_RATIO, DEFAULT_MAX_WORKERS, DEFAULT_TIMEOUT,
            OPTIMIZATION_ENABLED, SUBAGENT_THRESHOLD, CHECKPOINT_INTERVAL,
            PRIORITY_THRESHOLD, VALIDATION_CACHE_SIZE, COMPRESSION_THRESHOLD,
            PREDICTIVE_CACHE_SIZE, DEDUPLICATION_MIN_LENGTH
        )
...
def test_config_update():
    ...
    try:
        from optimization_config import get_optimization_config, update_optimization_config, reset_optimization_config
        ...
        print(f"  Workers: {original_workers} → {updated_config.context.default_max_workers}")
        print(f"  Timeout: {original_config.context.default_timeout} → {updated_config.context.default_timeout}")
        print(f"  Description length: {original_config.token.max_description_length} → {updated_config.token.max_description_length}")
        
        # Reset configuration
        reset_optimization_config()
        reset_config = get_optimization_config()
        print(f"✅ Configuration reset: {'PASSED' if reset_config.context.default_max_workers == original_workers else 'FAILED'}")
