...
class SimplePatternMatcher:
    ...
    def add_usage(
        self, query: str, skills: list[str], success: bool = True, tokens: int = 0
    ):
        ...
        if normalized_query not in self.patterns:
            self.patterns[normalized_query] = UsagePattern(
                query_pattern=normalized_query, skill_sequence=skills
            )
        else:
    ...
    def predict_next_skills(
        ...
        if normalized_query in self.patterns:
            ...
            return PredictionResult(
                ...
                tokens_saved=int(
                    pattern.avg_tokens * 0.7
                ),  # Estimate 70% token savings
            )
...
class PredictiveCache:
    ...
    def predict_and_preload(
        self, query: str, skill_loader, context: dict | None = None
    ) -> list[str]:
        ...
        if prediction.confidence >= self.prediction_threshold:
            logger.info(
                f"Preloading {len(prediction.predicted_skills)} skills with confidence {prediction.confidence:.2f}"
            )
    ...
    def record_usage(
        ...
        for skill_name in skills_used:
            if skill_name in self.cache_metadata:
                ...
                self.cache_metadata[skill_name]["usage_count"] = (
                    self.cache_metadata[skill_name].get("usage_count", 0) + 1
                )
    ...
    def _evict_least_used(self):
        ...
        worst_skill = min(
            self.cache_metadata.items(), key=lambda x: self._calculate_usage_score(x[1])
        )
    ...
    def _calculate_usage_score(self, metadata: dict) -> float:
        ...
        time_factor = 1.0 / (
            1.0 + (current_time - metadata.get("last_used", 0)) / 3600
        )  # Decay over hours
    ...
    def _calculate_efficiency(self) -> float:
        ...
        total_tokens_saved = sum(
            meta.get("tokens_used", 0) for meta in self.cache_metadata.values()
        )
        ...
        return (
            total_tokens_saved / (total_tokens_used + total_tokens_saved)
    ...
    def optimize_cache(self):
        ...
        unused_skills = [
            ...
            and time.time() - meta.get("cached_at", 0) > 3600  # 1 hour old
        ]
