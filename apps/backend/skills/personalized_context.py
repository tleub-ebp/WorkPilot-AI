    ...
    def get_relevance_score(self, skill_name: str, skill_metadata: Dict) -> float:
        ...
        if skill_metadata.get('category') == 'development':
            for framework in self.detected_frameworks:
                if framework.lower() in skill_name.lower() or skill_name.lower() in framework.lower():
                    score += 0.3
                    break
        
        # Language matching
        for language in self.detected_languages:
            if language.lower() in skill_name.lower() or skill_name.lower() in language.lower():
                score += 0.2
                break
...
class ContextLearner:
    ...
    def learn_from_interaction(self, query: str, selected_skills: List[str], 
        ...
        if success:
            self.success_patterns[pattern_key] = (
                self.success_patterns[pattern_key] * 0.9 + 0.1  # Exponential moving average
            )
        else:
            self.success_patterns[pattern_key] = (
    ...
    def predict_next_skills(self, query: str, context: Dict, limit: int = 5) -> List[Tuple[str, float]]:
        ...
        if normalized_query in self.query_patterns:
            ...
            # Sort by frequency
            skill_freq = [(skill, self.skill_transitions.get(skill, 0)) for skill in skills]
...
class PersonalizedSkillManager:
    ...
    def start_session(self, user_id: str, project_id: Optional[str] = None) -> str:
        ...
        session_id = hashlib.md5(f"{user_id}:{project_id}:{time.time()}").hexdigest()[:16]
        ...
        logger.info(f"Started personalized session: {session_id[:8]} for user {user_id}")
        return session_id
    ...
    def analyze_project(self, project_id: str):
        ...
        for framework, config_files in framework_configs:
            if any((project_path / config_file).exists() for config_file in config_files):
        ...
        logger.info(f"Analyzed project {project_id}: {context.project_type}, "
    ...
    def get_learning_insights(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'top_patterns': sorted(
                self.learner.success_patterns.items(),
                key=lambda x: x[1],
                reverse=True
            )[:10]
        }
