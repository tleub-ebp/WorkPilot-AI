...
class SkillValidationRule:
    ...
    optimizer: Optional[Callable[[Dict], Dict]] = None
    ...
    def validate(self) -> bool:
        ...
        for rule in self.validation_rules:
            if rule.max_tokens and self.token_count > rule.max_tokens:
                logger.error(f"Skill {self.name} exceeds token limit: {self.token_count} > {rule.max_tokens}")
                return False
...
class SkillRegistry:
    ...
    def _register_default_rules(self):
        ...
        self.register_validation_rule(SkillValidationRule(
            ...
            validator=lambda metadata: (
                isinstance(metadata.get('name'), str) and
                len(metadata['name']) <= 64 and
                metadata['name'].replace('-', '_').isalnum() and
                all(c not in metadata['name'] for c in ['<', '>', '&'])
            ),
            max_tokens=100
        ))
    ...
    def register_skill(self, skill_path: Path, validation_rules: Optional[List[SkillValidationRule]] = None) -> bool:
        ...
        try:
            ...
            if not self._check_dependencies(dependencies):
                logger.error(f"Skill {skill.name} has unmet dependencies: {dependencies}")
                return False
    ...
    def unregister_skill(self, skill_name: str) -> bool:
        ...
        if dependents:
            logger.warning(f"Cannot unregister {skill_name}: has dependents {dependents}")
            return False
    ...
    def get_skill(self, skill_name: str) -> Optional[DynamicSkill]:
        """Get a skill by name."""
...
class DynamicSkillManager:
    ...
    def register_skill(self, skill_path: str, validation_rules: Optional[List[SkillValidationRule]] = None) -> bool:
...
def create_validation_rule(name: str, validator: Callable, **kwargs) -> SkillValidationRule:
...
def create_optimization_rule(name: str, optimizer: Callable, **kwargs) -> SkillValidationRule:
