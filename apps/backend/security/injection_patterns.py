...
INJECTION_PATTERNS: list[tuple[re.Pattern[str], str, str]] = [
    ...
    (
        re.compile(r"forget\s+(everything|all|what)\s+(you|I)\s+", re.IGNORECASE),
    ...
    (
        re.compile(
            r"(print|show|reveal|output|display)\s+(your\s+)?(system\s+prompt|instructions|rules)",
            re.IGNORECASE,
        ),
        "System prompt extraction",
        "critical",
    ...
    (
        re.compile(
            r"repeat\s+(the\s+)?(text|words|instructions)\s+above", re.IGNORECASE
        ),
        "Context extraction",
        "high",
    ),
    ...
    # Encoding evasion
    (
        re.compile(r"(?:aWdub3Jl|aWdub3JlIGFsbCBw)", re.IGNORECASE),
    ...
    (
        re.compile(r"oublie[rz]?\s+tout\s+ce\s+qui", re.IGNORECASE),
        "French memory reset",
        "high",
    ),
    (
        re.compile(r"tu\s+es\s+maintenant\s+(un|une)", re.IGNORECASE),
