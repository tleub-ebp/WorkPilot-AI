...
class ConflictPredictorService:
    ...
    def analyze_project_conflicts(self, project_path: str) -> ConflictAnalysisResult:
        ...
        modified_files = self._get_modified_files_from_worktrees(
        ...
        try:
            ...
            if result.returncode == 0:
                for line in result.stdout.strip().split("\n"):
                    ...
                    if (
                        branch
                        and branch not in ["main", "master", "develop"]
                        and branch not in worktrees
                    ):
    ...
    def _get_modified_files_from_worktrees(
        self, project_path: str, worktrees: list[str]
    ...
    def _get_worktree_modifications(
        self, project_path: str, worktree_name: str
    ) -> list[FileModification]:
                    ...
                    if line.strip():
                        ...
                        if len(parts) >= 2:
                            ...
                            lines_added, lines_removed = self._get_line_changes(
                                project_path, branch_name, file_path
                            )
    ...
    def _get_line_changes(
        self, project_path: str, branch: str, file_path: str
    ) -> tuple[int, int]:
    ...
    def _analyze_file_conflict(
        self, mod1: FileModification, mod2: FileModification, file_path: str
    ) -> ConflictRisk | None:
        ...
        if mod1.modification_type == "deleted" or mod2.modification_type == "deleted":
            ...
            description += " - One worktree is deleting the file"
        ...
        if total_changes > 100:
            ...
            resolution_strategy = (
                "Consider rebasing one branch or creating integration branch"
            )
    ...
    def _detect_dependency_conflicts(
        ...
        config_files = [
            mod for mod in modifications if self._is_config_file(mod.file_path)
        ]
        dependency_files = [
            mod for mod in modifications if self._is_dependency_file(mod.file_path)
        ]
                ...
                for j in range(i + 1, len(config_files)):
                    ...
                    conflicts.append(
                        ConflictRisk(
                            ...
                            branch1=mod1.branch_name,
                            branch2=mod2.branch_name,
                            description="Configuration changes in multiple worktrees may conflict",
                ...
                for j in range(i + 1, len(dependency_files)):
                    ...
                    conflicts.append(
                        ConflictRisk(
                            ...
                            branch1=mod1.branch_name,
                            branch2=mod2.branch_name,
                            description="Dependency updates may conflict between worktrees",
    ...
    def _generate_recommendations(
        self, conflicts: list[ConflictRisk], modifications: list[FileModification]
    ) -> list[str]:
        ...
        if not conflicts:
            recommendations.append(
                "No conflicts detected - safe to proceed with parallel development"
            )
        ...
        if critical_count > 0:
            recommendations.append(
                f"URGENT: {critical_count} critical conflicts detected - immediate coordination required"
            )
            recommendations.append(
                "Consider creating an integration branch to resolve conflicts"
            )

        if high_count > 0:
            recommendations.append(
                f"High priority: {high_count} high-risk conflicts need attention"
            )
            recommendations.append(
                "Schedule a sync meeting between teams working on conflicting areas"
            )

        if medium_count > 0:
            recommendations.append(
                f"{medium_count} medium-risk conflicts - monitor during development"
            )

        # Specific recommendations based on conflict types
        dependency_conflicts = [
            c for c in conflicts if c.conflict_type == "dependency_conflict"
        ]
        if dependency_conflicts:
            recommendations.append(
                "Coordinate dependency updates in a dedicated branch first"
            )

        same_file_conflicts = [c for c in conflicts if c.conflict_type == "same_file"]
        if same_file_conflicts:
            recommendations.append(
                "Use feature flags or create integration branches for shared file modifications"
            )

        # General recommendations
        recommendations.append(
            "Set up regular sync meetings between parallel workstreams"
        )
        recommendations.append(
            "Consider using a shared integration branch for coordinated changes"
        )
        recommendations.append(
            "Implement automated conflict detection in CI/CD pipeline"
        )

        return recommendations

    def _determine_safe_merge_order(
        self, conflicts: list[ConflictRisk], worktrees: list[str]
    ) -> list[str]:
        ...
        worktree_conflict_counts = {
            worktree: len(conflict_graph[worktree]) for worktree in worktrees
        }

        sorted_worktrees = sorted(
            worktrees, key=lambda w: (worktree_conflict_counts[w], w)
        )
    ...
    def _identify_high_risk_areas(
        ...
        sorted_areas = sorted(conflict_areas.items(), key=lambda x: x[1], reverse=True)
