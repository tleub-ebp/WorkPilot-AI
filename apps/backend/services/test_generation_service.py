...
from agents.test_generator import TestGeneratorAgent, TestGenerationResult
...
class TestGenerationService:
    ...
    def __init__(self, project_path: Path):
        ...
        self.skip_existing_tests = os.getenv('SKIP_EXISTING_TESTS', 'false').lower() == 'true'
        ...
        for file_path in modified_files:
            try:
                ...
                if 'test_' in file_path or '/tests/' in file_path:
                    logger.debug(f"[TestGenerationService] Skipping test file: {file_path}")
                    continue
                    
                # Check if file exists
                full_path = project_root / file_path
                if not full_path.exists():
                    logger.warning(f"[TestGenerationService] File not found: {file_path}")
                    continue
    ...
    def _generate_tests_for_file(
        ...
        if existing_test_path and self.skip_existing_tests:
            logger.debug(f"[TestGenerationService] Skipping file with existing tests: {relative_path}")
            return None
        ...
        try:
            result: TestGenerationResult = self.agent.generate_unit_tests(
                str(full_path),
                existing_test_path,
                self.max_tests_per_function
            )
            
            if result.tests_generated == 0:
                logger.debug(f"[TestGenerationService] No tests generated for: {relative_path}")
                return None
            ...
            logger.error(f"[TestGenerationService] Failed to generate tests for {relative_path}: {e}")
            raise
    ...
    def analyze_coverage_only(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"[TestGenerationService] Coverage analysis failed for {file_path}: {e}")
            raise
