...
class SemanticHasher:
    ...
    def _extract_terms(self, context_data: Dict[str, Any]) -> List[str]:
        ...
        if 'project_structure' in context_data:
            terms.extend(self._extract_from_structure(context_data['project_structure']))
    ...
    def _extract_from_structure(self, structure: Dict) -> List[str]:
        ...
        for file_path, file_info in structure.items():
            ...
            if isinstance(file_info, dict) and 'content' in file_info:
                content_terms = file_info['content'].split()[:50]  # Limit to first 50 terms
...
class FreshnessScorer:
    ...
    def calculate_freshness_score(self, entry: ContextCacheEntry, project_path: Path) -> float:
    ...
    def _calculate_git_freshness(self, entry: ContextCacheEntry, project_path: Path) -> float:
        ...
        try:
            ...
            if self.git_integration.is_ancestor(project_path, entry.git_commit_hash, current_commit):
    ...
    def _calculate_file_freshness(self, entry: ContextCacheEntry, project_path: Path) -> float:
...
class GitIntegration:
    ...
    def is_ancestor(self, project_path: Path, ancestor_hash: str, descendant_hash: str) -> bool:
    ...
    def count_commits_between(self, project_path: Path, from_hash: str, to_hash: str) -> int:
...
class IntelligentContextCache:
    ...
    def __init__(self, project_path: Path, config: Optional[CacheConfig] = None):
    ...
    def get_context(self, context_request: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        with self._cache_lock:
            ...
            if cache_key in self._cache:
                ...
                logger.info(f"Cache hit for key {cache_key[:8]}... (saved {build_time:.2f}s)")
    ...
    def cache_context(self, context_request: Dict[str, Any], context_data: Dict[str, Any], 
                     build_time: float = 0.0, tokens_used: int = 0) -> str:
        ...
        semantic_signature = self.semantic_hasher.generate_semantic_signature(context_data)
        
        # Get current git state
        current_commit = self.freshness_scorer.git_integration.get_current_commit(self.project_path)
        ...
        logger.info(f"Cached context with key {cache_key[:8]}... ({len(context_data)} items)")
        return cache_key
    
    def _find_semantic_match(self, context_request: Dict[str, Any]) -> Optional[ContextCacheEntry]:
        """Find semantically similar cached context."""
        request_signature = self.semantic_hasher.generate_semantic_signature(context_request)
    ...
    def _evict_least_fresh(self):
        ...
        worst_entry = min(
            self._cache.values(),
            key=lambda e: e.freshness_score
        )
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute('DELETE FROM context_cache WHERE cache_key = ?', (worst_entry.cache_key,))
    ...
    def invalidate_cache(self, pattern: Optional[str] = None):
        ...
        with self._cache_lock:
            ...
            for key in keys_to_remove:
                ...
                with sqlite3.connect(self.db_path) as conn:
                    if pattern:
                        conn.execute('DELETE FROM context_cache WHERE cache_key LIKE ?', (f'%{pattern}%',))
                    else:
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        with self._cache_lock:
            ...
            total_build_time_saved = sum(entry.build_time_saved for entry in self._cache.values())
            total_tokens_saved = sum(entry.tokens_saved for entry in self._cache.values())
...
def get_context_cache(project_path: Path, config: Optional[CacheConfig] = None) -> IntelligentContextCache:
    """Get or create context cache instance for project."""
    project_key = str(project_path.resolve())
    
    with _cache_lock:
        if project_key not in _cache_instances:
            _cache_instances[project_key] = IntelligentContextCache(project_path, config)
