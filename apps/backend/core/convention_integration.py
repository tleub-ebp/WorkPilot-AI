...
from .convention_engine import ConventionEngine, create_convention_engine
from .learning_loop import LearningLoop, create_learning_loop
...
class ConventionAwareAgent:
    ...
    def validate_and_enforce_conventions(self, file_paths: List[str]) -> Dict[str, Any]:
        ...
        logger.info(f"Convention validation completed: {total_violations} violations found")
    ...
    def apply_convention_fixes(self, file_path: str, violations: List) -> Dict[str, Any]:
        ...
        for violation in violations:
            if violation.auto_fixable and violation.suggestion:
                try:
                    ...
                    logger.warning(f"Failed to apply auto-fix for {violation.rule_type}: {e}")
    ...
    def complete_build_tracking(self, success: bool, build_data: Optional[Dict[str, Any]] = None):
        ...
        build_record = {
            ...
            'performance_metrics': build_data.get('performance_metrics', {}) if build_data else {},
            'technologies_used': build_data.get('technologies_used', []) if build_data else [],
        ...
        if pending_evolutions:
            logger.info(f"Found {len(pending_evolutions)} pending convention evolutions")
...
class ConventionIntegration:
    ...
    def get_agent(self, agent_id: str) -> Optional[ConventionAwareAgent]:
        """Get a registered agent."""
        return self.agent_registry.get(agent_id)
    
    def validate_project_conventions(self, file_paths: Optional[List[str]] = None) -> Dict[str, Any]:
    ...
    def _generate_validation_summary(self, validation_results: Dict[str, List]) -> Dict[str, Any]:
...
_integration_instance: Optional[ConventionIntegration] = None
...
def get_convention_integration(project_root: str) -> ConventionIntegration:
    ...
    if _integration_instance is None or str(_integration_instance.project_root) != project_root:
