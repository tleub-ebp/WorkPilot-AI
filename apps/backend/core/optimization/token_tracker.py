...
class TokenTracker:
    ...
    def get_global_stats(self) -> TokenStats:
        ...
        global_stats.average_tokens_per_task = global_stats.total_tokens / max(
            total_tasks, 1
        )
    ...
    def check_budget(self, agent_id: str, requested_tokens: int) -> bool:
        ...
        if agent_id in self.agent_budgets:
            ...
            if (
                agent_stats.total_tokens + requested_tokens
    ...
    def get_optimization_recommendations(self, agent_id: str) -> list[str]:
        ...
        if (
            stats.failed_tasks / max(stats.successful_tasks + stats.failed_tasks, 1)
            > 0.2
        ):
            recommendations.append(
        ...
        if stats.peak_usage > stats.average_tokens_per_task * 2:
            recommendations.append("High peak usage detected - implement token pooling")

        return recommendations

    def estimate_tokens_for_task(
        self, task_description: str, complexity: float = 0.5
    ) -> int:
        ...
        logger.debug(
            f"Estimated {estimated} tokens for task: {task_description[:50]}..."
        )
        return estimated
    ...
    def get_efficiency_score(self, agent_id: str) -> float:
        ...
        success_rate = stats.successful_tasks / max(
            stats.successful_tasks + stats.failed_tasks, 1
        )
        ...
        consistency = (
            1.0 - (stats.peak_usage / max(stats.average_tokens_per_task, 1) - 1) / 2
        )
        consistency = max(0, min(1, consistency))

        # Weighted average
        efficiency = (
            (success_rate * 0.4) + (token_efficiency * 0.4) + (consistency * 0.2)
        )
