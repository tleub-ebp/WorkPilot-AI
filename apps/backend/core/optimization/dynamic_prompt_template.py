...
"""
    ...
    def generate(self, 
        ...
        logger.debug(f"Generated dynamic prompt with context level {context_level.value}")
        return prompt
    
    def _determine_context_level(self, 
                                task_description: str,
                                context_info: Optional[ContextInfo],
                                max_tokens: Optional[int]) -> ContextLevel:
    ...
    def _assess_task_complexity(self, task_description: str) -> str:
        ...
        complexity_indicators = {
            ...
            'very_complex': ['very complex', 'highly complex', 'extensive', 'comprehensive']
        }
    ...
    def _build_context_section(self, context_info: Optional[ContextInfo], level: ContextLevel) -> str:
    ...
    def _build_standard_context(self, context_info: ContextInfo) -> str:
        ...
        if context_info.dependencies:
            context_parts.append(f"Dependencies: {', '.join(context_info.dependencies)}")
    ...
    def _build_constraints_section(self, constraints: Optional[Dict[str, Any]]) -> str:
    ...
    def _build_examples_section(self, context_info: Optional[ContextInfo], task_description: str) -> str:
        """Build examples section"""
        if not context_info or not context_info.examples:
            return "No specific examples provided."
        
        # Use relevance-based selection
        examples = self._select_relevant_examples(context_info.examples, task_description)
    ...
    def _select_relevant_examples(self, examples: List[str], task_description: str) -> List[str]:
    ...
    def _select_similar_examples(self, examples: List[str], task_description: str) -> List[str]:
        """Select examples based on similarity"""
        # Simple implementation - could be enhanced with NLP
        return examples[:3]
    
    def _select_random_examples(self, examples: List[str], task_description: str) -> List[str]:
...
- Consistent formatting"""
    ...
    def _build_validation_section(self, constraints: Optional[Dict[str, Any]]) -> str:
        ...
        if constraints:
            validation_criteria.append(f"Meets all specified constraints")
    ...
    def _optimize_for_token_budget(self, prompt: str, max_tokens: int) -> str:
        ...
        logger.warning(f"Reduced prompt from {current_tokens} to ~{len(optimized_prompt)//4} tokens")
        return optimized_prompt
    
    def add_context_builder(self, 
                           level: ContextLevel,
                           builder: Callable[[ContextInfo], str]):
    ...
    def add_example_selector(self, 
                           name: str,
                           selector: Callable[[List[str], str], List[str]]):
