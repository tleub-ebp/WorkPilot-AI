...
class HierarchicalPrompt:
    ...
    def _create_ultra_minimal_prompt(
...
Ensure your solution passes all validation criteria."""
    ...
    def add_custom_template(
        self, name: str, content: str, token_estimate: int, priority: int = 5
    ):
        """Add a custom prompt template"""
        self.templates[name] = PromptTemplate(
            name=name, content=content, token_estimate=token_estimate, priority=priority
        )
    ...
    def analyze_prompt_efficiency(self, prompt: str) -> dict[str, Any]:
        ...
        if "example" not in prompt.lower() and tokens > 500:
            recommendations.append(
                "Add examples to improve clarity and reduce ambiguity"
            )
