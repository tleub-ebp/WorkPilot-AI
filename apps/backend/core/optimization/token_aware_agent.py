...
class TokenAwareAgentBase(ABC):
    ...
    def __init__(self, agent_id: str, token_budget: int = 1000):
        ...
        logger.info(f"Initialized TokenAwareAgent {agent_id} with budget {token_budget}")
    ...
    def combine_subtask_results(self, original_task: Task, subtask_results: list[TaskResult]) -> TaskResult:
        ...
        if all_successful:
            combined_result = "\n".join([
                str(result.result) for result in subtask_results 
    ...
    def estimate_tokens_for_task(self, task: Task) -> int:
        ...
        return self.token_tracker.estimate_tokens_for_task(task.description, complexity_value)
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        recommendations = self.token_tracker.get_optimization_recommendations(self.agent_id)
