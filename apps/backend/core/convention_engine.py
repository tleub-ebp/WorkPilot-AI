...
class ConventionEngine:
    ...
    def validate_project(self, file_paths: Optional[List[str]] = None) -> Dict[str, List[ConventionViolation]]:
    ...
    def _validate_python_file_naming(self, file_path: Path) -> List[ConventionViolation]:
    ...
    def _validate_tsx_component_naming(self, file_path: Path) -> List[ConventionViolation]:
        ...
        component_exports = re.findall(r'export\s+(?:function|const)\s+([A-Z][a-zA-Z0-9]*)', content)
    ...
    def _validate_python_imports(self, file_path: Path) -> List[ConventionViolation]:
        ...
        for i, line in enumerate(lines):
            ...
            elif import_section_start is not None and line.strip() and not line.strip().startswith(('import ', 'from ', '#')):
                break
    ...
    def _validate_typescript_strict_mode(self, file_path: Path) -> List[ConventionViolation]:
        ...
        if file_path.suffix == '.tsx':
            ...
            for match in matches:
                ...
                violations.append(ConventionViolation(
                    ...
                    file_path=str(file_path),
                    line_number=line_number,
                    suggestion=f"Use translation key instead of hardcoded text"
                ))
    ...
    def _validate_claude_sdk_usage(self, file_path: Path) -> List[ConventionViolation]:
        ...
        if 'anthropic.Anthropic(' in content or 'from anthropic import Anthropic' in content:
            violations.append(ConventionViolation(
    ...
    def _validate_platform_abstraction(self, file_path: Path) -> List[ConventionViolation]:
