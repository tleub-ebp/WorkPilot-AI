...
class LearningLoop:
    ...
    def _load_data(self):
        ...
        try:
            ...
            if self.patterns_file.exists():
                patterns_data = json.loads(self.patterns_file.read_text(encoding='utf-8'))
                self.patterns = {
            ...
            if self.evolutions_file.exists():
                evolutions_data = json.loads(self.evolutions_file.read_text(encoding='utf-8'))
                self.evolutions = {
            ...
            if self.build_history_file.exists():
                self.build_history = json.loads(self.build_history_file.read_text(encoding='utf-8'))
    ...
    def _save_data(self):
        ...
        try:
            ...
            # Save evolutions
            evolutions_data = {
                eid: evolution.to_dict() 
                for eid, evolution in self.evolutions.items()
            }
            self.evolutions_file.write_text(
                json.dumps(evolutions_data, indent=2), 
                encoding='utf-8'
            )
            
            # Save build history
            self.build_history_file.write_text(
                json.dumps(self.build_history, indent=2), 
                encoding='utf-8'
            )
    ...
    def _analyze_file_structure_patterns(self, build_record: Dict[str, Any]):
        ...
        for directory, extensions in directory_patterns.items():
            if len(extensions) >= 3:  # Significant pattern
                ...
                success_rate = len([b for b in similar_builds if b['success']]) / max(len(similar_builds), 1)
                ...
                if success_rate >= self.min_success_rate:
                    pattern = BuildPattern(
                        ...
                        confidence_score=min(success_rate * (len(similar_builds) / 10), 1.0),
    ...
    def _analyze_architecture_patterns(self, build_record: Dict[str, Any]):
        ...
        if len(agent_types) >= 2:
            ...
            if success_rate >= self.min_success_rate and len(similar_builds) >= self.min_frequency:
                pattern = BuildPattern(
                    pattern_id=pattern_id,
                    ...
                    success_rate=success_rate,
                    frequency=len(similar_builds),
                    confidence_score=min(success_rate * (len(similar_builds) / 20), 1.0),
                    discovered_at=datetime.now(),
                    last_seen=datetime.now()
                )
    ...
    def _analyze_performance_patterns(self, build_record: Dict[str, Any]):
        ...
        if duration > 0 and tokens_used > 0:
            ...
            similar_builds = self._find_similar_builds('performance', efficiency_category)
            success_rate = len([b for b in similar_builds if b['success']]) / max(len(similar_builds), 1)
            ...
            if success_rate >= self.min_success_rate:
                pattern = BuildPattern(
                    ...
                    success_rate=success_rate,
                    frequency=len(similar_builds),
                    confidence_score=min(success_rate * (len(similar_builds) / 15), 1.0),
                    discovered_at=datetime.now(),
                    last_seen=datetime.now()
                )
    ...
    def _generate_evolution_proposals(self):
        ...
        for pattern in self.patterns.values():
            ...
            if (pattern.confidence_score >= self.confidence_threshold and 
                ...
                existing_evolution = self._find_evolution_for_pattern(pattern.pattern_id)
    ...
    def _create_evolution_proposal(self, pattern: BuildPattern) -> Optional[ConventionEvolution]:
    ...
    def _create_architecture_evolution(self, pattern: BuildPattern) -> ConventionEvolution:
        ...
        return ConventionEvolution(
            ...
            section=section,
            proposed_change=proposed_change,
            rationale=f"Successful agent combination pattern",
            confidence_score=pattern.confidence_score,
            impact_assessment="Medium - Improves build success rate",
            created_at=datetime.now()
        )
    
    def _create_performance_evolution(self, pattern: BuildPattern) -> ConventionEvolution:
        ...
        return ConventionEvolution(
            ...
            section=section,
            proposed_change=proposed_change,
            rationale=f"Performance optimization pattern discovered",
            confidence_score=pattern.confidence_score,
            impact_assessment="Low - Performance guidance",
            created_at=datetime.now()
        )
    
    def _find_similar_builds(self, pattern_type: str, pattern_value: str) -> List[Dict[str, Any]]:
        ...
        for build in self.build_history:
            if pattern_type == 'naming':
                ...
                files = build.get('files_changed', [])
                if any(self._matches_structure_pattern(f, pattern_value) for f in files):
                    similar_builds.append(build)
                ...
                if duration > 0 and tokens > 0:
                    ...
                    if (pattern_value == 'high_efficiency' and efficiency < 100) or \
                       (pattern_value == 'medium_efficiency' and 100 <= efficiency < 500) or \
                       (pattern_value == 'low_efficiency' and efficiency >= 500):
                        similar_builds.append(build)
    ...
    def _find_evolution_for_pattern(self, pattern_id: str) -> Optional[ConventionEvolution]:
    ...
    def _generate_id(self) -> str:
        """Generate unique ID."""
        return hashlib.md5(f"{datetime.now().isoformat()}{os.urandom(8)}".encode()).hexdigest()[:12]
            ...
            if target_file.exists():
                ...
                if evolution.section in content:
                    ...
                    if section_pattern in content:
                        ...
                        new_content = parts[0] + section_pattern + "\n" + evolution.proposed_change + "\n" + parts[1]
    ...
    def get_learning_summary(self) -> Dict[str, Any]:
        ...
        recent_builds = [
            b for b in self.build_history 
            if datetime.fromisoformat(b['timestamp']) > datetime.now() - timedelta(days=30)
        ]
        
        recent_success_rate = len([b for b in recent_builds if b['success']]) / max(len(recent_builds), 1)
