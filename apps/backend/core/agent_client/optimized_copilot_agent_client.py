...
class OptimizedCopilotAgentClient(TokenAwareAgentBase):
    ...
    def __init__(self,
        ...
        logger.info(f"Initialized OptimizedCopilotAgentClient with model {model} and budget {token_budget}")
    ...
    def _convert_to_task_result(self, task: Task, response_data: Dict[str, Any]) -> TaskResult:
        ...
        return TaskResult(
            ...
            tokens_used=response_data.get('total_tokens', self._estimate_task_tokens(task)),
    ...
    def _build_context_info(self) -> ContextInfo:
        ...
        if self.cwd:
            ...
            context_info.files = [
                str(f) for f in working_path.iterdir() 
    ...
    def _estimate_task_tokens(self, task: Task) -> int:
        ...
        system_prompt_tokens = len(self.system_prompt or "") // 4 if self.system_prompt else 0
    ...
    async def run_subagents(self,
                           agents: dict[str, Any],
                           context_prompt: str) -> dict[str, str]:
        """
        ...
        try:
            ...
            estimated_tokens = len(context_prompt) // 4 + len(str(results)) // 4 + 500  # Approximate
