...
class MigrationAgentSkillWrapper:
    ...
    def analyze_stack(self) -> StackAnalysis:
        ...
        try:
            ...
            result = self.migration_skill.execute_script(
                "analyze_stack.py", {"project-root": self.project_root}
            )
    ...
    def create_migration_plan(
        ...
        try:
            ...
            breaking_changes_data = self.migration_skill.get_data(
            ...
            self._plans[plan_id] = plan
            logger.info(
                f"Created migration plan {plan_id}: {source_framework} {source_version} → {target_fw} {target_version}"
            )
    ...
    def _generate_migration_steps(
        self, plan: MigrationPlan, known: dict, custom_steps: list[dict] | None
    ) -> None:
    ...
    def _generate_basic_migration_steps(
        self, plan: MigrationPlan, custom_steps: list[dict] | None
    ) -> None:
    ...
    def execute_migration(self, plan_id: str, dry_run: bool = False) -> MigrationResult:
        ...
        try:
            ...
            result = self.migration_skill.execute_script(
                ...
                {"plan-file": plan_file, "dry-run": str(dry_run).lower()},
            )
    ...
    def rollback_migration(self, plan_id: str) -> MigrationResult:
        ...
        try:
            ...
            # Execute rollback
            result = self.migration_skill.execute_script(
                "execute_migration.py", {"plan-file": plan_file, "rollback": ""}
            )
