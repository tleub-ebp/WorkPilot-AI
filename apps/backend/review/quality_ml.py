...
class MLPatternDetector:
    ...
    def detect_violations(self, file_path: Path, content: str) -> list[QualityIssue]:
        ...
        for pattern in self.patterns:
            ...
            if pattern.pattern_id == 'naming_convention':
                ...
                violations = self._check_function_length_violations(file_path, content, pattern)
                issues.extend(violations)
            
            elif pattern.pattern_id == 'indentation_style':
                violations = self._check_indentation_violations(file_path, lines, pattern)
                issues.extend(violations)
    ...
    def _check_function_length_violations(
        ...
        func_starts = [(m.start(), m.group(), content[:m.start()].count('\n') + 1)
    ...
    def _check_indentation_violations(
        ...
        for i, line in enumerate(lines, 1):
            if line and line[0] in ' \t':
                if '4 spaces' in expected_style and line.startswith('  ') and not line.startswith('    '):
                    issues.append(QualityIssue(
                        category=QualityCategory.MAINTAINABILITY,
                        severity=IssueSeverity.LOW,
