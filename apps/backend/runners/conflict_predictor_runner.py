...
class ConflictPredictorRunner:
    ...
    def run_conflict_analysis(self, project_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            critical_conflicts = len([c for c in result.conflicts_detected if c.risk_level == 'critical'])
            high_conflicts = len([c for c in result.conflicts_detected if c.risk_level == 'high'])
            medium_conflicts = len([c for c in result.conflicts_detected if c.risk_level == 'medium'])
            ...
            structured_result['summary'] = {
                ...
                'risk_assessment': self._get_risk_assessment(critical_conflicts, high_conflicts, medium_conflicts)
            }
    ...
    def _stream_analysis_details(self, result: Dict[str, Any]):
        ...
        if result['active_worktrees']:
            ...
            self._emit_event('progress', {
                'status': f'Active worktrees: {worktree_list}'
            })
        ...
        if result['modified_files']:
            ...
            added_files = [f for f in result['modified_files'] if f['modification_type'] == 'added']
            modified_files = [f for f in result['modified_files'] if f['modification_type'] == 'modified']
            deleted_files = [f for f in result['modified_files'] if f['modification_type'] == 'deleted']
            
            if added_files:
                self._emit_event('progress', {
                    'status': f'{len(added_files)} files added'
                })
            if modified_files:
                self._emit_event('progress', {
                    'status': f'{len(modified_files)} files modified'
                })
            if deleted_files:
                self._emit_event('progress', {
                    'status': f'{len(deleted_files)} files deleted'
                })
        ...
        if result['safe_merge_order'] and len(result['safe_merge_order']) > 1:
            self._emit_event('progress', {
                'status': 'Safe merge order suggested:'
            })
