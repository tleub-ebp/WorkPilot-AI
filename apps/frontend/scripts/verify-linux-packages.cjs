	...
	for (const file of files) {
		...
		if (file.endsWith(".AppImage")) {
			if (!packages.appImage) {
				...
				logWarning(
					`Multiple AppImage files found, using first: ${path.basename(packages.appImage)}`,
				);
			}
			...
			if (!packages.deb) {
				...
				logWarning(
					`Multiple deb files found, using first: ${path.basename(packages.deb)}`,
				);
			}
			...
			if (!packages.flatpak) {
				...
				logWarning(
					`Multiple Flatpak files found, using first: ${path.basename(packages.flatpak)}`,
				);
			}
		}
	}
...
function verifyAppImage(appImagePath) {
	...
	if (!commandExists("bsdtar")) {
		logWarning(
			"bsdtar not found. Install with: sudo apt-get install libarchive-tools",
		);
	...
	if (result.error) {
		...
		return {
			verified: false,
			reason: `Command execution failed: ${result.error.message}`,
		};
	}
...
function verifyFlatpak(flatpakPath) {
	...
	if (!hasFlatpakCli) {
		logWarning(
			"flatpak command not found. Skipping detailed Flatpak verification",
		);
...
function main() {
	...
	if (hasFailures || hasCriticalSkips) {
		...
		if (hasFailures) {
			log(
				"Some packages are missing critical files. This will cause runtime errors.\n",
				colors.red,
			);
		}
		if (hasCriticalSkips) {
			log(
				"Some packages could not be verified due to missing required tools.\n",
				colors.red,
			);
