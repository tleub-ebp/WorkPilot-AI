...
const {
	...
	getCurrentPlatform,
	toNodePlatform,
} = require("../src/shared/platform.cjs");
...
function stageRuntimePackages(frontendDir, platform, arch) {
	...
	if (!fs.existsSync(outMainDir)) {
		throw new Error(
			"Missing build output. Run electron-vite build before staging node-pty.",
		);
	}
	...
	function stagePackage(pkgName) {
		...
		if (!fs.existsSync(rootDirPath)) {
			throw new Error(
				`Missing ${pkgName} in workspace. Run pnpm install before packaging.`,
			);
		}
...
async function main() {
	...
	const hasPublishFlag = builderArgs.some(
