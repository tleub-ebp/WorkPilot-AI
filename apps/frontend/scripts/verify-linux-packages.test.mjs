...
describe("verify-linux-packages", () => {
	describe("package finding logic", () => {
		it("should identify all three Linux package types", () => {
			...
			try {
				...
				assert.equal(
					result.appImage,
					"/test/dist/WorkPilot-AI-2.7.5-linux-x86_64.AppImage",
				);
				assert.equal(result.deb, "/test/dist/auto-claude_2.7.5_amd64.deb");
				assert.equal(
					result.flatpak,
					"/test/dist/com.workpilotai.app_2.7.5_linux_x86_64.flatpak",
				);
			} finally {
	...
	describe("critical packages list", () => {
		it("should contain all required Linux packages", () => {
			assert.ok(
				CRITICAL_PACKAGES.includes("secretstorage"),
				"secretstorage must be present for Linux OAuth",
			);
			assert.ok(
				CRITICAL_PACKAGES.includes("pydantic_core"),
				"pydantic_core must be present",
			);
			assert.ok(
				CRITICAL_PACKAGES.includes("claude_agent_sdk"),
				"claude_agent_sdk must be present",
			);
	...
	describe("file content verification logic", () => {
		...
		it("should detect Python binary with trailing slashes", () => {
			...
			assert.ok(
				result.verified,
				"Should detect Python binary directory with trailing slash",
			);
			assert.equal(result.issues.length, 0);
		});
		...
		it("should report missing packages", () => {
			...
			assert.ok(
				result.issues.includes("Python package not found: secretstorage"),
			);
			assert.ok(
				result.issues.includes("Python package not found: pydantic_core"),
			);
			assert.ok(
				result.issues.includes("Python package not found: claude_agent_sdk"),
			);
		...
		it("should not match python-site-packages when looking for python binary", () => {
			...
			assert.ok(
				result.issues.some((i) =>
					i.includes("Python binary directory not found"),
				),
			);
		});
		...
		it("should not match unrelated paths when looking for packages", () => {
			...
			assert.ok(
				result.issues.some((i) =>
					i.includes("Python package not found: secretstorage"),
				),
			);
		});
	});
	...
	describe("Flatpak file validation", () => {
		...
		it("should warn about suspiciously small Flatpak files", () => {
			...
			try {
				...
				assert.ok(
					!result.verified,
					"Should fail verification for too-small files",
				);
		...
		it("should accept reasonable Flatpak file sizes", () => {
			...
			try {
				...
				assert.ok(
					result.verified,
					"Should accept reasonable Flatpak file sizes",
				);
				assert.equal(result.issues.length, 0);
			} finally {
				existsSync.mock.restore();
			...
			const mockFn = (cmd) => {
				...
				if (cmd === "bsdtar") {
					return {
						status: 1,
						stdout: "",
						stderr: "bsdtar: Error: Not an AppImage file",
						error: undefined,
					};
				}
				return { status: 1, stderr: "Unknown command" };
			...
			const mockFn = (cmd) => {
				...
				if (cmd === "dpkg-deb") {
					return {
						status: 2,
						stdout: "",
						stderr: "dpkg-deb: error: cannot read archive",
						error: undefined,
					};
				}
				return { status: 1, stderr: "Unknown command" };
