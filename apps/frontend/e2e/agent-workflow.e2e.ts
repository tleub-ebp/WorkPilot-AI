...
test.describe("Agent Workflow E2E Tests", () => {
	...
	test("should create task spec directory for agent", () => {
		...
		expect(existsSync(path.join(SPECS_DIR, specId, "requirements.json"))).toBe(
			true,
		);
	...
	test("should simulate full agent execution: plan → code → review → complete", () => {
		...
		const status1 = JSON.parse(
			readFileSync(path.join(SPECS_DIR, specId, "status.json"), "utf-8"),
		);
		...
		agent.complete();
		const status4 = JSON.parse(
			readFileSync(path.join(SPECS_DIR, specId, "status.json"), "utf-8"),
		);
		expect(status4.status).toBe("human_review");

		const progress = readFileSync(
			path.join(SPECS_DIR, specId, "build-progress.txt"),
			"utf-8",
		);
	...
	test("should handle agent interruption and recovery", () => {
		...
		const pendingSubtask = plan.phases[0].subtasks.find(
	...
	test("should handle multiple concurrent task specs", () => {
		...
		const status0 = JSON.parse(
			readFileSync(path.join(SPECS_DIR, tasks[0], "status.json"), "utf-8"),
		);
		expect(status0.status).toBe("in_progress");

		const plan1 = JSON.parse(
			readFileSync(
				path.join(SPECS_DIR, tasks[1], "implementation_plan.json"),
				"utf-8",
			),
		);
		...
		const status2 = JSON.parse(
			readFileSync(path.join(SPECS_DIR, tasks[2], "status.json"), "utf-8"),
		);
	...
	test("should handle merge flow: human_review → done", () => {
		...
		expect(existsSync(path.join(SPECS_DIR, specId, "pr_created.json"))).toBe(
			true,
		);
		const pr = JSON.parse(
			readFileSync(path.join(SPECS_DIR, specId, "pr_created.json"), "utf-8"),
		);
