...
function shouldRunForPlatform(
	testPlatform: "all" | "windows" | "linux" | "mac",
): boolean {
...
test.describe("Terminal Copy/Paste Flows", () => {
	...
	test("should copy selected text to clipboard", async () => {
		...
		test.skip(
			!shouldRunForPlatform("all"),
			"Test not applicable to this platform",
		);
		...
		await expect(terminal).toContainText("test output for copy", {
			timeout: 5000,
		});
	...
	test("should handle clipboard errors gracefully", async () => {
		...
		await window.evaluate(async () => {
			...
			try {
				...
				console.warn(
					"Clipboard not accessible (expected in some environments)",
				);
			}
		});
		...
		await expect(terminal).toContainText("terminal still works", {
			timeout: 5000,
		});
	});
});
