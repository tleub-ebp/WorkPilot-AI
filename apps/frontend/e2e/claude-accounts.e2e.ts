...
test.describe("Claude Account Authentication Flow (Mock-based)", () => {
	...
	test("should simulate authentication terminal creation", () => {
		...
		expect(terminalConfig.env.CLAUDE_CONFIG_DIR).toBe(
			path.join(TEST_CONFIG_DIR, profileName),
		);
	});
...
test.describe("Claude Account Persistence", () => {
	...
	test("should persist multiple accounts across sessions", () => {
		...
		expect(existsSync(path.join(TEST_CONFIG_DIR, "personal-account"))).toBe(
			true,
		);
		...
		expect(
			existsSync(path.join(TEST_CONFIG_DIR, "personal-account", ".env")),
		).toBe(true);
		expect(existsSync(path.join(TEST_CONFIG_DIR, "work-account", ".env"))).toBe(
			true,
		);
		expect(existsSync(path.join(TEST_CONFIG_DIR, "test-account", ".env"))).toBe(
			false,
		);
	});
...
test.describe("Full Account Addition Workflow (Integration)", () => {
	...
	test("should complete full workflow: create → authenticate → persist", () => {
		...
		const savedProfile = JSON.parse(
			readFileSync(path.join(profileDir, "profile.json"), "utf-8"),
		);
...
test.describe
	.skip("Claude Account UI Tests (Electron)", () => {
		...
		test.skip('should click "+ Add" button and trigger authentication', async () => {
			...
			const input = await page
				.locator('input[placeholder*="account"], input[placeholder*="name"]')
				.first();
