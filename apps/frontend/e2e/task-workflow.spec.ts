...
function setupTestEnvironment(): void {
	...
	TEST_DATA_DIR = mkdtempSync(
		path.join(tmpdir(), "auto-claude-task-workflow-e2e-"),
	);
...
function simulateTaskResume(specId: string): void {
	...
	const pendingSubtask = plan.subtasks.find(
		(st: { status: string }) => st.status === "pending",
	);
	if (pendingSubtask) {
...
test.describe("Task Workflow E2E Tests", () => {
	...
	test("should resume task from checkpoint", () => {
		...
		let plan = JSON.parse(
			readFileSync(
				path.join(SPECS_DIR, specId, "implementation_plan.json"),
				"utf-8",
			),
		);
...
test.describe("Full Task Workflow Integration", () => {
	...
	test("should complete full workflow: create → spec → subtasks → resume → complete", () => {
		...
		createTaskWithSubtasks(specId, ["pending", "pending", "pending"]);
		let plan = JSON.parse(
			readFileSync(path.join(specDir, "implementation_plan.json"), "utf-8"),
		);
		expect(plan.subtasks.length).toBe(3);
	...
	test("should validate workflow data integrity", () => {
		...
		expect(existsSync(path.join(specDir, "implementation_plan.json"))).toBe(
			true,
		);
		...
		const requirements = JSON.parse(
			readFileSync(path.join(specDir, "requirements.json"), "utf-8"),
		);
