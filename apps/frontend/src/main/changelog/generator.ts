...
import { getBranchDiffCommits, getCommits } from "./git-integration";
...
export class ChangelogGenerator extends EventEmitter {
	private generationProcesses: Map<string, ReturnType<typeof spawn>> =
		new Map();
	...
	async generate(
		...
		if (sourceMode === "git-history" && request.gitHistory) {
			...
			const commits = getBranchDiffCommits(
				projectPath,
				request.branchDiff,
				this.debugEnabled,
			);
		...
		childProcess.stdout?.on("data", (data: Buffer) => {
			...
			output += chunk;
			this.debug("stdout chunk received", {
		...
		childProcess.on("exit", (code: number | null) => {
			...
			if (code === 0 && output.trim()) {
				...
				this.debug("Changelog extracted", {
	...
	private buildSpawnEnvironment(): Record<string, string> {
		...
		this.debug("Active profile environment", {
			...
			authMethod: profileEnv.CLAUDE_CODE_OAUTH_TOKEN
				? "oauth-token"
		...
		this.debug("Spawn environment", {
			...
			authMethod: spawnEnv.CLAUDE_CODE_OAUTH_TOKEN
				? "oauth-token"
				: spawnEnv.CLAUDE_CONFIG_DIR
					? `config-dir:${spawnEnv.CLAUDE_CONFIG_DIR}`
					: "default",
		});
