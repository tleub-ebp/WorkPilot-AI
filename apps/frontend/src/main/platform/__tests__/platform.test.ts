...
describe("Platform Module", () => {
	...
	describe("Binary Directories", () => {
		it("returns Windows-specific directories on Windows", () => {
			...
			expect(dirs.user).toContainEqual(expect.stringContaining("AppData"));
		...
		it("includes user-specific directories with home paths", () => {
			...
			mockPlatform("darwin");
			let dirs = getBinaryDirectories();
			const hasMacUserDir = dirs.user.some(
				(dir) => dir.includes(".local/bin") || dir.includes("bin"),
			);
			...
			mockPlatform("linux");
			dirs = getBinaryDirectories();
			const hasLinuxUserDir = dirs.user.some(
				(dir) => dir.includes(".local/bin") || dir.includes("bin"),
			);
			...
			mockPlatform("win32");
			dirs = getBinaryDirectories();
			const hasWindowsUserDir = dirs.user.some(
				(dir) => dir.includes("AppData") || dir.includes(".local"),
			);
		...
		it("Windows includes npm global directory", () => {
			...
			const hasNpmDir = dirs.user.some(
				(dir) => dir.includes("Roaming") && dir.includes("npm"),
			);
		...
		it("Windows includes System32 directory", () => {
			...
			const hasSystem32 = dirs.system.some((dir) => dir.includes("System32"));
	...
	describe("Shell Configuration", () => {
		it("returns PowerShell config on Windows by default", () => {
			...
			const isValidShell =
				config.executable.includes("pwsh.exe") ||
