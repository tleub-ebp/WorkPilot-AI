...
export function getPythonPaths(): string[] {
	...
	if (isWindows()) {
		...
		const userPythonPath = joinPaths(
			homeDir,
			"AppData",
			"Local",
			"Programs",
...
export function getWindowsShellPaths(): Record<string, string[]> {
	...
	return {
		...
		cygwin: [path.join("C:\\cygwin64", "bin", "bash.exe")],
		msys2: [path.join("C:\\msys64", "usr", "bin", "bash.exe")],
		wsl: [path.join(systemRoot, "System32", "wsl.exe")],
	};
}
 ...
 */
export function getWindowsToolPath(
	toolName: string,
	subPath?: string,
): string[] {
	if (!isWindows()) {
