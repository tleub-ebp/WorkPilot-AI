...
export class LiveCompanionService extends EventEmitter {
  ...
  private fileActivityMap: Map<string, { lastChange: number; changeCount: number }> = new Map();
  ...
  async start(projectDir: string): Promise<boolean> {
    ...
    try {
      ...
      watcher.on('change', (filePath: string) => {
        this.handleFileChange(projectDir, filePath as string, 'modified', config);
      });

      watcher.on('add', (filePath: string) => {
        this.handleFileChange(projectDir, filePath as string, 'created', config);
      });

      watcher.on('unlink', (filePath: string) => {
        this.handleFileChange(projectDir, filePath as string, 'deleted', config);
      });
      ...
      this.emit('error', `Failed to start file watcher: ${(error as Error).message}`);
      return false;
    }
  }
  ...
  getSuggestions(): LiveSuggestion[] {
    try {
      if (existsSync(SUGGESTIONS_FILE)) {
        ...
        return (data.suggestions || []).filter((s: LiveSuggestion) => s.status === 'active');
      }
    } catch { /* ignore */ }
    return [];
  }
  ...
  getTakeovers(): TakeoverProposal[] {
    try {
      if (existsSync(TAKEOVERS_FILE)) {
        ...
        return (data.takeovers || []).filter((t: TakeoverProposal) => t.status === 'proposed');
      }
    } catch { /* ignore */ }
    return [];
  }
  ...
  private handleFileChange(
    ...
    const existing = this.fileActivityMap.get(fullPath) || { lastChange: 0, changeCount: 0 };
  ...
  private async analyzePendingChanges(projectDir: string): Promise<void> {
    ...
    const runnerPath = path.join(autoBuildSource, 'runners', 'live_companion_runner.py');
    if (!existsSync(runnerPath)) return;

    // Write changes to temp file for batch analysis
    const tmpFile = path.join(os.tmpdir(), `workpilot_companion_${Date.now()}.json`);
    writeFileSync(tmpFile, JSON.stringify({ changes: changes.map(c => ({
    ...
    proc.on('close', () => {
      ...
      for (const line of stdout.split('\n')) {
        if (line.startsWith('LIVE_COMPANION_EVENT:')) {
          try {
            const event = JSON.parse(line.substring('LIVE_COMPANION_EVENT:'.length));
  ...
  private updateSuggestionStatus(suggestionId: string, status: string): boolean {
    try {
      if (!existsSync(SUGGESTIONS_FILE)) return false;
      const data = JSON.parse(readFileSync(SUGGESTIONS_FILE, 'utf-8'));
      const suggestion = (data.suggestions || []).find((s: LiveSuggestion) => s.suggestion_id === suggestionId);
  ...
  private updateTakeoverStatus(proposalId: string, status: string): boolean {
    try {
      ...
      const data = JSON.parse(readFileSync(TAKEOVERS_FILE, 'utf-8'));
      const takeover = (data.takeovers || []).find((t: TakeoverProposal) => t.proposal_id === proposalId);
  ...
  private buildProcessEnvironment(): Record<string, string> {
    ...
    try {
      ...
      if (existsSync(settingsPath)) {
        ...
        if (settings.globalClaudeOAuthToken) processEnv.CLAUDE_OAUTH_TOKEN = settings.globalClaudeOAuthToken;
        if (settings.globalAnthropicApiKey) processEnv.ANTHROPIC_API_KEY = settings.globalAnthropicApiKey;
        if (settings.globalOpenAIApiKey) processEnv.OPENAI_API_KEY = settings.globalOpenAIApiKey;
        if (settings.globalGoogleApiKey) processEnv.GOOGLE_API_KEY = settings.globalGoogleApiKey;
        if (settings.globalOpenRouterApiKey) processEnv.OPENROUTER_API_KEY = settings.globalOpenRouterApiKey;
      }
    } catch { /* ignore */ }
