...
export class TerminalSessionStore {
	 ...
	 */
	private pendingDeleteTimers: Map<string, ReturnType<typeof setTimeout>> =
		new Map();
	/**
	...
	private load(): SessionData {
		...
		if (mainResult.error) {
			console.warn(
				"[TerminalSessionStore] Main file corrupted, attempting backup recovery...",
			);
			const backupResult = this.tryLoadFile(this.backupPath);
			if (backupResult.success && backupResult.data) {
				console.warn(
					"[TerminalSessionStore] Successfully recovered from backup!",
				);
				...
				try {
					...
					console.error(
						"[TerminalSessionStore] Failed to restore main file:",
						writeError,
					);
				}
			...
			console.error(
				"[TerminalSessionStore] Backup recovery failed, starting fresh",
			);
		}
	 ...
	 */
	private tryLoadFile(filePath: string): {
		success: boolean;
		data?: SessionData;
		error?: Error;
	} {
		try {
			...
			if (data.version === 1 && data.sessions) {
				console.warn(
					"[TerminalSessionStore] Migrating from v1 to v2 structure",
				);
				const today = getDateString();
			...
			console.warn(
				"[TerminalSessionStore] Version mismatch, resetting sessions",
			);
			return { success: false };
		} catch (error) {
		...
		try {
			...
			if (existsSync(this.storePath)) {
				try {
					...
					console.warn(
						"[TerminalSessionStore] Current file corrupted, not backing up",
					);
	...
	private async saveAsync(): Promise<void> {
		...
		try {
			...
			if (
				this.consecutiveFailures >=
				TerminalSessionStore.MAX_FAILURES_BEFORE_WARNING
			) {
				console.error(
	...
	private cleanupOldSessions(): void {
		...
		if (removedCount > 0) {
			console.warn(
				`[TerminalSessionStore] Cleaned up sessions from ${removedCount} old dates`,
			);
			this.save();
		}
	}
	...
	private updateSessionInMemory(session: TerminalSession): boolean {
		...
		if (this.pendingDelete.has(session.id)) {
			console.warn(
				"[TerminalSessionStore] Skipping save for deleted session:",
				session.id,
			);
			return false;
		}
		...
		debugLog(
			"[TerminalSessionStore] Updating session in memory:",
			session.id,
			"incoming outputBuffer:",
			incomingBufferLen,
			"bytes",
			"isClaudeMode:",
		...
		const existingIndex = todaySessions[projectPath].findIndex(
			(s) => s.id === session.id,
		);
		if (existingIndex >= 0) {
			...
			const truncatedLen = session.outputBuffer.slice(
				-MAX_OUTPUT_BUFFER,
			).length;
			debugLog(
				"[TerminalSessionStore] Updating existing session:",
				session.id,
				"existing outputBuffer:",
				existingBufferLen,
				"bytes",
				"new outputBuffer (after truncation):",
				truncatedLen,
				"bytes",
			);
			...
			debugLog(
				"[TerminalSessionStore] Creating new session:",
				session.id,
				"outputBuffer (after truncation):",
				truncatedLen,
				"bytes",
			);
	 ...
	 */
	private validateWorktreeConfig(
		config: TerminalWorktreeConfig | undefined,
	): TerminalWorktreeConfig | undefined {
		if (!config) return undefined;

		// Check if the worktree path still exists
		if (!existsSync(config.worktreePath)) {
			console.warn(
				`[TerminalSessionStore] Worktree path no longer exists: ${config.worktreePath}, clearing config`,
			);
			return undefined;
		}
	...
	getSessions(projectPath: string): TerminalSession[] {
		...
		debugLog(
			"[TerminalSessionStore] Getting sessions for project:",
			projectPath,
			"date:",
			today,
		);
		...
		if (dates.length > 0) {
			...
			console.warn(
				`[TerminalSessionStore] No sessions today, migrating sessions from ${mostRecentDate} to today`,
			);
			const sessions =
				this.data.sessionsByDate[mostRecentDate][projectPath] || [];

			// Debug: Log outputBuffer info for sessions being migrated
			for (const session of sessions) {
				const bufferLen = session.outputBuffer?.length ?? 0;
				debugLog(
					"[TerminalSessionStore] Migrating session",
					session.id,
					"from",
					mostRecentDate,
					"outputBuffer:",
			...
			console.warn(
				`[TerminalSessionStore] Migrated ${migratedSessions.length} sessions from ${mostRecentDate} to ${today}`,
			);
	...
	updateOutputBuffer(
		...
		if (session) {
			...
			session.outputBuffer = (session.outputBuffer + output).slice(
				-MAX_OUTPUT_BUFFER,
			);
			...
			if (newLen - prevLen > 1000 || prevLen === 0) {
				debugLog(
					"[TerminalSessionStore] updateOutputBuffer:",
					sessionId,
					"prev:",
					prevLen,
					"bytes, added:",
	...
	updateClaudeSessionId(
		...
		if (session) {
			...
			this.save();
			console.warn(
				"[TerminalSessionStore] Saved Claude session ID:",
				claudeSessionId,
				"for terminal:",
				terminalId,
			);
