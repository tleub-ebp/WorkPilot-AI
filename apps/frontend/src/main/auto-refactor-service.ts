...
export class AutoRefactorService extends EventEmitter {
  ...
  private buildCommandArgs(request: AutoRefactorRequest, runnerPath: string): string[] {
    const args = [
      runnerPath,
  ...
  private setupProcessHandlers(proc: ChildProcess, processor: OutputProcessor): void {
  ...
  private handleProcessCompletion(code: number | null, processor: OutputProcessor): void {
  ...
  private handleProcessError(code: number, processor: OutputProcessor): void {
    ...
    if (combinedOutput.includes('rate_limit') || combinedOutput.includes('Rate limit')) {
      ...
      this.emit('error', `Analysis failed (exit code ${code}). ${processor.stderrOutput.slice(-500)}`);
    }
  }
...
class OutputProcessor {
  ...
  private handleAnalysisResult(line: string, emitter: AutoRefactorService): void {
    try {
  ...
  private handleExecutionResult(line: string, emitter: AutoRefactorService): void {
    try {
