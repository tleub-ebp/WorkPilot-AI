...
export interface ContextMeshRequest {
  command: 'analyze' | 'register' | 'unregister' | 'summary' | 'recommendations';
  projectDir?: string;
  phase?: string;
  model?: string;
...
export class ContextMeshService extends EventEmitter {
  ...
  async execute(request: ContextMeshRequest): Promise<void> {
    ...
    if (!autoBuildSource) {
      this.emit('error', 'WorkPilot AI source not found. Cannot locate context_mesh_runner.py');
      return;
    }

    const runnerPath = path.join(autoBuildSource, 'runners', 'context_mesh_runner.py');
  ...
  private buildCommandArgs(request: ContextMeshRequest, runnerPath: string): string[] {
    const args = [runnerPath, request.command];

    if (request.projectDir && ['register', 'unregister', 'recommendations'].includes(request.command)) {
    ...
    proc.on('close', (code) => {
      ...
      if (code === 0 && processor.completeResult) {
        ...
        if (combinedOutput.includes('rate_limit') || combinedOutput.includes('Rate limit')) {
          ...
          this.emit('error', 'Authentication error. Please check your credentials in Settings.');
        } else {
          this.emit('error', `Context mesh operation failed (exit code ${code}). ${processor.stderrOutput.slice(-500)}`);
        }
      } else {
        this.emit('error', 'Context mesh operation completed but produced no output.');
      }
    });

    proc.on('error', (err) => {
      this.activeProcess = null;
      this.emit('error', `Failed to start context mesh operation: ${err.message}`);
    });
  }
}
