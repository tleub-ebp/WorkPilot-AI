...
export class LogService {
	private activeSessions: Map<
		string,
		{ sessionId: string; logPath: string; startedAt: Date }
	> = new Map();
	...
	startSession(taskId: string, specDir: string): string {
		...
		console.warn(
			`[LogService] Started session ${sessionId} for task ${taskId}`,
		);
		return sessionId;
	}
	...
	appendLog(taskId: string, content: string): void {
		...
		if (!session) {
			...
			console.warn(
				`[LogService] No active session for task ${taskId}, log will be lost`,
			);
			return;
		}
	...
	private flushBuffer(taskId: string): void {
		...
		try {
			...
			console.error(
				`[LogService] Failed to flush logs for task ${taskId}:`,
				error,
			);
		}
	}
	...
	endSession(taskId: string, exitCode?: number | null): void {
		...
		try {
			...
			console.error(
				`[LogService] Failed to end session for task ${taskId}:`,
				error,
			);
		}
		...
		console.warn(
			`[LogService] Ended session for task ${taskId}, exit code: ${exitCode}`,
		);
	}
