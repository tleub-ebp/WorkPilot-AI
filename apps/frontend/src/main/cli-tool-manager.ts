...
export type ExecFileAsyncOptionsWithVerbatim =
...
class CLIToolManager {
	...
	getToolPath(tool: CLITool): string {
		...
		if (result.found && result.path) {
			...
			console.warn(
				`[CLI Tools] Detected ${tool}: ${result.path} (${result.source})`,
			);
			return result.path;
		}
	...
	private detectGit(): ToolDetectionResult {
		...
		if (this.userConfig.gitPath) {
			...
			if (isWrongPlatformPath(this.userConfig.gitPath)) {
				...
				console.warn(
					`[Git] User-configured path invalid: ${validation.message}`,
				);
			}
		}
		...
		if (isWindows()) {
			...
			const windowsPaths = getWindowsExecutablePaths(
				WINDOWS_GIT_PATHS,
				"[Git]",
			);
	...
	private detectGitHubCLI(): ToolDetectionResult {
		...
		if (this.userConfig.githubCLIPath) {
			...
			if (isWrongPlatformPath(this.userConfig.githubCLIPath)) {
				...
				const validation = this.validateGitHubCLI(
	...
	private detectGitLabCLI(): ToolDetectionResult {
		...
		if (this.userConfig.gitlabCLIPath) {
			...
			if (isWrongPlatformPath(this.userConfig.gitlabCLIPath)) {
				...
				const validation = this.validateGitLabCLI(
	...
	private detectCopilot(): ToolDetectionResult {
		...
		if (this.userConfig.copilotPath) {
			if (isWrongPlatformPath(this.userConfig.copilotPath)) {
				...
				console.warn(
					`[Copilot CLI] User-configured path invalid: ${validation.message}`,
				);
			}
		}
		...
		if (ghResult.found && ghResult.path) {
			...
			console.warn(
				`[Copilot CLI] gh found but copilot extension not available: ${validation.message}`,
			);
		}
		...
		return {
			...
			source: "fallback",
			message:
				"Copilot CLI not found. Requires GitHub CLI (gh) with copilot extension: gh extension install github/gh-copilot",
		};
	}
	...
	private validateClaude(claudeCmd: string): ToolValidation {
		try {
			...
			if (needsShell) {
				...
				try {
					...
					if (isWindows() && isSecurePath(unquotedCmd)) {
						console.warn(
							`[Claude CLI] Direct exec failed, retrying via cmd.exe: ${directError instanceof Error ? directError.message : String(directError)}`,
						);
						...
						try {
							...
							// Last fallback: PowerShell with EncodedCommand — avoids backslash escaping issues
							// with WinGet portable packages in Electron sandboxed environments.
							console.warn(
								`[Claude CLI] cmd.exe fallback failed, retrying via PowerShell: ${cmdError instanceof Error ? cmdError.message : String(cmdError)}`,
							);
			...
			console.warn(
				`[Claude CLI] Validation succeeded for "${claudeCmd}": version ${versionStr}`,
			);
			return {
				valid: true,
				version: versionStr,
			...
			console.warn(
				`[Claude CLI] Validation FAILED for "${claudeCmd}": ${errorMsg}`,
			);
			...
			if (existsSync(unquotedCmdForCheck)) {
				...
				if (!isWindows() || isSecurePath(unquotedCmdForCheck)) {
					console.warn(
						`[Claude CLI] File exists at "${unquotedCmdForCheck}" — accepting with unknown version (exec failed: ${errorMsg})`,
					);
					return {
						valid: true,
						version: "unknown",
	 ...
	 */
	private async detectToolPathAsync(
		tool: CLITool,
	 ...
	 */
	private async validateClaudeAsync(
		claudeCmd: string,
		...
		try {
			...
			if (needsShell) {
				...
				try {
					...
					if (isWindows() && isSecurePath(unquotedCmd)) {
						console.warn(
							`[Claude CLI] Async direct exec failed, retrying via cmd.exe: ${directError instanceof Error ? directError.message : String(directError)}`,
						);
						const cmdExeFallback =
							process.env.ComSpec ||
							path.join(
			...
			console.warn(
				`[Claude CLI] Async validation succeeded for "${claudeCmd}": version ${versionStr}`,
			);
			return {
				valid: true,
				version: versionStr,
			...
			const errorMsg = error instanceof Error ? error.message : String(error);
			console.warn(
				`[Claude CLI] Async validation FAILED for "${claudeCmd}": ${errorMsg}`,
			);
	 ...
	 * @returns Promise resolving to validation result
	 */
	private async validatePythonAsync(
		pythonCmd: string,
	): Promise<ToolValidation> {
		const MINIMUM_VERSION = "3.10.0";
	 ...
	 * @returns Promise resolving to validation result
	 */
	private async validateGitLabCLIAsync(
		glabCmd: string,
	): Promise<ToolValidation> {
		try {
	...
	private async detectClaudeAsync(): Promise<ToolDetectionResult> {
		...
		if (this.userConfig.claudePath) {
			if (isWrongPlatformPath(this.userConfig.claudePath)) {
				...
				const validation = await this.validateClaudeAsync(
					this.userConfig.claudePath,
				);
				const result = buildClaudeDetectionResult(
					this.userConfig.claudePath,
	...
	private async detectCopilotAsync(): Promise<ToolDetectionResult> {
		...
		if (this.userConfig.copilotPath) {
			if (isWrongPlatformPath(this.userConfig.copilotPath)) {
				...
				const validation = await this.validateCopilotAsync(
					this.userConfig.copilotPath,
				);
				if (validation.valid) {
					return {
						found: true,
	...
	private async detectPythonAsync(): Promise<ToolDetectionResult> {
		...
		if (this.userConfig.pythonPath) {
			if (isWrongPlatformPath(this.userConfig.pythonPath)) {
				...
				const validation = await this.validatePythonAsync(
					this.userConfig.pythonPath,
				);
				if (validation.valid) {
					return {
						found: true,
	...
	private async detectGitAsync(): Promise<ToolDetectionResult> {
		...
		if (isWindows()) {
			const whereGitPath = await findWindowsExecutableViaWhereAsync(
				"git",
				"[Git]",
			);
			if (whereGitPath) {
	...
	private async detectGitHubCLIAsync(): Promise<ToolDetectionResult> {
		...
		if (this.userConfig.githubCLIPath) {
			if (isWrongPlatformPath(this.userConfig.githubCLIPath)) {
				...
				const validation = await this.validateGitHubCLIAsync(
					this.userConfig.githubCLIPath,
				);
				if (validation.valid) {
					return {
	...
	private async detectGitLabCLIAsync(): Promise<ToolDetectionResult> {
		...
		if (this.userConfig.gitlabCLIPath) {
			if (isWrongPlatformPath(this.userConfig.gitlabCLIPath)) {
				...
				const validation = await this.validateGitLabCLIAsync(
					this.userConfig.gitlabCLIPath,
				);
				if (validation.valid) {
					return {
		...
		// 2. Homebrew (macOS)
		if (isMacOS()) {
			const homebrewPaths = ["/opt/homebrew/bin/glab", "/usr/local/bin/glab"];
 ...
 * ```
 */
export async function preWarmToolCache(
	tools: CLITool[] = ["claude"],
): Promise<void> {
