...
import type {
	PlaygroundType,
	SandboxType,
} from "../renderer/stores/code-playground-store";
...
export class CodePlaygroundService extends EventEmitter {
	...
	async generate(request: CodePlaygroundRequest): Promise<void> {
		...
		if (!autoBuildSource) {
			this.emit(
				"error",
				"WorkPilot AI source not found. Cannot locate code_playground_runner.py",
			);
			return;
		}
		...
		const runnerPath = path.join(
			autoBuildSource,
			"runners",
			"code_playground_runner.py",
		);
		if (!existsSync(runnerPath)) {
			this.emit(
				"error",
				"code_playground_runner.py not found in auto-claude directory",
			);
			return;
		}
			...
			if (code === 0 && playgroundResult) {
				...
				if (fullOutput.trim()) {
					this.emit("complete", {
						...
						integrationNotes:
							"Basic HTML playground generated from raw output.",
					} as PlaygroundResult);
				} else {
					this.emit(
						"error",
						"Playground generation completed but produced no output.",
					);
				}
			} else {
				...
				if (
					...
					this.emit(
						"error",
						`Playground generation failed (exit code ${code}). ${stderrOutput.slice(-500)}`,
					);
				}
			}
		});
		...
		proc.on("error", (err) => {
			...
			this.emit(
				"error",
				`Failed to start playground generator: ${err.message}`,
			);
		});
	}
}
