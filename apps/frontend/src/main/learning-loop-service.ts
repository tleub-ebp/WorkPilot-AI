...
export class LearningLoopService extends EventEmitter {
  ...
  async analyze(request: LearningLoopRequest): Promise<void> {
    ...
    if (!autoBuildSource) {
      this.emit('error', 'WorkPilot AI source not found. Cannot locate learning_loop_runner.py');
      return;
    }

    const runnerPath = path.join(autoBuildSource, 'runners', 'learning_loop_runner.py');
    if (!existsSync(runnerPath)) {
      this.emit('error', 'learning_loop_runner.py not found in auto-claude directory');
      return;
    }
  ...
  private buildCommandArgs(request: LearningLoopRequest, runnerPath: string): string[] {
    const args = [
      runnerPath,
  ...
  private async executeProcess(args: string[], processEnv: Record<string, string>, cwd: string): Promise<void> {
  ...
  private setupProcessHandlers(proc: ChildProcess, processor: LearningLoopOutputProcessor): void {
    ...
    proc.on('error', (err) => {
      this.activeProcess = null;
      this.emit('error', `Failed to start learning loop analysis: ${err.message}`);
    });
  }

  private handleProcessCompletion(code: number | null, processor: LearningLoopOutputProcessor): void {
    if (code === 0 && processor.completeResult) {
      this.emit('complete', processor.completeResult);
    } else if (code && code > 0) {
      this.handleProcessError(code, processor);
    } else {
      this.emit('error', 'Learning loop analysis completed but produced no output.');
    }
  }

  private handleProcessError(code: number, processor: LearningLoopOutputProcessor): void {
    ...
    if (combinedOutput.includes('rate_limit') || combinedOutput.includes('Rate limit')) {
      ...
      this.emit('error', `Learning loop analysis failed (exit code ${code}). ${processor.stderrOutput.slice(-500)}`);
    }
  }
}
