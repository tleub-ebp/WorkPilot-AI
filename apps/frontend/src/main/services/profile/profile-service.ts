 ...
 */
export async function validateProfileNameUnique(
	name: string,
): Promise<boolean> {
...
export async function testConnection(
	...
	const getUrlSuggestions = (url: string): string[] => {
		...
		if (domainMatch) {
			...
			if (
				domain.includes("anthropiic") ||
				domain.includes("anthhropic") ||
				domain.includes("anhtropic") ||
				domain.length < 10
			) {
	...
	try {
		...
		try {
			...
			const modelsErrorName =
				modelsError instanceof Error ? modelsError.name : "";
			if (
				modelsErrorName === "NotFoundError" ||
				modelsError instanceof NotFoundError
			) {
				...
				try {
					...
					const messagesErrorName =
						messagesError instanceof Error ? messagesError.name : "";
					...
					if (
						messagesErrorName === "BadRequestError" ||
						messagesErrorName === "InvalidRequestError" ||
						(messagesError instanceof Error &&
							"status" in messagesError &&
...
export async function discoverModels(
	...
	if (!validateApiKey(apiKey)) {
		const error: Error & { errorType?: string } = new Error(
	...
	if (signal?.aborted) {
		const error: Error & { errorType?: string } = new Error(
			"Connection timeout. The endpoint did not respond.",
		);
	...
	try {
		...
		if (
			...
			const authError: Error & { errorType?: string } = new Error(
				"Authentication failed. Please check your API key.",
			);
		...
		if (errorName === "NotFoundError" || error instanceof NotFoundError) {
			const notSupportedError: Error & { errorType?: string } = new Error(
				"This API endpoint does not support model listing. Please enter the model name manually.",
			);
		...
		if (
			...
			const timeoutError: Error & { errorType?: string } = new Error(
				"Connection timeout. The endpoint did not respond.",
			);
		...
		if (
			...
			const networkError: Error & { errorType?: string } = new Error(
		...
		// APIError or other errors
		const unknownError: Error & { errorType?: string } = new Error(
