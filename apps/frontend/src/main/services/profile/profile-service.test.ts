...
vi.mock("./profile-manager", () => ({
	...
	atomicModifyProfiles: vi.fn(
		async (modifier: (file: ProfilesFile) => ProfilesFile) => {
...
describe("profile-service", () => {
	...
	describe("testConnection", () => {
		...
		it("should return auth error for invalid API key (401 response)", async () => {
			mockModelsList.mockRejectedValue(
				createMockError("AuthenticationError", "Unauthorized"),
			);
		...
		it("should return network error for connection refused", async () => {
			mockModelsList.mockRejectedValue(
				createMockError("APIConnectionError", "ECONNREFUSED"),
			);
		...
		it("should return timeout error for AbortError", async () => {
			mockModelsList.mockRejectedValue(
				createMockError("APIConnectionTimeoutError", "Timeout"),
			);
	...
	describe("discoverModels", () => {
		...
		it("should throw not_supported error for 404 response", async () => {
			mockModelsList.mockRejectedValue(
				createMockError("NotFoundError", "Not Found"),
			);
