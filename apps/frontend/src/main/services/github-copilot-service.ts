...
export class GitHubCopilotService extends EventEmitter {
  ...
  constructor() {
    super();
    this.configPath = path.join(app.getPath('userData'), 'github-copilot-config.json');
    this.loadConfig();
    this.initialize();
  }
  ...
  private async checkStatus(): Promise<void> {
    try {
      ...
      const { stdout: versionOutput } = await execAsync('gh --version', { timeout: 5000 });
      this.status.installed = true;
      this.status.version = versionOutput.trim();

      // Vérifier l'authentification
      try {
        const { stdout: authOutput } = await execAsync('gh auth status', { timeout: 5000 });
      ...
      try {
        const { stdout: tokenOutput } = await execAsync('gh auth token', { timeout: 5000 });
        if (tokenOutput.trim()) {
      ...
      this.status.authenticated = false;
      this.status.error = error instanceof Error ? error.message : 'GitHub CLI not available';
    }
  }
  ...
  async setToken(token: string): Promise<void> {
    try {
      ...
      if (!token.startsWith('ghp_') && !token.startsWith('gho_') && !token.startsWith('ghu_')) {
        throw new Error('Invalid GitHub token format. Token should start with ghp_, gho_, or ghu_');
      }
