...
export class TerminalManager {
			...
			(terminal, data) => this.handleTerminalData(terminal, data),
			{
				...
				onResumeNeeded: (terminalId, sessionId) => {
					...
					this.resumeClaudeAsync(terminalId, sessionId).catch((error) => {
						debugError(
							"[terminal-manager] Failed to resume Claude session:",
							error,
						);
					});
				},
	...
	write(id: string, data: string): void {
		debugLog(
			"[TerminalManager:write] Writing to terminal:",
			id,
			"data length:",
			data.length,
		);
	...
	async switchClaudeProfile(
		...
		return ClaudeIntegration.switchClaudeProfile(
			...
			profileId,
			this.getWindow,
			async (terminalId, cwd, profileId, dangerouslySkipPermissions) =>
				this.invokeClaudeAsync(
					terminalId,
					cwd,
					profileId,
					dangerouslySkipPermissions,
	...
	private handleTerminalData(terminal: TerminalProcess, data: string): void {
		TerminalEventHandler.handleTerminalData(
			terminal,
			data,
			this.eventCallbacks,
		);
