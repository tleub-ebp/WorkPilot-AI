...
export function claimSessionId(sessionId: string, terminalId: string): boolean {
	...
	if (existingClaim && existingClaim !== terminalId) {
		debugLog(
			"[SessionHandler] Session ID already claimed:",
			sessionId,
			"by terminal:",
			existingClaim,
		);
	...
	debugLog(
		"[SessionHandler] Claimed session ID:",
		sessionId,
		"for terminal:",
		terminalId,
	);
...
export function releaseSessionId(terminalId: string): void {
	for (const [sessionId, claimedBy] of claimedSessionIds.entries()) {
		if (claimedBy === terminalId) {
			...
			debugLog(
				"[SessionHandler] Released session ID:",
				sessionId,
				"from terminal:",
				terminalId,
			);
 ...
 */
export function findMostRecentClaudeSession(
	projectPath: string,
): string | null {
	...
	try {
		if (!fs.existsSync(claudeProjectDir)) {
			debugLog(
				"[SessionHandler] Claude project directory not found:",
				claudeProjectDir,
			);
			return null;
		}
		...
		if (files.length === 0) {
			debugLog(
				"[SessionHandler] No Claude session files found in:",
				claudeProjectDir,
			);
			return null;
		}
...
export function findClaudeSessionAfter(
	...
	try {
		...
		debugLog(
			"[SessionHandler] Found unclaimed session after timestamp:",
			sessionId,
			"excluded:",
			excludeSessionIds?.size ?? 0,
		);
		return sessionId;
	} catch (error) {
		debugError("[SessionHandler] Error finding Claude session:", error);
 ...
 */
export async function persistAllSessionsAsync(
...
export function captureClaudeSessionId(
	...
	const checkForSession = () => {
		...
		if (terminal.claudeSessionId) {
			debugLog(
				"[SessionHandler] Terminal already has session ID, stopping capture:",
				terminalId,
			);
			return;
		}
		...
		const sessionId = findClaudeSessionAfter(
			projectPath,
			startTime,
			claimedIds,
		);
		...
		if (sessionId) {
			...
			if (claimSessionId(sessionId, terminalId)) {
				...
				debugLog(
					"[SessionHandler] Captured and claimed Claude session ID:",
					sessionId,
					"for terminal:",
					terminalId,
				);
				...
				debugLog(
					"[SessionHandler] Session ID was claimed by another terminal, continuing to poll:",
					sessionId,
				);
			...
			debugLog(
				"[SessionHandler] Could not capture Claude session ID after",
				maxAttempts,
				"attempts for terminal:",
				terminalId,
			);
		}
	};
