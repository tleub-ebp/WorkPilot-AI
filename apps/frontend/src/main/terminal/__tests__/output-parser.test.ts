...
describe("output-parser", () => {
	describe("detectClaudeExit", () => {
		describe("should detect shell prompts (Claude has exited)", () => {
			const shellPrompts = [
				...
				{
					input: "  [user@host ~]$ ",
					desc: "bracket prompt with leading space",
				},
		...
		describe("should NOT detect these as exit (Claude is still active or these are Claude output)", () => {
			const notExitPatterns = [
				...
				{
					input: "The path is ~/config $HOME",
					desc: "path in explanation (should NOT match)",
				},
				{
					input: "Use arr[index] to access elements",
					desc: "array access in explanation",
				},
				...
				{ input: "File path: ~/projects/test.js", desc: "file path mid-line" },
				{
					...
					desc: "email in text (mid-line)",
				},
				{
					input: "user@example.com: please review this",
					desc: "email at line start with colon (should NOT match)",
				},
				{
					input: "admin@company.org: check the logs",
					desc: "email at line start with text after colon",
				},
				{
					input: "The variable $HOME is set to /Users/dev",
					desc: "shell var in explanation",
				},
				{
					input: "Example: (main) branch is default",
					desc: "branch name in explanation",
				},
	...
	describe("isClaudeExitOutput", () => {
		...
		it("does not match user@hostname in middle of line", () => {
			...
			expect(isClaudeExitOutput("Contact user@hostname.com for help")).toBe(
				false,
			);
		});
	...
	describe("extractClaudeSessionId", () => {
		...
		it('extracts session ID from "Resuming session: xxx"', () => {
			expect(extractClaudeSessionId("Resuming session: sess_456")).toBe(
				"sess_456",
			);
		});
	...
	describe("extractRateLimitReset", () => {
		it("extracts rate limit reset time", () => {
			...
			).toBe("Dec 17 at 6am (Europe/Oslo)");
			expect(
				extractRateLimitReset("Limit reached • resets Jan 1 at noon"),
			).toBe("Jan 1 at noon");
		});
	...
	describe("extractEmail", () => {
		...
		it('extracts email from "Authenticated as" with space before email', () => {
			...
			expect(extractEmail("Authenticated as user@example.com")).toBe(
				"user@example.com",
			);
		});

		it('extracts email from "user@example.com\'s Organization" format', () => {
			expect(extractEmail("andre@mikalsenai.no's Organization")).toBe(
				"andre@mikalsenai.no",
			);
			expect(extractEmail("user@example.com's Organization")).toBe(
				"user@example.com",
			);
		});
		...
		describe("ANSI escape code handling", () => {
			...
			it("strips OSC 8 hyperlink sequences from email", () => {
				...
				const withHyperlink =
					"\x1b]8;;mailto:andre@mikalsenai.no\x07andre@mikalsenai.no\x1b]8;;\x07's Organization";
			...
			it("strips mixed ANSI codes (CSI + OSC)", () => {
				...
				const mixed =
					"\x1b[1m\x1b]8;;mailto:andre@mikalsenai.no\x07andre@mikalsenai.no\x1b]8;;\x07\x1b[0m's Organization";
			...
			it("strips OSC window title sequences", () => {
				...
				const withTitle =
					"\x1b]0;Claude Code\x07andre@mikalsenai.no's Organization";
			...
			it("strips 256-color and true-color codes", () => {
				...
				const with256Color =
					"\x1b[38;5;123mandre\x1b[0m@mikalsenai.no's Organization";
				const withTrueColor =
					"\x1b[38;2;255;128;0mandre\x1b[0m@mikalsenai.no's Organization";
			...
			it("strips private mode CSI sequences", () => {
				...
				const withPrivateMode =
					"\x1b[?25handre@mikalsenai.no\x1b[?25l's Organization";
			...
			it("handles email with formatting inside the address", () => {
				...
				const colorInsideEmail =
					"andr\x1b[1me\x1b[0m@mikalsenai.no's Organization";
