...
class SessionPersistence {
	...
	initialize(): TerminalRecoveryInfo {
		...
		console.warn(
			`[SessionPersistence] Initialized with ${sessions.length} sessions`,
		);
		return this.getRecoveryInfo();
	}
	...
	loadSessions(): TerminalSessionState[] {
		...
		try {
			...
			if (data.version !== 2) {
				console.warn(
					"[SessionPersistence] Incompatible version, starting fresh",
				);
				return [];
			}
	...
	getRecoveryInfo(): TerminalRecoveryInfo {
		...
		return {
			...
			recoverableSessions: sessions.filter((s) => s.bufferFile || s.daemonPtyId)
				.length,
	...
	updateSessionMetadata(
		...
		updates: Partial<
			Pick<
				TerminalSessionState,
				"title" | "isClaudeMode" | "claudeSessionId" | "daemonPtyId"
			>
		>,
	): void {
	...
	saveBuffer(sessionId: string, serializedBuffer: string): void {
		...
		if (!session) {
			console.warn(
				`[SessionPersistence] Cannot save buffer - session ${sessionId} not found`,
			);
			return;
		}
		...
		try {
			...
			console.warn(
				`[SessionPersistence] Saved buffer for session ${sessionId} (${serializedBuffer.length} bytes)`,
			);
		} catch (error) {
			console.error(
				`[SessionPersistence] Failed to save buffer for ${sessionId}:`,
				error,
			);
		}
	}
	...
	loadBuffer(sessionId: string): string | null {
		...
		if (!fs.existsSync(bufferPath)) {
			console.warn(
				`[SessionPersistence] Buffer file missing: ${session.bufferFile}`,
			);
			return null;
		}
		...
		try {
			...
			console.error(
				`[SessionPersistence] Failed to load buffer for ${sessionId}:`,
				error,
			);
			return null;
		}
	...
	private deleteBufferFile(bufferFile: string): void {
		...
		if (fs.existsSync(bufferPath)) {
			try {
				...
				console.error(
					`[SessionPersistence] Failed to delete buffer file ${bufferFile}:`,
					error,
				);
			}
		}
	...
	private saveToDisk(): void {
		...
		try {
			...
			console.warn(
				`[SessionPersistence] Saved ${data.sessions.length} sessions to disk`,
			);
		} catch (error) {
	...
	cleanupOrphanedBuffers(): void {
		...
		try {
			...
			if (cleanedCount > 0) {
				console.warn(
					`[SessionPersistence] Cleaned up ${cleanedCount} orphaned buffer files`,
				);
			}
		} catch (error) {
			console.error(
				"[SessionPersistence] Failed to cleanup orphaned buffers:",
				error,
			);
		}
	}
