...
// Module imports
import { decryptToken, encryptToken } from "./claude-profile/token-encryption";
...
export class ClaudeProfileManager {
	...
	private migrateCorruptedEmails(): void {
		...
		for (const profile of this.data.profiles) {
			...
			if (configEmail && profile.email !== configEmail) {
				console.warn(
					"[ClaudeProfileManager] Migrating corrupted email for profile:",
					{
	...
	private populateSubscriptionMetadata(): void {
		...
		for (const profile of this.data.profiles) {
			...
			const result = updateProfileSubscriptionMetadata(
				profile,
				expandedConfigDir,
				{ onlyIfMissing: true },
			);
			...
			if (result.subscriptionTypeUpdated) {
				needsSave = true;
				console.warn(
					"[ClaudeProfileManager] Populated subscriptionType for profile:",
					{
						profileId: profile.id,
			...
			if (result.rateLimitTierUpdated) {
				needsSave = true;
				console.warn(
					"[ClaudeProfileManager] Populated rateLimitTier for profile:",
					{
						profileId: profile.id,
		...
		if (needsSave) {
			this.save();
			console.warn(
				"[ClaudeProfileManager] Subscription metadata population complete",
			);
		}
	}
	...
	private createDefaultData(): ProfileStoreData {
		...
		const sanitizedName = initialProfileName
			.toLowerCase()
			.replace(/[^a-z0-9]+/g, "-");
	...
	getSettings(): ClaudeProfileSettings {
		...
		const profilesWithAuth = this.data.profiles.map((profile) => ({
			...profile,
			isAuthenticated:
				this.isProfileAuthenticated(profile) || hasValidToken(profile),
		}));
		...
		if (!active) {
			...
			if (defaultProfile) {
				if (process.env.DEBUG === "true") {
					console.warn(
						"[ClaudeProfileManager] getActiveProfile - using default:",
						{
			...
			if (process.env.DEBUG === "true") {
				console.warn(
					"[ClaudeProfileManager] getActiveProfile - using fallback:",
					{
						id: fallback.id,
						name: fallback.name,
	...
	setActiveProfile(profileId: string): boolean {
		...
		if (!profile) {
			console.warn(
				"[ClaudeProfileManager] setActiveProfile failed - profile not found:",
				{ profileId },
			);
			return false;
		}
	...
	getActiveProfileEnv(): Record<string, string> {
		...
		if (profile?.configDir) {
			...
			if (process.env.DEBUG === "true") {
				console.warn(
					"[ClaudeProfileManager] Using CLAUDE_CONFIG_DIR for profile:",
					profile.name,
					expandedConfigDir,
				);
			}
	...
	shouldProactivelySwitch(profileId: string): {
		...
		const settings = this.getAutoSwitchSettings();
		const priorityOrder = this.getAccountPriorityOrder();
		return shouldProactivelySwitchImpl(
			profile,
			this.data.profiles,
			settings,
			priorityOrder,
	...
	getProfileEnv(profileId: string): Record<string, string> {
		...
		try {
			...
			if (credentials.token) {
				...
				if (process.env.DEBUG === "true") {
					console.warn(
						"[ClaudeProfileManager] Retrieved OAuth token from Keychain for profile:",
						profile.name,
					);
				}
			}
			...
			console.error(
				"[ClaudeProfileManager] Failed to retrieve credentials from Keychain:",
				error,
			);
	...
	clearMigratedProfile(profileId: string): void {
		...
		this.data.migratedProfileIds = this.data.migratedProfileIds.filter(
			(id) => id !== profileId,
		);
