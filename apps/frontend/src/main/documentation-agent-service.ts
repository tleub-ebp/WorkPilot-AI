...
export interface DocumentationAgentResult {
  ...
  results_by_type: Record<string, { status: string; file?: string; error?: string; endpoints_found?: number; inserted?: number }>;
  coverage_before: {
...
export class DocumentationAgentService extends EventEmitter {
  ...
  private getAutoBuildSourcePath(): string | null {
    ...
    for (const p of possiblePaths) {
      if (existsSync(path.join(p, 'runners', 'documentation_agent_runner.py'))) {
  ...
  async generateDocs(request: DocumentationAgentRequest): Promise<void> {
    ...
    if (!sourcePath) {
      this.emit('error', 'WorkPilot AI source not found. Cannot locate documentation_agent_runner.py');
      return;
    }
    const runnerPath = path.join(sourcePath, 'runners', 'documentation_agent_runner.py');
    ...
    if (request.docTypes?.length) args.push('--doc-types', request.docTypes.join(','));
  ...
  private async executeProcess(args: string[], env: Record<string, string>, cwd: string): Promise<void> {
    ...
    proc.stdout?.on('data', (data: Buffer) => {
      for (const line of data.toString('utf-8').split('\n')) {
        if (line.startsWith('__DOC_RESULT__:')) {
          try { result = JSON.parse(line.substring('__DOC_RESULT__:'.length)); this.emit('status', 'Documentation complete'); } catch { /* ignore */ }
        } else if (line.trim()) {
          fullOutput += line + '\n';
    ...
    proc.on('close', (code) => {
      ...
      if (code === 0 && result) {
        ...
        this.emit('complete', { status: 'success', doc_types_processed: [], generated_files: [], results_by_type: {}, coverage_before: { coverage_percent: 0 }, coverage_after: { coverage_percent: 0 }, outdated_found: 0, summary: {} });
      } else {
        this.emit('error', `Documentation generation failed (exit code ${code}). ${stderrOutput.slice(-500)}`);
      }
    });
