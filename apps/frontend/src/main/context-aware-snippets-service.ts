...
export class ContextAwareSnippetsService extends EventEmitter {
	...
	async generateSnippet(request: ContextAwareSnippetRequest): Promise<void> {
		...
		if (!autoBuildSource) {
			this.emit(
				"error",
				"WorkPilot AI source not found. Cannot locate context_aware_snippets_runner.py",
			);
			return;
		}
		...
		if (!existsSync(runnerPath)) {
			this.emit(
				"error",
				"context_aware_snippets_runner.py not found in auto-claude directory",
			);
			return;
		}
			...
			for (const line of lines) {
				...
				if (line.startsWith("__CONTEXT_AWARE_SNIPPET__:")) {
					try {
						...
						console.error(
							"[ContextAwareSnippets] Failed to parse result:",
							parseErr,
						);
					}
		...
		proc.on("close", (code) => {
			...
			if (code !== 0) {
				...
				if (
					...
					this.emit(
						"error",
						`Snippet generation failed (exit code ${code}). ${stderrOutput.slice(-500)}`,
					);
				}
				...
				this.emit("complete", {
					...
					reasoning:
						"The generator completed but did not produce a structured result.",
				} as ContextAwareSnippetResult);
			} else {
				this.emit(
					"error",
					"Snippet generation completed but produced no output.",
				);
			}
		});
