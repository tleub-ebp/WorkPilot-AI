...
import { getSpawnCommand, getSpawnOptions, shouldUseShell } from "../env-utils";
...
describe("shouldUseShell", () => {
	...
	describe("Windows platform", () => {
		...
		it("should return true for .cmd files", () => {
			...
			expect(
				shouldUseShell("C:\\Users\\admin\\AppData\\Roaming\\npm\\claude.cmd"),
			).toBe(true);
		});
...
describe("getSpawnCommand", () => {
	...
	describe("Windows platform", () => {
		...
		it("should quote .bat files with spaces", () => {
			const cmd = getSpawnCommand(
				"D:\\Program Files (x86)\\scripts\\setup.bat",
			);
		...
		it("should handle uppercase .CMD and .BAT extensions", () => {
			expect(getSpawnCommand("D:\\Tools\\CLAUDE.CMD")).toBe(
				'"D:\\Tools\\CLAUDE.CMD"',
			);
			expect(getSpawnCommand("C:\\Scripts\\SETUP.BAT")).toBe(
				'"C:\\Scripts\\SETUP.BAT"',
			);
		});
	...
	describe("Non-Windows platforms", () => {
		it("should NOT quote commands on macOS", () => {
			...
			expect(getSpawnCommand("/usr/local/bin/claude")).toBe(
				"/usr/local/bin/claude",
			);
			expect(getSpawnCommand("/opt/homebrew/bin/claude.cmd")).toBe(
				"/opt/homebrew/bin/claude.cmd",
			);
		});
		...
		it("should NOT quote commands on Linux", () => {
			...
			expect(getSpawnCommand("/home/user/.local/bin/claude.bat")).toBe(
				"/home/user/.local/bin/claude.bat",
			);
		});
		...
		it("should trim whitespace on macOS", () => {
			...
			expect(getSpawnCommand("\t/opt/homebrew/bin/claude\t")).toBe(
		...
		it("should trim whitespace on Linux", () => {
			...
			expect(getSpawnCommand("  /usr/bin/claude  ")).toBe("/usr/bin/claude");
			expect(getSpawnCommand("\t/home/user/.local/bin/claude\t")).toBe(
