...
function createMockProfileManager(
	...
	const bestProfile =
		options.bestAvailableProfile === undefined
			? profiles.mu
			: options.bestAvailableProfile;

	return {
		getActiveProfile: vi.fn(
			() => profiles[activeId === "profile-mai" ? "mai" : "mu"],
		),
	...
	if (exitCode !== 0) {
		...
		if (isRateLimited) {
			...
			if (settings.enabled && settings.autoSwitchOnRateLimit) {
				const bestProfile =
					profileManager.getBestAvailableProfile("current-profile");

				if (bestProfile) {
					// Auto-swap
					profileManager.setActiveProfile(bestProfile.id);
					result.autoSwapped = true;
					result.swappedToProfile = {
						id: bestProfile.id,
...
Stack trace follows`;
	...
	describe("Auto-Switch Settings Verification", () => {
		...
		it("should have both enabled and autoSwitchOnRateLimit for auto-recovery", () => {
			...
			const shouldAutoRecover =
				settings.enabled && settings.autoSwitchOnRateLimit;
	...
	describe("Profile Scoring and Selection", () => {
		it("should return alternative profile when one is available", () => {
			const bestProfile =
				mockProfileManager.getBestAvailableProfile("profile-mai");
		...
		it("should return null when no alternative profile is available", () => {
			...
			const bestProfile =
				noAlternativeManager.getBestAvailableProfile("profile-mai");
	...
	describe("Event Emission Verification", () => {
		...
		it("should handle event chain: rate-limit → swap → restart", () => {
			...
			emitter.on("auto-swap-restart-task", () =>
				events.push("auto-swap-restart-task"),
			);

			// Simulate the flow
			emitter.emit("sdk-rate-limit", {
				/* info */
			});
			emitter.emit("auto-swap-restart-task", "task-123", "profile-mu");
...
describe("Rate Limit Edge Cases", () => {
	...
	describe("Pattern Matching Edge Cases", () => {
		...
		it("should handle different bullet characters", async () => {
			...
			// Middle dot (·)
			expect(detectRateLimit("Limit reached · resets 5pm").isRateLimited).toBe(
				true,
			);
		...
		it("should handle 12-hour and 24-hour time formats", async () => {
			...
			expect(
				detectRateLimit("Limit reached · resets 11:59pm").isRateLimited,
			).toBe(true);
			expect(detectRateLimit("Limit reached · resets 6am").isRateLimited).toBe(
				true,
			);
			expect(
				detectRateLimit("Limit reached · resets 18:00").isRateLimited,
			).toBe(true);
		});
	...
	describe("Both Profiles Rate Limited", () => {
		...
		it("should show manual modal when no profiles available", () => {
			...
			const bestProfile =
				bothLimitedManager.getBestAvailableProfile("profile-mai");

			// Even with auto-switch enabled, should show modal since no alternative
			const shouldShowManualModal =
				settings.enabled && settings.autoSwitchOnRateLimit && !bestProfile;
