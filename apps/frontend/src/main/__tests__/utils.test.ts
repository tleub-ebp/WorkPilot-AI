...
describe("safeSendToRenderer", () => {
	...
	beforeEach(async () => {
		...
		const { _clearWarnTimestampsForTest } = await import(
			"../ipc-handlers/utils"
		);
	...
	describe("warning cooldown behavior", () => {
		it("returns false for multiple consecutive calls to destroyed windows", () => {
			...
			const result1 = safeSendToRenderer(
				getMainWindow,
				"test-channel",
				"data1",
			);
			const result2 = safeSendToRenderer(
				getMainWindow,
				"test-channel",
				"data2",
			);
			const result3 = safeSendToRenderer(
				getMainWindow,
				"test-channel",
				"data3",
			);
		...
		it("logs console.warn only once for multiple consecutive calls to same channel", () => {
			...
			expect(consoleWarnSpy).toHaveBeenCalledWith(
				expect.stringContaining(
					"Skipping send to destroyed window: test-channel",
				),
			);
