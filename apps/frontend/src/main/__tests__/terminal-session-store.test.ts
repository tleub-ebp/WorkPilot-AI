...
function initTestPaths(): void {
	...
	TEST_DIR = mkdtempSync(
		path.join(os.tmpdir(), "terminal-session-store-test-"),
	);
...
function createValidStoreData(
	sessionsByDate: Record<string, Record<string, unknown[]>> = {},
): string {
...
describe("TerminalSessionStore", () => {
	...
	describe("initialization", () => {
		it("should create sessions directory if not exists", async () => {
			...
			const { TerminalSessionStore } = await import(
	...
	describe("atomic writes", () => {
		...
		it("should rotate current file to backup before overwriting", async () => {
			...
			const backupSessions =
				backupContent.sessionsByDate[today][TEST_PROJECT_PATH];
			expect(
				backupSessions.some((s: { id: string }) => s.id === "original-session"),
			).toBe(true);
		});
	...
	describe("race condition prevention", () => {
		...
		it("should track session in pendingDelete after removal", async () => {
			...
			const result = await store.saveSessionAsync(
	...
	describe("write serialization", () => {
		it("should serialize concurrent async writes", async () => {
			...
			const promises = [
				...
				store.saveSessionAsync(
					createTestSession({ id: "session-2", title: "Second" }),
				),
				store.saveSessionAsync(
					createTestSession({ id: "session-3", title: "Third" }),
				),
			];
	...
	describe("date-based organization", () => {
		...
		it("should filter available dates by project", async () => {
			...
			store.saveSession(
				createTestSession({ id: "other", projectPath: otherProjectPath }),
			);
