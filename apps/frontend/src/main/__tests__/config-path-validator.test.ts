...
describe("isValidConfigDir - Security Validation", () => {
	...
	describe("Valid paths - Should ACCEPT", () => {
		test("accepts paths within home directory", () => {
			...
			expect(isValidConfigDir(path.join(homeDir, "Documents", "configs"))).toBe(
				true,
			);
			expect(
				isValidConfigDir(path.join(homeDir, "any", "nested", "path")),
			).toBe(true);
		});
		...
		test("accepts paths within ~/.claude", () => {
			...
			expect(isValidConfigDir(path.join(homeDir, ".claude", "config"))).toBe(
				true,
			);
			expect(
				isValidConfigDir(
					path.join(homeDir, ".claude", "deep", "nested", "path"),
				),
			).toBe(true);
		...
		test("accepts ~/.claude-profiles directory", () => {
			...
			expect(isValidConfigDir(path.join(homeDir, ".claude-profiles"))).toBe(
				true,
			);
		...
		test("accepts paths within ~/.claude-profiles", () => {
			...
			expect(
				isValidConfigDir(path.join(homeDir, ".claude-profiles", "profile1")),
			).toBe(true);
			expect(
				isValidConfigDir(
					path.join(homeDir, ".claude-profiles", "profile2", "config"),
				),
			).toBe(true);
		...
		test("accepts paths with . and .. that resolve within boundaries", () => {
			...
			expect(
				isValidConfigDir(path.join(homeDir, ".claude", "foo", "..", "bar")),
			).toBe(true);
			expect(isValidConfigDir("~/.claude/foo/../bar")).toBe(true);

			// Path that navigates but stays within bounds
			expect(
				isValidConfigDir(path.join(homeDir, "Documents", "..", "Downloads")),
			).toBe(true);
		});
	});
	...
	describe("Path traversal attacks - Should REJECT", () => {
		...
		test("rejects paths that traverse beyond home directory boundaries", () => {
			...
			expect(
				isValidConfigDir(path.join(homeDir, "Documents", "..", "..", "etc")),
			).toBe(false);
			expect(
				isValidConfigDir(path.join(homeDir, ".claude", "..", "..", "usr")),
			).toBe(false);
	...
	describe("Boundary attack vectors - Should REJECT", () => {
		...
		test("enforces path separator boundary checks", () => {
			...
			if (
				!similarName.startsWith(homeDir + path.sep) &&
				similarName !== homeDir
			) {
	...
	describe("Edge cases and special inputs", () => {
		test("handles empty string based on cwd resolution", () => {
			...
			const homeDir = os.homedir();
			const shouldBeValid =
				resolvedPath === homeDir || resolvedPath.startsWith(homeDir + path.sep);
		...
		test("handles relative paths based on cwd resolution", () => {
			...
			if (cwdInHome) {
				...
				const parentShouldBeValid =
					parentDir === homeDir || parentDir.startsWith(homeDir + path.sep);
	...
	describe("Real-world attack scenarios", () => {
		test("prevents symbolic link style attacks via path traversal", () => {
			...
			expect(isValidConfigDir("~/.claude/../../../../../etc/passwd")).toBe(
				false,
			);
		});
	...
	describe("Tilde expansion behavior", () => {
		test("expands tilde to home directory before validation", () => {
			...
			expect(isValidConfigDir("~/.claude")).toBe(
				isValidConfigDir(path.join(homeDir, ".claude")),
			);
			expect(isValidConfigDir("~/Documents")).toBe(
				isValidConfigDir(path.join(homeDir, "Documents")),
			);
		});
