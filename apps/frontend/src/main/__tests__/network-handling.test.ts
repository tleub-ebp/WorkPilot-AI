...
describe("Network Error Handling", () => {
  beforeEach(() => {
    ...
    if (typeof globalThis !== 'undefined' && (globalThis as Record<string, unknown>).cachedLatestVersion) {
  ...
  it("should retry on network errors and eventually succeed", async () => {
    ...
    const { isNetworkError } = await import("../ipc-handlers/claude-code-handlers");
  ...
  it("should handle proxy configuration", async () => {
    ...
    try {
      ...
      const { createProxiedFetch } = await import("../ipc-handlers/claude-code-handlers");
  ...
  it("should use fallback registry when primary fails", async () => {
    ...
    await expect(mockFetch("https://registry.npmjs.org/test")).rejects.toThrow("ECONNRESET");

    // Second call fails with socket disconnected
    await expect(mockFetch("https://registry.npmjs.org/test")).rejects.toThrow("socket disconnected");
