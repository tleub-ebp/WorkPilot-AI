...
describe("SETTINGS_CLAUDE_CODE_GET_ONBOARDING_STATUS handler", () => {
	...
	beforeEach(async () => {
		...
		const { registerSettingsHandlers } = await import(
			"../ipc-handlers/settings-handlers"
		);
		...
		if (!handler) {
			throw new Error(
				"SETTINGS_CLAUDE_CODE_GET_ONBOARDING_STATUS handler not registered",
			);
		}
	...
	describe("error handling", () => {
		test("should handle read errors gracefully", async () => {
			...
			const originalExistsSync: ExistsSyncFn =
				(existsSyncMock.getMockImplementation() as ExistsSyncFn | undefined) ??
				(() => false);
			const originalReadFileSync: ReadFileSyncFn =
				(readFileSyncMock.getMockImplementation() as
					| ReadFileSyncFn
					| undefined) ?? (() => "");
