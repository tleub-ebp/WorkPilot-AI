...
describe("Phase Event Parser", () => {
	...
	describe("parsePhaseEvent", () => {
		describe("Basic Parsing", () => {
			it("should parse valid phase event", () => {
				const line =
					'__EXEC_PHASE__:{"phase":"coding","message":"Starting implementation"}';
			...
			it("should handle marker at start of line", () => {
				const line =
					'__EXEC_PHASE__:{"phase":"planning","message":"Creating plan"}';
				const result = parsePhaseEvent(line);
			...
			it("should handle marker with prefix text", () => {
				const line =
					'[2024-01-01 12:00:00] INFO: __EXEC_PHASE__:{"phase":"coding","message":"Working"}';
				const result = parsePhaseEvent(line);
			...
			it("should handle ANSI color codes around JSON by extracting valid JSON", () => {
				const line =
					'\x1b[32m__EXEC_PHASE__:{"phase":"coding","message":"Test"}\x1b[0m';
				const result = parsePhaseEvent(line);
		...
		describe("Message Handling", () => {
			it("should extract message field", () => {
				const line =
					'__EXEC_PHASE__:{"phase":"coding","message":"Building feature X"}';
				const result = parsePhaseEvent(line);
			...
			it("should handle unicode in message", () => {
				const line =
					'__EXEC_PHASE__:{"phase":"coding","message":"Building 🚀 feature with émojis"}';
				const result = parsePhaseEvent(line);
			...
			it("should handle escaped quotes in message", () => {
				const line =
					'__EXEC_PHASE__:{"phase":"coding","message":"Message with \\"quotes\\""}';
				const result = parsePhaseEvent(line);
			...
			it("should handle escaped newlines in message (JSON format)", () => {
				const line =
					'__EXEC_PHASE__:{"phase":"coding","message":"Line1\\nLine2"}';
				const result = parsePhaseEvent(line);
			...
			it("should handle escaped backslashes", () => {
				const line =
					'__EXEC_PHASE__:{"phase":"coding","message":"path\\\\to\\\\file"}';
				const result = parsePhaseEvent(line);
		...
		describe("Optional Fields", () => {
			it("should extract progress when present", () => {
				const line =
					'__EXEC_PHASE__:{"phase":"coding","message":"Working","progress":50}';
				const result = parsePhaseEvent(line);
			...
			it("should handle progress of 0", () => {
				const line =
					'__EXEC_PHASE__:{"phase":"coding","message":"Starting","progress":0}';
				const result = parsePhaseEvent(line);
			...
			it("should handle progress of 100", () => {
				const line =
					'__EXEC_PHASE__:{"phase":"complete","message":"Done","progress":100}';
				const result = parsePhaseEvent(line);
			...
			it("should reject progress below 0", () => {
				const line =
					'__EXEC_PHASE__:{"phase":"coding","message":"Working","progress":-1}';
				const result = parsePhaseEvent(line);
				expect(result).toBeNull();
			});
			...
			it("should reject progress above 100", () => {
				const line =
					'__EXEC_PHASE__:{"phase":"coding","message":"Working","progress":101}';
				const result = parsePhaseEvent(line);
				expect(result).toBeNull();
			});
			...
			it("should reject non-integer progress", () => {
				const line =
					'__EXEC_PHASE__:{"phase":"coding","message":"Working","progress":50.5}';
				const result = parsePhaseEvent(line);
				expect(result).toBeNull();
			});
			...
			it("should extract subtask when present", () => {
				const line =
					'__EXEC_PHASE__:{"phase":"coding","message":"Working","subtask":"task-123"}';
				const result = parsePhaseEvent(line);
			...
			it("should handle subtask with special characters", () => {
				const line =
					'__EXEC_PHASE__:{"phase":"coding","message":"Working","subtask":"feat/add-login#123"}';
				const result = parsePhaseEvent(line);
			...
			it("should reject non-string subtask", () => {
				const line =
					'__EXEC_PHASE__:{"phase":"coding","message":"Working","subtask":123}';
				const result = parsePhaseEvent(line);
				expect(result).toBeNull();
			});
			...
			it("should handle all optional fields together", () => {
				const line =
					'__EXEC_PHASE__:{"phase":"coding","message":"Working","progress":75,"subtask":"feat-1"}';
				const result = parsePhaseEvent(line);
			...
			it("should ignore unknown fields", () => {
				const line =
					'__EXEC_PHASE__:{"phase":"coding","message":"Working","unknown":"field","extra":123}';
				const result = parsePhaseEvent(line);
				expect(result).not.toBeNull();
				expect(result?.phase).toBe("coding");
		...
		describe("Error Handling", () => {
			...
			it("should handle whitespace after marker", () => {
				const line =
					'__EXEC_PHASE__:  {"phase":"coding","message":"With spaces"}';
				const result = parsePhaseEvent(line);
				expect(result).not.toBeNull();
				expect(result?.phase).toBe("coding");
			});
		...
		describe("Real-world Scenarios", () => {
			...
			it("should parse typical coding event with subtask", () => {
				const line =
					'__EXEC_PHASE__:{"phase":"coding","message":"Implementing feature","subtask":"1/3","progress":33}';
				const result = parsePhaseEvent(line);
				expect(result?.phase).toBe("coding");
			...
			it("should parse QA review event", () => {
				const line =
					'__EXEC_PHASE__:{"phase":"qa_review","message":"Running QA validation iteration 1"}';
				const result = parsePhaseEvent(line);
				expect(result?.phase).toBe("qa_review");
			});
			...
			it("should parse QA fixing event", () => {
				const line =
					'__EXEC_PHASE__:{"phase":"qa_fixing","message":"Fixing QA issues"}';
				const result = parsePhaseEvent(line);
				expect(result?.phase).toBe("qa_fixing");
			});
			...
			it("should parse complete event", () => {
				const line =
					'__EXEC_PHASE__:{"phase":"complete","message":"QA validation passed","progress":100}';
				const result = parsePhaseEvent(line);
				expect(result?.phase).toBe("complete");
				expect(result?.progress).toBe(100);
			});
			...
			it("should parse failed event with error message", () => {
				const line =
					'__EXEC_PHASE__:{"phase":"failed","message":"Build failed: TypeError: Cannot read property of undefined"}';
				const result = parsePhaseEvent(line);
				expect(result?.phase).toBe("failed");
	...
	describe("Type Safety", () => {
		it("should return correct PhaseEvent type", () => {
			const line =
				'__EXEC_PHASE__:{"phase":"coding","message":"Test","progress":50,"subtask":"t1"}';
			const result = parsePhaseEvent(line);
