...
import {
	...
	validateArgs,
} from "../../../scripts/package-with-python.cjs";
	...
	describe("on Windows (shell injection risk)", () => {
		...
		describe("should throw for shell metacharacters", () => {
			...
			it("should include offending argument in error message", () => {
				expect(() => validateArgs(["file&evil.exe"])).toThrowError(
					/Argument: "file&evil\.exe"/,
				);
			});
		});
		...
		describe("should throw for non-string arguments", () => {
			...
			it("should throw TypeError for undefined argument", () => {
				...
				expect(() => validateArgs([undefined])).toThrowError(
					/must be a string/,
				);
			});
			...
			it("should throw TypeError for object argument", () => {
				...
				expect(() => validateArgs([{ key: "value" }])).toThrowError(
					/got object/,
				);
			});
		...
		describe("should NOT throw for safe inputs", () => {
			...
			it("should allow alphanumeric arguments", () => {
				expect(() =>
					validateArgs(["build", "test", "production"]),
				).not.toThrow();
			});

			it("should allow flag arguments", () => {
				expect(() =>
					validateArgs(["--win", "--x64", "--publish=never"]),
				).not.toThrow();
			});
			...
			it("should allow dots and hyphens", () => {
				expect(() =>
					validateArgs(["--config.file", "my-config.json"]),
				).not.toThrow();
			});

			it("should allow underscores", () => {
				expect(() =>
					validateArgs(["my_config_file", "--output_dir"]),
				).not.toThrow();
			});
