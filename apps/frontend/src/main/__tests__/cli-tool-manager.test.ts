...
type SpawnOptions = Parameters<
	typeof import("../env-utils")["getSpawnOptions"]
>[1];
type MockDirent = import("node:fs").Dirent<
...
const {
	...
	const mockExecFile = vi.fn(
		(_cmd: unknown, _args: unknown, _options: unknown, callback: unknown) => {
			const childProcess = {
				stdout: { on: vi.fn() },
				stderr: { on: vi.fn() },
				on: vi.fn(),
	...
	const mockExec = vi.fn(
		(_cmd: unknown, _options: unknown, callback: unknown) => {
			const childProcess = {
				stdout: { on: vi.fn() },
				stderr: { on: vi.fn() },
				on: vi.fn(),
...
describe("cli-tool-manager - Claude CLI NVM detection", () => {
	...
	describe("NVM path detection on Unix/Linux/macOS", () => {
		...
		it("should validate Claude CLI before returning NVM path", () => {
			...
			const mockDirents: MockDirent[] = [createDirent("v22.17.0", true)];
			vi.mocked(readdirSync).mockReturnValue(mockDirents);
	...
	describe("Platform-specific path detection", () => {
		...
		it("should ignore insecure Windows Claude CLI path from where.exe", () => {
			...
			expect(isSecurePath).toHaveBeenCalledWith(
				String.raw`D:\Tools\claude.cmd`,
			);
		});
		...
		it("should detect Claude CLI in Unix .local/bin path", () => {
			...
			vi.mocked(existsSync).mockImplementation((filePath) => {
				const pathStr = String(filePath);
				if (
					pathStr.includes(".local/bin/claude") ||
					pathStr.includes(".local\\bin\\claude")
				) {
					return true;
				}
...
describe("cli-tool-manager - Helper Functions", () => {
	describe("getClaudeDetectionPaths", () => {
		...
		it("should return Windows paths on win32", () => {
			...
			expect(paths.platformPaths.some((p) => p.includes("Program Files"))).toBe(
				true,
			);
		});
		...
		it("should return Unix paths on Linux", () => {
			...
			expect(
				paths.platformPaths.some(
					(p) =>
						p.includes(".local") && p.includes("bin") && p.includes("claude"),
				),
			).toBe(true);
			expect(
				paths.platformPaths.some(
					(p) => p.includes("bin") && p.includes("claude"),
				),
			).toBe(true);
		});
...
describe("cli-tool-manager - Claude CLI async Windows where.exe detection", () => {
	...
	it("should detect Claude CLI via where.exe asynchronously", async () => {
		...
		expect(findWindowsExecutableViaWhereAsync).toHaveBeenCalledWith(
			"claude",
			"[Claude CLI]",
		);
	});
