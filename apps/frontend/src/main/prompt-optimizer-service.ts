...
export class PromptOptimizerService extends EventEmitter {
  ...
  async optimize(request: PromptOptimizeRequest): Promise<void> {
    ...
    if (!autoBuildSource) {
      this.emit('error', 'WorkPilot AI source not found. Cannot locate prompt_optimizer_runner.py');
      return;
    }

    const runnerPath = path.join(autoBuildSource, 'runners', 'prompt_optimizer_runner.py');
    if (!existsSync(runnerPath)) {
      this.emit('error', 'prompt_optimizer_runner.py not found in auto-claude directory');
      return;
    }
      ...
      for (const line of lines) {
        ...
        if (line.startsWith('__OPTIMIZED_PROMPT__:')) {
          try {
            ...
            console.error('[PromptOptimizer] Failed to parse result:', parseErr);
          }
    ...
    proc.on('close', (code) => {
      ...
      if (code === 0 && optimizerResult) {
        ...
        if (combinedOutput.includes('rate_limit') || combinedOutput.includes('Rate limit')) {
          ...
          this.emit('error', `Optimization failed (exit code ${code}). ${stderrOutput.slice(-500)}`);
        }
      } else {
        ...
        if (fullOutput.trim()) {
          this.emit('complete', {
            ...
            changes: ['Raw optimization output (structured parsing unavailable)'],
            reasoning: 'The optimizer completed but did not produce a structured result.',
