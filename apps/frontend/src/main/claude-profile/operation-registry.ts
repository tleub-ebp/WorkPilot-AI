...
class ClaudeOperationRegistry extends EventEmitter {
	...
	registerOperation(
		...
		const operation: RegisteredOperation = {
			...
			estimatedCapacityPerHour:
				options?.estimatedCapacityPerHour ?? DEFAULT_CAPACITY_ESTIMATES[type],
		};
		...
		for (const op of operations) {
			try {
				...
				if (success) {
					...
					console.warn(
						"[OperationRegistry] Operation restart returned false:",
						op.id,
					);
				}
			} catch (error) {
				console.error(
					"[OperationRegistry] Failed to restart operation:",
					op.id,
					error,
				);
			}
	...
	onOperationRegistered(
		callback: (operation: RegisteredOperation) => void,
	): () => void {
	 ...
	 */
	onOperationUnregistered(
		callback: (operationId: string, type: OperationType) => void,
	): () => void {
