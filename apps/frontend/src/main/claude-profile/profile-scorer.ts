	...
	if (params.isRateLimited) {
		...
		if (params.resetAt) {
			const hoursUntilReset =
				(params.resetAt.getTime() - Date.now()) / (1000 * 60 * 60);
...
function calculateFallbackScore(
	...
	if (profile.usage) {
		const weeklyOverage = Math.max(
			0,
			profile.usage.weeklyUsagePercent - settings.weeklyThreshold,
		);
		const sessionOverage = Math.max(
			0,
			profile.usage.sessionUsagePercent - settings.sessionThreshold,
		);
...
export function getBestAvailableProfile(
	...
	const candidates = profiles.filter(
		(p) => p.id !== excludeProfileId && !lockedIds.has(p.id),
	);
	...
	if (isDebug) {
		console.warn(
			"[ProfileScorer] Evaluating",
			candidates.length,
			"candidate profiles (excluding:",
			excludeProfileId,
			", locked:",
		...
		console.warn(
			"[ProfileScorer] Thresholds: session =",
	...
	const scoredProfiles: ScoredProfile[] = candidates.map((profile) => {
		...
		if (isDebug) {
			...
			console.warn(
				"[ProfileScorer]   Priority index:",
				priorityIndex === -1 ? "not in list (Infinity)" : priorityIndex,
			);
			console.warn(
				"[ProfileScorer]   Available:",
			...
			console.warn(
				"[ProfileScorer]   Usage:",
				profile.usage
					? `session=${profile.usage.sessionUsagePercent}%, weekly=${profile.usage.weeklyUsagePercent}%`
					: "unknown",
			);
			console.warn("[ProfileScorer]   Fallback score:", fallbackScore);
			if (velocityData) {
				console.warn(
					"[ProfileScorer]   Velocity:",
					`session=${velocityData.sessionVelocity.toFixed(2)}%/min, weekly=${velocityData.weeklyVelocity.toFixed(2)}%/min (${velocityData.confidence})`,
				);
			}
			if (reservedCapacity) {
				console.warn(
					"[ProfileScorer]   Reserved capacity:",
					reservedCapacity,
					"%/hour",
				);
			}
	...
	if (best.isAvailable) {
		console.warn(
			"[ProfileScorer] Best available profile:",
			best.profile.name,
			"(priority index:",
			best.priorityIndex,
			", score:",
	...
	if (best.score > 0) {
		console.warn(
			"[ProfileScorer] No ideal profile available, using least-bad option:",
			best.profile.name,
			"(score:",
			best.score,
			", reason:",
...
export function shouldProactivelySwitch(
	...
	if (usage.weeklyUsagePercent >= settings.weeklyThreshold) {
		const bestProfile = getBestAvailableProfile(
			allProfiles,
			settings,
			profile.id,
			priorityOrder,
 ...
 */
export function getProfilesSortedByAvailability(
	profiles: ClaudeProfile[],
): ClaudeProfile[] {
