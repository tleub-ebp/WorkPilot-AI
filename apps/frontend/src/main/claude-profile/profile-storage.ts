...
function migrateProfileToIsolatedDirectory(profile: ClaudeProfile): string {
	...
	const baseName =
		profile.name.toLowerCase().replace(/[^a-z0-9]+/g, "-") || "primary";
	...
	console.warn(
		`[ProfileStorage] Migrated profile "${profile.name}" from ~/.claude to ${isolatedDir}`,
	);
	console.warn(
		"[ProfileStorage] NOTE: Credentials remain at ~/.claude - user should re-authenticate in Settings > Accounts",
	);
 ...
 */
function parseAndMigrateProfileData(
	...
	if (data.version === STORE_VERSION) {
		...
		data.profiles = profiles.map((p: ClaudeProfile) => {
			...
			if (p.oauthToken) {
				console.warn(
					"[ProfileStorage] Migrating profile - removing cached oauthToken:",
					p.name,
				);
			}
		...
		const existingMigrated =
			(data.migratedProfileIds as string[] | undefined) || [];
		const allMigratedIds = [
			...new Set([...existingMigrated, ...newlyMigratedProfileIds]),
		];
