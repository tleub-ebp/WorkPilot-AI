 ...
 */
export async function createProfileDirectory(
	profileName: string,
): Promise<string> {
	...
	if (existsSync(claudeJsonPath)) {
		try {
			...
			if (
				data &&
				typeof data === "object" &&
				(data.oauthAccount?.accountUuid || data.oauthAccount?.emailAddress)
			) {
				...
				console.warn(
					`[profile-utils] Profile has .claude.json but no platform credentials for: ${configDir}, checking .credentials.json fallback...`,
				);
			}
		} catch (error) {
			// Log parse errors for debugging, but fall through to legacy checks
			console.warn(
				`[profile-utils] Failed to read or parse ${claudeJsonPath}:`,
				error,
			);
		}
	}
	...
	if (existsSync(credentialsJsonPath)) {
		try {
			...
			if (
				data &&
				typeof data === "object" &&
				(data.accessToken || data.refreshToken || data.token)
			) {
				return true;
			}
		} catch (error) {
			// Log parse errors for debugging, but fall through to legacy checks
			console.warn(
				`[profile-utils] Failed to read or parse ${credentialsJsonPath}:`,
				error,
			);
		}
	}
...
export function hasValidToken(profile: ClaudeProfile): boolean {
	...
	console.warn(
		"[hasValidToken] DEPRECATED: Profile has cached oauthToken. Using CLAUDE_CONFIG_DIR for fresh tokens.",
	);
	return true;
}
...
export function getEmailFromConfigDir(configDir?: string): string | null {
	...
	if (existsSync(claudeJsonPath)) {
		try {
			...
			console.warn(
				`[profile-utils] Failed to read email from ${claudeJsonPath}:`,
				error,
			);
		}
	}
	...
	if (existsSync(credentialsJsonPath)) {
		try {
			...
			console.warn(
				`[profile-utils] Failed to read email from ${credentialsJsonPath}:`,
				error,
			);
		}
	}
