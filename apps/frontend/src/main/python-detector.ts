	...
	if (bundledPython) {
		try {
			...
			if (validation.valid) {
				...
				console.warn(
					`[Python] Bundled Python version issue: ${validation.message}`,
				);
			}
		} catch (err) {
...
function matchesAllowedPattern(pythonPath: string): boolean {
	...
	return ALLOWED_PATH_PATTERNS.some(
		(pattern) => pattern.test(pythonPath) || pattern.test(normalizedPath),
	);
}
	...
	if (isFilePath) {
		...
		if (!matchesAllowedPattern(normalizedPath)) {
			return {
				valid: false,
				reason:
					"Path does not match allowed Python locations. Expected: system Python, Homebrew, pyenv, or virtual environment paths",
			};
		}
...
export function getValidatedPythonPath(
	...
	console.error(
		`[${serviceName}] Invalid Python path rejected: ${validation.reason}`,
	);
	return findPythonCommand() || "python";
}
