...
export class CodeMigrationService extends EventEmitter {
  ...
  async startMigration(request: CodeMigrationRequest): Promise<void> {
    ...
    if (!sourcePath) {
      this.emit('error', 'WorkPilot AI source not found. Cannot locate code_migration_runner.py');
      return;
    }
    const runnerPath = path.join(sourcePath, 'runners', 'code_migration_runner.py');
    ...
    const args = [
      ...
      '--migration-description', request.migrationDescription,
    ];
    if (request.model) {
  ...
  private async executeProcess(args: string[], env: Record<string, string>, cwd: string): Promise<void> {
    ...
    proc.stdout?.on('data', (data: Buffer) => {
      ...
      for (const line of text.split('\n')) {
        if (line.startsWith('__MIGRATION_RESULT__:')) {
          ...
          try {
            const progress = JSON.parse(line.substring('__TASK_PROGRESS__:'.length));
    ...
    proc.on('close', (code) => {
      ...
      if (code === 0 && result) {
        ...
        this.emit('complete', { status: 'success', migration_description: '', dry_run: false, plan: {}, execution: {}, summary: { files_modified: 0 } });
      } else {
        ...
        if (combined.includes('rate_limit')) {
          ...
          this.emit('error', `Migration failed (exit code ${code}). ${stderrOutput.slice(-500)}`);
        }
      }
    });
