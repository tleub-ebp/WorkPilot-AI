...
export class PipelineGeneratorService extends EventEmitter {
  ...
  async generate(request: PipelineGeneratorRequest): Promise<void> {
    ...
    if (!sourcePath) {
      this.emit('error', 'WorkPilot AI source not found. Cannot locate pipeline_generator_runner.py');
      return;
    }

    const runnerPath = path.join(sourcePath, 'runners', 'pipeline_generator_runner.py');
    ...
    const args = [runnerPath, '--project', request.projectDir, '--platforms', platforms];
  ...
  private async executeProcess(args: string[], env: Record<string, string>, cwd: string): Promise<void> {
    ...
    proc.stdout?.on('data', (data: Buffer) => {
      ...
      for (const line of text.split('\n')) {
        if (line.startsWith('__PIPELINE_GENERATOR_RESULT__:')) {
          try {
            result = JSON.parse(line.substring('__PIPELINE_GENERATOR_RESULT__:'.length));
          } catch { /* ignore */ }
        } else if (line.trim()) {
          ...
          } else if (line.includes('✅') && line.includes('pipeline generated')) {
    ...
    proc.on('close', (code) => {
      ...
      if (code === 0 && result) {
        ...
        this.emit('error', `Pipeline generation failed (exit code ${code}). ${stderrOutput.slice(-500)}`);
      }
    });
