...
export class ReleaseService extends EventEmitter {
	...
	parseChangelogVersions(projectPath: string): ReleaseableVersion[] {
		...
		const versionRegex =
			/^## \[(\d+\.\d+\.\d+(?:-[a-zA-Z0-9.]+)?)\](?: - (\d{4}-\d{2}-\d{2}))?/gm;
	...
	async getReleaseableVersions(
		...
		for (const version of versions) {
			const { specIds } = this.getTasksForVersion(
				projectPath,
				version.version,
				tasks,
			);
			...
			try {
				...
				if (tagExists) {
					...
					version.releaseUrl = this.getGitHubReleaseUrl(
						projectPath,
						version.tagName,
					);
				}
	...
	async runPreflightChecks(
		...
		try {
			...
			if (!gitStatus) {
				...
				status.blockers.push(
					`Uncommitted changes: ${uncommittedFiles.length} file(s)`,
				);
			}
		} catch {
			status.checks.gitClean = {
		...
		try {
			...
			if (!unpushed) {
				...
				status.blockers.push(
					`${unpushedCount} unpushed commit(s) - push before releasing`,
				);
			}
		} catch {
		...
		if (!tagExists) {
			...
			status.blockers.push(
				`Tag ${tagName} already exists - use a different version`,
			);
		}
		...
		status.canRelease = Object.values(status.checks).every(
			(check: ReleasePreflightCheck) => check.passed,
		);
	...
	private async checkWorktreesForVersion(
		...
		for (const specId of releaseSpecIds) {
			...
			const matchingFolder = worktreeFolders.find(
				(folder) => folder === specId || folder.startsWith(`${specId}-`),
			);
	...
	private async isWorktreeMerged(
		...
		try {
			...
			try {
				unmergedCommits = execFileSync(
					getToolPath("git"),
					["log", `${mainBranch}..${worktreeBranch}`, "--oneline"],
					{
						cwd: projectPath,
						encoding: "utf-8",
					},
	...
	async bumpVersion(
		...
		try {
			...
			if (hadChanges) {
				...
				execFileSync(
					getToolPath("git"),
					["stash", "push", "-m", "auto-claude-release-temp"],
					{
						cwd: projectPath,
						encoding: "utf-8",
					},
			...
			if (stashCreated) {
				try {
					...
					console.warn(
						'[ReleaseService] Failed to pop stash - user may need to run "git stash pop" manually',
					);
				}
			}
		}
