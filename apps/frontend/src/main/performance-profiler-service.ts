...
export class PerformanceProfilerService extends EventEmitter {
	...
	private getAutoBuildSourcePath(): string | null {
		...
		for (const p of possiblePaths) {
			if (
				existsSync(path.join(p, "runners", "performance_profiler_runner.py"))
			) {
	...
	async startProfiling(request: PerformanceProfilerRequest): Promise<void> {
		...
		if (!sourcePath) {
			this.emit(
				"error",
				"WorkPilot AI source not found. Cannot locate performance_profiler_runner.py",
			);
			return;
		}
		const runnerPath = path.join(
			sourcePath,
			"runners",
			"performance_profiler_runner.py",
		);
	...
	private async executeProcess(
		...
		proc.stdout?.on("data", (data: Buffer) => {
			for (const line of data.toString("utf-8").split("\n")) {
				if (line.startsWith("__PERF_RESULT__:")) {
					try {
						result = JSON.parse(line.substring("__PERF_RESULT__:".length));
						this.emit("status", "Profiling complete");
					} catch {
						/* ignore */
					}
				} else if (line.startsWith("__PERF_IMPLEMENTATION__:")) {
					try {
						implResult = JSON.parse(
							line.substring("__PERF_IMPLEMENTATION__:".length),
						);
					} catch {
						/* ignore */
					}
		...
		proc.on("close", (code) => {
			...
			if (code === 0 && result) {
				...
				this.emit(
					"error",
					`Performance profiling failed (exit code ${code}). ${stderrOutput.slice(-500)}`,
				);
			}
		});
		...
		proc.on("error", (err) => {
			...
			this.emit(
				"error",
				`Failed to start performance profiler: ${err.message}`,
			);
		});
	}
}
