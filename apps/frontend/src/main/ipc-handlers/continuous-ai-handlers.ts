...
export function registerContinuousAIHandlers(
  ...
  ipcMain.handle(
    ...
    async (_event, config: ContinuousAIConfig, projectPath?: string) => {
      try {
        ...
        const runnerPath = path.join(backendDir, 'runners', 'continuous_ai_runner.py');

        const proc = spawn(pythonPath, [
          runnerPath,
          '--project-dir', resolvedProject,
        ...
        proc.stderr?.on('data', (data: Buffer) => {
          appLog.warn(`[ContinuousAI] stderr: ${data.toString().slice(0, 500)}`);
        });
   ...
   * Get daemon status.
   */
  ipcMain.handle('continuousAI:status', async (_event, projectPath?: string) => {
    const resolvedProject = projectPath || process.cwd();
    const statusFile = path.join(resolvedProject, '.workpilot', 'continuous-ai', 'status.json');
   ...
   */
  ipcMain.handle('continuousAI:approveAction', async (_event, actionId: string) => {
   ...
   */
  ipcMain.handle('continuousAI:rejectAction', async (_event, actionId: string) => {
    if (activeDaemonProcess?.stdin) {
      try {
        activeDaemonProcess.stdin.write(
  ...
  ipcMain.handle('continuousAI:isRunning', async () => {
    return { running: activeDaemonProcess !== null, pid: activeDaemonProcess?.pid ?? null };
  });
}
