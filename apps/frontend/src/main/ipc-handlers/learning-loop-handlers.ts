...
export function registerLearningLoopHandlers(
  ...
  ipcMain.handle(IPC_CHANNELS.LEARNING_LOOP_GET_PATTERNS, async (_event, projectDir: string) => {
    try {
  ...
  ipcMain.handle(IPC_CHANNELS.LEARNING_LOOP_GET_SUMMARY, async (_event, projectDir: string) => {
    try {
      const { readFileSync, existsSync } = require('node:fs');
      const path = require('node:path');
      const patternsPath = path.join(projectDir, '.workpilot', 'learning_loop', 'patterns.json');
  ...
  ipcMain.handle(IPC_CHANNELS.LEARNING_LOOP_DELETE_PATTERN, async (_event, projectDir: string, patternId: string) => {
    try {
  ...
  ipcMain.handle(IPC_CHANNELS.LEARNING_LOOP_TOGGLE_PATTERN, async (_event, projectDir: string, patternId: string) => {
    try {
      const { readFileSync, writeFileSync, existsSync } = require('node:fs');
      const path = require('node:path');
      const patternsPath = path.join(projectDir, '.workpilot', 'learning_loop', 'patterns.json');
  ...
  ipcMain.on(IPC_CHANNELS.LEARNING_LOOP_RUN_ANALYSIS, (_event, projectDir: string, specId?: string) => {
  ...
  const handleStatus = (status: string): void => {
    safeSendToRenderer(getMainWindow, IPC_CHANNELS.LEARNING_LOOP_STATUS, status);
  };

  const handleStreamChunk = (chunk: string): void => {
    safeSendToRenderer(getMainWindow, IPC_CHANNELS.LEARNING_LOOP_STREAM_CHUNK, chunk);
  };
