...
export function registerBrowserAgentHandlers(
  ...
  ipcMain.handle(IPC_CHANNELS.BROWSER_AGENT_GET_DASHBOARD, async (_, projectPath: string) => {
    try {
      const result = await runBrowserAgentCommand(projectPath, ['dashboard', '--json']);
    ...
    async (_, projectPath: string, name: string, url: string) => {
      try {
        ...
        const result = await runBrowserAgentCommand(projectPath, [
          'screenshot', '--url', url, '--name', name, '--json',
        ]);
        const data = JSON.parse(result);

        if (mainWindow) {
          mainWindow.webContents.send(IPC_CHANNELS.BROWSER_AGENT_SCREENSHOT_READY, data);
        }
  ...
  ipcMain.handle(
    ...
    async (_, projectPath: string, url: string) => {
      try {
        const result = await runBrowserAgentCommand(projectPath, [
          'screenshot', '--url', url, '--name', 'navigation_preview', '--json',
        ]);
  ...
  ipcMain.handle(
    ...
    async (_, projectPath: string) => {
      try {
        const result = await runBrowserAgentCommand(projectPath, ['baseline', 'list', '--json']);
        return JSON.parse(result);
      } catch (_error) {
  ...
  ipcMain.handle(
    ...
    async (_, projectPath: string, name: string) => {
      try {
        const result = await runBrowserAgentCommand(projectPath, [
          'baseline', 'delete', '--name', name, '--json',
        ]);
        return JSON.parse(result);
      } catch (error) {
        return { success: false, error: String(error) };
  ...
  ipcMain.handle(
    ...
    async (_, projectPath: string) => {
      try {
        ...
        const result = await runBrowserAgentCommand(projectPath, ['tests', '--json'], 300000);
        const data = JSON.parse(result);

        if (mainWindow) {
          mainWindow.webContents.send(IPC_CHANNELS.BROWSER_AGENT_TEST_COMPLETE, data);
        }
...
function runBrowserAgentCommand(
  ...
  return new Promise((resolve, reject) => {
    const runnerPath = path.join(
      __dirname, '..', '..', '..', '..', 'backend', 'runners', 'browser_agent_runner.py'
    );
    const proc = spawn('python', [runnerPath, '--project', projectPath, '--json', ...args], {
      cwd: projectPath,
    ...
    proc.on('close', (code: number | null) => {
      if (code === 0) {
        ...
        reject(new Error(`Browser agent runner exited with code ${code}: ${stderr}`));
      }
    });
