	...
	for (const specDir of recentSpecDirs) {
		...
		if (existsSync(codebaseMapPath)) {
			try {
				...
				if (
					mapData.discovered_files &&
					Object.keys(mapData.discovered_files).length > 0
				) {
					memories.push({
...
export function searchFileBasedMemories(
	...
	const allSpecDirs = readdirSync(specsDir).filter((f: string) => {
		try {
			const specPath = path.join(specsDir, f);
			return statSync(specPath).isDirectory();
		} catch {
	...
	for (const specDir of allSpecDirs) {
		...
		const memoryFiles = readdirSync(memoryDir).filter((f: string) =>
			f.endsWith(".json"),
		);
...
export function registerMemoryDataHandlers(
	...
	ipcMain.handle(
		...
		async (
			...
			const projectEnvVars = loadProjectEnvVars(
				project.path,
			...
			if (graphitiEnabled && isKuzuAvailable()) {
				try {
					...
					console.warn(
						"Failed to get memories from LadybugDB, falling back to file-based:",
						error,
					);
				}
			}
		...
		async (
			...
			if (graphitiEnabled && isKuzuAvailable()) {
				try {
					...
					console.warn(
						"Failed to search LadybugDB, falling back to file-based:",
						error,
					);
				}
			}
