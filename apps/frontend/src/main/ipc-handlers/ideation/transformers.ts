...
function isValidIdeationType(value: unknown): value is IdeationType {
	return (
		typeof value === "string" && VALID_IDEATION_TYPES.has(value as IdeationType)
	);
}
...
function validateEnabledTypes(rawTypes: unknown): IdeationType[] {
	...
	if (invalidTypes.length > 0) {
		debugLog(
			"[Transformers] Dropped invalid IdeationType values:",
			invalidTypes,
		);
	}
	return validTypes;
	...
	if (idea.type === "code_improvements") {
		return {
			...
			implementationApproach:
				idea.implementation_approach || idea.implementationApproach || "",
		...
		return {
			...
			affectedComponents:
				idea.affected_components || idea.affectedComponents || [],
		...
		return {
			...
			targetAudience:
				idea.target_audience || idea.targetAudience || "developers",
			affectedAreas: idea.affected_areas || idea.affectedAreas || [],
			currentDocumentation:
				idea.current_documentation || idea.currentDocumentation || "",
		...
		return {
			...
			expectedImprovement:
				idea.expected_improvement || idea.expectedImprovement || "",
			implementation: idea.implementation || "",
			tradeoffs: idea.tradeoffs || "",
			estimatedEffort:
				idea.estimated_effort || idea.estimatedEffort || "medium",
...
export function transformSessionFromSnakeCase(
	...
	const rawEnabledTypes =
		rawSession.config?.enabled_types || rawSession.config?.enabledTypes || [];
	...
	return {
		...
		config: {
			enabledTypes,
			includeRoadmapContext:
				rawSession.config?.include_roadmap_context ??
				rawSession.config?.includeRoadmapContext ??
				true,
			includeKanbanContext:
				rawSession.config?.include_kanban_context ??
				rawSession.config?.includeKanbanContext ??
				true,
			maxIdeasPerType:
				rawSession.config?.max_ideas_per_type ||
				rawSession.config?.maxIdeasPerType ||
				5,
		},
		ideas: (rawSession.ideas || []).map((idea) =>
			transformIdeaFromSnakeCase(idea),
		),
		projectContext: {
			existingFeatures:
				rawSession.project_context?.existing_features ||
				rawSession.projectContext?.existingFeatures ||
				[],
			techStack:
				rawSession.project_context?.tech_stack ||
				rawSession.projectContext?.techStack ||
				[],
			targetAudience:
				rawSession.project_context?.target_audience ||
				rawSession.projectContext?.targetAudience,
			plannedFeatures:
				rawSession.project_context?.planned_features ||
				rawSession.projectContext?.plannedFeatures ||
				[],
		},
		generatedAt: rawSession.generated_at
			? new Date(rawSession.generated_at)
			: new Date(),
		updatedAt: rawSession.updated_at
			? new Date(rawSession.updated_at)
			: new Date(),
	};
}
