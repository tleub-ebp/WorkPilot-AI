 ...
 */
export function registerAppEmulatorHandlers(
	...
	ipcMain.handle(
		"app-emulator:start",
		async (_event, { config }: { config: AppEmulatorConfig }) => {
			if (startLock) {
				return {
					success: false,
					error: "Already starting — ignoring duplicate request",
				};
			}
			startLock = true;
			try {
