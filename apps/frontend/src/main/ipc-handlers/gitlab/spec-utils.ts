...
function sanitizeAssignees(value: unknown): Array<{ username: string }> {
	...
	for (const assignee of value) {
		...
		const username = sanitizeText(
			(assignee as { username?: unknown }).username,
			100,
		);
		if (username) {
...
function sanitizeIssueForSpec(
	issue: IssueLike,
	instanceUrl: string,
): SanitizedGitLabIssue {
	const issueIid = sanitizeIssueNumber(issue.iid);
	const title =
		sanitizeText(issue.title, 200) || `Issue ${issueIid || "unknown"}`;
	return {
...
export function buildIssueContext(
	...
	lines.push(
		`**Created:** ${new Date(safeIssue.created_at).toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric" })}`,
	);
	...
	if (safeIssue.assignees.length > 0) {
		lines.push(
			`**Assignees:** ${safeIssue.assignees.map((a) => a.username).join(", ")}`,
		);
	}
...
export async function createSpecForIssue(
	...
	try {
		...
		if (await pathExists(specDir)) {
			debugLog("Spec already exists for issue:", {
				iid: safeIssue.iid,
				specDir,
			});
