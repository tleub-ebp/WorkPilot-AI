...
import type { GitLabIssue, GitLabNote, IPCResult } from "../../../shared/types";
...
function transformIssue(
	...
	if (apiIssue.milestone) {
		...
		if (rawState === "active" || rawState === "closed") {
			...
			console.warn(
				`[GitLab Issues] Unknown milestone state '${rawState}' for issue #${apiIssue.iid} (id: ${apiIssue.id}), defaulting to 'active'`,
			);
		...
		async (
			...
			try {
				...
				const issues = apiIssues.map((issue) =>
					transformIssue(issue, config.project),
				);
