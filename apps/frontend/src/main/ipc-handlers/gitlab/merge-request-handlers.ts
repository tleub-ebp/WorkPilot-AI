...
import type { GitLabMergeRequest, IPCResult } from "../../../shared/types";
import { projectStore } from "../../project-store";
import type { CreateMergeRequestOptions, GitLabAPIMergeRequest } from "./types";
 ...
 */
function transformMergeRequest(
	apiMr: GitLabAPIMergeRequest,
): GitLabMergeRequest {
	return {
		id: apiMr.id,
		iid: apiMr.iid,
...
export function registerCreateMergeRequest(): void {
	ipcMain.handle(
		...
		async (
			...
			debugLog("createGitLabMergeRequest handler called", {
				title: options.title,
			});
		...
		async (
			...
			try {
				...
				if (updates.description !== undefined)
					mrBody.description = updates.description;
				if (updates.targetBranch !== undefined)
					mrBody.target_branch = updates.targetBranch;
				if (updates.labels !== undefined)
					mrBody.labels = updates.labels.join(",");
				if (updates.assigneeIds !== undefined)
					mrBody.assignee_ids = updates.assigneeIds;
