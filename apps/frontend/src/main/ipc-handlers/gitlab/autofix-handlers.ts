...
function sanitizeIssueUrl(rawUrl: unknown, instanceUrl: string): string {
	...
	try {
		...
		if (parsedInstanceUrl.protocol !== "https:") {
			console.warn(
				`[GitLab AutoFix] Instance URL does not use HTTPS: ${instanceUrl}`,
			);
			return "";
		}
 ...
 */
function saveAutoFixConfig(
	project: Project,
	config: GitLabAutoFixConfig,
): void {
...
function sendProgress(
	...
	mainWindow.webContents.send(
		IPC_CHANNELS.GITLAB_AUTOFIX_PROGRESS,
		projectId,
		progress,
	);
}
...
function sendError(
	...
	mainWindow.webContents.send(
		IPC_CHANNELS.GITLAB_AUTOFIX_ERROR,
		projectId,
		error,
	);
}
...
function sendComplete(
	...
	mainWindow.webContents.send(
		IPC_CHANNELS.GITLAB_AUTOFIX_COMPLETE,
		projectId,
		data,
	);
}
...
async function startAutoFix(
	...
	const sanitizedIssueUrl = sanitizeIssueUrl(
		issue.web_url,
		glConfig.instanceUrl,
	);
	const sanitizedProject =
		typeof glConfig.project === "string" ? glConfig.project : "";
...
export function registerAutoFixHandlers(
	...
	ipcMain.handle(
		...
		async (
			_,
			projectId: string,
			approvedBatches: GitLabIssueBatch[],
		): Promise<{
			success: boolean;
			batches?: GitLabIssueBatch[];
			error?: string;
		}> => {
