...
export async function getGitLabConfig(
	...
	try {
		...
		const instanceUrl = normalizeInstanceUrl(
			vars[GITLAB_ENV_KEYS.INSTANCE_URL],
		);
		if (!instanceUrl) return null;

		// If no token in .env, try to get it from glab CLI
		if (!token) {
			const glabToken = sanitizeToken(
				getTokenFromGlabCli(instanceUrl) ?? undefined,
			);
			if (glabToken) {
				token = glabToken;
			}
 ...
 */
export function normalizeProjectReference(
	project: string,
...
export async function gitlabFetch(
	...
	try {
		...
		if (error instanceof Error && error.name === "AbortError") {
			throw new Error(
				`GitLab API timeout after ${GITLAB_API_TIMEOUT_MS / 1000}s: ${url}`,
			);
		}
		throw error;
	} finally {
 ...
 */
export function detectGitLabProjectFromRemote(
	projectPath: string,
): { project: string; instanceUrl: string } | null {
	try {
