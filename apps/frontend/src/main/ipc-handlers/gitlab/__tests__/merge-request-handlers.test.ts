...
describe("GitLab Merge Request Handlers", () => {
	...
	describe("transformMergeRequest", () => {
		const baseApiMr: GitLabAPIMergeRequest = {
			...
			author: {
				username: "developer",
				avatar_url: "https://gitlab.com/dev.png",
			},
			assignees: [
				{ username: "reviewer", avatar_url: "https://gitlab.com/rev.png" },
			],
			labels: ["bug", "security"],
		...
		it("should handle assignees with missing username", () => {
			...
			expect(result.assignees[0].avatarUrl).toBe(
		...
		it("should preserve merge status", () => {
			...
			expect(transformMergeRequest(cannotMerge).mergeStatus).toBe(
				"cannot_be_merged",
			);
		});
