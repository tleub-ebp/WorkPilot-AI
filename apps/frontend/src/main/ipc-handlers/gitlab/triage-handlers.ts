		...
		if (file.startsWith("triage_") && file.endsWith(".json")) {
			try {
				const data = JSON.parse(
					fs.readFileSync(path.join(triageDir, file), "utf-8"),
				);
				results.push({
...
function sendProgress(
	...
	mainWindow.webContents.send(
		IPC_CHANNELS.GITLAB_TRIAGE_PROGRESS,
		projectId,
		progress,
	);
}
...
function sendError(
	...
	mainWindow.webContents.send(
		IPC_CHANNELS.GITLAB_TRIAGE_ERROR,
		projectId,
		error,
	);
}
...
function sendComplete(
	...
	mainWindow.webContents.send(
		IPC_CHANNELS.GITLAB_TRIAGE_COMPLETE,
		projectId,
		results,
	);
}
			...
			try {
				await withProjectOrNull(projectId, async (project) => {
					...
					for (let i = 0; i < filteredIssues.length; i++) {
						...
						if (!issueIid) {
							debugLog("Skipping issue with invalid IID", {
								issueIid: issue.iid,
							});
							continue;
						}
						...
						if (!sanitizedResult) {
							debugLog("Skipping triage result with invalid IID", {
