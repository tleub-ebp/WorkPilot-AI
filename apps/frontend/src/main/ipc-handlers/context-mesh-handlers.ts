...
export function registerContextMeshHandlers(
	...
	ipcMain.handle(IPC_CHANNELS.CONTEXT_MESH_GET_PROJECTS, async () => {
		try {
			const data = readMeshJson("projects.json", { projects: [] }) as {
				projects: unknown[];
			};
	...
	ipcMain.handle(
		IPC_CHANNELS.CONTEXT_MESH_REGISTER_PROJECT,
	...
	ipcMain.handle(
		IPC_CHANNELS.CONTEXT_MESH_UNREGISTER_PROJECT,
		async (_event, projectDir: string) => {
			try {
				const data = readMeshJson("projects.json", { projects: [] }) as {
					projects: Array<Record<string, unknown>>;
	...
	ipcMain.handle(IPC_CHANNELS.CONTEXT_MESH_GET_PATTERNS, async () => {
		try {
			const data = readMeshJson("patterns.json", { patterns: [] }) as {
				patterns: unknown[];
			};
	...
	ipcMain.handle(
		IPC_CHANNELS.CONTEXT_MESH_DELETE_PATTERN,
		async (_event, patternId: string) => {
			try {
				const data = readMeshJson("patterns.json", { patterns: [] }) as {
	...
	ipcMain.handle(IPC_CHANNELS.CONTEXT_MESH_GET_HANDBOOK, async () => {
		try {
			const data = readMeshJson("handbook.json", { entries: [] }) as {
				entries: unknown[];
			};
	...
	ipcMain.handle(
		IPC_CHANNELS.CONTEXT_MESH_DELETE_HANDBOOK_ENTRY,
	...
	ipcMain.handle(IPC_CHANNELS.CONTEXT_MESH_GET_SKILL_TRANSFERS, async () => {
		try {
			const data = readMeshJson("skill_transfers.json", { transfers: [] }) as {
				transfers: unknown[];
			};
	...
	ipcMain.handle(
		IPC_CHANNELS.CONTEXT_MESH_UPDATE_TRANSFER_STATUS,
		async (_event, transferId: string, status: string) => {
			try {
				const data = readMeshJson("skill_transfers.json", {
					transfers: [],
	...
	ipcMain.handle(
		IPC_CHANNELS.CONTEXT_MESH_GET_RECOMMENDATIONS,
		async (_event, targetProject?: string) => {
			try {
				const data = readMeshJson("recommendations.json", {
					recommendations: [],
	...
	ipcMain.handle(
		IPC_CHANNELS.CONTEXT_MESH_UPDATE_RECOMMENDATION_STATUS,
		async (_event, recId: string, status: string) => {
			try {
				const data = readMeshJson("recommendations.json", {
					recommendations: [],
				}) as { recommendations: Array<Record<string, unknown>> };
				const rec = (data.recommendations || []).find(
					(r) => r.recommendation_id === recId,
				);
				if (!rec) {
	...
	ipcMain.handle(IPC_CHANNELS.CONTEXT_MESH_GET_SUMMARY, async () => {
		try {
			const projects =
				(
					readMeshJson("projects.json", { projects: [] }) as {
						projects: unknown[];
					}
				).projects || [];
			const patterns =
				(
					readMeshJson("patterns.json", { patterns: [] }) as {
						patterns: unknown[];
					}
				).patterns || [];
			const handbook =
				(
					readMeshJson("handbook.json", { entries: [] }) as {
						entries: unknown[];
					}
				).entries || [];
			const transfers =
				(
					readMeshJson("skill_transfers.json", { transfers: [] }) as {
						transfers: Array<Record<string, unknown>>;
					}
				).transfers || [];
			const recs =
				(
					readMeshJson("recommendations.json", { recommendations: [] }) as {
			...
			return {
				...
				data: {
					...
					active_recommendations: recs.filter((r) => r.status === "active")
						.length,
	...
	ipcMain.on(
		IPC_CHANNELS.CONTEXT_MESH_RUN_ANALYSIS,
		(_event, model?: string, thinkingLevel?: string) => {
	...
	const handleStreamChunk = (chunk: string): void => {
		safeSendToRenderer(
			getMainWindow,
			IPC_CHANNELS.CONTEXT_MESH_STREAM_CHUNK,
			chunk,
		);
	};
	...
	const handleComplete = (result: unknown): void => {
		safeSendToRenderer(
			getMainWindow,
			IPC_CHANNELS.CONTEXT_MESH_COMPLETE,
			result,
		);
	};
