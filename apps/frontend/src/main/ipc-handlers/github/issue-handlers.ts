...
import type { GitHubAPIComment, GitHubAPIIssue } from "./types";
...
export function registerGetIssues(): void {
	ipcMain.handle(
		...
		async (
			...
			debugLog("[GitHub Issues] getIssues handler called", {
				projectId,
				state,
				page,
				fetchAll,
			...
			try {
				...
				debugLog(
					"[GitHub Issues] Fetching issues from:",
					normalizedRepo,
					"state:",
					state,
				);

				const maxPagesPerRequest = fetchAll
					? MAX_PAGES_FETCH_ALL
					: MAX_PAGES_PAGINATED;
				...
				if (fetchAll) {
					...
					const issuesOnly = allIssues.filter(
						(issue: GitHubAPIIssue) => !issue.pull_request,
					);
					const result: GitHubIssue[] = issuesOnly.map(
						(issue: GitHubAPIIssue) => transformIssue(issue, normalizedRepo),
					);

					debugLog(
						"[GitHub Issues] fetchAll complete:",
						result.length,
						"issues",
					);
				...
				while (
					collectedIssues.length < targetEndIndex &&
					apiPage <= maxPagesPerRequest &&
					hasMoreFromAPI
				) {
					debugLog(
						"[GitHub Issues] Fetching API page",
						apiPage,
						"collected so far:",
					...
					if (!Array.isArray(pageItems)) {
						debugLog(
							"[GitHub Issues] Unexpected response format:",
							typeof pageItems,
						);
						break;
					}
					...
					const issuesFromPage = pageItems.filter(
						(issue: GitHubAPIIssue) => !issue.pull_request,
					);
					collectedIssues.push(...issuesFromPage);

					debugLog(
						"[GitHub Issues] API page",
						apiPage,
						":",
						pageItems.length,
						"items,",
						issuesFromPage.length,
						"actual issues",
					);
				...
				const pageIssues = collectedIssues.slice(
					targetStartIndex,
					targetEndIndex,
				);
