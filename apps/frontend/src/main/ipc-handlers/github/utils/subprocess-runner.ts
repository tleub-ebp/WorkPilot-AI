...
import {
	...
	type OperationType,
} from "../../../claude-profile/operation-registry";
...
export function runPythonSubprocess<T = unknown>(
	...
	if (options.operationRegistration) {
		const { operationId, operationType, metadata, restartFn } =
	...
	const promise = new Promise<SubprocessResult<T>>((resolve) => {
						...
						if (!isWindows()) {
							...
							execFile(
								...
								(err: Error | null) => {
									if (err)
										console.warn(
											"[SubprocessRunner] taskkill error (process may have already exited):",
											err.message,
										);
								},
							);
			...
			if (billingResult.isBillingFailure) {
				...
				const billingFailureInfo: BillingFailureInfo = {
					...
					message:
						...
						"Billing or credit error. Please check your account.",
				...
				try {
					...
					console.error(
						"[SubprocessRunner] onBillingFailure callback threw:",
						e,
					);
				}
		...
		child.on("close", (code: number | null) => {
			...
			if (options.operationRegistration) {
				getOperationRegistry().unregisterOperation(
					options.operationRegistration.operationId,
				);
			}
...
export function getBackendPath(project: Project): string | null {
	...
	if (
		fs.existsSync(path.join(appsBackendPath, "runners", "github", "runner.py"))
	) {
 ...
 */
export function validateRunner(backendPath: string | null): {
	valid: boolean;
	error?: string;
} {
	if (!backendPath) {
		return {
			valid: false,
			error:
				"GitHub runner not found. Make sure the GitHub automation module is installed.",
		};
	}
 ...
 */
export async function validateGitHubModule(
	project: Project,
): Promise<GitHubModuleValidation> {
	...
	if (!backendPath) {
		result.error =
			"GitHub automation module not installed. This project does not have the GitHub runner configured.";
		return result;
	}
...
export function parseJSONFromOutput<T>(stdout: string): T {
	...
	if (jsonStart >= 0 && jsonEnd > jsonStart) {
		...
		try {
			...
			throw new Error(
				"Failed to parse JSON response from backend. The analysis completed but the response format was invalid.",
			);
		}
	}
