...
async function checkAutoFixLabels(project: Project): Promise<number[]> {
	...
	for (const issue of issues) {
		...
		const hasMatchingLabel = config.labels.some((label) =>
...
async function startAutoFix(
	...
	try {
		...
		sendProgress({
			...
			issueNumber,
			progress: 100,
			message: "Auto-fix spec creation started!",
		});
		sendComplete(state);
	} catch (error) {
		...
		sendProgress({
			...
			issueNumber,
			progress: 100,
			message: "Spec directory created. Click Start to begin.",
		});
		sendComplete(state);
	}
}
 ...
 */
function convertAnalyzePreviewResult(
	...
	return {
		...
		proposedBatches: (
			(result.proposed_batches as Array<Record<string, unknown>>) ?? []
		).map((b) => ({
		...
		singleIssues: (
			(result.single_issues as Array<Record<string, unknown>>) ?? []
		).map((i) => ({
			issueNumber: i.issue_number as number,
			title: i.title as string,
			labels: (i.labels as string[]) ?? [],
...
export function registerAutoFixHandlers(
	...
	ipcMain.handle(
		...
		async (_, projectId: string): Promise<AutoFixConfig | null> => {
			...
			return withProjectOrNull(projectId, async (project) => {
				const config = getAutoFixConfig(project);
				debugLog("AutoFix config loaded", {
	...
	ipcMain.handle(
		...
		async (_, projectId: string, config: AutoFixConfig): Promise<boolean> => {
			debugLog("saveAutoFixConfig handler called", {
				projectId,
				enabled: config.enabled,
			});
	...
	ipcMain.handle(
		...
		async (_, projectId: string): Promise<number[]> => {
			...
			const result = await withProjectOrNull(projectId, async (project) => {
				...
				debugLog("Issues with auto-fix labels", {
					count: issues.length,
					issues,
				});
				return issues;
			...
			try {
				await withProjectOrNull(projectId, async (project) => {
					...
					const additionalArgs =
						issueNumbers && issueNumbers.length > 0
					...
					const { promise } = runPythonSubprocess<IssueBatch[]>({
						...
						onComplete: () => {
							...
							debugLog("Batch auto-fix completed", {
				...
				debugLog("Batch auto-fix failed", {
					error: error instanceof Error ? error.message : error,
				});
				const { sendError } = createIPCCommunicators<
					BatchProgress,
				...
				sendError(
					error instanceof Error ? error.message : "Failed to batch issues",
				);
			}
		},
	);
	...
	ipcMain.on(
		...
		async (
			...
			debugLog("analyzePreview handler called", {
				projectId,
				issueNumbers,
				maxIssues,
			});
			...
			try {
				await withProjectOrNull(projectId, async (project) => {
					...
					sendProgress({
						phase: "analyzing",
						progress: 10,
						message: "Fetching issues for analysis...",
					});
					...
					const { promise } = runPythonSubprocess<AnalyzePreviewResult>({
						...
						onComplete: (stdout) => {
							const rawResult =
								parseJSONFromOutput<Record<string, unknown>>(stdout);
							const convertedResult = convertAnalyzePreviewResult(rawResult);
							debugLog("Analyze preview completed", {
								batchCount: convertedResult.proposedBatches.length,
							});
				...
				debugLog("Analyze preview failed", {
					error: error instanceof Error ? error.message : error,
				});
				const { sendError } = createIPCCommunicators<
				...
				if (error instanceof Error) {
					if (error.message.includes("JSON")) {
						userMessage =
							"Analysis completed, but there was an error processing the results. Please try again.";
					} else if (error.message.includes("No JSON found")) {
						userMessage =
							"No analysis results returned. Please check your GitHub connection and try again.";
					} else {
	...
	ipcMain.handle(
		...
		async (
			_,
			projectId: string,
			approvedBatches: Array<Record<string, unknown>>,
		): Promise<{
			success: boolean;
			batches?: IssueBatch[];
			error?: string;
		}> => {
			debugLog("approveBatches handler called", {
				projectId,
			...
			const result = await withProjectOrNull(projectId, async (project) => {
				try {
					...
					fs.writeFileSync(
						tempFile,
						JSON.stringify(pythonBatches, null, 2),
						"utf-8",
					);
					...
					debugLog("Approve batches failed", {
						error: error instanceof Error ? error.message : error,
					});
					return {
						success: false,
