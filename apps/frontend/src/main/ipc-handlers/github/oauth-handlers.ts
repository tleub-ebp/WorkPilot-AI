 ...
 */
function sendDeviceCodeToRenderer(
	deviceCode: string,
	authUrl: string,
 ...
 */
function sendAuthChangedToRenderer(
	oldUsername: string | null,
	newUsername: string,
): void {
	debugLog("Sending auth changed event to renderer windows", {
		oldUsername,
		newUsername,
	});
	const windows = BrowserWindow.getAllWindows();
	...
	ipcMain.handle(
		...
		> => {
			...
			try {
				...
				const authStatus = execFileSync(getToolPath("gh"), ["auth", "status"], {
					encoding: "utf-8",
					stdio: "pipe",
					env,
				});
...
export function registerStartGhAuth(): void {
	ipcMain.handle(
		...
		async (): Promise<IPCResult<GitHubAuthStartResult>> => {
			...
			debugLog(
				"Username before auth:",
				usernameBeforeAuth || "(not authenticated)",
			);
				...
				try {
					...
					const tryExtractAndOpenBrowser = async () => {
						...
						if (deviceFlowInfo.deviceCode) {
							...
							debugLog(
								"Device code extracted successfully (code redacted for security)",
							);
							...
							sendDeviceCodeToRenderer(
								extractedDeviceCode,
								extractedAuthUrl,
								browserOpenedSuccessfully,
							);
					...
					ghProcess.on("close", async (code) => {
						...
						if (code === 0) {
							...
							if (
								...
								debugLog(
									"WARNING: Auth succeeded but could not fetch username to detect account change",
								);
							}
							...
							resolve({
								...
								data: {
									...
									fallbackUrl: !browserOpenedSuccessfully
										? extractedAuthUrl
										: undefined,
								},
							});
							...
							const fallbackUrlForManualAuth = extractedDeviceCode
								? extractedAuthUrl
								: GITHUB_DEVICE_URL;
							...
							resolve({
								...
								data: {
									...
									message:
										"Authentication failed. Please visit the URL manually to complete authentication.",
								},
							});
						}
					});
					...
					ghProcess.on("error", (error) => {
						...
						resolve({
							...
							data: {
								...
								message:
									"Failed to start GitHub CLI. Please visit the URL manually to authenticate.",
							},
						});
					});
				} catch (error) {
					debugLog(
						"Exception in startGitHubAuth:",
						error instanceof Error ? error.message : error,
					);
					resolve({
						success: false,
						...
						data: {
							...
							fallbackUrl: GITHUB_DEVICE_URL,
							message:
								"An unexpected error occurred. Please visit the URL manually to authenticate.",
						},
					});
				}
			});
	...
	ipcMain.handle(
		...
		> => {
			...
			try {
				...
				debugLog(
					"Running: gh repo list --limit 100 --json nameWithOwner,description,isPrivate",
				);
				...
				try {
					...
					try {
						...
						if (existingRemote !== remoteUrl) {
							debugLog("Updating existing origin remote:", {
								existingRemote,
								remoteUrl,
							});
							execFileSync(
