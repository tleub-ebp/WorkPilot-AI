...
import type { PromptOptimizeRequest, PromptOptimizerResult } from '../prompt-optimizer-service';
import { safeSendToRenderer } from './utils';

/**
 * Read prompt optimizer feature settings from the settings file
 */
function getPromptOptimizerSettings(): { model: string; thinkingLevel: string } {
  ...
  try {
    if (existsSync(settingsPath)) {
      ...
      return {
        model: featureModels.promptOptimizer ?? DEFAULT_FEATURE_MODELS.promptOptimizer,
        thinkingLevel: featureThinking.promptOptimizer ?? DEFAULT_FEATURE_THINKING.promptOptimizer,
      };
    }
  } catch (error) {
    debugError('[PromptOptimizer Handler] Failed to read feature settings:', error);
  }
 ...
 */
export function registerPromptOptimizerHandlers(getMainWindow: () => BrowserWindow | null): void {
  // ============================================
  ...
  ipcMain.on(
    ...
    async (
      ...
      try {
        ...
        debugError('[PromptOptimizer Handler] Failed to read settings for configuration:', error);
      }
  ...
  promptOptimizerService.on('stream-chunk', (chunk: string) => {
    safeSendToRenderer(getMainWindow, IPC_CHANNELS.PROMPT_OPTIMIZER_STREAM_CHUNK, chunk);
  });

  // Forward status updates to renderer
  promptOptimizerService.on('status', (status: string) => {
    safeSendToRenderer(getMainWindow, IPC_CHANNELS.PROMPT_OPTIMIZER_STATUS, status);
  });

  // Forward errors to renderer
  promptOptimizerService.on('error', (error: string) => {
    safeSendToRenderer(getMainWindow, IPC_CHANNELS.PROMPT_OPTIMIZER_ERROR, error);
  });

  // Forward completion to renderer with structured result
  promptOptimizerService.on('complete', (result: PromptOptimizerResult) => {
    safeSendToRenderer(getMainWindow, IPC_CHANNELS.PROMPT_OPTIMIZER_COMPLETE, result);
  });
}
