...
export function registerCostHandlers(): void {
  ...
  ipcMain.handle('costs:getSummary', async (_, projectPath: string) => {
    try {
      ...
      const recent = data.usages.filter(u => new Date(u.timestamp) >= since30);
      const previous = data.usages.filter(
        u => new Date(u.timestamp) >= since60 && new Date(u.timestamp) < since30
      );
  ...
  ipcMain.handle('costs:getBudget', async (_, projectPath: string) => {
    try {
      ...
      if (budget.period === 'monthly') {
        ...
        forecastEndOfMonth = dayOfMonth > 0 ? (spent / dayOfMonth) * daysInMonth : 0;
      }
      ...
      if (pct >= 1) {
        alerts.push(`Budget dépassé ! Dépensé $${spent.toFixed(2)} / $${budget.limit.toFixed(2)}`);
      } else if (pct >= budget.critical_threshold) {
        alerts.push(`Critique : proche du plafond. Dépensé $${spent.toFixed(2)} / $${budget.limit.toFixed(2)} (${utilizationPct.toFixed(0)}%)`);
      } else if (pct >= budget.warning_threshold) {
        alerts.push(`Attention : utilisation élevée. Dépensé $${spent.toFixed(2)} / $${budget.limit.toFixed(2)} (${utilizationPct.toFixed(0)}%)`);
      }
  ...
  ipcMain.handle(
    'costs:setBudget',
    async (_, projectPath: string, limit: number, period: string = 'monthly') => {
      try {
        const data = loadCostData(projectPath);
