 ...
 */
export function setupTestGenerationHandlers(getMainWindow: () => BrowserWindow | null): void {
  ...
  ipcMain.handle('test-generation:analyze-coverage', async (_, params: { filePath: string; existingTestPath?: string; projectPath?: string }) => {
    try {
      await testGenerationService.analyzeCoverage(params.filePath, params.existingTestPath, params.projectPath);
      return { success: true };
    } catch (error) {
  ...
  // Generate unit tests
  ipcMain.handle('test-generation:generate-unit', async (_, params: { filePath: string; existingTestPath?: string; coverageTarget?: number; projectPath?: string }) => {
    try {
      await testGenerationService.generateUnitTests(params.filePath, params.existingTestPath, params.coverageTarget, params.projectPath);
      return { success: true };
    } catch (error) {
  ...
  // Generate E2E tests
  ipcMain.handle('test-generation:generate-e2e', async (_, params: { userStory: string; targetModule: string; projectPath?: string }) => {
    try {
      await testGenerationService.generateE2ETests(params.userStory, params.targetModule, params.projectPath);
      return { success: true };
    } catch (error) {
  ...
  // Generate TDD tests
  ipcMain.handle('test-generation:generate-tdd', async (_, params: { description: string; language: string; snippetType: string; projectPath?: string }) => {
    try {
      await testGenerationService.generateTDDTests(params.description, params.language, params.snippetType, params.projectPath);
      return { success: true };
    } catch (error) {
