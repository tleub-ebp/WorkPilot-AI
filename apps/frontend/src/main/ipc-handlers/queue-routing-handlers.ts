...
export function registerQueueRoutingHandlers(
	...
	ipcMain.handle(
		...
		async (): Promise<{
			...
			try {
				...
				console.error(
					"[QueueRouting] Failed to get running tasks by profile:",
					error,
				);
				return {
					success: false,
	...
	ipcMain.handle(
		...
		async (
			...
			try {
				...
				console.error(
					"[QueueRouting] Failed to get best profile for task:",
					error,
				);
				return {
					success: false,
	...
	ipcMain.handle(
		...
		async (
			...
			try {
				...
				console.error(
					"[QueueRouting] Failed to assign profile to task:",
					error,
				);
				return {
					success: false,
	...
	agentManager.on("profile-swapped", (taskId: string, swap: unknown) => {
		...
		if (win) {
			win.webContents.send(IPC_CHANNELS.QUEUE_PROFILE_SWAPPED, {
				taskId,
				swap,
			});
		}
