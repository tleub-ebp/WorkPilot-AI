 ...
 */
export const findTaskAndProject = (
	taskId: string,
	projectId?: string,
): { task: Task | undefined; project: Project | undefined } => {
	...
	if (projectId) {
		...
		if (!targetProject) {
			console.warn(
				`[findTaskAndProject] projectId "${projectId}" not found in projects list, returning undefined`,
			);
