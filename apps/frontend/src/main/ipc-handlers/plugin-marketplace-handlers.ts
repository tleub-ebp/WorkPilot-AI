...
function buildPluginCatalog(): MarketplacePlugin[] {
	return [
		// === AGENT PLUGINS ===
		{
			...
			description:
				"A specialized AI agent that audits your codebase for OWASP Top 10 vulnerabilities, insecure dependencies, hardcoded secrets, and SQL injection risks. Generates detailed security reports with remediation steps.",
			...
			agentConfig: {
				systemPrompt:
					"You are a security audit specialist. Analyze code for OWASP Top 10 vulnerabilities.",
		...
		},
		{
			...
			description:
				"Analyzes your UI components for WCAG 2.1 AA compliance. Detects missing ARIA labels, poor color contrast, keyboard navigation issues, and auto-generates fixes.",
			...
			agentConfig: {
				systemPrompt:
					"You are an accessibility expert. Analyze UI code for WCAG 2.1 compliance.",
		...
		},
		{
			...
			description:
				"Identifies N+1 queries, unnecessary re-renders, memory leaks, and bundle size issues. Provides actionable optimizations with benchmarks.",
			author: "perf-guild",
			authorVerified: true,
			type: "agent",
			icon: "Zap",
			...
			agentConfig: {
				systemPrompt:
					"You are a performance optimization expert. Find and fix performance issues.",
		...
		},
		{
			...
			description:
				"Scans your codebase for hardcoded UI strings, extracts them to i18n translation files, and replaces them with the appropriate translation function calls.",
			author: "localize-dev",
			authorVerified: false,
			type: "agent",
			icon: "Globe",
			...
			agentConfig: {
				systemPrompt:
					"You are an internationalization expert. Extract hardcoded strings to i18n files.",
		...
		// === INTEGRATION PLUGINS ===
		{
			...
			description:
				"Automatically notify your team when tasks complete, QA passes, or builds fail. Configure per-project channels and notification filters.",
			author: "WorkPilot Community",
			authorVerified: true,
			type: "integration",
			icon: "MessageSquare",
				...
				requiredEnvVars: [
					{
						...
						description: "Incoming webhook URL from your Slack app",
						secret: true,
					},
				],
			},
		...
		},
		{
			...
			description:
				"Bi-directional sync between WorkPilot tasks and Notion databases. Automatically create Notion pages from specs and update them as work progresses.",
			author: "notion-tools",
			authorVerified: false,
			type: "integration",
			icon: "FileText",
			...
			integrationConfig: {
				...
				requiredEnvVars: [
					...
					},
					{
						...
						label: "Database ID",
						description: "The Notion database ID to sync to",
						secret: false,
					},
				],
			},
		...
		},
		{
			...
			description:
				"Send WorkPilot agent metrics, task durations, and error rates to Datadog dashboards. Track the ROI of your AI-assisted development.",
		...
		// === SPEC TEMPLATE PLUGINS ===
		{
			...
			description:
				"Comprehensive spec template for building REST APIs with authentication, CRUD operations, error handling, validation, pagination, and OpenAPI documentation.",
			author: "spec-masters",
			authorVerified: true,
			type: "spec-template",
			icon: "Code",
		...
		},
		{
			...
			description:
				"Complete template for React component library development: design tokens, Storybook stories, accessibility requirements, unit tests, and documentation.",
		...
		},
		{
			...
			description:
				"Full-stack microservice spec covering Docker, Kubernetes manifests, health checks, distributed tracing, circuit breakers, and service mesh configuration.",
		...
		// === THEME PLUGINS ===
		{
			...
			description:
				"A vibrant cyberpunk theme with neon cyan and magenta accents, dark grid backgrounds, and futuristic UI elements. Perfect for late-night coding sessions.",
			author: "theme-forge",
			authorVerified: false,
			type: "theme",
			icon: "Palette",
		...
		},
		{
			...
			description:
				"The beloved GitHub Dark Dimmed color scheme, faithfully recreated for WorkPilot AI. Easy on the eyes with muted blues and subtle contrasts.",
			author: "github-fans",
			authorVerified: false,
			type: "theme",
			icon: "Moon",
		...
		},
		{
			...
			description:
				"Ethan Schoonover's iconic Solarized Light palette adapted for WorkPilot AI. Scientifically designed for reduced eye strain during day coding.",
		...
		// === CUSTOM PROMPT PLUGINS ===
		{
			...
			description:
				"A curated set of code review prompts that simulate a senior engineer's perspective: performance, maintainability, security, DRY principles, and architectural concerns.",
			author: "prompt-labs",
			authorVerified: true,
			type: "custom-prompt",
			icon: "Search",
			...
			promptContent:
				"Review this code as a senior engineer with 10+ years of experience...",
			addedAt: "2025-01-10T00:00:00Z",
			updatedAt: "2025-03-05T00:00:00Z",
		},
		{
			...
			description:
				"A comprehensive prompt pack for practicing strict TDD: red-green-refactor cycles, property-based testing, boundary analysis, and mutation testing strategies.",
			author: "tdd-zealots",
			authorVerified: false,
			type: "custom-prompt",
			icon: "TestTube",
		...
		},
		{
			...
			description:
				"Expert prompts for applying Domain-Driven Design patterns: bounded contexts, aggregates, value objects, domain events, and CQRS. Includes refactoring from CRUD to DDD.",
...
export function registerPluginMarketplaceHandlers(): void {
	...
	ipcMain.handle(
		...
		async (_event, { pluginId }: { pluginId: string }) => {
			try {
				const installed = readInstalledPlugins();
		...
		async (
			...
			try {
				...
				if (fs.existsSync(pluginsDir)) {
					return {
						success: false,
						error: `A plugin with ID "${data.id}" already exists`,
					};
				}
				...
				if (data.type === "agent") {
					...
					if (data.triggerKeywords?.trim()) {
						config.triggers = data.triggerKeywords
							.split(",")
							.map((k) => k.trim())
							.filter(Boolean);
					}
					...
					data.type === "spec-template" ||
					data.type === "custom-prompt"
				) {
				...
				let localConfig: {
					localPlugins: Array<{ path: string; enabled: boolean }>;
				} = { localPlugins: [] };
				...
				appLog.info(
					`[Plugin Marketplace] Created plugin: ${data.name} at ${pluginsDir}`,
				);
