...
import type { AppUpdateInfo, IPCResult } from "../../shared/types";
import {
	checkForUpdates,
	...
	ipcMain.handle(
		...
		async (): Promise<IPCResult> => {
			try {
				...
				console.error(
					"[app-update-handlers] Download stable version failed:",
					error,
				);
				return {
					success: false,
					error:
						error instanceof Error
							? error.message
							: "Failed to download stable version",
				};
			}
		},
	);
	...
	ipcMain.handle(
		...
		async (): Promise<IPCResult<AppUpdateInfo | null>> => {
			try {
				...
				console.error(
					"[app-update-handlers] Get downloaded update info failed:",
					error,
				);
				return {
					success: false,
					error:
						error instanceof Error
							? error.message
							: "Failed to get downloaded update info",
				};
			}
		},
	);
