...
export function registerDebugHandlers(): void {
	...
	ipcMain.handle(
		IPC_CHANNELS.DEBUG_OPEN_LOGS_FOLDER,
	...
	ipcMain.handle(
		IPC_CHANNELS.DEBUG_COPY_DEBUG_INFO,
		async (): Promise<{ success: boolean; error?: string }> => {
			try {
	...
	// Get recent errors
	ipcMain.handle(
		IPC_CHANNELS.DEBUG_GET_RECENT_ERRORS,
		async (_, maxCount?: number): Promise<string[]> => {
	...
	// List log files
	ipcMain.handle(
		IPC_CHANNELS.DEBUG_LIST_LOG_FILES,
		async (): Promise<LogFileInfo[]> => {
