...
const EXCLUDED_DIRS = new Set([
  ...
  'coverage', '.cache', '.venv', 'venv', 'out', '.turbo', '.worktrees',
  'vendor', 'target', '.gradle', '.maven', 'obj', 'bin', '.vs',
]);

function walkFiles(dir: string, extensions: string[], maxDepth = 12, depth = 0): string[] {
  ...
  for (const entry of entries) {
    ...
    if (stat.isDirectory()) {
      results = results.concat(walkFiles(full, extensions, maxDepth, depth + 1));
...
function detectDotnet(projectPath: string): DetectedRoute[] {
  ...
  for (const filePath of files) {
    ...
    if (classMatch) {
      ...
      classBase = classMatch[1].replace('[controller]', controllerName.toLowerCase());
    } else {
    ...
    while ((m = methodRe.exec(content)) !== null) {
      ...
      const requiresAuth = /\[Authorize\b/.test(ctx) && !/\[AllowAnonymous\]/.test(ctx);

      routes.push({ path: fullPath, methods: [method], summary, tag, file: path.relative(projectPath, filePath), framework: 'ASP.NET Core', requiresAuth });
    }
  }
  return routes;
}
  ...
  for (const filePath of files) {
    ...
    while ((m = flaskRe.exec(content)) !== null) {
      const methods = m[2]
        ...
        : ['GET'];
      routes.push({ path: m[1], methods, tag, file: path.relative(projectPath, filePath), framework: 'Flask', requiresAuth: /login_required/.test(content.slice(Math.max(0, m.index - 100), m.index)) });
    }
  }
  return routes;
}
...
function detectSpring(projectPath: string): DetectedRoute[] {
  ...
  for (const filePath of files) {
    ...
    const tag = path.basename(filePath, '.java').replace('Controller', '').toLowerCase();

    // Class-level @RequestMapping
    const clsMatch = content.match(/@RequestMapping\(\s*["']?([^"')\s]+)["']?\s*\)/);
    const classBase = clsMatch ? (clsMatch[1].startsWith('/') ? clsMatch[1] : `/${clsMatch[1]}`) : '';
    ...
    while ((m = methodRe.exec(content)) !== null) {
      ...
      routes.push({ path: fullPath, methods: [method], tag, file: path.relative(projectPath, filePath), framework: 'Spring Boot', requiresAuth: false });
    }
  }
  return routes;
}
...
function detectRails(projectPath: string): DetectedRoute[] {
  const routes: DetectedRoute[] = [];
  const files = walkFiles(projectPath, ['.rb']).filter((f) => f.endsWith('routes.rb'));
  ...
  for (const filePath of files) {
    ...
    while ((m = resourcesRe.exec(content)) !== null) {
      ...
      for (const [p, method] of [[base, 'GET'], [base, 'POST'], [`${base}/{id}`, 'GET'], [`${base}/{id}`, 'PUT'], [`${base}/{id}`, 'DELETE']] as [string, string][]) {
        routes.push({ path: p, methods: [method], tag: m[1], file: path.relative(projectPath, filePath), framework: 'Rails', requiresAuth: false });
      }
    }
  }
  return routes;
...
function buildOpenApiSpec(routes: DetectedRoute[], projectName: string): Record<string, unknown> {
  ...
  for (const route of routes) {
    ...
    for (const method of route.methods) {
      const op: Record<string, unknown> = {
        ...
        operationId: `${method.toLowerCase()}_${openApiPath.replace(/[^a-zA-Z0-9]/g, '_').replace(/_+/g, '_').replace(/^_|_$/g, '')}`,
      ...
      const pathParams = [...openApiPath.matchAll(/\{([^}]+)\}/g)].map((pm) => ({
        name: pm[1],
        in: 'path',
        required: true,
        schema: { type: 'string' },
