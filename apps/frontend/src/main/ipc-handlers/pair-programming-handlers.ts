...
export function registerPairProgrammingHandlers(
  ...
  ipcMain.handle(
    ...
    async (_, params: StartSessionParams): Promise<IPCResult<PairSession>> => {
      ...
      proc.on('close', (code) => {
        ...
        safeSendToRenderer(getMainWindow, IPC_CHANNELS.PAIR_PROGRAMMING_STATUS,
          projectId, code === 0 ? 'completed' : 'error', '');
      });
  ...
  ipcMain.handle(
    ...
    async (_, projectId: string, _sessionId: string): Promise<IPCResult> => {
      ...
      activeSessions.delete(projectId);
      safeSendToRenderer(getMainWindow, IPC_CHANNELS.PAIR_PROGRAMMING_STATUS, projectId, 'completed', 'Session stopped by user.');
      return { success: true };
    }
  );
...
function handleRunnerEvent(
  ...
  switch (type) {
    ...
    case 'action': {
      ...
      activeSession.session.updatedAt = new Date().toISOString();
      safeSendToRenderer(getMainWindow, IPC_CHANNELS.PAIR_PROGRAMMING_AI_ACTION, projectId, action);
      break;
    }
    ...
    case 'conflict': {
      ...
      const message = event.message as string;
      safeSendToRenderer(getMainWindow, IPC_CHANNELS.PAIR_PROGRAMMING_CONFLICT, projectId, filePath, message);
      break;
    }
    ...
    case 'done': {
      ...
      safeSendToRenderer(getMainWindow, IPC_CHANNELS.PAIR_PROGRAMMING_COMPLETE, projectId, summary);
      break;
    }
    ...
    case 'error': {
      ...
      activeSession.session.updatedAt = new Date().toISOString();
      safeSendToRenderer(getMainWindow, IPC_CHANNELS.PAIR_PROGRAMMING_ERROR, projectId, errMsg);
      break;
    }
