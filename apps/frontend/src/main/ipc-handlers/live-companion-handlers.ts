...
export function registerLiveCompanionHandlers(
	...
	ipcMain.handle(
		IPC_CHANNELS.LIVE_COMPANION_START,
	...
	ipcMain.handle(
		IPC_CHANNELS.LIVE_COMPANION_DISMISS_SUGGESTION,
	...
	ipcMain.handle(
		IPC_CHANNELS.LIVE_COMPANION_APPLY_SUGGESTION,
		async (_event, suggestionId: string) => {
			try {
	...
	ipcMain.handle(
		IPC_CHANNELS.LIVE_COMPANION_ACCEPT_TAKEOVER,
	...
	ipcMain.handle(
		IPC_CHANNELS.LIVE_COMPANION_DECLINE_TAKEOVER,
		async (_event, proposalId: string) => {
			try {
	...
	ipcMain.handle(
		IPC_CHANNELS.LIVE_COMPANION_UPDATE_CONFIG,
	...
	const handleStateChanged = (state: unknown): void => {
		safeSendToRenderer(
			getMainWindow,
			IPC_CHANNELS.LIVE_COMPANION_STATE_CHANGED,
			state,
		);
	};
	...
	const handleSuggestion = (suggestion: unknown): void => {
		safeSendToRenderer(
			getMainWindow,
			IPC_CHANNELS.LIVE_COMPANION_SUGGESTION,
			suggestion,
		);
	};
	...
	const handleTakeoverProposal = (proposal: unknown): void => {
		safeSendToRenderer(
			getMainWindow,
			IPC_CHANNELS.LIVE_COMPANION_TAKEOVER_PROPOSAL,
			proposal,
		);
	};
	...
	const handleFileChange = (event: unknown): void => {
		safeSendToRenderer(
			getMainWindow,
			IPC_CHANNELS.LIVE_COMPANION_FILE_CHANGE,
			event,
		);
	};
