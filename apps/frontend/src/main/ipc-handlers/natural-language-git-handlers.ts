...
export function setupNaturalLanguageGitHandlers(): void {
  ...
  ipcMain.handle('execute-natural-language-git', async (event, request: NaturalLanguageGitRequest) => {
    try {
      ...
      if (win) {
        win.webContents.send('natural-language-git-error', error instanceof Error ? error.message : 'Unknown error');
      }
    }
  });
