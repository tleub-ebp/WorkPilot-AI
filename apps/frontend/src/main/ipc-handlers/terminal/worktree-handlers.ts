...
function fixMisconfiguredBareRepo(projectPath: string): boolean {
	try {
		...
		if (hasExactMatch) {
			...
			const hasGlobMatch = GLOB_MARKERS.some((pattern) => {
				...
				if (pattern.includes("..") || pattern.includes("/")) {
					debugLog(
						"[TerminalWorktree] Unsupported glob pattern ignored:",
						pattern,
					);
					return false;
				}
				...
				if (directoryFiles === null) {
					try {
						...
						if (allFiles.length > MAX_FILES_TO_CHECK) {
							debugLog(
								`[TerminalWorktree] Directory has ${allFiles.length} entries, checking only first ${MAX_FILES_TO_CHECK}`,
							);
						}
					} catch (error) {
		...
		debugLog(
			"[TerminalWorktree] Detected misconfigured bare repository with source files. Auto-fixing by unsetting core.bare...",
		);
		...
		debugLog(
			"[TerminalWorktree] Fixed: core.bare has been unset. Git operations should now work correctly.",
		);
		return true;
	} catch {
		return false;
...
function getDefaultBranch(projectPath: string): string {
	...
	if (project?.settings?.mainBranch) {
		debugLog(
			"[TerminalWorktree] Using mainBranch from project settings:",
	...
	try {
		...
		debugLog(
			"[TerminalWorktree] Falling back to current branch:",
			currentBranch,
		);
 ...
 */
function symlinkNodeModulesToWorktree(
	projectPath: string,
	worktreePath: string,
): string[] {
	...
	for (const [sourceRel, targetRel] of nodeModulesLocations) {
		...
		if (!existsSync(sourcePath)) {
			debugLog(
				"[TerminalWorktree] Skipping symlink - source does not exist:",
				sourceRel,
			);
			continue;
		}
		...
		if (existsSync(targetPath)) {
			debugLog(
				"[TerminalWorktree] Skipping symlink - target already exists:",
				targetRel,
			);
			continue;
		}
		...
		try {
			...
			debugLog(
				"[TerminalWorktree] Skipping symlink - target exists (possibly broken symlink):",
				targetRel,
			);
			continue;
		} catch {
		...
		try {
			...
			if (isWindows()) {
				...
				debugLog(
					"[TerminalWorktree] Created junction (Windows):",
					targetRel,
					"->",
					sourcePath,
				);
				...
				debugLog(
					"[TerminalWorktree] Created symlink (Unix):",
					targetRel,
					"->",
					relativePath,
				);
			...
			debugError(
				"[TerminalWorktree] Could not create symlink for",
				targetRel,
				":",
				error,
			);
			console.warn(
				`[TerminalWorktree] Warning: Failed to link ${targetRel} - TypeScript checks may fail in this worktree`,
			);
		}
	}
...
function loadWorktreeConfig(
	...
	if (existsSync(metadataPath)) {
		try {
			...
			debugError(
				"[TerminalWorktree] Corrupted config at:",
				metadataPath,
				error,
			);
			return null;
	...
	const legacyConfigPath = path.join(
		getTerminalWorktreePath(projectPath, name),
		"config.json",
	);
	if (existsSync(legacyConfigPath)) {
		try {
			...
			try {
				...
				debugLog(
					"[TerminalWorktree] Migrated config from legacy location:",
					name,
				);
			} catch {
				debugLog(
					"[TerminalWorktree] Could not remove legacy config:",
					legacyConfigPath,
				);
			}
			return config;
			...
			debugError(
				"[TerminalWorktree] Corrupted legacy config at:",
				legacyConfigPath,
				error,
			);
			return null;
...
async function createTerminalWorktree(
	...
	debugLog("[TerminalWorktree] Creating worktree:", {
		name,
		taskId,
		createGitBranch,
		projectPath,
	...
	if (!WORKTREE_NAME_REGEX.test(name)) {
		return {
			success: false,
			error:
				"Invalid worktree name. Use lowercase letters, numbers, dashes, and underscores. Must start and end with alphanumeric.",
		};
	}
	...
	if (fixMisconfiguredBareRepo(projectPath)) {
		debugLog(
			"[TerminalWorktree] Fixed misconfigured bare repository at:",
			projectPath,
		);
	}
	...
	try {
		...
		try {
			...
			debugLog(
				"[TerminalWorktree] Could not fetch from remote, continuing with local branch",
			);
		}
		...
		if (isRemoteRef) {
			...
			try {
				...
				debugLog(
					"[TerminalWorktree] Remote ref not found, using local branch:",
					baseBranch,
				);
			}
		}
		...
		if (createGitBranch) {
			...
			debugLog(
				"[TerminalWorktree] Created worktree with branch:",
				branchName,
				"from",
				baseRef,
			);
			...
			try {
				...
				debugLog(
					"[TerminalWorktree] No origin remote found, skipping push for local-only repo",
				);
			}
			...
			if (hasOrigin) {
				try {
					...
					debugLog(
						"[TerminalWorktree] Pushed branch to remote with tracking:",
						branchName,
					);
				} catch (pushError) {
					...
					debugLog(
						"[TerminalWorktree] Could not push to remote (worktree still usable):",
						message,
					);
				}
			}
			...
			debugLog(
				"[TerminalWorktree] Created worktree in detached HEAD mode from",
				baseRef,
			);
		}
		...
		const symlinkedModules = symlinkNodeModulesToWorktree(
			projectPath,
			worktreePath,
		);
		if (symlinkedModules.length > 0) {
			debugLog(
				"[TerminalWorktree] Symlinked dependencies:",
				symlinkedModules.join(", "),
			);
		}
		...
		if (directoryCreated && existsSync(worktreePath)) {
			try {
				...
				debugLog(
					"[TerminalWorktree] Cleaned up failed worktree directory:",
					worktreePath,
				);
				...
				debugError(
					"[TerminalWorktree] Failed to cleanup worktree directory:",
					cleanupError,
				);
			}
		}
...
async function listTerminalWorktrees(
	projectPath: string,
): Promise<TerminalWorktreeConfig[]> {
	// Validate projectPath against registered projects
	if (!isValidProjectPath(projectPath)) {
		debugError(
			"[TerminalWorktree] Invalid project path for listing:",
			projectPath,
		);
		return [];
	}
				...
				if (file.isFile() && file.name.endsWith(".json")) {
					...
					if (config) {
						...
						if (existsSync(config.worktreePath)) {
							...
							debugLog(
								"[TerminalWorktree] Found stale metadata for deleted worktree:",
								name,
							);
						}
					}
	...
	if (staleMetadataFiles.length > 0) {
		for (const filePath of staleMetadataFiles) {
			try {
				rmSync(filePath);
				debugLog(
					"[TerminalWorktree] Cleaned up stale metadata file:",
					filePath,
				);
			} catch (error) {
				debugError(
					"[TerminalWorktree] Failed to cleanup stale metadata:",
					filePath,
					error,
				);
			}
 ...
 */
async function listOtherWorktrees(
	projectPath: string,
): Promise<OtherWorktreeInfo[]> {
	// Validate projectPath against registered projects
	if (!isValidProjectPath(projectPath)) {
		debugError(
			"[TerminalWorktree] Invalid project path for listing other worktrees:",
			projectPath,
		);
		return [];
	}
	...
	try {
		...
		for (const line of output.split("\n")) {
			if (line.startsWith(GIT_PORCELAIN.WORKTREE_PREFIX)) {
				...
				currentWorktree = {
					path: line.substring(GIT_PORCELAIN.WORKTREE_PREFIX.length),
				};
...
function processOtherWorktree(
	...
	for (const excludePrefix of excludePrefixes) {
		if (
			normalizedPath.startsWith(excludePrefix + path.sep) ||
			normalizedPath === excludePrefix
		) {
...
async function removeTerminalWorktree(
	...
	debugLog("[TerminalWorktree] Removing worktree:", {
		name,
		deleteBranch,
		projectPath,
	});
	...
	try {
		...
		if (deleteBranch && config.hasGitBranch && config.branchName) {
			...
			if (!GIT_BRANCH_REGEX.test(config.branchName)) {
				debugError(
					"[TerminalWorktree] Invalid branch name in config:",
					config.branchName,
				);
			} else {
				try {
					...
					debugLog(
						"[TerminalWorktree] Branch not found or already deleted:",
						config.branchName,
					);
				}
			}
		...
		if (existsSync(metadataPath)) {
			try {
				...
				debugLog(
					"[TerminalWorktree] Could not remove metadata file:",
					metadataPath,
				);
			}
		}
			...
			try {
				...
				return {
					...
					error:
						error instanceof Error
							? error.message
							: "Failed to list other worktrees",
				};
			}
		},
	);
