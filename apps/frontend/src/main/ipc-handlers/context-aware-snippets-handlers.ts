...
export function setupContextAwareSnippetsHandlers(): void {
  ...
  ipcMain.handle('context-aware-snippets:generate', async (_, request: ContextAwareSnippetRequest) => {
    try {
  ...
  ipcMain.handle('context-aware-snippets:configure', async (_, config: { pythonPath?: string; autoBuildSourcePath?: string }) => {
    try {
      contextAwareSnippetsService.configure(config.pythonPath, config.autoBuildSourcePath);
      return { success: true };
    } catch (error) {
