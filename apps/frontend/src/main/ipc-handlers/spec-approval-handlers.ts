...
function saveApprovals(projectDir: string, records: SpecApprovalRecord[]): void {
  const p = getApprovalDbPath(projectDir);
  ...
  try {
    for (const entry of fs.readdirSync(specsDir)) {
      ...
      if (fs.existsSync(pendingMarker) && fs.existsSync(specMd)) {
        ...
        const alreadyDecided = existing.find(r => r.specNumber === specNumber);
...
export function registerSpecApprovalHandlers(): void {
  ipcMain.handle('specApproval:getPendingSpecs', (_event, projectDir: string) => {
    try {
  ...
  ipcMain.handle('specApproval:getSpec', (_event, projectDir: string, specNumber: string) => {
    try {
      ...
      for (const fname of ['spec.md', 'requirements.json', 'context.json', 'implementation_plan.json']) {
  ...
  ipcMain.handle('specApproval:approve', (_event, projectDir: string, params: {
    specNumber: string;
    amendments?: string;
    autoApproveThreshold?: number;
  }) => {
    try {
      ...
      const pendingMarker = path.join(specDir, 'APPROVAL_PENDING');
      if (fs.existsSync(pendingMarker)) { fs.unlinkSync(pendingMarker); }
      fs.writeFileSync(path.join(specDir, 'APPROVED'), JSON.stringify({
      ...
      if (params.amendments?.trim()) {
        ...
        if (fs.existsSync(specMd)) {
          ...
          fs.writeFileSync(specMd, existing + '\n\n## Human Amendments\n\n' + params.amendments, 'utf-8');
        }
      }
  ...
  ipcMain.handle('specApproval:reject', (_event, projectDir: string, params: {
    specNumber: string;
    feedback: string;
  }) => {
    try {
      ...
      const pendingMarker = path.join(specDir, 'APPROVAL_PENDING');
      if (fs.existsSync(pendingMarker)) { fs.unlinkSync(pendingMarker); }
      fs.writeFileSync(path.join(specDir, 'REJECTED'), JSON.stringify({
  ...
  ipcMain.handle('specApproval:requestChanges', (_event, projectDir: string, params: {
    specNumber: string;
    feedback: string;
  }) => {
    try {
      ...
      fs.writeFileSync(path.join(specDir, 'CHANGES_REQUESTED'), JSON.stringify({
  ...
  ipcMain.handle('specApproval:getHistory', (_event, projectDir: string, limit: number = 100) => {
    try {
...
export function setupSpecApprovalEventForwarding(_getMainWindow: () => BrowserWindow | null): void {
