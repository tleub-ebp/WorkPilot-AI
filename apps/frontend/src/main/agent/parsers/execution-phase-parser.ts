 ...
 */
export interface ExecutionParserContext
	extends PhaseParserContext<ExecutionPhase> {
...
export class ExecutionPhaseParser extends BasePhaseParser<ExecutionPhase> {
	...
	private parseRunPhase(
		...
		const subtaskMatch = originalLog.match(
		...
		if (
			...
			const completedSubtask = originalLog.match(
