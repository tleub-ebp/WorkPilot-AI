 ...
 */
export const IDEATION_TERMINAL_PHASES: ReadonlySet<IdeationPhase> = new Set([
	"complete",
]);
 ...
 */
export interface IdeationParserContext
	extends PhaseParserContext<IdeationPhase> {
...
export class IdeationPhaseParser extends BasePhaseParser<IdeationPhase> {
	...
	parse(
		log: string,
		context: IdeationParserContext,
	): IdeationParseResult | null {
		...
		if (!result) {
			...
			if (
				...
				context.completedTypes.size > 0
			) {
				const progress = this.calculateGeneratingProgress(
					context.completedTypes.size,
					context.totalTypes,
				);
				return {
					phase: "generating",
		...
		if (result.phase === "generating" && context.completedTypes.size > 0) {
			progress = this.calculateGeneratingProgress(
				context.completedTypes.size,
				context.totalTypes,
			);
		}
	 ...
	 */
	private calculateGeneratingProgress(
