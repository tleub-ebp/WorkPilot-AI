...
describe("task-event-parser", () => {
	...
	it("should extract JSON with escaped quotes correctly", () => {
		const input =
			'__TASK_EVENT__:{"message": "Hello \\"world\\"", "type": "log"}some trailing text';
	...
	it("should extract JSON with nested objects correctly", () => {
		const input =
			'__TASK_EVENT__:{"data": {"nested": {"value": 42}}, "type": "result"}';
		const result = parseTaskEvent(input);
		expect(result).not.toBeNull();
	...
	it("should handle JSON with escaped backslashes correctly", () => {
		const input =
			'__TASK_EVENT__:{"path": "C:\\\\Users\\\\test", "type": "file"}';
		const result = parseTaskEvent(input);
		expect(result).not.toBeNull();
	...
	it("should handle complex nested JSON with strings and escapes", () => {
		const input =
			'__TASK_EVENT__:{"config": {"path": "C:\\\\temp\\\\file.txt", "description": "A \\"special\\" file"}, "status": "ready"}extra garbage';
		const result = parseTaskEvent(input);
		expect(result).not.toBeNull();
