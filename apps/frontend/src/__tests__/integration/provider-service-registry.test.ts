...
import {
	...
	getProviders,
	providerRegistry,
} from "../../shared/services/providerRegistry";
...
describe("ProviderRegistry — Unit & Integration", () => {
		...
		it("model entries should have required fields", () => {
			...
			for (const provider of providers) {
				for (const model of provider.models) {
					...
					expect(["flagship", "standard", "fast", "local"]).toContain(
						model.tier,
					);
				}
			}
	...
	describe("Dynamic provider management", () => {
		...
		it("should update an existing provider", () => {
			...
			const updated = providerRegistry.updateProvider("update-test", {
				label: "After Update",
			});
			expect(updated).toBe(true);
			expect(providerRegistry.getProvider("update-test")?.label).toBe(
				"After Update",
			);
		...
		it("should return false when updating non-existent provider", () => {
			const updated = providerRegistry.updateProvider("nonexistent", {
				label: "New",
			});
	...
	describe("checkProviderStatus", () => {
		...
		it("should check API key providers against profiles", async () => {
			...
			const status = await providerRegistry.checkProviderStatus(
				"openai",
				profiles,
			);
			expect(status.available).toBe(true);
