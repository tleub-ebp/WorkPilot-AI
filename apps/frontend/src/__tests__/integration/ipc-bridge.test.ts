...
describe('IPC Bridge Integration', () => {
  ...
  describe('Project operations', () => {
    ...
    it('should have addProject method that invokes IPC', async () => {
      (vi.mocked(ipcRenderer.invoke)).mockResolvedValue({ success: true, data: { id: '1' } });

      const addProject = electronAPI['addProject'] as (path: string) => Promise<unknown>;
      await addProject('/test/path');

      expect(ipcRenderer.invoke).toHaveBeenCalledWith('project:add', '/test/path');
    });
    ...
    it('should have removeProject method', async () => {
      ...
      expect(ipcRenderer.invoke).toHaveBeenCalledWith('project:remove', 'project-id');
    });
  ...
  describe('Task operations', () => {
    ...
    it('should have getTasks method', async () => {
      ...
      expect(ipcRenderer.invoke).toHaveBeenCalledWith('task:list', 'project-id', undefined);
    });
    ...
    it('should have startTask method using send', async () => {
      ...
      expect(ipcRenderer.send).toHaveBeenCalledWith('task:start', 'task-id', { parallel: true });
    });
