	...
	app: {
		...
		getPath: vi.fn((name: string) => {
			const paths: Record<string, string> = {
				userData: process.env.CI
					? "/tmp/ci-test-app-data"
					: "/tmp/test-app-data",
...
vi.mock("../../main/python-detector", async (importOriginal) => {
	const actual =
		await importOriginal<typeof import("../../main/python-detector")>();
	return {
		...actual,
...
describe("Subprocess Spawn Integration", () => {
	...
	describe("AgentManager", () => {
		...
		it("should spawn Python process for task execution", async () => {
			...
			// Start the async operation
			const promise = manager.startTaskExecution(
				"task-1",
				TEST_PROJECT_PATH,
				"spec-001",
			);
		...
		it("should spawn Python process for QA process", async () => {
			...
			// Start the async operation
			const promise = manager.startQAProcess(
				"task-1",
				TEST_PROJECT_PATH,
				"spec-001",
			);
		...
		it("should track running tasks", async () => {
			...
			const promise1 = manager.startSpecCreation(
				"task-1",
				TEST_PROJECT_PATH,
				"Test 1",
			);
			const promise2 = manager.startTaskExecution(
				"task-2",
				TEST_PROJECT_PATH,
				"spec-001",
			);
		...
		it("should allow sequential execution of same task", async () => {
			...
			const promise2 = manager.startSpecCreation(
				"task-1",
				TEST_PROJECT_PATH,
				"Test 2",
			);
