...
describe('Frontend ↔ Backend Provider API Integration', () => {
  ...
  describe('Scenario: Provider selection flow', () => {
    ...
    it('should switch active profile', async () => {
      useSettingsStore.setState({
        profiles: [
          { id: 'p1', name: 'Profile 1', baseUrl: 'https://api.openai.com', apiKey: 'key1', createdAt: Date.now(), updatedAt: Date.now() },
          { id: 'p2', name: 'Profile 2', baseUrl: 'https://api.anthropic.com', apiKey: 'key2', createdAt: Date.now(), updatedAt: Date.now() },
        ],
        activeProfileId: 'p1',
      });
  ...
  describe('Scenario: Model discovery flow', () => {
    it('should discover models from a valid endpoint', async () => {
      ...
      const models = await useSettingsStore.getState().discoverModels(
        'https://api.openai.com/v1',
        'sk-valid-key'
      );
    ...
    it('should cache models and reuse for same endpoint', async () => {
      ...
      await useSettingsStore.getState().discoverModels('https://api.test.com', 'sk-test-key1');
      // Second call with same key - should use cache
      const models = await useSettingsStore.getState().discoverModels('https://api.test.com', 'sk-test-key1');
  ...
  describe('Scenario: Full profile lifecycle', () => {
    it('should create → test → activate → use → delete a profile', async () => {
      ...
      const testResult = await useSettingsStore.getState().testConnection(
        'https://api.openai.com/v1',
        'sk-test-lifecycle'
      );
      ...
      const deleted = await useSettingsStore.getState().deleteProfile('new-profile');
  ...
  describe('Scenario: Settings + profiles loading on app init', () => {
    it('should load settings and profiles in sequence', async () => {
      ...
      mockGetAPIProfiles.mockResolvedValue({
        ...
        data: {
          profiles: [
            { id: 'p1', name: 'Profile', baseUrl: 'https://api.test.com', apiKey: 'key', createdAt: Date.now(), updatedAt: Date.now() },
          ],
          activeProfileId: 'p1',
        },
      });
    ...
    it('should handle partial failures gracefully', async () => {
      ...
      mockGetAPIProfiles.mockRejectedValue(new Error('IPC channel not available'));
