...
vi.mock("../../lib/terminal-font-constants", () => ({
	...
	isValidFontWeight: vi.fn(
		(value: number) => value >= 100 && value <= 900 && value % 100 === 0,
	),
	...
	isValidCursorStyle: vi.fn((value: string) =>
		["block", "underline", "bar"].includes(value),
	),
	isValidHexColor: vi.fn((value: string) =>
		/^#([0-9A-Fa-f]{3}|[0-9A-Fa-f]{6}|[0-9A-Fa-f]{8})$/.test(value),
	),
	isValidFontFamily: vi.fn(
...
describe("terminal-font-settings-store", () => {
	...
	beforeEach(async () => {
		...
		vi.doMock("../../lib/terminal-font-constants", () => ({
			...
			isValidLineHeight: vi.fn((value: number) => value >= 1.0 && value <= 2.0),
			isValidLetterSpacing: vi.fn((value: number) => value >= -2 && value <= 5),
			isValidScrollback: vi.fn(
				(value: number) => value >= 1000 && value <= 100000,
			),
			isValidCursorStyle: vi.fn((value: string) =>
				["block", "underline", "bar"].includes(value),
			),
	...
	describe("Store initialization", () => {
		it("should initialize with OS-specific defaults", () => {
			...
			expect(state.fontFamily).toEqual([
				"Ubuntu Mono",
				"Source Code Pro",
				"Liberation Mono",
				"DejaVu Sans Mono",
	...
	describe("OS-specific defaults", () => {
		it("should initialize with Windows defaults", async () => {
			...
			vi.doMock("../../lib/terminal-font-constants", () => ({
				...
				isValidLineHeight: vi.fn(
					(value: number) => value >= 1.0 && value <= 2.0,
				),
				isValidLetterSpacing: vi.fn(
					(value: number) => value >= -2 && value <= 5,
				),
				isValidScrollback: vi.fn(
					(value: number) => value >= 1000 && value <= 100000,
				),
			...
			const windowsStoreModule = await import(
				"../terminal-font-settings-store"
			);
			const windowsStore =
				windowsStoreModule.useTerminalFontSettingsStore.getState();

			expect(windowsStore.fontFamily).toEqual([
				"Cascadia Code",
				"Consolas",
				"Courier New",
				"monospace",
		...
		it("should initialize with macOS defaults", async () => {
			...
			expect(macStore.fontFamily).toEqual([
				"SF Mono",
				"Menlo",
				"Monaco",
				"monospace",
	...
	describe("applySettings", () => {
		...
		it("should preserve unspecified settings", () => {
			...
			expect(useTerminalFontSettingsStore.getState().fontFamily).toEqual(
				originalFontFamily,
			);
		});
	});
