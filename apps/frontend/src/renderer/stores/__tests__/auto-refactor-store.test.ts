...
describe("AutoRefactorStore", () => {
	...
	describe("phase management", () => {
		it("should update phase correctly", () => {
			...
			const phases: Array<AutoRefactorPhase> = [
				"idle",
				"analyzing",
				"executing",
				"complete",
...
describe("AutoRefactorStore Actions", () => {
	...
	describe("startAutoRefactor", () => {
		it("should call electronAPI with correct parameters", () => {
			...
			act(() => {
				useAutoRefactorStore
					.getState()
					.openDialog(true, "claude-3.5-sonnet", "high");
			});
	...
	describe("setupAutoRefactorListeners", () => {
		...
		it("should setup all event listeners", () => {
			...
			expect(
				mockElectronAPI.onAutoRefactorExecutionComplete,
			).toHaveBeenCalled();
		...
		it("should handle status updates", () => {
			const { result } = renderHook(() => useAutoRefactorStore());
			const mockCallback =
				mockElectronAPI.onAutoRefactorStatus.mock.calls[0][0];
		...
		it("should handle stream chunks", () => {
			const { result } = renderHook(() => useAutoRefactorStore());
			const mockCallback =
				mockElectronAPI.onAutoRefactorStreamChunk.mock.calls[0][0];
		...
		it("should handle completion", () => {
			const { result } = renderHook(() => useAutoRefactorStore());
			const mockCallback =
				mockElectronAPI.onAutoRefactorComplete.mock.calls[0][0];
		...
		it("should handle execution completion", () => {
			const { result } = renderHook(() => useAutoRefactorStore());
			const mockCallback =
				mockElectronAPI.onAutoRefactorExecutionComplete.mock.calls[0][0];
