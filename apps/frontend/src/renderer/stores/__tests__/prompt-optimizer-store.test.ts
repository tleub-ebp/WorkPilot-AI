...
describe('Prompt Optimizer Store', () => {
  ...
  describe('setStatus', () => {
    it('should update status text', () => {
      ...
      expect(usePromptOptimizerStore.getState().status).toBe('Analyzing prompt...');
    });
  });
  ...
  describe('appendStreamingOutput', () => {
    it('should append chunks to streaming output', () => {
      ...
      expect(usePromptOptimizerStore.getState().streamingOutput).toBe('Hello world!');
    });
    ...
    it('should handle empty chunks', () => {
      ...
      expect(usePromptOptimizerStore.getState().streamingOutput).toBe('initial');
    });
  });
  ...
  describe('setupPromptOptimizerListeners', () => {
    ...
    it('should update store on stream chunk event', () => {
      mockOnPromptOptimizerStreamChunk.mockImplementation((cb: (chunk: string) => void) => {
        cb('first chunk ');
        cb('second chunk');
        return vi.fn();
      });
      ...
      expect(usePromptOptimizerStore.getState().streamingOutput).toBe('first chunk second chunk');
    });

    it('should update store on status event', () => {
      mockOnPromptOptimizerStreamChunk.mockReturnValue(vi.fn());
      mockOnPromptOptimizerStatus.mockImplementation((cb: (status: string) => void) => {
      ...
      expect(usePromptOptimizerStore.getState().status).toBe('Analyzing project context...');
    });

    it('should update store on error event', () => {
      mockOnPromptOptimizerStreamChunk.mockReturnValue(vi.fn());
      mockOnPromptOptimizerStatus.mockReturnValue(vi.fn());
      mockOnPromptOptimizerError.mockImplementation((cb: (error: string) => void) => {
        cb('Process crashed');
        return vi.fn();
      });
      mockOnPromptOptimizerComplete.mockReturnValue(vi.fn());
