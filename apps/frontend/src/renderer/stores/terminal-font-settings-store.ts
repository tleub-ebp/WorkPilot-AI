...
import { isLinux, isMacOS, isWindows } from "../lib/os-detection";
import {
	isValidCursorStyle,
	isValidFontFamily,
	isValidFontSize,
				...
				try {
					...
					if (parsed.fontSize !== undefined) {
						if (
							...
							!isValidFontSize(parsed.fontSize)
						) {
							return false;
						}
					...
					if (parsed.fontWeight !== undefined) {
						if (
							...
							!isValidFontWeight(parsed.fontWeight)
						) {
							return false;
						}
					...
					if (parsed.lineHeight !== undefined) {
						if (
							...
							!isValidLineHeight(parsed.lineHeight)
						) {
							return false;
						}
					...
					if (parsed.letterSpacing !== undefined) {
						if (
							...
							!isValidLetterSpacing(parsed.letterSpacing)
						) {
							return false;
						}
					...
					if (parsed.scrollback !== undefined) {
						if (
							...
							!isValidScrollback(parsed.scrollback)
						) {
							return false;
						}
					...
					if (parsed.cursorAccentColor !== undefined) {
						if (
							...
							!isValidHexColor(parsed.cursorAccentColor)
						) {
							return false;
						}
