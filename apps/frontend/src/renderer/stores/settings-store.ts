...
interface SettingsState {
	...
	setProviderPriorityOrder: (
		order: import("../components/settings/multiconnector/types").LLMProvider[],
	) => void;
}
...
export const useSettingsStore = create<SettingsState>((set, _get) => ({
	...
	discoverModels: async (
		...
		try {
			const result = await window.electronAPI.discoverModels(
				baseUrl,
				apiKey,
				signal,
			);
			...
			if (result.success && result.data) {
				...
				// Cache the results
				set((state) => ({
					discoveredModels: new Map(state.discoveredModels).set(
						cacheKey,
						models,
					),
					modelsLoading: false,
 ...
 */
async function migrateOnboardingCompleted(
	settings: AppSettings,
): Promise<AppSettings> {
	...
	try {
		const claudeCodeResult =
			await window.electronAPI.getClaudeCodeOnboardingStatus();
		if (
			claudeCodeResult.success &&
			claudeCodeResult.data?.hasCompletedOnboarding
		) {
		...
		console.warn(
			"[settings-store] Failed to check Claude Code onboarding status:",
			error,
		);
	}
 ...
 */
export async function saveActiveView(
	view: import("../components/Sidebar").SidebarView,
): Promise<boolean> {
