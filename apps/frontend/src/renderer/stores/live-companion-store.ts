...
export async function startCompanion(projectDir: string): Promise<boolean> {
  ...
  try {
    const result = await window.electronAPI.invoke('liveCompanion:start', projectDir);
...
export async function updateCompanionConfig(updates: Partial<CompanionConfig>): Promise<boolean> {
  try {
    const result = await window.electronAPI.invoke('liveCompanion:updateConfig', updates);
    if (result.success) {
      useLiveCompanionStore.getState().setConfig(result.data);
      return true;
    }
...
export async function loadSuggestions(): Promise<void> {
  try {
    const result = await window.electronAPI.invoke('liveCompanion:getSuggestions');
    if (result.success) {
...
export async function dismissSuggestion(suggestionId: string): Promise<boolean> {
  try {
    const result = await window.electronAPI.invoke('liveCompanion:dismissSuggestion', suggestionId);
    if (result.success) {
...
export async function applySuggestion(suggestionId: string): Promise<boolean> {
  try {
    const result = await window.electronAPI.invoke('liveCompanion:applySuggestion', suggestionId);
    if (result.success) {
      useLiveCompanionStore.getState().removeSuggestion(suggestionId);
      return true;
    }
...
export async function loadTakeovers(): Promise<void> {
  try {
    const result = await window.electronAPI.invoke('liveCompanion:getTakeovers');
    if (result.success) {
...
export async function acceptTakeover(proposalId: string): Promise<boolean> {
  try {
    const result = await window.electronAPI.invoke('liveCompanion:acceptTakeover', proposalId);
    if (result.success) {
...
export async function declineTakeover(proposalId: string): Promise<boolean> {
  try {
    const result = await window.electronAPI.invoke('liveCompanion:declineTakeover', proposalId);
    if (result.success) {
      useLiveCompanionStore.getState().removeTakeover(proposalId);
      return true;
    }
