...
interface MemoryLifecycleState {
  ...
  loadEpisodes: (projectDir: string, page?: number, pageSize?: number) => Promise<void>;
  runPrune: (projectDir: string, options?: {
    strategy?: PruneStrategy;
    maxAgeDays?: number;
    maxCount?: number;
    dryRun?: boolean;
  }) => Promise<{ pruned: number; remaining: number } | null>;
  setPolicy: (projectDir: string, policy: Partial<Pick<MemoryStatus, 'retention_days' | 'max_episodes' | 'prune_strategy' | 'auto_prune'>>) => Promise<void>;
  deleteEpisode: (projectDir: string, episodeId: string) => Promise<void>;
  exportMemories: (projectDir: string, outputPath: string) => Promise<{ count: number; path: string } | null>;
  clearError: () => void;
}
...
export const useMemoryLifecycleStore = create<MemoryLifecycleState>((set) => ({
  ...
  loadStatus: async (projectDir) => {
    ...
    try {
      const result = await globalThis.electronAPI.invoke('memoryLifecycle:getStatus', projectDir);
      if (result.success) {
  ...
  loadEpisodes: async (projectDir, page = 0, pageSize = 50) => {
    set({ isLoading: true, error: null });
    try {
      const result = await globalThis.electronAPI.invoke('memoryLifecycle:listMemories', projectDir, page, pageSize);
      if (result.success) {
  ...
  runPrune: async (projectDir, options = {}) => {
    ...
    try {
      const result = await globalThis.electronAPI.invoke('memoryLifecycle:prune', projectDir, {
  ...
  setPolicy: async (projectDir, policy) => {
    try {
      ...
      if (policy.retention_days != null) mapped.retention_days = policy.retention_days;
      if (policy.max_episodes != null) mapped.max_episodes = policy.max_episodes;
      if (policy.prune_strategy != null) mapped.prune_strategy = policy.prune_strategy;
      if (policy.auto_prune != null) mapped.auto_prune = policy.auto_prune;
      await globalThis.electronAPI.invoke('memoryLifecycle:setPolicy', projectDir, mapped);
    } catch (err) {
      set({ error: String(err) });
    }
  },
  ...
  deleteEpisode: async (projectDir, episodeId) => {
    try {
      await globalThis.electronAPI.invoke('memoryLifecycle:deleteMemory', projectDir, episodeId);
      set((state) => ({
  ...
  exportMemories: async (projectDir, outputPath) => {
    try {
      const result = await globalThis.electronAPI.invoke('memoryLifecycle:export', projectDir, outputPath);
