	...
	try {
		...
		if (result.success && result.data) {
			...
			if (!store.selectedVersion) {
				const firstUnreleased = result.data.find(
					(v: ReleaseableVersion) => !v.isReleased,
				);
				if (firstUnreleased) {
...
export async function runPreflightCheck(projectId: string): Promise<void> {
	...
	try {
		const result = await window.electronAPI.runReleasePreflightCheck(
			projectId,
			version,
		);
		if (result.success && result.data) {
...
export function createRelease(projectId: string): void {
	...
	const versionInfo = store.releaseableVersions.find(
		(v) => v.version === version,
	);
	if (!versionInfo) {
