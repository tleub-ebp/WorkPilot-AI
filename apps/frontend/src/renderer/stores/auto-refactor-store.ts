...
export type AutoRefactorPhase = 'idle' | 'analyzing' | 'complete' | 'error' | 'executing';
...
interface AutoRefactorState {
  ...
  // Actions
  openDialog: (autoExecute?: boolean, model?: string, thinkingLevel?: string) => void;
  closeDialog: () => void;
...
export function startAutoRefactor(projectId: string): void {
  ...
  useAutoRefactorStore.setState({ streamingOutput: '', error: null, result: null, executionResult: null });
...
export function setupAutoRefactorListeners(): () => void {
  ...
  const unsubChunk = window.electronAPI.onAutoRefactorStreamChunk((chunk: string) => {
    store().appendStreamingOutput(chunk);
  });

  // Listen for status updates
  const unsubStatus = window.electronAPI.onAutoRefactorStatus((status: string) => {
