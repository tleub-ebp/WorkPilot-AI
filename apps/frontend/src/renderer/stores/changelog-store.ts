...
export const useChangelogStore = create<ChangelogState>((set, get) => ({
	...
	// Git history options actions
	setGitHistoryType: (type) =>
		set({ gitHistoryType: type, previewCommits: [] }),
	...
	setCompareBranch: (branch) =>
		set({ compareBranch: branch, previewCommits: [] }),
...
export async function loadChangelogData(projectId: string): Promise<void> {
	...
	try {
		...
		const tasksResult = await window.electronAPI.getChangelogDoneTasks(
			projectId,
			tasks,
		);
		...
		const changelogResult =
			await window.electronAPI.readExistingChangelog(projectId);
...
export async function loadGitData(projectId: string): Promise<void> {
	...
	try {
		...
		store.setError(
			error instanceof Error ? error.message : "Failed to load git data",
		);
	} finally {
	...
	try {
		...
		if (store.sourceMode === "git-history") {
			...
			options = {
				...
				fromTag:
					store.gitHistoryType === "since-version"
		...
		const result = await window.electronAPI.getChangelogCommitsPreview(
			projectId,
			options,
			mode,
		);
		...
		store.setError(
			error instanceof Error ? error.message : "Failed to load commits preview",
		);
		store.setPreviewCommits([]);
	} finally {
		store.setIsLoadingCommits(false);
