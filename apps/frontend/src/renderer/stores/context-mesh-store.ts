...
export type ContextMeshTab = 'overview' | 'patterns' | 'handbook' | 'transfers' | 'recommendations' | 'projects';
...
export const useContextMeshStore = create<ContextMeshState>((set) => ({
  ...
  removeHandbookEntry: (entryId) =>
    set((state) => ({
      handbookEntries: state.handbookEntries.filter((e) => e.entry_id !== entryId),
    })),

  updateTransferStatus: (transferId, status) =>
    set((state) => ({
      skillTransfers: state.skillTransfers.map((t) =>
        t.transfer_id === transferId ? { ...t, status: status as SkillTransfer['status'] } : t
      ),
    })),
  ...
  updateRecommendationStatus: (recId, status) =>
    set((state) => ({
      recommendations: state.recommendations.map((r) =>
        r.recommendation_id === recId ? { ...r, status: status as ContextualRecommendation['status'] } : r
      ),
    })),
...
export async function loadContextMeshSkillTransfers(): Promise<void> {
  try {
    const result = await window.electronAPI.invoke('contextMesh:getSkillTransfers');
    if (result.success) {
...
export async function loadContextMeshRecommendations(targetProject?: string): Promise<void> {
  try {
    const result = await window.electronAPI.invoke('contextMesh:getRecommendations', targetProject);
    if (result.success) {
...
export async function registerProject(projectDir: string): Promise<boolean> {
  try {
    const result = await window.electronAPI.invoke('contextMesh:registerProject', projectDir);
    if (result.success) {
...
export async function unregisterProject(projectDir: string): Promise<boolean> {
  try {
    const result = await window.electronAPI.invoke('contextMesh:unregisterProject', projectDir);
    if (result.success) {
      await loadContextMeshProjects();
      await loadContextMeshSummary();
      return true;
...
export async function deletePattern(patternId: string): Promise<boolean> {
  try {
    const result = await window.electronAPI.invoke('contextMesh:deletePattern', patternId);
    if (result.success) {
...
export async function deleteHandbookEntry(entryId: string): Promise<boolean> {
  try {
    const result = await window.electronAPI.invoke('contextMesh:deleteHandbookEntry', entryId);
    if (result.success) {
...
export async function updateSkillTransferStatus(transferId: string, status: string): Promise<boolean> {
  try {
    const result = await window.electronAPI.invoke('contextMesh:updateTransferStatus', transferId, status);
    if (result.success) {
...
export async function updateRecommendationStatus(recId: string, status: string): Promise<boolean> {
  try {
    const result = await window.electronAPI.invoke('contextMesh:updateRecommendationStatus', recId, status);
    if (result.success) {
    ...
    console.error('[ContextMesh] Failed to update recommendation status:', error);
    return false;
  }
}

export function startContextMeshAnalysis(model?: string, thinkingLevel?: string): void {
  const store = useContextMeshStore.getState();
