...
interface PluginMarketplaceState {
  ...
  updateInstalledPlugin: (pluginId: string, updates: Partial<InstalledPlugin>) => void;
...
export const usePluginMarketplaceStore = create<PluginMarketplaceState>((set, get) => ({
  ...
  isPluginInstalled: (pluginId) => get().installed.some((p) => p.pluginId === pluginId),
  getInstalledPlugin: (pluginId) => get().installed.find((p) => p.pluginId === pluginId),
}));
...
export async function loadPluginCatalog(): Promise<void> {
  ...
  store.setError(null);
  try {
    const result = await globalThis.electronAPI?.invoke('pluginMarketplace:getCatalog');
    if (result?.success) {
    ...
    store.setError(err instanceof Error ? err.message : 'Failed to load catalog');
  } finally {
...
export async function loadInstalledPlugins(): Promise<void> {
  ...
  try {
    const result = await globalThis.electronAPI?.invoke('pluginMarketplace:getInstalled');
    if (result?.success) {
      store.setInstalled(result.data);
    }
  } catch (err) {
    console.error('[Plugin Marketplace] Failed to load installed plugins:', err);
  } finally {
...
export async function installPlugin(
  ...
  try {
    const result = await globalThis.electronAPI?.invoke('pluginMarketplace:install', {
      pluginId,
      config,
    });
    if (result?.success) {
...
export async function uninstallPlugin(pluginId: string): Promise<boolean> {
  try {
    const result = await globalThis.electronAPI?.invoke('pluginMarketplace:uninstall', {
      pluginId,
    });
    if (result?.success) {
...
export async function togglePlugin(pluginId: string, enabled: boolean): Promise<void> {
  try {
    await globalThis.electronAPI?.invoke('pluginMarketplace:toggle', { pluginId, enabled });
    usePluginMarketplaceStore.getState().updateInstalledPlugin(pluginId, { enabled });
  } catch (err) {
