...
export type EventType =
  ...
  | 'task_updated' | 'task_locked' | 'task_unlocked'
  | 'task_created' | 'task_deleted' | 'task_moved'
  | 'chat_message' | 'agent_started' | 'agent_completed'
  | 'notification' | 'conflict_detected' | 'sync_request' | 'sync_response';
...
interface CollaborationState {
  ...
  // Actions — locks
  lockTask: (taskId: string, lockedBy: string, lockType: LockType, reason?: string) => void;
...
export const useCollaborationStore = create<CollaborationState>((set, get) => ({
      ...
      if (exists) {
        return {
          users: state.users.map((u) =>
            u.userId === user.userId ? { ...u, ...user, status: 'online' as UserStatus } : u
          ),
        };
      }
      return {
        users: [...state.users, { ...user, avatarColor: getAvatarColor(user.userId) }],
      };
    });
  },
  ...
  updateUserStatus: (userId, status) => {
    set((state) => ({
      users: state.users.map((u) =>
        u.userId === userId ? { ...u, status, lastActivity: new Date().toISOString() } : u
      ),
    }));
  },
  ...
  setUserCurrentTask: (userId, taskId) => {
    set((state) => ({
      users: state.users.map((u) =>
        u.userId === userId ? { ...u, currentTask: taskId, lastActivity: new Date().toISOString() } : u
      ),
    }));
  },
  ...
  resolveConflict: (conflictId, resolvedValue) => {
    set((state) => ({
      conflicts: state.conflicts.map((c) =>
        c.conflictId === conflictId ? { ...c, resolved: true, resolvedValue } : c
      ),
    }));
  },
  ...
  addEvent: (event) => {
    _evtCounter++;
    set((state) => ({
      events: [...state.events.slice(-99), { ...event, eventId: event.eventId || `evt-${_evtCounter.toString().padStart(6, '0')}` }],
    }));
  },
