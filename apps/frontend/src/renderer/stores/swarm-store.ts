...
export const useSwarmStore = create<SwarmState & SwarmActions>()((set, get) => ({
  ...
  handleSubtaskStarted: (event) => {
    set((state) => {
      ...
      if (existing) {
        nodes[event.subtaskId] = { ...existing, state: 'running' as SubtaskState };
      }
      return {
        status: {
          ...state.status,
      ...
      return {
        status: {
          ...
          progressPercent: event.success
            ? Math.round(
                ((state.status.completedSubtasks + 1) / state.status.totalSubtasks) * 100,
              )
  ...
  handleWaveStarted: (event) => {
    set((state) => {
      ...
      if (wave) {
        waves[event.waveIndex] = { ...wave, state: 'running' as SubtaskState };
      }
      return {
        status: {
          ...state.status,
    ...
    set((state) => {
      ...
      if (wave) {
        waves[event.waveIndex] = {
          ...wave,
          state: (event.allSucceeded ? 'completed' : 'failed') as SubtaskState,
          durationSeconds: event.durationSeconds ?? null,
        };
      }
