...
export type PipelineRunStatus = 'pending' | 'triggered' | 'running' | 'success' | 'failed';
...
interface CICDTriggersState {
  ...
  triggerPipeline: (projectDir: string, params: {
    provider: string;
    prUrl?: string;
    branch?: string;
    workflow?: string;
...
export const useCICDTriggersStore = create<CICDTriggersState>((set, get) => ({
  ...
  loadConfig: async (projectDir) => {
    ...
    try {
      const result = await globalThis.electronAPI.invoke('cicdTriggers:getConfig', projectDir);
      if (result.success) {
  ...
  setConfig: async (projectDir, config) => {
    try {
      ...
      await globalThis.electronAPI.invoke('cicdTriggers:setConfig', projectDir, flat);
      set((state) => ({
        config: state.config ? { ...state.config, ...config } : config as CICDConfig,
      }));
    } catch (err) {
      set({ error: String(err) });
    }
  ...
  loadRuns: async (projectDir, limit = 50) => {
    try {
      const result = await globalThis.electronAPI.invoke('cicdTriggers:listRuns', projectDir, limit);
      if (result.success) {
  ...
  triggerPipeline: async (projectDir, params) => {
    ...
    try {
      const result = await globalThis.electronAPI.invoke('cicdTriggers:trigger', projectDir, {
