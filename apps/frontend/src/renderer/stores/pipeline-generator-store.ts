...
export type PipelineGeneratorPhase =
	| "idle"
	| "generating"
	| "complete"
	| "error";
...
export const usePipelineGeneratorStore = create<PipelineGeneratorState>(
	(set) => ({
		...initialState,
		...
		setSelectedPlatformView: (selectedPlatformView) =>
			set({ selectedPlatformView }),
...
export function generatePipelines(projectDir: string): void {
	...
	usePipelineGeneratorStore.setState({
		streamingOutput: "",
		error: null,
		result: null,
	});
...
export function setupPipelineGeneratorListeners(): () => void {
	...
	const unsubChunk = window.electronAPI.onPipelineGeneratorStreamChunk(
		(chunk: string) => {
			store().appendStreamingOutput(chunk);
		},
	);

	const unsubStatus = window.electronAPI.onPipelineGeneratorStatus(
		(status: string) => {
			store().setStatus(status);
		},
	);

	const unsubError = window.electronAPI.onPipelineGeneratorError(
		(error: string) => {
			store().setError(error);
		},
	);

	const unsubComplete = window.electronAPI.onPipelineGeneratorComplete(
		(result: PipelineGeneratorResult) => {
			store().setResult(result);
			const firstPlatform = result.platforms_generated?.[0] as
				| CiPlatform
				| undefined;
