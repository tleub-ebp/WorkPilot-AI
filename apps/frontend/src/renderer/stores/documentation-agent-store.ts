...
export const useDocumentationAgentStore = create<DocumentationAgentState>((set) => ({
  ...initialState,
...
export function generateDocumentation(projectDir: string): void {
  ...
  useDocumentationAgentStore.setState({ streamingOutput: '', error: null, result: null });
...
export function setupDocumentationAgentListeners(): () => void {
  ...
  const unsubChunk = window.electronAPI.onDocumentationAgentStreamChunk((chunk: string) => {
    store().appendStreamingOutput(chunk);
  });

  const unsubStatus = window.electronAPI.onDocumentationAgentStatus((status: string) => {
    store().setStatus(status);
    if (status.toLowerCase().includes('phase 1') || status.toLowerCase().includes('analyz')) {
      store().setPhase('analyzing');
    } else if (status.toLowerCase().includes('phase 2') || status.toLowerCase().includes('generat')) {
      store().setPhase('generating');
    } else if (status.toLowerCase().includes('phase 3') || status.toLowerCase().includes('updat')) {
      store().setPhase('updating');
    }
  });

  const unsubError = window.electronAPI.onDocumentationAgentError((error: string) => {
    store().setError(error);
  });

  const unsubComplete = window.electronAPI.onDocumentationAgentComplete((result: DocumentationAgentResult) => {
