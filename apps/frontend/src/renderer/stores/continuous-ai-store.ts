...
interface ContinuousAIActions {
  ...
  setModuleConfig: (module: ModuleName, partial: Record<string, unknown>) => void;
...
export const useContinuousAIStore = create<ContinuousAIState & ContinuousAIActions>()(
  persist(
    (set, get) => ({
      ...initialState,
        ...
        switch (event.type) {
          ...
          case 'action_completed': {
            ...
            if (completedAction.id) {
              set((state) => ({
                recentActions: [...state.recentActions, completedAction].slice(-50),
