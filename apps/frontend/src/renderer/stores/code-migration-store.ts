...
export const useCodeMigrationStore = create<CodeMigrationState>((set) => ({
  ...
  setMigrationDescription: (migrationDescription) => set({ migrationDescription }),
...
export function startCodeMigration(projectDir: string): void {
  ...
  useCodeMigrationStore.setState({ streamingOutput: '', error: null, result: null, taskProgress: null });
...
export function setupCodeMigrationListeners(): () => void {
  ...
  const unsubChunk = window.electronAPI.onCodeMigrationStreamChunk((chunk: string) => {
    store().appendStreamingOutput(chunk);
  });

  const unsubStatus = window.electronAPI.onCodeMigrationStatus((status: string) => {
    store().setStatus(status);
    if (status.toLowerCase().includes('phase 1') || status.toLowerCase().includes('analyz')) {
      store().setPhase('analyzing');
    } else if (status.toLowerCase().includes('phase 2') || status.toLowerCase().includes('plan')) {
      store().setPhase('planning');
    } else if (status.toLowerCase().includes('phase 3') || status.toLowerCase().includes('execut')) {
  ...
  const unsubError = window.electronAPI.onCodeMigrationError((error: string) => {
    store().setError(error);
  });

  const unsubComplete = window.electronAPI.onCodeMigrationComplete((result: CodeMigrationResult) => {
    store().setResult(result);
  });

  const unsubProgress = window.electronAPI.onCodeMigrationTaskProgress((progress: TaskProgress) => {
