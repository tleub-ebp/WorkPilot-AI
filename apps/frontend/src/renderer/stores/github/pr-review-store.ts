...
interface PRReviewStoreState {
	...
	setPRReviewResult: (
		...
		options?: { preserveNewCommitsCheck?: boolean },
	) => void;
	setPRReviewError: (
		projectId: string,
		prNumber: number,
...
export const usePRReviewStore = create<PRReviewStoreState>((set, get) => ({
	...
	// Actions
	startPRReview: (projectId: string, prNumber: number) =>
		set((state) => {
	...
	startFollowupReview: (projectId: string, prNumber: number) =>
		set((state) => {
			const key = `${projectId}:${prNumber}`;
			const existing = state.prReviews[key];
	...
	setPRReviewProgress: (projectId: string, progress: PRReviewProgress) =>
		set((state) => {
			...
			return {
				prReviews: {
					...
					[key]: {
						...
						newCommitsCheck: options?.preserveNewCommitsCheck
	...
	setPRReviewError: (projectId: string, prNumber: number, error: string) =>
		set((state) => {
			const key = `${projectId}:${prNumber}`;
			const existing = state.prReviews[key];
			return {
	...
	clearPRReview: (projectId: string, prNumber: number) =>
		set((state) => {
			const key = `${projectId}:${prNumber}`;
