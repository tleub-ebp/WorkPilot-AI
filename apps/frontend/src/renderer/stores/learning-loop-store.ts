 ...
 */
export function startLearningAnalysis(
	projectDir: string,
	specId?: string,
): void {
	const store = useLearningLoopStore.getState();
 ...
 * Delete a pattern
 */
export async function deleteLearningPattern(
	projectDir: string,
	patternId: string,
): Promise<boolean> {
	try {
		const result = await globalThis.electronAPI.deleteLearningPattern(
			projectDir,
			patternId,
		);
		if (result.success) {
 ...
 */
export async function toggleLearningPattern(
	projectDir: string,
	patternId: string,
): Promise<boolean> {
	try {
		const result = await globalThis.electronAPI.toggleLearningPattern(
			projectDir,
			patternId,
		);
		if (result.success && result.data !== undefined) {
			useLearningLoopStore
				.getState()
				.updatePatternInList(patternId, { enabled: result.data });
			return true;
		}
		return false;
	} catch {
