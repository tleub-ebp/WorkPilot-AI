...
export const useMRReviewStore = create<MRReviewStoreState>((set, get) => ({
	...
	// Actions
	startMRReview: (projectId: string, mrIid: number) =>
		set((state) => {
	...
	setMRReviewProgress: (projectId: string, progress: GitLabMRReviewProgress) =>
		set((state) => {
	...
	setMRReviewResult: (projectId: string, result: GitLabMRReviewResult) =>
		set((state) => {
	...
	setMRReviewError: (projectId: string, mrIid: number, error: string) =>
		set((state) => {
			const key = `${projectId}:${mrIid}`;
			const existing = state.mrReviews[key];
			return {
	...
	clearMRReview: (projectId: string, mrIid: number) =>
		set((state) => {
			const key = `${projectId}:${mrIid}`;
...
export function initializeMRReviewListeners(): void {
	...
	if (!window.electronAPI?.onGitLabMRReviewProgress) {
		console.warn(
			"[GitLab MR Store] GitLab MR Review API not available, skipping listener setup",
		);
		return;
	}
	...
	if (typeof api.removeGitLabMRReviewProgress === "function") {
		cleanupFunctions.push(() =>
			(api.removeGitLabMRReviewProgress as (handler: unknown) => void)?.(
				progressHandler,
			),
		);
	}
	if (typeof api.removeGitLabMRReviewComplete === "function") {
		cleanupFunctions.push(() =>
			(api.removeGitLabMRReviewComplete as (handler: unknown) => void)?.(
				completeHandler,
			),
		);
	}
	if (typeof api.removeGitLabMRReviewError === "function") {
		cleanupFunctions.push(() =>
			(api.removeGitLabMRReviewError as (handler: unknown) => void)?.(
				errorHandler,
			),
		);
	}
