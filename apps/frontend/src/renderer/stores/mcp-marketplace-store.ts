...
interface McpMarketplaceState {
  ...
  updateInstalledServer: (serverId: string, updates: Partial<McpInstalledServer>) => void;
  addBuilderProject: (project: McpBuilderProject) => void;
  updateBuilderProject: (projectId: string, updates: Partial<McpBuilderProject>) => void;
...
export const useMcpMarketplaceStore = create<McpMarketplaceState>((set, get) => ({
  ...
  removeBuilderProject: (projectId) =>
    set((state) => ({
      builderProjects: state.builderProjects.filter((p) => p.id !== projectId),
    })),
...
export async function uninstallMarketplaceServer(serverId: string): Promise<boolean> {
  const store = useMcpMarketplaceStore.getState();
...
export async function saveBuilderProject(project: McpBuilderProject): Promise<boolean> {
  const store = useMcpMarketplaceStore.getState();
...
export async function deleteBuilderProject(projectId: string): Promise<boolean> {
  const store = useMcpMarketplaceStore.getState();
