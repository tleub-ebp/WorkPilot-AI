...
export function setupVoiceControlListeners(): () => void {
	...
	const unsubChunk = window.electronAPI.onVoiceControlStreamChunk(
		(chunk: string) => {
	...
	const unsubStatus = window.electronAPI.onVoiceControlStatus(
		(status: string) => {
	...
	const unsubAudioLevel = window.electronAPI.onVoiceControlAudioLevel(
		(level: number) => {
	...
	const unsubDuration = window.electronAPI.onVoiceControlDuration(
		(duration: number) => {
