...
export interface DecisionNode {
  ...
  node_type: 'root' | 'thinking' | 'tool_call' | 'decision' | 'result' | 'error' | 'branch';
...
export interface MissionControlState {
  ...
  createAgent: (name: string, role: string, provider: string, model: string, modelLabel: string) => Promise<AgentSlot | null>;
  removeAgent: (agentId: string) => Promise<void>;
  updateAgentConfig: (agentId: string, config: Partial<{ provider: string; model: string; model_label: string; name: string; role: string }>) => Promise<void>;
...
async function mcFetch<T = Record<string, unknown>>(path: string, options?: RequestInit): Promise<T> {
...
export const useMissionControlStore = create<MissionControlState>((set, get) => ({
  ...
  startSession: async (name = 'Mission Control') => {
    ...
    try {
      const res = await mcFetch<{ success: boolean; session: MCSession }>('/session/start', {
        method: 'POST',
  ...
  stopSession: async () => {
    ...
    set({ session: null, isActive: false, agents: [], decisionTrees: {}, events: [] });
  },
  ...
  createAgent: async (name, role, provider, model, modelLabel) => {
    set({ isLoading: true });
    try {
      const res = await mcFetch<{ success: boolean; agent: AgentSlot }>('/agents', {
        method: 'POST',
        body: JSON.stringify({ name, role, provider, model, model_label: modelLabel }),
      });
      if (res.success) {
  ...
  updateAgentConfig: async (agentId, config) => {
    try {
      const res = await mcFetch<{ success: boolean; agent: AgentSlot }>(`/agents/${agentId}`, {
        method: 'PUT',
  ...
  fetchDecisionTree: async (agentId) => {
    try {
      const res = await mcFetch<{ success: boolean; tree: DecisionTree }>(`/agents/${agentId}/decision-tree`);
      if (res.success) {
        set((s) => ({
