...
export const useDecisionLoggerStore = create<DecisionLoggerState>((set, _get) => ({
  ...
  addLiveEntry: (taskId, entry) =>
    set((state) => {
      ...
      if (existing.some((e) => e.id === entry.id && e.session_id === entry.session_id)) {
        return state;
      }
      ...
      return {
        entriesByTask: {
          ...state.entriesByTask,
          [taskId]: updated.length > MAX_LIVE_ENTRIES
  ...
  setHistoricalEntries: (taskId, entries) =>
    set((state) => {
      ...
      const historicalIds = new Set(entries.map((e) => `${e.session_id}:${e.id}`));
      const liveOnly = live.filter((e) => !historicalIds.has(`${e.session_id}:${e.id}`));
