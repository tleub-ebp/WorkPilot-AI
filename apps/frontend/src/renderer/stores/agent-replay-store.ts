...
type ReplayTab = 'timeline' | 'diff' | 'heatmap' | 'tokens' | 'debug' | 'compare';
...
export interface AgentReplayState {
  ...
  // A/B Comparison
  compareSessionsAction: (sessionAId: string, sessionBId: string) => Promise<void>;
  clearComparison: () => void;

  // Breakpoints
  addBreakpoint: (sessionId: string, type: string, condition: string, description?: string) => Promise<void>;
...
async function replayFetch<T = Record<string, unknown>>(path: string, options?: RequestInit): Promise<T> {
...
export const useAgentReplayStore = create<AgentReplayState>((set, get) => ({
  ...
  fetchSessions: async () => {
    ...
    try {
      const res = await replayFetch<{ success: boolean; sessions: ReplaySessionSummary[] }>('/sessions');
  ...
  loadSession: async (sessionId: string) => {
    set({ activeSessionLoading: true, error: null, currentStepIndex: 0, isPlaying: false });
    try {
      const res = await replayFetch<{ success: boolean; session: ReplaySessionFull }>(`/sessions/${sessionId}`);
      set({
  ...
  deleteSession: async (sessionId: string) => {
    try {
      ...
      set((s) => ({
        ...
        activeSession: s.activeSession?.session_id === sessionId ? null : s.activeSession,
      }));
    } catch (e) {
  ...
  play: () => {
    ...
    if (!activeSession || currentStepIndex >= activeSession.steps.length - 1) return;

    set({ isPlaying: true });

    const advanceStep = () => {
      const { currentStepIndex: idx, activeSession: sess, isPlaying: playing } = get();
      ...
      if (nextStep.is_breakpoint) {
        set({ currentStepIndex: nextIdx, selectedStepId: nextStep.id, isPlaying: false, playbackTimer: null });
        return;
      }
  ...
  stop: () => {
    const { playbackTimer } = get();
    if (playbackTimer) clearTimeout(playbackTimer);
    set({ isPlaying: false, playbackTimer: null, currentStepIndex: 0, selectedStepId: null });
  },
  ...
  selectStep: (stepId: string | null) => {
    ...
    set({ selectedStepId: stepId, currentStepIndex: idx >= 0 ? idx : get().currentStepIndex });
  },

  // --- Heatmap ---
  fetchHeatmap: async (sessionId: string) => {
    try {
      const res = await replayFetch<{ success: boolean; heatmap: HeatmapEntry[] }>(`/sessions/${sessionId}/heatmap`);
  ...
  clearComparison: () => {
    set({ comparison: null, comparisonSessionA: null, comparisonSessionB: null });
  },

  // --- Breakpoints ---
  addBreakpoint: async (sessionId: string, type: string, condition: string, description?: string) => {
    try {
      const res = await replayFetch<{ success: boolean; breakpoint: Breakpoint }>(`/sessions/${sessionId}/breakpoints`, {
        method: 'POST',
        body: JSON.stringify({ breakpoint_type: type, condition, description: description || '' }),
      });
      if (res.success) {
  ...
  removeBreakpoint: async (sessionId: string, breakpointId: string) => {
    try {
      await replayFetch(`/sessions/${sessionId}/breakpoints/${breakpointId}`, { method: 'DELETE' });
      set((s) => ({ breakpoints: s.breakpoints.filter((b) => b.id !== breakpointId) }));
    } catch (e) {
      set({ error: (e as Error).message });
    }
  },
