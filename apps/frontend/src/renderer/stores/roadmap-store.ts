		...
		set((state) => {
			...
			return {
				generationStatus: {
					...
					lastActivityAt: isStoppingGeneration
						? undefined
...
export async function loadRoadmap(projectId: string): Promise<void> {
	...
	if (statusResult.success && statusResult.data?.isRunning) {
		...
		const progressResult =
			await window.electronAPI.loadRoadmapProgress(projectId);
		if (progressResult.success && progressResult.data) {
			...
			store.setGenerationStatus({
				phase:
					persistedProgress.phase !== "idle"
						? persistedProgress.phase
						: "analyzing",
				progress: persistedProgress.progress,
				message:
					persistedProgress.message || "Roadmap generation in progress...",
				startedAt: parseDate(persistedProgress.startedAt) ?? new Date(),
				lastActivityAt:
					parseDate(persistedProgress.lastActivityAt) ?? new Date(),
			});
		} else {
	...
	if (result.success && result.data) {
		...
		if (migratedRoadmap !== result.data) {
			window.electronAPI
				.saveRoadmap(projectId, migratedRoadmap)
				.catch((err) => {
...
export function getFeatureStats(roadmap: Roadmap | null): {
	...
	roadmap.features.forEach((feature) => {
		...
		byComplexity[feature.complexity] =
			(byComplexity[feature.complexity] || 0) + 1;
	});
