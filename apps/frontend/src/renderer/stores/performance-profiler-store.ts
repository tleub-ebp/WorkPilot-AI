...
export const usePerformanceProfilerStore = create<PerformanceProfilerState>((set) => ({
  ...initialState,
  ...
  setImplementationResult: (implementationResult) => set({ implementationResult }),
...
export function startPerformanceProfiling(projectDir: string): void {
  ...
  usePerformanceProfilerStore.setState({ streamingOutput: '', error: null, result: null, implementationResult: null });
...
export function setupPerformanceProfilerListeners(): () => void {
  ...
  const unsubChunk = window.electronAPI.onPerformanceProfilerStreamChunk((chunk: string) => {
    store().appendStreamingOutput(chunk);
  });

  const unsubStatus = window.electronAPI.onPerformanceProfilerStatus((status: string) => {
    store().setStatus(status);
    if (status.toLowerCase().includes('phase 1') || status.toLowerCase().includes('detect')) {
      store().setPhase('profiling');
    } else if (status.toLowerCase().includes('phase 2') || status.toLowerCase().includes('benchmark')) {
      store().setPhase('analyzing');
    } else if (status.toLowerCase().includes('phase 3') || status.toLowerCase().includes('suggest')) {
      store().setPhase('optimizing');
    }
  });

  const unsubError = window.electronAPI.onPerformanceProfilerError((error: string) => {
    store().setError(error);
  });

  const unsubComplete = window.electronAPI.onPerformanceProfilerComplete((result: PerformanceProfilerResult) => {
    store().setResult(result);
  });

  const unsubImpl = window.electronAPI.onPerformanceProfilerImplementationComplete((result: unknown) => {
