  ...
  return (
    <div className="rounded-lg border border-[var(--border-primary)] bg-[var(--bg-secondary)] p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className={`text-xs px-2 py-0.5 rounded-full ${operation.success ? 'bg-green-500/20 text-green-400' : operation.completed_at ? 'bg-red-500/20 text-red-400' : 'bg-blue-500/20 text-blue-400'}`}>
          {operation.success ? t('selfHealing:resolved') : operation.completed_at ? t('selfHealing:failed') : t('selfHealing:inProgress')}
        </span>
      </div>

      <div className="relative pl-4">
        {operation.steps.map((step, index) => (
          <div key={`${step.name}-${index}`} className="relative pb-4 last:pb-0">
            {/* Vertical line */}
            ...
            <div className="ml-3">
              ...
              {step.detail && (
                <p className="text-xs text-[var(--text-tertiary)] mt-0.5">{step.detail}</p>
              )}
            </div>
          </div>
        ))}
