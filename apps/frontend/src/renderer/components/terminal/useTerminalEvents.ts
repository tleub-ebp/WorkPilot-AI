...
export function useTerminalEvents({
	...
	useEffect(() => {
		const cleanup = window.electronAPI.onTerminalWorktreeConfigChange(
			(id, config) => {
				if (id === terminalId) {
	...
	useEffect(() => {
		const cleanup = window.electronAPI.onTerminalClaudeSession(
			(id, sessionId) => {
				if (id === terminalId) {
					const store = useTerminalStore.getState();
	...
	useEffect(() => {
		const cleanup = window.electronAPI.onTerminalClaudeExit((id: string) => {
			if (id === terminalId) {
				...
				console.warn(
					"[Terminal] Claude exited, reset mode for terminal:",
					terminalId,
				);
			}
		});
	...
	useEffect(() => {
		const cleanup = window.electronAPI.onTerminalPendingResume(
			(id, _sessionId) => {
				if (id === terminalId) {
