...
export function useAutoNaming({ terminalId, cwd }: UseAutoNamingOptions) {
	...
	const autoNameTerminals = useSettingsStore(
		(state) => state.settings.autoNameTerminals,
	);
	const autoNameClaudeTerminals = useSettingsStore(
		(state) => state.settings.autoNameClaudeTerminals,
	);
	...
	const setClaudeNamedOnce = useTerminalStore(
		(state) => state.setClaudeNamedOnce,
	);
	...
	const triggerAutoNaming = useCallback(async () => {
		...
		try {
			const result = await window.electronAPI.generateTerminalName(
				command,
				terminal?.cwd || cwd,
			);
		...
		autoNameTerminals,
		autoNameClaudeTerminals,
		terminal?.isClaudeMode,
		terminal?.claudeNamedOnce,
		terminal?.cwd,
		cwd,
		terminalId,
		updateTerminal,
