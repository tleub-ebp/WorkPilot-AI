...
export function usePtyProcess({
	 ...
	 */
	const scheduleRetryOrFail = useCallback(
		(error: string): boolean => {
			if (
				isRecreatingRef?.current &&
				recreationRetryCountRef.current < MAX_RECREATION_RETRIES
			) {
	...
	useEffect(() => {
		...
		if (skipCreation && isRecreatingRef?.current) {
			debugLog(
				`[usePtyProcess] Skipping PTY creation for terminal: ${terminalId} - dimensions not ready during recreation, scheduling retry`,
			);
		...
		if (isCreatingRef.current || isCreatedRef.current) {
			debugLog(
				`[usePtyProcess] Skipping PTY creation for terminal: ${terminalId} - already creating: ${isCreatingRef.current}, already created: ${isCreatedRef.current}`,
			);
			return;
		}
		...
		const alreadyRunning =
			...
			terminalState?.status === "claude-active";
		const isRestored = terminalState?.isRestored;

		debugLog(
			`[usePtyProcess] Starting PTY creation for terminal: ${terminalId}`,
		);
		debugLog(
			`[usePtyProcess] Terminal ${terminalId} state: isRestored=${isRestored}, status=${terminalState?.status}`,
		);
		debugLog(
			`[usePtyProcess] Terminal ${terminalId} dimensions for PTY: cols=${cols}, rows=${rows}`,
		);
		...
		if (isRestored && terminalState) {
			// Restored session
			debugLog(
				`[usePtyProcess] Restoring session for terminal: ${terminalId}, cwd: ${terminalState.cwd}, isClaudeMode: ${terminalState.isClaudeMode}, claudeSessionId: ${terminalState.claudeSessionId || "none"}`,
			);
			window.electronAPI
				...
				.then((result) => {
					if (result.success && result.data?.success) {
						debugLog(
							`[usePtyProcess] Successfully restored PTY session for terminal: ${terminalId}`,
						);
						handleSuccess();
						const store = getStore();
						store.setTerminalStatus(
							terminalId,
							terminalState.isClaudeMode ? "claude-active" : "running",
						);
						...
						debugError(
							`[usePtyProcess] Failed to restore PTY session for terminal: ${terminalId}, error: ${errorMsg}`,
						);
				...
				.catch((err) => {
					debugError(
						`[usePtyProcess] Exception restoring PTY session for terminal: ${terminalId}, error:`,
						err,
					);
			...
			// New terminal
			debugLog(
				`[usePtyProcess] Creating new PTY for terminal: ${terminalId}, cwd: ${cwd}, projectPath: ${projectPath}`,
			);
			window.electronAPI
				.createTerminal({
					id: terminalId,
				...
				.then((result) => {
					if (result.success) {
						debugLog(
							`[usePtyProcess] Successfully created PTY for terminal: ${terminalId}`,
						);
						handleSuccess();
						if (!alreadyRunning) {
						...
						debugError(
							`[usePtyProcess] Failed to create PTY for terminal: ${terminalId}, error: ${errorMsg}`,
						);
						handleError(errorMsg);
					}
				})
				.catch((err) => {
					debugError(
						`[usePtyProcess] Exception creating PTY for terminal: ${terminalId}, error:`,
						err,
					);
					handleError(err.message);
				});
