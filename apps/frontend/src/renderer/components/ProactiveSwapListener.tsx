...
export function ProactiveSwapListener() {
	...
	const [notification, setNotification] = useState<SwapNotification | null>(
		null,
	);
	const [isVisible, setIsVisible] = useState(false);

	useEffect(() => {
		const unsubscribe = window.electronAPI.onProactiveSwapNotification(
			(data) => {
				...
				<div className="flex items-start gap-3">
					...
					<div className="flex-1 min-w-0">
						<p className="text-sm font-semibold text-foreground">
							{t("notification.accountSwitched")}
						</p>
						<p className="text-xs text-muted-foreground mt-1">
							{t("notification.swapFrom")}{" "}
							<strong>{notification.fromProfile}</strong>{" "}
							{t("notification.swapTo")}{" "}
