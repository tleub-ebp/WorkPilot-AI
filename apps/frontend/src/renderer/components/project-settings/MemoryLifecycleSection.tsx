...
export function MemoryLifecycleSection({
	projectPath,
}: MemoryLifecycleSectionProps) {
	...
	return (
		<div className="space-y-4">
			...
			<div className="flex items-center justify-between">
				<div>
					<h4 className="text-sm font-semibold text-foreground">
						{t("memoryLifecycle.title")}
					</h4>
					<p className="text-xs text-muted-foreground mt-0.5">
						{t("memoryLifecycle.description")}
					</p>
				</div>
				<Button
					variant="ghost"
				...
				<div className="grid grid-cols-2 gap-3 rounded-md border border-border bg-muted/30 p-3">
					<div>
						<p className="text-xs text-muted-foreground">
							{t("memoryLifecycle.episodeCount")}
						</p>
						<p className="text-sm font-medium">
							{status.episode_count.toLocaleString()}
						</p>
					</div>
					<div>
						<p className="text-xs text-muted-foreground">
							{t("memoryLifecycle.diskUsage")}
						</p>
						<p className="text-sm font-medium">
			...
			<div className="grid grid-cols-2 gap-4">
				<div className="space-y-1">
					<Label className="text-xs text-muted-foreground">
						{t("memoryLifecycle.retentionDays")}
					</Label>
					<Input
						type="number"
						min={1}
						...
						onBlur={(e) =>
							setPolicy(projectPath, {
								retention_days: Number(e.target.value) || 90,
							})
						}
					/>
				</div>
				<div className="space-y-1">
					...
					<Input
						...
						onBlur={(e) =>
							setPolicy(projectPath, {
								max_episodes: Number(e.target.value) || 10000,
							})
						}
					/>
				</div>
			...
			<div className="space-y-1">
				<Label className="text-xs text-muted-foreground">
					{t("memoryLifecycle.pruneStrategy")}
				</Label>
				<Select
					value={pruneStrategy}
					onValueChange={(v) => {
				...
				>
					...
					<SelectContent>
						<SelectItem value="lru">
							{t("memoryLifecycle.pruneStrategies.lru")}
						</SelectItem>
						<SelectItem value="oldest">
							{t("memoryLifecycle.pruneStrategies.oldest")}
						</SelectItem>
						<SelectItem value="duplicates">
							{t("memoryLifecycle.pruneStrategies.duplicates")}
						</SelectItem>
					</SelectContent>
				</Select>
			</div>
