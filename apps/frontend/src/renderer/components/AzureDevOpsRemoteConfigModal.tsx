...
export function AzureDevOpsRemoteConfigModal({
	open,
	onOpenChange,
	onSave,
	initialConfig,
}: AzureDevOpsRemoteConfigModalProps) {
	...
	const isValidUrl = (url: string) =>
		/^https:\/\/dev\.azure\.com\/.+/.test(url.trim());
	...
	return (
		...
		>
			...
			>
				<DialogHeader>
					<DialogTitle>
						{t("azureConfig.title", "Configurer Azure DevOps")}
					</DialogTitle>
					<DialogDescription>
						{t(
							"azureConfig.desc",
							"Renseignez l’URL de l’organisation et le token PAT.",
						)}
					</DialogDescription>
				</DialogHeader>
				...
				<DialogFooter>
					<Button
						onClick={handleSave}
						disabled={!orgUrl.trim() || !pat.trim() || !isValidUrl(orgUrl)}
					>
						{t("azureConfig.save", "Valider")}
					</Button>
				</DialogFooter>
			</DialogContent>
		</Dialog>
