...
function createFileReferenceDragData(
	path: string,
	name: string,
	isDirectory = false,
): FileReferenceDropData {
	return {
		type: "file-reference",
		path,
		name,
...
describe("useTerminalFileDrop Hook", () => {
	describe("handleNativeDrop - File Path Insertion", () => {
		it("should call sendTerminalInput with escaped file path when dropping valid file reference", () => {
			...
			const dragData = createFileReferenceDragData(
				"/path/to/file.ts",
				"file.ts",
			);
	...
	describe("Edge Cases", () => {
		...
		it("should handle multiple drops with different terminal IDs", () => {
			...
			expect(mockSendTerminalInput).toHaveBeenNthCalledWith(
				1,
				"terminal-a",
				"'/path/to/file.ts' ",
			);
