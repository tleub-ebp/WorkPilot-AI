...
describe("AnalyticsDashboard", () => {
	beforeEach(() => {
		...
		(globalThis as unknown as { electronAPI?: ElectronAPIMock }).electronAPI = {
			...(globalThis as unknown as { electronAPI?: ElectronAPIMock })
				.electronAPI,
	...
	it("renders the no-project empty state when projectPath is not provided", () => {
		...
		expect(
			screen.getByText("Select a project to view analytics"),
		).toBeInTheDocument();
	});
	...
	it("renders loading state initially when projectPath is provided", () => {
		...
		mockGetDashboardSnapshot.mockReturnValue(
			new Promise(() => {
				/* noop */
			}),
		);
	...
	it("renders the dashboard header after data loads", async () => {
		...
		expect(
			screen.getByText(
				"Monitor agent performance, token usage, and build metrics",
			),
		).toBeInTheDocument();
	});
	...
	it("switches to Performance tab", async () => {
		...
		await waitFor(() => {
			...
			expect(
				screen.getByText("Avg. Completion by Complexity"),
			).toBeInTheDocument();
		});
	});
