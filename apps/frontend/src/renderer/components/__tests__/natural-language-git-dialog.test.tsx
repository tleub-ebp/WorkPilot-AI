...
describe("NaturalLanguageGitDialog", () => {
	...
	beforeEach(() => {
		...
		(useProjectStore as any).mockImplementation(
			(selector: (s: typeof mockProjectStore) => unknown) => {
	...
	it("should not render when dialog is closed", () => {
		...
		expect(
			screen.queryByText("naturalLanguageGit:title"),
	...
	it("should render when dialog is open", () => {
		...
		expect(
			screen.getByText("naturalLanguageGit:description"),
		).toBeInTheDocument();
	});
	...
	it("should show command input field", () => {
		...
		expect(textarea).toHaveAttribute(
			"placeholder",
			"naturalLanguageGit:command.placeholder",
		);
	});
	...
	it("should show examples text", () => {
		...
		expect(
			screen.getByText("naturalLanguageGit:command.examples"),
		).toBeInTheDocument();
	});
	...
	it("should show no project warning when no project selected", () => {
		...
		expect(
			screen.getByText("naturalLanguageGit:errors.noProject"),
		).toBeInTheDocument();
	});
	...
	it("should show execute button when idle", () => {
		...
		expect(
			screen.getByText("naturalLanguageGit:actions.execute"),
		).toBeInTheDocument();
		expect(
			screen.getByText("naturalLanguageGit:actions.close"),
		).toBeInTheDocument();
	});
	...
	it("should disable execute button when no command", () => {
		...
		const executeButton = screen.getByText(
			"naturalLanguageGit:actions.execute",
		);
	...
	it("should show processing state", () => {
		...
		expect(
			screen.getByText("naturalLanguageGit:status.processing"),
		).toBeInTheDocument();
		expect(
			screen.getByText("naturalLanguageGit:actions.close"),
		).toBeInTheDocument();
	...
	it("should show error state", () => {
		...
		expect(
			screen.getByText("naturalLanguageGit:status.error"),
		).toBeInTheDocument();
		expect(screen.getByText("Test error")).toBeInTheDocument();
		expect(
			screen.getByText("naturalLanguageGit:actions.tryAgain"),
		).toBeInTheDocument();
	});
	...
	it("should show streaming output during processing", () => {
		...
		expect(
			screen.getByText("naturalLanguageGit:result.streamingOutput"),
		).toBeInTheDocument();
		expect(
			screen.getByText(/Processing\.\.\..*Analyzing\.\.\./s),
		).toBeInTheDocument();
	});
	...
	it("should show result when complete", () => {
		...
		expect(
			screen.getByText("naturalLanguageGit:result.generatedCommand"),
		).toBeInTheDocument();
		expect(screen.getByText("git status")).toBeInTheDocument();
		expect(
			screen.getByText("naturalLanguageGit:result.explanation"),
		).toBeInTheDocument();
		expect(
			screen.getByText("Shows the working tree status"),
		).toBeInTheDocument();
		expect(
			screen.getByText("naturalLanguageGit:result.executionOutput"),
		).toBeInTheDocument();
		expect(
			screen.getByText(/On branch main.*nothing to commit/s),
		).toBeInTheDocument();
		expect(
			screen.getByText("naturalLanguageGit:result.executionSuccess"),
		).toBeInTheDocument();
		expect(
			screen.getByText("naturalLanguageGit:actions.newCommand"),
		).toBeInTheDocument();
	});
	...
	it("should call setNaturalLanguageCommand when input changes", async () => {
		...
		expect(mockStore.setNaturalLanguageCommand).toHaveBeenCalledWith(
			"undo last commit",
		);
	});
	...
	it("should call reset when try again clicked", async () => {
		...
		const tryAgainButton = screen.getByText(
			"naturalLanguageGit:actions.tryAgain",
		);
	...
	it("should call reset when new command clicked", async () => {
		...
		const newCommandButton = screen.getByText(
			"naturalLanguageGit:actions.newCommand",
		);
