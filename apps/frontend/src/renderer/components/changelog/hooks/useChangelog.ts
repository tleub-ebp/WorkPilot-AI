...
export function useChangelog() {
	...
	const existingChangelog = useChangelogStore(
		(state) => state.existingChangelog,
	);
	...
	const gitHistorySinceDate = useChangelogStore(
		(state) => state.gitHistorySinceDate,
	);
	const gitHistoryFromTag = useChangelogStore(
		(state) => state.gitHistoryFromTag,
	);
	const gitHistoryToTag = useChangelogStore((state) => state.gitHistoryToTag);
	const gitHistorySinceVersion = useChangelogStore(
		(state) => state.gitHistorySinceVersion,
	);
	const includeMergeCommits = useChangelogStore(
		(state) => state.includeMergeCommits,
	);
	...
	const customInstructions = useChangelogStore(
		(state) => state.customInstructions,
	);
	const generationProgress = useChangelogStore(
		(state) => state.generationProgress,
	);
	const generatedChangelog = useChangelogStore(
		(state) => state.generatedChangelog,
	);
	const isGenerating = useChangelogStore((state) => state.isGenerating);
	const error = useChangelogStore((state) => state.error);

	// Task actions
	const toggleTaskSelection = useChangelogStore(
	...
	const setGitHistoryType = useChangelogStore(
		(state) => state.setGitHistoryType,
	);
	const setGitHistoryCount = useChangelogStore(
		(state) => state.setGitHistoryCount,
	);
	const setGitHistorySinceDate = useChangelogStore(
		(state) => state.setGitHistorySinceDate,
	);
	const setGitHistoryFromTag = useChangelogStore(
		(state) => state.setGitHistoryFromTag,
	);
	const setGitHistoryToTag = useChangelogStore(
		(state) => state.setGitHistoryToTag,
	);
	const setGitHistorySinceVersion = useChangelogStore(
		(state) => state.setGitHistorySinceVersion,
	);
	const setIncludeMergeCommits = useChangelogStore(
		(state) => state.setIncludeMergeCommits,
	);
	...
	const setCustomInstructions = useChangelogStore(
		(state) => state.setCustomInstructions,
	);
	const updateGeneratedChangelog = useChangelogStore(
		(state) => state.updateGeneratedChangelog,
	);
	const setError = useChangelogStore((state) => state.setError);
	const setIsGenerating = useChangelogStore((state) => state.setIsGenerating);
	const setGenerationProgress = useChangelogStore(
	...
	const initializeFromSettings = useChangelogStore(
		(state) => state.initializeFromSettings,
	);
	useEffect(() => {
		...
		if (selectedProjectId) {
			try {
				...
				if (sourceMode === "tasks" && selectedTaskIds.length > 0) {
					...
					const result =
						await window.electronAPI.suggestChangelogVersionFromCommits(
							selectedProjectId,
							previewCommits,
						);
					if (result.success && result.data) {
	...
	return {
		...
		handleDone,
		handleRefresh: () =>
			selectedProjectId && loadChangelogData(selectedProjectId),
	};
}
