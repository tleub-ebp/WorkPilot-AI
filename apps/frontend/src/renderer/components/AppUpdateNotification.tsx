...
export function AppUpdateNotification() {
	...
	const [updateInfo, setUpdateInfo] = useState<AppUpdateAvailableEvent | null>(
		null,
	);
	...
	const handleDownload = async () => {
		...
		try {
			...
			if (!result.success) {
				setDownloadError(
					result.error ||
						t("dialogs:appUpdate.downloadError", "Failed to download update"),
				);
				setIsDownloading(false);
			}
		} catch (error) {
			...
			<DialogContent className="max-w-2xl">
				<DialogHeader>
					...
					<DialogDescription>
						{t(
							...
							"A new version of WorkPilot AI is ready to download",
						)}
					</DialogDescription>
				</DialogHeader>
				...
				<div className="space-y-4">
					...
					<Button
						...
						onClick={() =>
							window.electronAPI.openExternal(CLAUDE_CODE_CHANGELOG_URL)
						}
						aria-label={t(
					...
					>
						{t(
							"dialogs:appUpdate.claudeCodeChangelog",
						...
						<div className="flex items-start gap-3 text-sm text-warning bg-warning/10 border border-warning/30 rounded-lg p-3">
							...
							<div className="space-y-1">
								<p className="font-medium">
									{t(
										"dialogs:appUpdate.readOnlyVolumeTitle",
										"Cannot install from disk image",
									)}
								</p>
								<p className="text-muted-foreground">
									{t(
										"dialogs:appUpdate.readOnlyVolumeDescription",
										"Please move WorkPilot AI to your Applications folder before updating.",
									)}
								</p>
							</div>
						</div>
