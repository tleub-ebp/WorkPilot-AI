...
export function McpBuilder() {
  ...
  const [editingProject, setEditingProject] = useState<McpBuilderProject | null>(null);
  ...
  const handleUpdateTool = (toolId: string, updates: Partial<McpBuilderTool>) => {
    if (!editingProject) return;
    setEditingProject({
      ...editingProject,
  ...
  const editingTool = editingProject?.tools.find((t) => t.id === editingToolId) || null;
    ...
    <div className="flex-1 flex overflow-hidden">
      ...
      <div className="w-64 shrink-0 border-r border-border bg-muted/20 flex flex-col">
        <div className="p-3 border-b border-border">
          <Button onClick={handleNewProject} className="w-full h-8 text-xs gap-1.5">
        ...
        <div className="flex-1 overflow-auto p-2 space-y-1">
          {builderProjects.map((project) => (
            ...
            >
              ...
              <span className="truncate">{project.name || t('mcpMarketplace.builder.untitled')}</span>
            </button>
          ))}
      ...
      {editingProject ? (
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {editingProject.tools.length} {t('mcpMarketplace.builder.tools')}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <Button variant="outline" size="sm" onClick={() => handleDeleteProject(editingProject.id)}>
            ...
            <div className="w-80 shrink-0 border-r border-border overflow-auto p-4 space-y-5">
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  <div>
                    <Label className="text-xs">{t('mcpMarketplace.builder.serverName')}</Label>
                    <Input
                  ...
                  <div>
                    <Label className="text-xs">{t('mcpMarketplace.builder.description')}</Label>
                    <Textarea
                  ...
                  <div>
                    <Label className="text-xs">{t('mcpMarketplace.builder.baseUrl')}</Label>
                    <Input
                      value={editingProject.baseUrl || ''}
                      onChange={(e) => updateProject({ baseUrl: e.target.value })}
                      placeholder="https://api.example.com"
                      className="h-8 text-sm mt-1"
                    />
                  </div>
                  <div>
                    <Label className="text-xs">{t('mcpMarketplace.builder.color')}</Label>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <Button variant="ghost" size="icon" className="h-6 w-6" onClick={handleAddTool}>
                    <Plus className="h-3.5 w-3.5" />
                  </Button>
                </div>
                    ...
                    {editingProject.tools.map((tool) => (
                      ...
                      >
                        ...
                        <span className="truncate flex-1">
                          {tool.name || t('mcpMarketplace.builder.untitledTool')}
                        </span>
                        <Badge variant="secondary" className="text-[9px] px-1">
                          {tool.action.type === 'http_request' ? 'HTTP' : tool.action.type}
                        </Badge>
              ...
              {editingTool ? (
                <div className="max-w-2xl space-y-5">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <Label className="text-xs">{t('mcpMarketplace.builder.toolName')}</Label>
                      <Input
                        value={editingTool.name}
                        onChange={(e) =>
                          handleUpdateTool(editingTool.id, { name: e.target.value })
                        }
                    ...
                    <div>
                      <Label className="text-xs">{t('mcpMarketplace.builder.toolDescription')}</Label>
                      <Input
                        value={editingTool.description}
                        onChange={(e) =>
                          handleUpdateTool(editingTool.id, { description: e.target.value })
                        }
                  ...
                  {/* Action type */}
                  <div>
                    <Label className="text-xs">{t('mcpMarketplace.builder.actionType')}</Label>
                    <Select
                      value={editingTool.action.type}
                      onValueChange={(val) =>
                        handleUpdateTool(editingTool.id, {
                          action: { ...editingTool.action, type: val as McpBuilderActionType },
                        })
                      }
                    >
                  ...
                  {editingTool.action.type === 'http_request' && (
                    <div className="space-y-3 p-3 rounded-lg border border-border bg-muted/20">
                            ...
                            handleUpdateTool(editingTool.id, {
                              action: {
                                ...
                                http: editingTool.action.http ? {
                                  ...
                                  method: val as 'GET' | 'POST' | 'PUT' | 'PATCH' | 'DELETE',
                                } : {
                                  method: val as 'GET' | 'POST' | 'PUT' | 'PATCH' | 'DELETE',
                                  url: '',
                                },
                      ...
                      <div>
                        <Label className="text-xs">{t('mcpMarketplace.builder.bodyTemplate')}</Label>
                        <Textarea
                          ...
                          >
                            <div className="grid grid-cols-3 gap-2 flex-1">
                              <Input
                                value={param.name}
                                onChange={(e) =>
                                  handleUpdateParam(editingTool.id, idx, { name: e.target.value })
                                }
                              ...
                              <Input
                                ...
                                onChange={(e) =>
                                  handleUpdateParam(editingTool.id, idx, { description: e.target.value })
                                }
                            ...
                            <Button
                              ...
                              onClick={() => handleRemoveParam(editingTool.id, idx)}
                            >
                ...
                <div className="flex flex-col items-center justify-center h-full text-muted-foreground">
                  ...
                  <p className="text-sm">{t('mcpMarketplace.builder.selectTool')}</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground px-6">
          ...
          <h2 className="text-lg font-medium mb-1">{t('mcpMarketplace.builder.welcome')}</h2>
