...
export function McpServerCard({ server }: McpServerCardProps) {
  ...
  const canInstall = !hasRequiredEnvVars || server.requiredEnvVars?.every((v) => envVars[v.name]?.trim());
      ...
      >
        ...
        <div className="flex flex-col flex-1 p-4">
          ...
          <div className="flex items-center gap-3 text-xs text-muted-foreground mb-3">
            ...
            <span className="ml-auto text-[10px] opacity-60">v{server.version}</span>
          </div>

          {/* Tools preview */}
          <div className="flex flex-wrap gap-1 mb-3">
            {server.tools.slice(0, 3).map((tool) => (
              <Badge key={tool.name} variant="secondary" className="text-[10px] px-1.5 py-0">
                {tool.name}
              </Badge>
            ))}
          ...
          >
            ...
            {showDetails ? t('mcpMarketplace.hideDetails') : t('mcpMarketplace.showDetails')}
          </button>
          ...
          {showDetails && (
            <div className="text-xs text-muted-foreground mb-3 space-y-2">
              ...
              {server.requiredEnvVars && server.requiredEnvVars.length > 0 && (
                <div>
                  <span className="font-medium text-foreground">{t('mcpMarketplace.requiredConfig')}:</span>
      ...
      <Dialog open={showInstallDialog} onOpenChange={setShowInstallDialog}>
        <DialogContent className="sm:max-w-[500px]">
                ...
                <div className="flex gap-2">
                  ...
                  {envVar.helpUrl && (
                    <Button variant="outline" size="icon" className="h-9 w-9 shrink-0" asChild>
                      <a href={envVar.helpUrl} target="_blank" rel="noopener noreferrer">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleConfirmInstall} disabled={!canInstall || installing}>
              {installing ? (
                <>
