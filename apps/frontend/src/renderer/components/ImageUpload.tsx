	...
	const processFiles = useCallback(
		async (files: FileList | File[]) => {
			...
			if (fileArray.length > remainingSlots) {
				setError(
					`Only ${remainingSlots} more image(s) can be added. Some files were skipped.`,
				);
			}
			...
			for (const file of filesToProcess) {
				...
				if (!isValidImageType(file)) {
					errors.push(
						`"${file.name}" is not a valid image type. Allowed: ${ALLOWED_IMAGE_TYPES_DISPLAY}`,
					);
					continue;
				}
				...
				if (file.size > MAX_IMAGE_SIZE) {
					errors.push(
						`"${file.name}" is larger than 10MB. Consider compressing it for better performance.`,
					);
