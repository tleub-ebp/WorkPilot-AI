...
export function BulkPRDialog({
	...
	// Progress tracking
	const [step, setStep] = useState<"options" | "creating" | "results">(
		"options",
	);
	...
	const handleCreatePRs = useCallback(async () => {
		...
		for (let i = 0; i < tasks.length; i++) {
			...
			try {
				const prResult = await window.electronAPI?.createWorktreePR(
					tasks[i].id,
					{
						...
						prev.map((r, idx) =>
							idx === i
								? {
										...
										error: data.success
											? undefined
											: data.error || t("taskReview:pr.errors.unknown"),
									}
								: r,
						),
					);
					...
					setTaskResults((prev) =>
						prev.map((r, idx) =>
							idx === i
								? {
										...r,
										status: isWorktreeRelatedError(errorMsg)
											? ("skipped" as const)
											: ("error" as const),
							...
							? {
									...
									error: isWorktreeRelatedError(errorMsg)
										...
										: err instanceof Error
											? err.message
											: t("taskReview:pr.errors.unknown"),
								}
							: r,
					),
				);
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="sm:max-w-[600px]">
				<DialogHeader>
					...
					<DialogDescription>
						{step === "options" &&
							t("taskReview:bulkPR.description", { count: tasks.length })}
						{step === "creating" &&
							t("taskReview:bulkPR.creating", {
								current: currentIndex + 1,
								total: tasks.length,
							})}
						{step === "results" &&
							(skippedCount > 0
								? t("taskReview:bulkPR.resultsDescriptionWithSkipped", {
										success: successCount,
										skipped: skippedCount,
										failed: errorCount,
									})
								: t("taskReview:bulkPR.resultsDescription", {
										success: successCount,
										failed: errorCount,
									}))}
					</DialogDescription>
				...
				{step === "options" && (
					<div className="space-y-4">
						...
						<div className="space-y-4 pt-2">
							<div className="space-y-2">
								<Label htmlFor="bulkTargetBranch">
							...
							<div className="flex items-center gap-2">
								...
								/>
								<label
									htmlFor="bulk-draft-pr-checkbox"
						...
						<DialogFooter>
							...
							<Button
								...
								disabled={tasks.length === 0 || step !== "options"}
							>
						...
						<div className="flex flex-col items-center justify-center py-4 space-y-4">
							...
							<div className="text-center space-y-1">
								<p className="text-sm font-medium">
									{t("taskReview:bulkPR.creatingPR", {
										current: currentIndex + 1,
										total: tasks.length,
									})}
								</p>
							...
							{successCount > 0 && (
								<div className="flex items-center gap-2 text-success">
									...
									<span className="font-medium">
										{successCount} {t("taskReview:bulkPR.succeeded")}
									</span>
								</div>
							)}
							{skippedCount > 0 && (
								<div className="flex items-center gap-2 text-muted-foreground">
									...
									<span className="font-medium">
										{skippedCount} {t("taskReview:bulkPR.skipped")}
									</span>
								</div>
							)}
							{errorCount > 0 && (
								<div className="flex items-center gap-2 text-destructive">
									...
									<span className="font-medium">
										{errorCount} {t("taskReview:bulkPR.failed")}
									</span>
								</div>
							)}
						</div>
