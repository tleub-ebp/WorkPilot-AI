...
function isPRReview(memory: MemoryEpisode): boolean {
	if (
		["pr_review", "pr_finding", "pr_pattern", "pr_gotcha"].includes(memory.type)
	) {
		return true;
	}
	try {
...
function getMemoryCategory(memory: MemoryEpisode): FilterCategory {
	...
	if (["session_insight", "task_outcome"].includes(memory.type))
		return "sessions";
	if (["codebase_discovery", "codebase_map"].includes(memory.type))
		return "codebase";
...
export function MemoriesTab({
	...
	const filteredMemories = useMemo(() => {
		...
		return recentMemories.filter(
			(memory) => getMemoryCategory(memory) === activeFilter,
		);
			...
			<div className="p-6 space-y-6">
					...
					<CardContent className="space-y-3">
						{memoryStatus?.available ? (
							...
							<div className="text-sm text-muted-foreground">
								<p>
									{memoryStatus?.reason
										? t(`context:memories.${memoryStatus.reason}`, {
												defaultValue: memoryStatus.reason,
											})
				...
				<div className="space-y-4">
					...
					<div className="flex gap-2">
						...
						<Button onClick={handleSearch} disabled={searchLoading}>
							<Search
								className={cn("h-4 w-4", searchLoading && "animate-pulse")}
							/>
						</Button>
					</div>
					...
					{searchResults.length > 0 && (
						<div className="space-y-3">
							<p className="text-sm text-muted-foreground">
								{searchResults.length} {t("context:memories.result")}
				...
				<div className="space-y-4">
					<div className="flex items-center justify-between">
						...
						<span className="text-xs text-muted-foreground">
							{filteredMemories.length} {t("context:memories.of")}{" "}
							{recentMemories.length} {t("context:memories.memories")}
						</span>
					</div>

					{/* Filter Pills */}
					<div className="flex flex-wrap gap-2">
						{(Object.keys(memoryFilterCategories) as FilterCategory[]).map(
							(category) => {
					...
					{!memoriesLoading &&
						...
						recentMemories.length === 0 && (
							<div className="flex flex-col items-center justify-center py-8 text-center">
								...
								<p className="text-sm text-muted-foreground">
									{t(
										"context:memories.noMemoriesYet",
					...
					{!memoriesLoading &&
						...
						recentMemories.length > 0 && (
							<div className="flex flex-col items-center justify-center py-8 text-center">
								...
								<p className="text-sm text-muted-foreground">
									{t(
										"context:memories.noMatchingMemories",
