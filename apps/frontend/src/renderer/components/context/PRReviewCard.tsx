...
export function PRReviewCard({ memory }: PRReviewCardProps) {
	...
	const parsed = useMemo(
		() => parsePRReviewContent(memory.content),
		[memory.content],
	);
	...
	const totalFindings =
		...
		(finding_counts?.medium || 0) +
		(finding_counts?.low || 0);
						...
						<div className="flex-1 min-w-0">
							...
							{totalFindings > 0 && (
								<div className="flex items-center gap-1.5 mt-2 flex-wrap">
									<SeverityBadge
										severity="critical"
										count={finding_counts?.critical || 0}
									/>
									<SeverityBadge
										severity="high"
										count={finding_counts?.high || 0}
									/>
									<SeverityBadge
										severity="medium"
										count={finding_counts?.medium || 0}
									/>
									<SeverityBadge
										severity="low"
										count={finding_counts?.low || 0}
									/>
								</div>
							)}
						...
						{parsed.repo && parsed.prNumber && (
							<div className="pt-2">
								<Button
									...
									onClick={() =>
										window.open(
											`https://github.com/${parsed.repo}/pull/${parsed.prNumber}`,
											"_blank",
										)
									}
								>
