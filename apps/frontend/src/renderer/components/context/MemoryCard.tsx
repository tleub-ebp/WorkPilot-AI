...
interface ParsedSessionInsight {
	...
	discoveries?: {
		...
		patterns_discovered?: Array<
			{ pattern?: string; applies_to?: string } | string
		>;
		gotchas_discovered?: Array<
			{ gotcha?: string; trigger?: string; solution?: string } | string
		>;
		approach_outcome?: {
			success?: boolean;
...
function isPRReviewMemory(memory: MemoryEpisode): boolean {
	// Check by type first
	if (
		memory.type === "pr_review" ||
		memory.type === "pr_finding" ||
		memory.type === "pr_pattern" ||
		memory.type === "pr_gotcha"
	) {
		return true;
	}
...
export function MemoryCard({ memory }: MemoryCardProps) {
	...
	const parsed = useMemo(
		() => parseMemoryContent(memory.content),
		[memory.content],
	);
					...
					<div className="mt-4 space-y-4 pt-4 border-t border-border/50">
						...
						{parsed.discoveries?.approach_outcome?.approach_used && (
							<div>
								<SectionHeader
									icon={
										parsed.discoveries.approach_outcome.success
											? CheckCircle2
											: AlertTriangle
									}
									title="Approach"
						...
						{/* File Insights */}
						{parsed.discoveries?.file_insights &&
							parsed.discoveries.file_insights.length > 0 && (
								<div>
									<SectionHeader
										icon={FileCode}
										title="File Insights"
						...
						{/* Subtasks Completed */}
						{parsed.subtasks_completed &&
							parsed.subtasks_completed.length > 0 && (
								<div>
									<SectionHeader
										icon={CheckCircle2}
