...
import { AlertTriangle, CheckCircle, CheckSquare, Square } from "lucide-react";
import { useMemo, useState } from "react";
import { cn } from "../../../lib/utils";
import { Button } from "../../ui/button";
import type { SeverityGroup } from "../constants/severity-config";
import { SEVERITY_CONFIG, SEVERITY_ORDER } from "../constants/severity-config";
...
export function ReviewFindings({
	...
	return (
		<div className="space-y-4">
			{/* Summary Stats Bar */}
			<FindingsSummary findings={findings} selectedCount={selectedIds.size} />
			...
			<div className="space-y-3">
				{SEVERITY_ORDER.map((severity) => {
					...
					const selectedInGroup = group.filter((f) =>
					...
					return (
						<div
							key={severity}
							className={cn("rounded-lg border", config.bgColor)}
						>
							{/* Group Header */}
							<SeverityGroupHeader
								severity={severity}
