...
export function useGitLabMRs(
	...
	const getActiveMRReviews = useMRReviewStore(
		(state) => state.getActiveMRReviews,
	);
	...
	const getReviewStateForMR = useCallback(
		(mrIid: number) => {
	...
	const selectedMR =
		mergeRequests.find((mr) => mr.iid === selectedMRIid) || null;
	...
	const fetchMRs = useCallback(async () => {
		...
		try {
			...
			const connectionResult =
				await window.electronAPI.checkGitLabConnection(projectId);
			if (connectionResult.success && connectionResult.data) {
				...
				if (connectionResult.data.connected) {
					// Fetch MRs
					const result = await window.electronAPI.getGitLabMergeRequests(
						projectId,
						stateFilter,
					);
					if (result.success && result.data) {
						...
						result.data.forEach((mr) => {
							...
							if (
								...
								window.electronAPI.getGitLabMRReview
							) {
								window.electronAPI
									.getGitLabMRReview(projectId, mr.iid)
									.then((reviewResult) => {
										if (reviewResult) {
											// Update store with the loaded result
											useMRReviewStore
												.getState()
												.setMRReviewResult(projectId, reviewResult);
										}
									});
							}
						});
		...
		(mrIid: number | null) => {
			...
			if (mrIid && projectId) {
				...
				if (!existingState?.result && window.electronAPI.getGitLabMRReview) {
					window.electronAPI
						.getGitLabMRReview(projectId, mrIid)
						.then((result) => {
							if (result) {
								// Update store with the loaded result
								useMRReviewStore
	...
	const checkNewCommits = useCallback(
		async (mrIid: number): Promise<GitLabNewCommitsCheck> => {
			if (!projectId || !window.electronAPI.checkGitLabMRNewCommits) {
				return { hasNewCommits: false };
			}

			try {
				const result = await window.electronAPI.checkGitLabMRNewCommits(
					projectId,
					mrIid,
				);
				...
				useMRReviewStore
					.getState()
					.setNewCommitsCheck(projectId, mrIid, result);
				return result;
			} catch (err) {
				setError(
					err instanceof Error
						? err.message
						: "Failed to check for new commits",
				);
				return { hasNewCommits: false };
			}
		},
	...
	const cancelReview = useCallback(
		async (mrIid: number): Promise<boolean> => {
			if (!projectId || !window.electronAPI.cancelGitLabMRReview) return false;

			try {
				const success = await window.electronAPI.cancelGitLabMRReview(
					projectId,
					mrIid,
				);
				if (success) {
					useMRReviewStore
						.getState()
						.setMRReviewError(projectId, mrIid, "Review cancelled by user");
				}
				return success;
			} catch (err) {
				setError(
	...
	const postReview = useCallback(
		async (mrIid: number, selectedFindingIds?: string[]): Promise<boolean> => {
			if (!projectId || !window.electronAPI.postGitLabMRReview) return false;

			try {
				return await window.electronAPI.postGitLabMRReview(
					projectId,
					mrIid,
					selectedFindingIds,
				);
	...
	const postNote = useCallback(
		async (mrIid: number, body: string): Promise<boolean> => {
			if (!projectId || !window.electronAPI.postGitLabMRNote) return false;

			try {
				return await window.electronAPI.postGitLabMRNote(
					projectId,
					mrIid,
					body,
				);
	...
	const mergeMR = useCallback(
		async (
			...
			try {
				const success = await window.electronAPI.mergeGitLabMR(
					projectId,
					mrIid,
					mergeMethod,
				);
	...
	const assignMR = useCallback(
		async (mrIid: number, userIds: number[]): Promise<boolean> => {
			if (!projectId || !window.electronAPI.assignGitLabMR) return false;

			try {
				const success = await window.electronAPI.assignGitLabMR(
					projectId,
					mrIid,
					userIds,
				);
