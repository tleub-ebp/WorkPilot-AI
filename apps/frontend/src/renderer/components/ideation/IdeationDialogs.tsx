								...
								<>
									...
									>
										<div className="flex items-center gap-3">
											<div
												className={`p-2 rounded-md ${IDEATION_TYPE_COLORS[type]}`}
											>
												<TypeIcon type={type} />
											</div>
											<div>
							...
							<div className="flex items-center justify-between">
								...
								<Switch
									...
									onCheckedChange={(checked) =>
										onSetConfig({ includeRoadmapContext: checked })
									}
								/>
							</div>
							<div className="flex items-center justify-between">
								...
								<Switch
									...
									onCheckedChange={(checked) =>
										onSetConfig({ includeKanbanContext: checked })
									}
								/>
							</div>
						</div>
			...
			<Dialog open={showAddMoreDialog} onOpenChange={onCloseAddMoreDialog}>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>{t("addMore.description")}</DialogDescription>
					</DialogHeader>
						...
						{availableTypesToAdd.length === 0 ? (
							...
							availableTypesToAdd.map((type) => (
								<>
									...
									{/* biome-ignore lint/a11y/noNoninteractiveElementInteractions: selectable type row */}
									<div
										key={type}
					...
					<DialogFooter className="flex items-center justify-between">
						<div className="text-sm text-muted-foreground">
							{typesToAdd.length > 0 &&
								t("header.selectedCount", { count: typesToAdd.length })}
						</div>
