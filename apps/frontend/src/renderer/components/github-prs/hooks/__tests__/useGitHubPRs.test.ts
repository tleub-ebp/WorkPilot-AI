...
function createMockNewCommitsCheck(
	overrides: Partial<NewCommitsCheck> = {},
): NewCommitsCheck {
	return {
		hasNewCommits: false,
		newCommitCount: 0,
...
async function simulateSelectPR(
	params: SelectPRTestParams,
): Promise<SelectPRTestResult> {
	const {
		prNumber,
		projectId,
		existingState,
	...
	const checkNewCommitsForPR = async (
...
describe("useGitHubPRs - selectPR triggering checkNewCommits", () => {
	...
	beforeEach(() => {
		mockCheckNewCommits = vi
			.fn()
			.mockResolvedValue(createMockNewCommitsCheck());
	...
	describe("checkNewCommits NOT triggered during active review", () => {
		...
		it("should still call checkNewCommits when previous result exists during active review", async () => {
			const previousReview = createMockReviewResult({
				reviewedCommitSha: "old123",
			});
