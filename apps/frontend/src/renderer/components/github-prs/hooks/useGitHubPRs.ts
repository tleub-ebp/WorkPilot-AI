...
export function useGitHubPRs(
	...
	const [selectedPRDetails, setSelectedPRDetails] = useState<PRData | null>(
		null,
	);
	...
	const getActivePRReviews = usePRReviewStore(
		(state) => state.getActivePRReviews,
	);
	const setNewCommitsCheckAction = usePRReviewStore(
		(state) => state.setNewCommitsCheck,
	);
	...
	const selectedPR = useMemo(() => {
		...
		return (
			matchingDetails ||
			prs.find((pr) => pr.number === selectedPRNumber) ||
			null
		);
	...
	const fetchPRs = useCallback(async () => {
		...
		try {
			...
			const connectionResult =
				await window.electronAPI.github.checkGitHubConnection(projectId);
				...
				if (connectionResult.data.connected) {
					...
					if (result) {
						...
						if (prsNeedingPreload.length > 0) {
							...
							const batchReviews =
								await window.electronAPI.github.getPRReviewsBatch(
									projectId,
									prNumbers,
								);
		...
		(prNumber: number | null) => {
			...
			if (prNumber && projectId) {
				...
				const checkNewCommitsForPR = (
					...
					window.electronAPI.github
						...
						.catch((err) => {
							...
							console.warn(
								`Failed to check new commits for PR #${prNumber}:`,
								err,
							);
						});
				};
				...
				if (!existingState?.result && !existingState?.isReviewing) {
					window.electronAPI.github
						.getPRReview(projectId, prNumber)
						.then((result) => {
	...
	const loadMore = useCallback(async () => {
		...
		try {
			const result = await window.electronAPI.github.listMorePRs(
				projectId,
				endCursor,
			);
			...
			if (result) {
				...
				const isFailureResponse =
					result.prs.length === 0 && !result.hasNextPage && !result.endCursor;
				...
				if (!isFailureResponse) {
					...
					setPrs((prevPrs) => {
						...
						const newPrs = result.prs.filter(
							(pr) => !existingNumbers.has(pr.number),
						);
				...
				if (prsNeedingPreload.length > 0) {
					...
					for (const reviewResult of Object.values(batchReviews)) {
						if (reviewResult) {
							usePRReviewStore
								.getState()
								.setPRReviewResult(requestProjectId, reviewResult, {
									preserveNewCommitsCheck: true,
								});
						}
					}
			...
			if (
				...
				setError(
					err instanceof Error ? err.message : "Failed to load more PRs",
				);
			}
		} finally {
			setIsLoadingMore(false);
	...
	const checkNewCommits = useCallback(
		async (prNumber: number): Promise<NewCommitsCheck> => {
			...
			try {
				const result = await window.electronAPI.github.checkNewCommits(
					projectId,
					prNumber,
				);
	...
	const cancelReview = useCallback(
		async (prNumber: number): Promise<boolean> => {
			...
			try {
				const success = await window.electronAPI.github.cancelPRReview(
					projectId,
					prNumber,
				);
		...
		async (
			...
			try {
				...
				if (success) {
					...
					const result = await window.electronAPI.github.getPRReview(
						projectId,
						prNumber,
					);
					if (result) {
	...
	const postComment = useCallback(
		async (prNumber: number, body: string): Promise<boolean> => {
			...
			try {
				return await window.electronAPI.github.postPRComment(
					projectId,
					prNumber,
					body,
				);
	...
	const mergePR = useCallback(
		async (
			...
			try {
				const success = await window.electronAPI.github.mergePR(
					projectId,
					prNumber,
					mergeMethod,
				);
	...
	const assignPR = useCallback(
		async (prNumber: number, username: string): Promise<boolean> => {
			...
			try {
				const success = await window.electronAPI.github.assignPR(
					projectId,
					prNumber,
					username,
				);
	...
	const markReviewPosted = useCallback(
		async (prNumber: number): Promise<void> => {
			...
			const success = await window.electronAPI.github.markReviewPosted(
				projectId,
				prNumber,
			);
			if (!success) return;
