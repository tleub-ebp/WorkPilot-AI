...
function createMockCleanReviewResult(
...
function createMockPostedReviewResult(
	overrides: Partial<PRReviewResult> = {},
): PRReviewResult {
	return {
		prNumber: 123,
...
describe("PRDetail - Clean Review State Reset Integration", () => {
	...
	it("should reset cleanReviewPosted state when pr.number changes", async () => {
		...
		const postCleanReviewButton = screen.getByRole("button", {
			name: /post clean review/i,
		});
		...
		expect(
			screen.queryByRole("button", { name: /post clean review/i }),
		...
		const postCleanReviewButtonAfterChange = screen.queryByRole("button", {
			name: /post clean review/i,
		});
		// Either the button exists or it doesn't - both are valid states
		expect(
			postCleanReviewButtonAfterChange === null ||
				postCleanReviewButtonAfterChange,
		).toBeTruthy();
		unmount();
	...
	it("should not show Post Clean Review button when review has HIGH severity findings", async () => {
		...
		const postCleanReviewButton = screen.queryByRole("button", {
			name: /post clean review/i,
		});
	...
	it("should show correct button state based on review cleanliness", async () => {
		...
		const postCleanReviewButtonDirty = screen.queryByRole("button", {
			name: /post clean review/i,
		});
	...
	it("should show error message when posting clean review fails", async () => {
		...
		await waitFor(() => {
			expect(
				screen.getByText(/Failed to post clean review/i),
			).toBeInTheDocument();
		});
...
describe("PRDetail - Follow-up Review Trigger Integration", () => {
	...
	function renderPRDetailForFollowup(
		overrides: {
			pr?: PRData;
			reviewResult?: PRReviewResult;
		...
		const onRunFollowupReviewMock =
			overrides.onRunFollowupReview ?? mockOnRunFollowupReview;
	...
	it('should show "Run Follow-up" button when new commits overlap with findings', async () => {
		...
		await waitFor(() => {
			expect(
				screen.getByRole("button", { name: /run follow-up/i }),
			).toBeInTheDocument();
		});
	...
	it("should NOT show follow-up prompt when hasCommitsAfterPosting is false", async () => {
		...
		await waitFor(() => {
			expect(
				screen.queryByRole("button", { name: /run follow-up/i }),
			).not.toBeInTheDocument();
		});
