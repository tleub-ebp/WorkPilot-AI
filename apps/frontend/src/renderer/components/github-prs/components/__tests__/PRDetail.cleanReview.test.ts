...
function createTestFinding(
	severity: PRReviewFinding["severity"],
	id: string = "finding-1",
): PRReviewFinding {
	return {
		id,
...
describe("PRDetail Clean Review Functionality", () => {
	...
	describe("isCleanReview Logic", () => {
		...
		it("should return false for review with CRITICAL severity findings", () => {
			const reviewResult = createReviewResult({
				findings: [createTestFinding("critical", "critical-1")],
			});
	...
	describe("Clean Review Comment Format", () => {
		it("should format clean review comment correctly", () => {
			...
			const cleanReviewMessage = `## ✅ WorkPilot AI PR Review - PASSED
...
*This automated review found no issues. Generated by WorkPilot AI.*`;

			expect(cleanReviewMessage).toContain(
				"## ✅ WorkPilot AI PR Review - PASSED",
			);
		...
		it("should include custom summary in clean review comment", () => {
			const customSummary =
				"Review completed: 5 files checked, 0 issues found. Code follows best practices.";
			const reviewResult = createReviewResult({
...
*This automated review found no issues. Generated by WorkPilot AI.*`;
	...
	describe("Edge Cases", () => {
		...
		it("should handle overallStatus comment correctly", () => {
			...
			const shouldShowButton = reviewResult.overallStatus !== "request_changes";
