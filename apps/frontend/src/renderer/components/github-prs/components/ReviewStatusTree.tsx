...
export function ReviewStatusTree({
		...
		return (
			<div className="flex flex-wrap items-center justify-between gap-y-3 p-4 border rounded-lg bg-card shadow-sm">
				...
				<Button
					onClick={onRunReview}
					size="sm"
					className="gap-2 shrink-0 ml-auto sm:ml-0"
				>
	...
	if (isFollowupInProgress) {
		...
		if (previousReviewResult) {
			steps.push({
				id: "prev_review",
				label: t("prReview.previousReview", {
					count: previousReviewResult.findings.length,
				}),
				status: "completed",
			...
			const prevPostedCount =
				previousReviewResult.postedFindingIds?.length ?? 0;
		...
		if (isReviewing) {
			...
			steps.push({
				id: "analysis",
				label: t("prReview.analysisComplete", {
		...
		if (postedCount > 0 || reviewResult?.hasPostedFindings) {
			steps.push({
				...
				date:
					...
					(lastPostedAt ? new Date(lastPostedAt).toISOString() : null),
			});
		...
		const hasPostedFindings =
			postedCount > 0 || reviewResult?.hasPostedFindings;
		if (
			!isReviewing &&
			hasPostedFindings &&
			newCommitsCheck?.hasNewCommits &&
			newCommitsCheck?.hasCommitsAfterPosting
		) {
			...
			if (hasOverlap) {
				...
				steps.push({
					...
					label: newCommitsCheck.isMergeFromBase
						? t("prReview.branchSynced", {
								count: newCommitsCheck.newCommitCount,
							})
						: t("prReview.newCommitsNoOverlap", {
								count: newCommitsCheck.newCommitCount,
							}),
					status: "completed",
					date: null,
