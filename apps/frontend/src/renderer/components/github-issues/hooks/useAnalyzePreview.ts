...
export function useAnalyzePreview({
	projectId,
}: UseAnalyzePreviewProps): UseAnalyzePreviewReturn {
	const [isWizardOpen, setIsWizardOpen] = useState(false);
	const [isAnalyzing, setIsAnalyzing] = useState(false);
	const [isApproving, setIsApproving] = useState(false);
	const [analysisProgress, setAnalysisProgress] =
		useState<AnalyzePreviewProgress | null>(null);
	const [analysisResult, setAnalysisResult] =
		useState<AnalyzePreviewResult | null>(null);
	...
	const approveBatches = useCallback(
		async (batches: ProposedBatch[]) => {
			if (!projectId || batches.length === 0) return;

			setIsApproving(true);
			try {
				const result = await window.electronAPI.github.approveBatches(
					projectId,
					batches,
				);
				if (!result.success) {
				...
				setAnalysisError(
					error instanceof Error ? error.message : "Failed to approve batches",
				);
				throw error;
			} finally {
				setIsApproving(false);
