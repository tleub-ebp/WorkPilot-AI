...
export function BatchReviewWizard({
	...
	const [selectedSingleIssueNumbers, setSelectedSingleIssueNumbers] = useState<
		Set<number>
	>(new Set());
	// Track which batches are expanded
	const [expandedBatchIds, setExpandedBatchIds] = useState<Set<number>>(
		new Set(),
	);
	// Current wizard step
	const [step, setStep] = useState<
		"intro" | "analyzing" | "review" | "approving" | "done"
	>("intro");
	...
	useEffect(() => {
		if (isAnalyzing) {
			...
			if (
				analysisResult.proposedBatches.length === 0 &&
	...
	const handleApprove = useCallback(async () => {
		...
		const selectedBatches = analysisResult.proposedBatches.filter((_, idx) =>
			selectedBatchIds.has(idx),
		);

		// Convert selected single issues into batches (each single issue becomes a batch of 1)
		const selectedSingleIssueBatches: ProposedBatch[] =
			analysisResult.singleIssues
			...
			<div className="text-center space-y-2">
				...
				<p className="text-sm text-muted-foreground">
					{analysisProgress?.message ||
						"Computing similarity and validating batches..."}
				</p>
			</div>
			...
			<div className="flex flex-col h-[60vh]">
								...
								{singleIssues.slice(0, 10).map((issue) => (
									<>
										...
										<div
											...
											onClick={() =>
												toggleSingleIssueSelection(issue.issueNumber)
											}
										...
										>
											<Checkbox
												checked={selectedSingleIssueNumbers.has(
													issue.issueNumber,
												)}
				...
				<div className="flex items-center justify-between pt-4 mt-4 border-t border-border">
					<div className="text-sm text-muted-foreground">
						...
						{totalIssuesInSelected} issues)
						{selectedSingleIssueNumbers.size > 0 && (
							<>
								{" "}
								+ {selectedSingleIssueNumbers.size} single issue
								{selectedSingleIssueNumbers.size !== 1 ? "s" : ""}
							</>
						)}
					</div>
				</div>
		...
		<Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
			<DialogContent className="max-w-3xl">
				<DialogHeader>
					...
					<DialogDescription>
						{step === "intro" &&
							"Analyze open issues and group similar ones for batch processing."}
						{step === "analyzing" &&
							"Analyzing issues for semantic similarity..."}
						{step === "review" &&
							"Review and approve the proposed issue batches."}
						...
						>
							{isApproving ? (
								...
								<>
									...
									Approve & Create (
									{selectedBatchIds.size + selectedSingleIssueNumbers.size}{" "}
									{selectedBatchIds.size + selectedSingleIssueNumbers.size === 1
										? "batch"
										: "batches"}
									)
								</>
