...
export const CHARACTER_PALETTES = [
  { skin: '#F5D0A9', hair: '#3B2314', shirt: '#4A90D9', pants: '#2C3E50', shoes: '#1A1A2E' },
  { skin: '#8D5524', hair: '#1A1A1A', shirt: '#E74C3C', pants: '#34495E', shoes: '#2C2C3E' },
  { skin: '#FFDBB4', hair: '#D4A017', shirt: '#27AE60', pants: '#2C3E50', shoes: '#1A1A2E' },
  { skin: '#C68642', hair: '#2C1810', shirt: '#9B59B6', pants: '#2C3E50', shoes: '#1A1A2E' },
  { skin: '#F1C27D', hair: '#6B3A2A', shirt: '#F39C12', pants: '#34495E', shoes: '#2C2C3E' },
  { skin: '#FFDBAC', hair: '#8B4513', shirt: '#1ABC9C', pants: '#2C3E50', shoes: '#1A1A2E' },
];
...
function getCharacterPixels(
  ...
  if (direction === 'down' || direction === 'left' || direction === 'right') {
    ...
    // Eyes
    if (direction === 'down') {
      pixels.push({ x: 6, y: 5, color: '#1A1A2E' }, { x: 9, y: 5, color: '#1A1A2E' });
    } else if (direction === 'left') {
      pixels.push({ x: 5, y: 5, color: '#1A1A2E' }, { x: 8, y: 5, color: '#1A1A2E' });
    } else {
      pixels.push({ x: 7, y: 5, color: '#1A1A2E' }, { x: 10, y: 5, color: '#1A1A2E' });
    }
  ...
  pixels.push({ x: 3, y: 9 + armOffset, color: skin }, { x: 12, y: 9 + (armOffset === 0 ? 1 : 0), color: skin });
  ...
  // Shoes (rows 18-19)
  for (let x = 5 - legSpread; x <= 7; x++) pixels.push({ x, y: 18, color: shoes });
  for (let x = 8; x <= 10 + legSpread; x++) pixels.push({ x, y: 18, color: shoes });
...
export function getCharacterSprite(
  ...
  const palette = CHARACTER_PALETTES[characterIndex % CHARACTER_PALETTES.length];
...
export type ActivityIcon = 'typing' | 'reading' | 'running' | 'waiting' | 'idle';

export function getActivityIcon(activity: ActivityIcon, frame: number): HTMLCanvasElement {
