...
export function useTaskDetailHandlers({ task, state, onOpenChange }: TaskDetailHandlersProps) {
  ...
  const handleMerge = async () => {
    ...
    try {
      const result = await globalThis.window.electronAPI.mergeWorktree(task.id, { noCommit: state.stageOnly });
  ...
  const handleCreatePR = async (options: WorktreeCreatePROptions) => {
    ...
    try {
      const result = await globalThis.window.electronAPI.createWorktreePR(task.id, options);
