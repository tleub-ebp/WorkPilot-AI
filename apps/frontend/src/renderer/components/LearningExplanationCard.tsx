...
export function LearningExplanationCard({
	explanation,
	defaultExpanded = false,
}: LearningExplanationCardProps) {
	const [isExpanded, setIsExpanded] = useState(defaultExpanded);

	const Icon = CATEGORY_ICONS[explanation.category] || BookOpen;
	const categoryLabel =
		CATEGORY_LABELS[explanation.category] || explanation.category;
	const categoryColor =
		CATEGORY_COLORS[explanation.category] ||
		...
		<Card className={cn("border-l-4 transition-all", categoryColor)}>
			...
			{isExpanded && (
				<CardContent className="pt-0 space-y-4">
					...
					{explanation.alternative_approaches &&
						explanation.alternative_approaches.length > 0 && (
							<div>
								<div className="text-xs font-medium text-muted-foreground mb-2">
