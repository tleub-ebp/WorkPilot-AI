...
import { ROADMAP_PRIORITY_LABELS } from "../../shared/constants";
import type {
...
export function AddFeatureDialog({
	...
	const handleSave = async () => {
		...
		try {
			...
			const newFeatureId = addFeature({
				...
				rationale:
					rationale.trim() || `User-created feature for ${title.trim()}`,
				priority,
				complexity,
				impact,
				phaseId,
			...
			if (roadmap) {
				...
				const result = await globalThis.electronAPI.saveRoadmap(
					roadmap.projectId,
					roadmap,
				);
				if (!result.success) {
			...
			setError(
				err instanceof Error ? err.message : t("addFeature.failedToAdd"),
			);
		} finally {
			setIsSaving(false);
		}
	...
	// Form validation
	const isValid =
		title.trim().length > 0 && description.trim().length > 0 && phaseId !== "";
		...
		<Dialog open={open} onOpenChange={handleClose}>
			<DialogContent className="sm:max-w-[550px] max-h-[90vh] overflow-y-auto">
				<DialogHeader>
					...
					<DialogDescription>{t("addFeature.description")}</DialogDescription>
				</DialogHeader>
				...
				<div className="space-y-5 py-4">
					...
					<div className="space-y-2">
						<Label
							htmlFor="add-feature-title"
							className="text-sm font-medium text-foreground"
						>
							{t("addFeature.featureTitle")}{" "}
					...
					<div className="space-y-2">
						<Label
							...
							className="text-sm font-medium text-foreground"
						>
							{t("addFeature.featureDescription")}{" "}
							<span className="text-destructive">*</span>
						</Label>
						<Textarea
					...
					<div className="space-y-2">
						<Label
							htmlFor="add-feature-rationale"
							className="text-sm font-medium text-foreground"
						>
							{t("addFeature.rationale")}{" "}
							<span className="text-muted-foreground font-normal">
								({t("addFeature.optional")})
							</span>
						</Label>
						<Textarea
					...
					<div className="grid grid-cols-2 gap-4">
						...
						<div className="space-y-2">
							...
							<Select
								value={priority}
								onValueChange={(value) =>
									setPriority(value as RoadmapFeaturePriority)
								}
								disabled={isSaving}
							>
								...
								<SelectContent>
									{Object.entries(ROADMAP_PRIORITY_LABELS).map(
										([value, label]) => (
						...
						<div className="space-y-2">
							<Label
								htmlFor="add-feature-complexity"
								className="text-sm font-medium text-foreground"
							>
							...
							<Select
								value={complexity}
								onValueChange={(value) =>
									setComplexity(value as ComplexityLevel)
								}
								disabled={isSaving}
							>
