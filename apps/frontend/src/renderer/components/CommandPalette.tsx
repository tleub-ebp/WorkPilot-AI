...
function fuzzyMatch(query: string, text: string): { match: boolean; score: number } {
  ...
  for (let ti = 0; ti < t.length && qi < q.length; ti++) {
    if (t[ti] === q[qi]) {
      ...
      // Word boundary bonus
      else if (ti === 0 || t[ti - 1] === ' ' || t[ti - 1] === '-' || t[ti - 1] === '_') {
        score += 6;
      } else {
        score += 4;
      }
...
export function CommandPalette({
  ...
  const results = useMemo(() => {
    if (!query.trim()) {
      ...
      return commands.sort((a, b) => {
        const order: CommandCategory[] = ['navigation', 'tasks', 'agents', 'terminal', 'settings', 'providers', 'help'];
    ...
    const scored = commands
      .map((cmd) => {
        ...
        return { cmd, score: bestScore, match: labelMatch.match || descMatch.match || keywordScore > 0 };
      })
      .filter((r) => r.match)
  ...
  const grouped = results.reduce<Record<string, PaletteCommand[]>>((acc, cmd) => {
        ...
        <div className="overflow-hidden rounded-xl border border-border bg-card shadow-2xl">
          ...
          <div className="flex items-center justify-between border-t border-border px-4 py-2 text-xs text-muted-foreground">
            <div className="flex items-center gap-3">
              ...
              <span className="flex items-center gap-1">
                <kbd className="rounded border border-border bg-secondary px-1 font-mono text-[10px]">esc</kbd>
                close
              </span>
            </div>
