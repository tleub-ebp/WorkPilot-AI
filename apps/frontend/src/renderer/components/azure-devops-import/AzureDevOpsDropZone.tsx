...
export function AzureDevOpsDropZone({
	...
	const [draggedWorkItems, setDraggedWorkItems] = useState<
		AzureDevOpsWorkItem[]
	>([]);
	...
	return (
		...
		<section
			...
			style={{ zIndex: 100 }}
			aria-label={t("azureDevOpsImport.dropZoneAriaLabel", {
				status: columnStatus,
			})}
		>
			{children}
					...
					<div className="text-center space-y-2 p-4 bg-background/98 rounded-lg border border-primary/30 shadow-lg">
						...
						<div className="space-y-1">
							<p className="font-medium text-primary">
								{hasItems
									? t("azureDevOpsImport.dropZoneItems", {
											count: draggedWorkItems.length,
										})
