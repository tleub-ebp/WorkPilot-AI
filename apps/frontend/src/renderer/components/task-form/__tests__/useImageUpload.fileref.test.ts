...
function createMockImageDragEvent(
	files: File[],
): React.DragEvent<HTMLTextAreaElement> {
	const fileList = {
		length: files.length,
...
describe("useImageUpload - File Reference Handling", () => {
	...
	describe("parseFileReferenceData (via handleDrop)", () => {
		it("should detect valid file reference drops and call onFileReferenceDrop callback", async () => {
			...
			const fileRefData = createFileReferenceData(
				"/path/to/file.ts",
				"file.ts",
			);
			...
			expect(mockOnFileReferenceDrop).toHaveBeenCalledWith(
				"@file.ts",
				fileRefData,
			);
		...
		it("should handle directory references the same as file references", async () => {
			...
			const dirRefData = createFileReferenceData(
				"/path/to/directory",
				"directory",
				true,
			);
	...
	describe("Edge Cases", () => {
		...
		it("should handle multiple rapid drops", async () => {
			...
			const fileRefData1 = createFileReferenceData(
				"/path/to/file1.ts",
				"file1.ts",
			);
			const fileRefData2 = createFileReferenceData(
				"/path/to/file2.ts",
				"file2.ts",
			);
			const fileRefData3 = createFileReferenceData(
				"/path/to/file3.ts",
				"file3.ts",
			);
			...
			expect(mockOnFileReferenceDrop).toHaveBeenNthCalledWith(
				1,
				"@file1.ts",
				fileRefData1,
			);
	...
	describe("Callback Data Shape", () => {
		it("should pass complete FileReferenceData to callback", async () => {
			...
			const mockEvent = createMockFileRefDragEvent(
				fileRefData,
				"@component.tsx",
			);
			...
			const passedData = mockOnFileReferenceDrop.mock
				.calls[0][1] as FileReferenceData;
