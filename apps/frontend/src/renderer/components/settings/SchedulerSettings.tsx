import { BarChart3, CalendarClock, Link2, Play, Plus } from "lucide-react";
...
export function SchedulerSettings() {
	...
	const [scheduledTasks] = useState<ScheduledTaskEntry[]>([
		{
			id: "sched-1",
			name: t("tasks.sampleTasks.dailyScan.name"),
			cron: "0 22 * * *",
			action: "security_scan",
			priority: 2,
			status: "pending",
			nextRun: t("tasks.sampleTasks.dailyScan.nextRun"),
		},
		{
			id: "sched-2",
			name: t("tasks.sampleTasks.weeklyUpdate.name"),
			cron: "0 3 * * 1",
			action: "update_deps",
			priority: 5,
			status: "pending",
			nextRun: t("tasks.sampleTasks.weeklyUpdate.nextRun"),
		},
		{
			id: "sched-3",
			name: t("tasks.sampleTasks.hourlyLog.name"),
			cron: "0 * * * *",
			action: "check_logs",
			priority: 8,
			status: "paused",
	...
	const [chains] = useState<TaskChainEntry[]>([
		{
			name: t("tasks.sampleTasks.ciPipeline"),
					...
					<div className="space-y-4">
						<div className="flex items-center justify-between rounded-lg border border-border p-3">
							<div>
								<p className="text-sm font-medium">
									{t("tasks.settings.autoStart.label")}
								</p>
								<p className="text-xs text-muted-foreground">
									{t("tasks.settings.autoStart.description")}
								</p>
							</div>
							<button
								type="button"
						...
						<div className="space-y-1">
							...
							<p className="text-[10px] text-muted-foreground">
								{t("tasks.settings.checkInterval.description")}
							</p>
						</div>
					</div>
				</div>
									...
									{chain.tasks.map((task, taskIndex) => {
										...
										return (
											<span key={task} className="flex items-center gap-1.5">
												<span className={cn(taskStyleClass)}>{task}</span>
					...
					<div className="rounded-md border border-border overflow-hidden">
						<table className="w-full text-xs">
							<thead>
								<tr className="border-b border-border bg-muted/40">
									<th className="text-left px-3 py-2 font-medium text-muted-foreground">
										{t("tasks.cronReference.table.expression")}
									</th>
									<th className="text-left px-3 py-2 font-medium text-muted-foreground">
										{t("tasks.cronReference.table.description")}
									</th>
								</tr>
							</thead>
							<tbody>
								{[
									{
										expr: "0 22 * * *",
										desc: t("tasks.cronReference.examples.daily"),
									},
									{
										expr: "*/15 * * * *",
										desc: t("tasks.cronReference.examples.every15min"),
									},
									{
										expr: "0 3 * * 1",
										desc: t("tasks.cronReference.examples.weekly"),
									},
									{
										expr: "0 0 1 * *",
										desc: t("tasks.cronReference.examples.monthly"),
									},
									{
										expr: "0 9-17 * * 1-5",
										desc: t("tasks.cronReference.examples.workHours"),
									},
								].map((row) => (
									<tr
										key={`cron-${row.expr}`}
