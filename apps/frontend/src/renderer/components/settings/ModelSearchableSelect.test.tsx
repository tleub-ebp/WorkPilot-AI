...
describe("ModelSearchableSelect", () => {
	...
	beforeEach(() => {
		...
		vi.mocked(useSettingsStore).mockImplementation(
			// biome-ignore lint/suspicious/noExplicitAny: Zustand mock requires flexible types in tests
	...
	it("should fetch models when dropdown opens", async () => {
		...
		const input = screen.getByPlaceholderText(
			"Select a model or type manually",
		);
	...
	it("should show fallback mode on fetch failure", async () => {
		...
		await waitFor(() => {
			...
			expect(
				screen.getByText(/Model discovery not available/),
			).toBeInTheDocument();
		});
	});
