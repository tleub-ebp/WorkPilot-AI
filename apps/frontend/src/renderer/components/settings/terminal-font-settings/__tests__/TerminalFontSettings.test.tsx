...
describe("TerminalFontSettings - Infinite Re-render Loop Fix", () => {
	...
	describe("Component Rendering", () => {
		...
		it("should render all expected sections", () => {
			...
			expect(screen.getAllByText(/copy to clipboard/i).length).toBeGreaterThan(
				0,
			);

			// Configuration sections
			expect(screen.getAllByText(/font configuration/i).length).toBeGreaterThan(
				0,
			);
			expect(
				screen.getAllByText(/cursor configuration/i).length,
			).toBeGreaterThan(0);
			expect(
				screen.getAllByText(/performance settings/i).length,
			).toBeGreaterThan(0);
		...
		it("should handle concurrent updates without race conditions", async () => {
			...
			const promises = [
				Promise.resolve().then(() =>
					act(() => useTerminalFontSettingsStore.getState().setFontSize(16)),
				),
				Promise.resolve().then(() =>
					act(() => useTerminalFontSettingsStore.getState().setFontWeight(500)),
				),
				Promise.resolve().then(() =>
					act(() => useTerminalFontSettingsStore.getState().setLineHeight(1.5)),
				),
			];
	...
	describe("Import/Export Operations", () => {
		...
		it("should import settings and update store state", () => {
			...
			const success = useTerminalFontSettingsStore
				.getState()
				.importSettings(json);
	...
	describe("Regression Prevention", () => {
		...
		it("should not log React warnings about getSnapshot caching", () => {
			...
			const warnCalls = consoleWarnSpy.mock.calls.filter((call) =>
				call.some(
					(arg) => typeof arg === "string" && arg.includes("getSnapshot"),
				),
			);
		...
		it('should not cause "Maximum update depth exceeded" error', () => {
			...
			const errorCalls = consoleErrorSpy.mock.calls.filter((call) =>
				call.some(
					(arg) =>
						typeof arg === "string" &&
						(arg.includes("Maximum update depth") ||
