      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between rounded-lg border border-border p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <Label className="text-sm font-medium">{t('continuousAI:enable')}</Label>
                    ...
                    <div className="flex items-center justify-between p-3">
                      ...
                      >
                        <div className={cn(
                          ...
                          <Icon className={cn('h-4 w-4', moduleConfig.enabled ? mod.color : 'text-muted-foreground')} />
                        </div>
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="text-xs text-muted-foreground truncate">
                            {t(`continuousAI:modules.${mod.configKey}.description`)}
                          </div>
                        </div>
                      ...
                      <div className="px-4 pb-4 pt-1 space-y-4 border-t border-border/50">
                        ...
                        <div className="border-t border-border/50 pt-3 space-y-3">
                          {mod.key === 'cicd_watcher' && (
                            <>
                              <div className="flex items-center justify-between">
                                <Label className="text-xs">{t('continuousAI:modules.cicdWatcher.autoFix')}</Label>
                                <Switch
                                  checked={config.cicdWatcher.autoFix}
                                  onCheckedChange={(v) => setModuleConfig('cicd_watcher', { autoFix: v })}
                                />
                              </div>
                              <div className="flex items-center justify-between">
                                <Label className="text-xs">{t('continuousAI:modules.cicdWatcher.autoCreatePr')}</Label>
                                <Switch
                                  checked={config.cicdWatcher.autoCreatePr}
                                  onCheckedChange={(v) => setModuleConfig('cicd_watcher', { autoCreatePr: v })}
                                />
                              </div>
                            </>
                          )}
                          ...
                          {mod.key === 'dependency_sentinel' && (
                            <>
                              <div className="flex items-center justify-between">
                                <Label className="text-xs">{t('continuousAI:modules.dependencySentinel.autoPatchMinor')}</Label>
                                <Switch
                                  checked={config.dependencySentinel.autoPatchMinor}
                                  onCheckedChange={(v) => setModuleConfig('dependency_sentinel', { autoPatchMinor: v })}
                                />
                              </div>
                              <div className="flex items-center justify-between">
                                <Label className="text-xs">{t('continuousAI:modules.dependencySentinel.autoPatchMajor')}</Label>
                                <Switch
                                  checked={config.dependencySentinel.autoPatchMajor}
                                  onCheckedChange={(v) => setModuleConfig('dependency_sentinel', { autoPatchMajor: v })}
                                />
                              </div>
                            </>
                          )}
                          ...
                          {mod.key === 'issue_responder' && (
                            <>
                              <div className="flex items-center justify-between">
                                <Label className="text-xs">{t('continuousAI:modules.issueResponder.autoTriage')}</Label>
                                <Switch
                                  checked={config.issueResponder.autoTriage}
                                  onCheckedChange={(v) => setModuleConfig('issue_responder', { autoTriage: v })}
                                />
                              </div>
                              <div className="flex items-center justify-between">
                                <Label className="text-xs">{t('continuousAI:modules.issueResponder.autoInvestigate')}</Label>
                                <Switch
                                  checked={config.issueResponder.autoInvestigateBugs}
                                  onCheckedChange={(v) => setModuleConfig('issue_responder', { autoInvestigateBugs: v })}
                                />
                              </div>
                              <div className="flex items-center justify-between">
                                <Label className="text-xs">{t('continuousAI:modules.issueResponder.autoCreateSpecs')}</Label>
                                <Switch
                                  checked={config.issueResponder.autoCreateSpecs}
                                  onCheckedChange={(v) => setModuleConfig('issue_responder', { autoCreateSpecs: v })}
                                />
                              </div>
                            </>
                          )}
                          ...
                          {mod.key === 'pr_reviewer' && (
                            <>
                              <div className="flex items-center justify-between">
                                <Label className="text-xs">{t('continuousAI:modules.prReviewer.autoApproveTrivial')}</Label>
                                <Switch
                                  checked={config.prReviewer.autoApproveTrivial}
                                  onCheckedChange={(v) => setModuleConfig('pr_reviewer', { autoApproveTrivial: v })}
                                />
                              </div>
                              <div className="flex items-center justify-between">
                                <Label className="text-xs">{t('continuousAI:modules.prReviewer.reviewExternalOnly')}</Label>
                                <Switch
                                  checked={config.prReviewer.reviewExternalOnly}
                                  onCheckedChange={(v) => setModuleConfig('pr_reviewer', { reviewExternalOnly: v })}
                                />
                              </div>
                              <div className="flex items-center justify-between">
                                <Label className="text-xs">{t('continuousAI:modules.prReviewer.postComments')}</Label>
                                <Switch
                                  checked={config.prReviewer.postReviewComments}
                                  onCheckedChange={(v) => setModuleConfig('pr_reviewer', { postReviewComments: v })}
                                />
                              </div>
                            </>
                          )}
