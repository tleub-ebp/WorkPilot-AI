...
export function DevToolsSettings({
	...
	const detectTools = useCallback(async () => {
		...
		try {
			...
			setDetectError(
				err instanceof Error ? err.message : "Failed to detect tools",
			);
		} finally {
			setIsDetecting(false);
		}
	...
	const handleTerminalChange = (terminal: SupportedTerminal) => {
		onSettingsChange({
			...
			// Clear custom path when switching away from custom
			customTerminalPath:
				terminal === "custom" ? settings.customTerminalPath : undefined,
		});
	};
	...
	const detectedTerminalIds = new Set(
		detectedTools?.terminals.map((t) => t.id) || [],
	);
	...
	return (
		<SettingsSection
			...
			description={t(
				...
				"Configure your preferred IDE and terminal for working with worktrees",
			)}
		>
			<div className="space-y-6">
				...
				<div className="space-y-2">
					...
					>
						<SelectTrigger id="preferred-ide">
							<SelectValue
								placeholder={t("devtools.ide.placeholder", "Select IDE...")}
							/>
						</SelectTrigger>
						<SelectContent>
					...
					<p className="text-xs text-muted-foreground">
						{t(
							"devtools.ide.description",
							"WorkPilot AI will open worktrees in this editor",
						)}
					</p>
				...
				<div className="space-y-2">
					...
					<Select
						...
						onValueChange={(value) =>
							handleTerminalChange(value as SupportedTerminal)
						}
					>
						<SelectTrigger id="preferred-terminal">
							<SelectValue
								placeholder={t(
									"devtools.terminal.placeholder",
									"Select terminal...",
								)}
							/>
						</SelectTrigger>
					...
					<p className="text-xs text-muted-foreground">
						{t(
							"devtools.terminal.description",
							"WorkPilot AI will open terminal sessions here",
						)}
					</p>
						...
						<div className="mt-3 space-y-2">
							...
							<div className="flex gap-2">
								<Input
									...
									onChange={(e) =>
										handleCustomTerminalPathChange(e.target.value)
									}
				...
				<div className="space-y-3 pt-2 border-t border-border">
					<div className="flex items-center justify-between">
						<div className="space-y-0.5">
							<Label
								htmlFor="auto-name-claude-terminals"
								className="text-sm font-medium"
							>
								{t(
									"devtools.autoNameClaude.label",
									"Auto-name Claude terminals",
								)}
							</Label>
							<p className="text-xs text-muted-foreground">
								{t(
									"devtools.autoNameClaude.description",
									"Use AI to generate a descriptive name for Claude terminals based on your first message",
								)}
							</p>
						</div>
				...
				<div className="space-y-3 rounded-md border border-amber-500/30 bg-amber-500/5 p-4">
					...
					<p className="text-xs text-amber-400/80">
						{t(
							"devtools.yoloMode.description",
							"Start Claude with --dangerously-skip-permissions flag, bypassing all safety prompts. Use with extreme caution.",
						)}
					</p>
				...
				{detectedTools && !isDetecting && (
					<div className="text-xs text-muted-foreground bg-muted/50 p-3 rounded-md">
						...
						<ul className="list-disc list-inside space-y-0.5">
							...
							{detectedTools.terminals
								.filter((t) => t.id !== "system")
								.map((term) => (
									<li key={term.id}>{term.name}</li>
								))}
							{detectedTools.ides.length === 0 &&
								detectedTools.terminals.filter((t) => t.id !== "system")
									.length === 0 && (
									<li>
										{t(
											"devtools.noToolsDetected",
