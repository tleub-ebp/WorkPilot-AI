...
import { AlertCircle, CheckCircle, Loader2, LogIn, LogOut, User, Shield } from 'lucide-react';
    ...
    try {
      ...
      if (cliAuthResponse.success && cliAuthResponse.data?.authenticated) {
        setAuthStatus({
          authenticated: true,
          profiles: [{
            username: cliAuthResponse.data.username || t('settings:copilotOAuth.defaultUser'),
        ...
        const pollInterval = setInterval(async () => {
          try {
            ...
            if (statusResponse.success && statusResponse.data.authenticated) {
              ...
              const latestProfile = statusResponse.data.profiles[statusResponse.data.profiles.length - 1];
              
              toast({
                title: t('settings:copilotOAuth.toast.authSuccess'),
                description: t('settings:copilotOAuth.toast.authSuccessDesc', { username: latestProfile.username }),
              });

              if (onAuthSuccess) {
                onAuthSuccess(latestProfile.username, latestProfile.profileName);
              }
    ...
    try {
      ...
      if (response.success) {
        toast({
          ...
          description: t('settings:copilotOAuth.toast.authRevokedDesc', { username }),
        });
      ...
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          <Badge variant={authStatus.authenticated ? 'default' : 'secondary'}>
            {authStatus.authenticated ? t('settings:copilotOAuth.authenticated') : t('settings:copilotOAuth.notAuthenticated')}
          </Badge>
        </CardTitle>
        <CardDescription>
      ...
      <CardContent className="space-y-4">
        {authStatus.authenticated && authStatus.profiles.length > 0 ? (
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {t('settings:copilotOAuth.addedDate', { date: new Date(profile.createdAt).toLocaleDateString() })}
                      </p>
                    </div>
                  </div>
                  <Button
          ...
          <div className="text-center py-8">
            ...
            <h4 className="font-medium mb-2">{t('settings:copilotOAuth.noAuthFound')}</h4>
            ...
            >
              {isLoading || isStartingOAuth ? (
                <>
                  <Loader2 className="w-4 h-4 animate-spin mr-2" />
                  {isStartingOAuth ? t('settings:copilotOAuth.openingBrowser') : t('settings:copilotOAuth.authenticating')}
                </>
              ) : (
                <>
                  <LogIn className="w-4 h-4 mr-2" />
