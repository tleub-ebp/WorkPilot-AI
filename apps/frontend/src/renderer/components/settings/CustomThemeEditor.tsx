...
import { Plus, Download, Upload, Trash2, Palette, Check } from 'lucide-react';
 ...
 */
export function CustomThemeEditor({ settings, onSettingsChange }: CustomThemeEditorProps) {
  ...
  const handleImport = () => {
    ...
    input.onchange = async (e) => {
      ...
      try {
        ...
        if (!data.name || !data.colors?.bg || !data.colors?.accent || !data.colors?.darkBg || !data.colors?.darkAccent) {
          setImportError('Invalid theme file: missing required fields (name, colors.bg, colors.accent, colors.darkBg, colors.darkAccent)');
          return;
        }
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <div>
            <Label className="text-sm font-medium text-foreground">Custom Themes</Label>
            <p className="text-sm text-muted-foreground">Create your own color themes or import shared ones</p>
          </div>
          <div className="flex gap-2">
            ...
            <Button size="sm" variant="outline" onClick={() => setIsCreating(!isCreating)} className="gap-1.5">
        ...
        {isCreating && (
          <div className="rounded-lg border border-primary/30 bg-primary/5 p-4 space-y-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {(Object.keys(DEFAULT_CUSTOM_COLORS) as Array<keyof typeof DEFAULT_CUSTOM_COLORS>).map((key) => (
            ...
            <div className="flex gap-2 items-center">
              ...
              <div className="flex -space-x-1">
                <div className="w-6 h-6 rounded-full border-2 border-background shadow-sm" style={{ backgroundColor: editColors.bg }} />
                <div className="w-6 h-6 rounded-full border-2 border-background shadow-sm" style={{ backgroundColor: editColors.accent }} />
                <div className="w-6 h-6 rounded-full border-2 border-background shadow-sm" style={{ backgroundColor: editColors.darkBg }} />
                <div className="w-6 h-6 rounded-full border-2 border-background shadow-sm" style={{ backgroundColor: editColors.darkAccent }} />
              </div>
            </div>

            <div className="flex gap-2 justify-end">
              <Button size="sm" variant="ghost" onClick={() => setIsCreating(false)}>Cancel</Button>
              <Button size="sm" onClick={handleCreate} disabled={!editName.trim()}>Create Theme</Button>
            </div>
          </div>
        )}
        ...
        {customThemes.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 pt-1">
            {customThemes.map((theme) => {
              ...
              const accentColor = isDark ? theme.colors.darkAccent : theme.colors.accent;
        ...
        {customThemes.length === 0 && !isCreating && (
          <div className="rounded-lg border border-dashed border-border p-6 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1">Create a new theme or import one to get started</p>
          </div>
        )}
      </div>
    </div>
