...
import { Eye, EyeOff, CheckCircle2, AlertCircle, ExternalLink, MessageSquare } from 'lucide-react';
 ...
 */
export function TeamsIntegration({ envConfig, updateEnvConfig }: TeamsIntegrationProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h3 className="font-medium text-foreground">{t('settings:teams.title')}</h3>
          <p className="text-sm text-muted-foreground">{t('settings:teams.description')}</p>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between">
        <div>
          <Label className="font-normal text-foreground">{t('settings:teams.enableNotifications')}</Label>
          <p className="text-xs text-muted-foreground mt-0.5">{t('settings:teams.enableNotificationsHint')}</p>
        </div>
        <Switch
          checked={enabled}
          onCheckedChange={(checked) => updateEnvConfig({ teamsNotificationsEnabled: checked })}
        />
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="relative">
          <Input
            ...
            value={webhookUrl}
            onChange={(e) => updateEnvConfig({ teamsWebhookUrl: e.target.value })}
        ...
        <p className="text-xs text-muted-foreground">{t('settings:teams.webhookUrlHint')}</p>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        >
          {testStatus === 'testing' ? t('common:testing') : t('settings:teams.testWebhook')}
        </Button>
