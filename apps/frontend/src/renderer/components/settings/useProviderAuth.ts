...
export function useProviderAuth() {
  ...
  const handleClaudeAuth = useCallback(async (_providerId: string, providerName: string) => {
    try {
      ...
      if (!activeProfileId) {
        ...
        return { success: false, error: 'No Claude profile found. Please restart the application.' };
      }
      ...
      if (result.success && result.data) {
        ...
        return { success: false, error: result.error || 'Failed to prepare authentication' };
      }
    } catch (error) {
  ...
  const handleFallbackAuth = useCallback((providerId: string, providerName: string) => {
