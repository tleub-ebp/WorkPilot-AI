...
const providerIcons: Record<string, React.ReactNode> = {
  ...
  windsurf: (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Windsurf">
      ...
      <path d="M12 2v20c5.16-1.26 9-6.45 9-12V7l-9-4.5z" fill="#00B894" opacity="0.8"/>
    </svg>
  ),
  ...
  cursor: (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Cursor">
      ...
      <path d="M3 3l7.07 16.97 2.51-7.39 7.39-2.51L3 3z" stroke="#000000" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
  ),
  // Grok (xAI) — logo stylisé
  grok: (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Grok">
      <title>Grok</title>
      <path d="M12 2L2 7v10c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V7l-10-5z" fill="#FF6B35"/>
      <path d="M12 2v20c5.16-1.26 9-6.45 9-12V7l-9-4.5z" fill="#E85D2D" opacity="0.8"/>
    </svg>
  ),
...
const getProviderIcon = (providerId: string, _category: string, isConfigured: boolean) => {
  ...
  if (!icon) {
    ...
    return (
      <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label={providerId}>
        <title>{providerId}</title>
        <circle cx="10" cy="10" r="9" stroke={isConfigured ? "#4B5563" : "#BDBDBD"} strokeWidth="2" fill={isConfigured ? "#F5F5F5" : "#F5F5F5"} />
        <text x="50%" y="55%" textAnchor="middle" fontSize="10" fill={isConfigured ? "#4B5563" : "#BDBDBD"} fontWeight="bold" dy=".3em">{providerId.charAt(0).toUpperCase()}</text>
      </svg>
    );
  }
  return icon;
...
export function ElegantProviderCard({
  ...
  const ProviderIcon = getProviderIcon(provider.id, provider.category, provider.isConfigured);
  const categoryGradient = categoryGradients[provider.category] || categoryGradients['independent'];
    ...
    if (!provider.isConfigured) {
      return (
        <div className="flex items-center gap-2 px-3 py-1.5 bg-gray-100/50 rounded-full">
          ...
          <span className="text-xs font-medium text-gray-600">Non configuré</span>
        </div>
      );
    }
              ...
              {provider.isConfigured && (
                ...
                >
                  <div className={cn(
                    ...
                    provider.isWorking === false ? 'translate-x-0.5' : 'translate-x-5'
                  )} />
                </button>
              )}
            </div>
