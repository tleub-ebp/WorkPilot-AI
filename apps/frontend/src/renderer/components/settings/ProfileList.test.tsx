...
describe("ProfileList - Delete Confirmation Dialog", () => {
	...
	it("should show delete confirmation dialog with profile name", () => {
		...
		expect(
			screen.getByText(i18n.t("settings:apiProfiles.dialog.deleteTitle")),
		).toBeInTheDocument();
		expect(
			screen.getByText(
				i18n.t("settings:apiProfiles.dialog.deleteDescription", {
					name: "Production API",
				}),
			),
		).toBeInTheDocument();
		expect(
			screen.getByText(i18n.t("settings:apiProfiles.dialog.cancel")),
		).toBeInTheDocument();
		expect(
			screen.getByText(i18n.t("settings:apiProfiles.dialog.delete")),
		).toBeInTheDocument();
	});
	...
	it("should close dialog when cancel is clicked", async () => {
		...
		const cancelButton = await screen.findByText(
			i18n.t("settings:apiProfiles.dialog.cancel"),
		);
		fireEvent.click(cancelButton);

		// Dialog should be closed
		expect(
			screen.queryByText(i18n.t("settings:apiProfiles.dialog.deleteTitle")),
...
describe("ProfileList - Switch to OAuth Button", () => {
	...
	it('should show "Switch to OAuth" button when a profile is active', () => {
		...
		expect(
			screen.getByText(i18n.t("settings:apiProfiles.switchToOauth.label")),
		).toBeInTheDocument();
	});
	...
	it('should NOT show "Switch to OAuth" button when no profile is active', () => {
		...
		expect(
			screen.queryByText(i18n.t("settings:apiProfiles.switchToOauth.label")),
		).not.toBeInTheDocument();
	});
