...
export function ModelSearchableSelect({
	...
	const [modelDiscoveryNotSupported, setModelDiscoveryNotSupported] =
		useState(false);
	...
	const fetchModels = async () => {
		...
		try {
			...
			if (err instanceof Error && err.name !== "AbortError") {
				...
				if (
					err.message.includes("does not support model listing") ||
					...
					setModelDiscoveryNotSupported(true);
					console.warn(
						"[ModelSearchableSelect] Model discovery failed:",
						err.message,
					);
				}
