...
export function CustomProviderConfig() {
  ...
  const [newModelTier, setNewModelTier] = useState<'flagship' | 'standard' | 'fast'>('standard');
    ...
    try {
      ...
      if (Array.isArray(data.data)) {
        ...
        data.data.forEach((model: any, index: number) => {
          discoveredModels.push({
            ...
            supportsThinking: model.id?.toLowerCase().includes('thinking') || 
        ...
        data.forEach((model: any, index: number) => {
          discoveredModels.push({
            ...
            supportsThinking: (model.name || model.id || '').toLowerCase().includes('thinking') ||
                           (model.name || model.id || '').toLowerCase().includes('reasoning') ||
      ...
      if (discoveredModels.length > 0) {
        ...
        toast({
          ...
          description: t('customProvider.modelsDiscoveredDesc', { count: discoveredModels.length }),
        });
      } else {
        toast({
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : t('customProvider.discoveryFailedDesc'),
        variant: 'destructive',
      });
    } finally {
  ...
  const getTierFromModelName = (modelName: string): 'flagship' | 'standard' | 'fast' => {
    const name = modelName.toLowerCase();
    if (name.includes('gpt-4') || name.includes('claude-3') || name.includes('gemini-pro') || 
        name.includes('llama-3') || name.includes('mixtral') || name.includes('opus')) {
      return 'flagship';
    } else if (name.includes('gpt-3.5') || name.includes('claude-instant') || name.includes('gemini')) {
      return 'standard';
    }
    return 'fast';
  };
    ...
    >
      <div className="space-y-6">
        ...
        <div className="space-y-4">
          <h3 className="text-sm font-medium text-foreground">{t('customProvider.apiConfig')}</h3>
          ...
          <div className="space-y-3">
            <div>
              ...
              <Input
                ...
                onChange={(e) => saveSettings({ customProviderUrl: e.target.value })}
                className="mt-1"
              />
            </div>

            <div>
              <Label htmlFor="custom-key" className="text-xs text-muted-foreground">
              ...
              <Input
                ...
                onChange={(e) => saveSettings({ customProviderApiKey: e.target.value })}
                className="mt-1"
              />
            </div>
          </div>
          ...
          >
            ...
            {isDiscovering ? t('customProvider.discovering') : t('customProvider.discoverModels')}
          </Button>
        </div>
        ...
        <div className="space-y-4">
          <h3 className="text-sm font-medium text-foreground">{t('customProvider.manualModels')}</h3>
          ...
          <div className="space-y-3 p-4 border border-border rounded-lg bg-muted/30">
            ...
            <div className="flex items-center gap-4">
              <div className="flex items-center space-x-2">
                <Label htmlFor="model-tier" className="text-xs text-muted-foreground">
                ...
                >
                  <option value="flagship">{t('customProvider.tierFlagship')}</option>
                  <option value="standard">{t('customProvider.tierStandard')}</option>
              ...
              <div className="flex items-center space-x-2">
                <input
                  ...
                  className="rounded"
                />
                <Label htmlFor="model-thinking" className="text-xs text-muted-foreground">
                  {t('customProvider.supportsThinking')}
                </Label>
              </div>

              <Button onClick={handleAddCustomModel} size="sm" className="ml-auto">
          ...
          {customModels.length === 0 && (
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p className="text-sm">{t('customProvider.noModelsConfigured')}</p>
              <p className="text-xs mt-1">{t('customProvider.addModelsDesc')}</p>
            </div>
          )}
        </div>
      </div>
