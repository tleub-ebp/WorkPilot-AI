...
export function CopilotAuthTerminal({
  ...
  const [status, setStatus] = useState<'loading' | 'running' | 'success' | 'error'>('loading');
  ...
  const handleTerminalData = useCallback((data: string) => {
    ...
    if (data.includes('Logged in to') || data.includes('Authentication complete')) {
      setStatus('success');
    ...
    <div className="flex flex-col h-full bg-gray-900 rounded-lg overflow-hidden">
      ...
      {statusMessage && (
        <div className={cn(
          ...
          status === 'success' ? 'bg-green-900/20 text-green-400 border-green-800' :
          status === 'error' ? 'bg-red-900/20 text-red-400 border-red-800' :
