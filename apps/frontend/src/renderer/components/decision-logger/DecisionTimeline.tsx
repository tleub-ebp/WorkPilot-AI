...
import {
	selectEntriesForTask,
	useDecisionLoggerStore,
} from "../../stores/decision-logger-store";
...
export function DecisionTimeline({
	...
	useEffect(() => {
		...
		const unsub = api.onDecisionLogEntry(
			(incomingTaskId: string, entry: DecisionEntry) => {
	...
	useEffect(() => {
		...
		api
			...
			.then(
				(result: {
					success: boolean;
					data?: import("../../../shared/types/decision-logger").DecisionLog;
					error?: string;
				}) => {
	...
	if (store.isLoadingHistory) {
		return (
			<div className="flex items-center justify-center py-8 text-muted-foreground text-sm gap-2">
				...
				<span>
					{t("tasks:decisionLogger.loading", "Loading decision log…")}
				</span>
			</div>
		);
	}
	...
	if (entries.length === 0) {
		return (
			<div className="flex flex-col items-center justify-center py-8 text-muted-foreground text-sm gap-2">
				...
				<span>
					{t("tasks:decisionLogger.empty", "No decisions recorded yet")}
				</span>
				<span className="text-xs">
					{t(
						"tasks:decisionLogger.emptyHint",
						"Decisions appear here while the agent is running.",
					)}
				</span>
			</div>
		);
