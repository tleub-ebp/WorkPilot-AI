...
export function UsageIndicatorAgnostic({ selectedProvider }: UsageIndicatorAgnosticProps = {}) {
  ...
  const Icon = getIcon();
  const sessionResetTime = getFormattedResetTime(usageData.metrics.sessionResetTimestamp);
  const periodicResetTime = getFormattedResetTime(usageData.metrics.periodicResetTimestamp);
            ...
            <div className="py-2 space-y-3">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-xs font-semibold ${getColorClass('text')}`}>
                    {Math.round(sessionPercent)}%
                  </span>
                </div>
                  ...
                  {usageData.details.anthropic && (
                    <div className="space-y-1">
                      {usageData.details.anthropic.subscriptionType && (
                        <div>Subscription: {usageData.details.anthropic.subscriptionType}</div>
                      )}
                      {usageData.details.anthropic.opusUsagePercent && (
                        <div>Opus: {usageData.details.anthropic.opusUsagePercent}%</div>
                      )}
                    </div>
                  )}
                  ...
                  {usageData.details.openai && (
                    <div className="space-y-1">
                      {usageData.details.openai.estimatedCost && (
                        <div>Cost: ${usageData.details.openai.estimatedCost.toFixed(2)}</div>
                      )}
                    </div>
                  )}
                  ...
                  {usageData.details.copilot && (
                    <div className="space-y-1">
                      {usageData.details.copilot.totalTokens && (
                        <div>Tokens: {usageData.details.copilot.totalTokens.toLocaleString()}</div>
                      )}
                      {usageData.details.copilot.acceptanceRate && (
                        <div>Acceptance: {usageData.details.copilot.acceptanceRate.toFixed(1)}%</div>
                      )}
                    </div>
                  )}
                </div>
          ...
          >
            <div className="relative">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${
                ...
                <span className={`text-xs font-semibold ${
                  ...
                  {getInitials(activeCredential?.credentials?.profileName || usageData.profileName)}
                </span>
              </div>
              {needsReauth && (
            ...
            <div className="flex-1 min-w-0 text-left">
              ...
              <p className="text-[10px] text-muted-foreground truncate">
                {activeCredential?.credentials?.profileName || usageData.profileName || usageData.profileEmail || 'Unknown Account'}
              </p>
            </div>
