...
export function BrowserTab({ projectPath }: BrowserTabProps) {
  ...
  const isNavigating = browserStatus === 'launching' || browserStatus === 'navigating';
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 text-[11px]">
          {browserStatus === 'ready' ? (
            <>
              ...
              <span className="text-emerald-400/80 font-medium">{t('browserAgent:browser.ready')}</span>
              <span className="text-[var(--text-tertiary)]">·</span>
              <span className="text-[var(--text-tertiary)] truncate">{currentUrl}</span>
            </>
          ) : browserStatus === 'error' ? (
            <>
              <WifiOff className="w-3 h-3 text-red-400" />
              <span className="text-red-400/80 font-medium">{t('browserAgent:errors.screenshotFailed')}</span>
            </>
          ) : null}
        </div>
      )}
