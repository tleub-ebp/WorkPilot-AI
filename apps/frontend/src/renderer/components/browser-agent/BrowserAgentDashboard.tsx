      ...
      <div className="px-6 py-4 border-b border-[var(--border-primary)] bg-[var(--bg-primary)]">
        ...
        <div className="grid grid-cols-4 gap-3">
          ...
          <StatCard
            ...
            color={stats.passRate >= 80 ? 'emerald' : stats.passRate > 0 ? 'amber' : 'emerald'}
          />
          <StatCard
            icon={Camera}
...
const STAT_COLORS: Record<StatColor, { icon: string; value: string; bg: string; border: string }> = {
  blue: {
    icon: 'text-blue-400',
    value: 'text-blue-400',
    bg: 'bg-blue-500/10',
...
function StatCard({
  ...
  return (
    <div className={`px-3 py-2.5 rounded-lg ${colors.bg} border ${colors.border} flex items-center gap-3`}>
      <Icon className={`w-4.5 h-4.5 ${colors.icon} shrink-0`} />
      <div className="min-w-0">
        <p className="text-[11px] text-[var(--text-tertiary)] truncate">{label}</p>
        <p className={`text-lg font-semibold ${colors.value} leading-tight`}>{value}</p>
      </div>
    </div>
  );
}
