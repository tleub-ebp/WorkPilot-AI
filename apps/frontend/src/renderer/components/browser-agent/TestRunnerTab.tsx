        ...
        {recentTestRun && (
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1 text-xs text-blue-400/80 bg-blue-500/10 px-2 py-1 rounded-md border border-blue-500/15">
              ...
              <span className="font-mono">{(recentTestRun.durationMs / 1000).toFixed(1)}s</span>
            </div>
          </div>
        )}
      </div>
      ...
      {recentTestRun && (
        <div className="rounded-lg border border-[var(--border-primary)] overflow-hidden">
          {recentTestRun.results.map((result, index) => {
            ...
            const { icon: StatusIcon, color, bgColor, borderColor } = getStatusConfig(result.status);
              ...
              <div key={key}>
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-2.5 hover:bg-[var(--bg-hover)] text-left transition-colors ${
                    !isLast || isExpanded ? 'border-b border-[var(--border-primary)]' : ''
                ...
                >
                  ...
                  <div className="flex-1 min-w-0 flex items-center gap-2">
                    <span className="text-sm text-[var(--text-primary)] truncate">{result.name}</span>
...
function getStatusConfig(status: string): {
  ...
  switch (status) {
    case 'passed':
      return { icon: CheckCircle2, color: 'text-emerald-400', bgColor: 'bg-emerald-500/5', borderColor: 'border-emerald-500/15' };
    case 'failed':
      return { icon: XCircle, color: 'text-rose-400', bgColor: 'bg-rose-500/5', borderColor: 'border-rose-500/15' };
    case 'skipped':
      return { icon: MinusCircle, color: 'text-amber-400', bgColor: 'bg-amber-500/5', borderColor: 'border-amber-500/15' };
    case 'error':
      return { icon: AlertCircle, color: 'text-red-500', bgColor: 'bg-red-500/5', borderColor: 'border-red-500/15' };
    default:
      return { icon: AlertCircle, color: 'text-[var(--text-tertiary)]', bgColor: 'bg-[var(--bg-secondary)]', borderColor: 'border-[var(--border-primary)]' };
  }
}
