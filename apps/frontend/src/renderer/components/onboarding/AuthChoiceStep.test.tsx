...
describe("AuthChoiceStep", () => {
	...
	describe("Rendering", () => {
		...
		it("should show icons for each auth option", () => {
			...
			const iconContainers =
				document.querySelectorAll(String.raw`.bg-primary\/10`);
	...
	describe("OAuth Button Handler", () => {
		it("should call onNext when OAuth button is clicked", () => {
			...
			const oauthButton = screen
				.getByText("Sign in with Anthropic")
	...
	describe("API Key Button Handler", () => {
		it("should open ProfileEditDialog when API Key button is clicked", () => {
			...
			const apiKeyButton = screen
				.getByText("Use Custom API Key")
				.closest(".cursor-pointer");
			// biome-ignore lint/style/noNonNullAssertion: value is guaranteed by context
	...
	describe("Accessibility", () => {
		it("should have descriptive text for each auth option", () => {
			...
			expect(
				screen.getByText(/Use your Anthropic account to authenticate/),
			).toBeInTheDocument();
		...
		it("should have helper text explaining both options", () => {
			...
			expect(
				screen.getByText(
					/Both options provide full access to Claude Code features/,
				),
			).toBeInTheDocument();
		});
	});
