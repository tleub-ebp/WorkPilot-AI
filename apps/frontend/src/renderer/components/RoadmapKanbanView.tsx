...
import { Calendar, Check, Eye, Inbox, Play, Plus } from "lucide-react";
		...
		>
			...
			<div className="flex items-center justify-between p-4 border-b border-white/5">
				<div className="flex items-center gap-2.5">
					...
					<span className="column-count-badge">{features.length}</span>
				</div>
			</div>
...
export function RoadmapKanbanView({
	...
	const [activeFeature, setActiveFeature] = useState<RoadmapFeature | null>(
		null,
	);
	const [overColumnId, setOverColumnId] = useState<string | null>(null);

	const updateFeatureStatus = useRoadmapStore(
		(state) => state.updateFeatureStatus,
	);
	...
	const featuresByStatus = useMemo(() => {
		...
		ROADMAP_STATUS_COLUMNS.forEach((column) => {
			grouped[column.id] = roadmap.features.filter(
				(f) => f.status === column.id,
			);
		});
		return grouped;
	}, [roadmap.features]);
	...
	const handleDragEnd = (event: DragEndEvent) => {
		...
		const overId = over.id as string;
		const draggedFeature = roadmap.features.find(
	...
	const getStatusLabelForFeature = (feature: RoadmapFeature) => {
		const statusColumn = ROADMAP_STATUS_COLUMNS.find(
			(c) => c.id === feature.status,
		);
