...
import type { RepoDependencyGraph, RepoExecutionState, RepoExecutionStatus } from '@shared/types';
 ...
 */
export function DependencyGraphView({ graph, executionOrder, repoStates }: DependencyGraphViewProps) {
    ...
    <div className="rounded-lg border border-border bg-card p-4">
      ...
      {executionOrder.length > 0 && (
        <div className="mt-3 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
          <span className="font-medium">{t('multiRepo:graph.executionOrder')}:</span>
          {executionOrder.map((repo, i) => (
            <span key={repo} className="flex items-center gap-1">
              ...
              {repo.split('/').pop()}
              {i < executionOrder.length - 1 && <span className="text-border ml-1">→</span>}
            </span>
          ))}
        </div>
      )}
