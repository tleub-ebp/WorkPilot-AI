import { type CanonicalProvider, getProviders } from "@shared/utils/providers";
	...
	useEffect(() => {
		if (open) {
			...
			getProviders().then(({ providers }) => {
				...
				if (
					defaultProvider &&
					providers.some((p) => p.name === defaultProvider)
				) {
			...
			<DialogContent className="sm:max-w-lg">
				...
				<div className="py-2">
					...
					<div className="mb-1">
						...
						<span className="font-mono">{selectedProviderLabel}</span>
					</div>
				</div>
				<DialogFooter>
					...
					<Button
						onClick={() => onConfirm(description, selectedProvider)}
						disabled={!selectedProvider || !description}
					>
