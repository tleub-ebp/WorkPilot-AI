...
export function AuthFailureModal({ onOpenSettings }: AuthFailureModalProps) {
	...
	const profileName =
		authFailureInfo.profileName ||
		t("auth.failure.unknownProfile", "Unknown Profile");
	...
	const getFailureMessage = () => {
		switch (authFailureInfo.failureType) {
			case "expired":
				return t(
					"auth.failure.tokenExpired",
					"Your authentication token has expired.",
				);
			case "invalid":
				return t(
					"auth.failure.tokenInvalid",
	...
	return (
		<Dialog
			open={isModalOpen}
			onOpenChange={(open) => !open && hideAuthFailureModal()}
		>
				...
				<div className="space-y-4 py-4">
					...
					<p className="text-sm text-muted-foreground">
						{t(
							...
							"Please re-authenticate your Claude profile to continue using WorkPilot AI.",
						)}
					</p>
					...
					{authFailureInfo.taskId && (
						<div className="rounded-md bg-muted p-3 text-xs">
							<p className="text-muted-foreground">
								{t("auth.failure.taskAffected", "Task affected")}:{" "}
								<span className="font-mono">{authFailureInfo.taskId}</span>
							</p>
						</div>
					)}
