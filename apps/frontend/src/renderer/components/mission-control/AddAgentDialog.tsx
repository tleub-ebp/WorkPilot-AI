...
interface AddAgentDialogProps {
  ...
  readonly onAdd: (name: string, role: string, provider: string, model: string, modelLabel: string) => void;
}

const ROLES = [
  { value: 'architect', label: '🏗️ Architect', desc: 'System design & architecture' },
...
export function AddAgentDialog({ open, onOpenChange, onAdd }: AddAgentDialogProps) {
  ...
  useEffect(() => {
    ...
    const recommended = providerData.models.find((m) => m.tier === recommendedTier);
              ...
              <SelectContent>
                {providerData?.models.map((m) => (
                  <SelectItem key={m.value} value={m.value}>
                    <div className="flex items-center gap-2">
                      <span>{m.label}</span>
                      <span className={`text-[10px] capitalize px-1 rounded ${
                        m.tier === 'flagship' ? 'bg-amber-500/10 text-amber-600' :
                        m.tier === 'fast' ? 'bg-green-500/10 text-green-600' :
