...
import { AgentPanel, AddAgentDialog, DecisionTreeViewer, AgentEventLog } from './index';
...
export function MissionControlDashboard() {
  ...
  const totalCost = agents.reduce((sum, a) => sum + a.tokens.estimated_cost_usd, 0);
      ...
      <div className="flex flex-col items-center justify-center h-full gap-8 p-8">
        <div className="flex flex-col items-center gap-4 max-w-lg text-center">
          ...
          <p className="text-muted-foreground text-lg">
            {t('missionControl:launchDescription', 'Multi-Agent Orchestration Hub — Orchestrate multiple AI agents simultaneously with full visibility on reasoning, files, and token consumption.')}
          </p>
          <div className="grid grid-cols-2 gap-3 w-full max-w-sm text-sm text-muted-foreground">
            <div className="flex items-center gap-2 bg-card/50 rounded-lg p-3 border border-border/50">
              ...
              <span>{t('missionControl:featureMultiAgent', 'Multi-Agent')}</span>
            </div>
            <div className="flex items-center gap-2 bg-card/50 rounded-lg p-3 border border-border/50">
              <Brain className="h-4 w-4 text-primary" />
              <span>{t('missionControl:featureModelMixing', 'Model Mixing')}</span>
            </div>
            <div className="flex items-center gap-2 bg-card/50 rounded-lg p-3 border border-border/50">
              <GitBranch className="h-4 w-4 text-primary" />
              <span>{t('missionControl:featureDecisionTree', 'Decision Tree')}</span>
            </div>
            <div className="flex items-center gap-2 bg-card/50 rounded-lg p-3 border border-border/50">
      ...
      <div className="p-4 space-y-4 max-w-[1800px] mx-auto">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="outline" size="sm" onClick={() => setShowAddAgent(true)} className="gap-1.5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <Card className="border-border/50">
            <CardContent className="p-3 flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-muted-foreground">{t('missionControl:statAgents', 'Agents')}</p>
                <p className="text-lg font-bold">
                  {runningCount}<span className="text-sm font-normal text-muted-foreground">/{agents.length}</span>
                </p>
              </div>
            </CardContent>
          </Card>
          <Card className="border-border/50">
            <CardContent className="p-3 flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-muted-foreground">{t('missionControl:statTokens', 'Tokens')}</p>
          ...
          <Card className="border-border/50">
            <CardContent className="p-3 flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-muted-foreground">{t('missionControl:statCost', 'Cost')}</p>
          ...
          <Card className="border-border/50">
            <CardContent className="p-3 flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-muted-foreground">{t('missionControl:statElapsed', 'Elapsed')}</p>
                <p className="text-lg font-bold">{formatElapsed(session?.elapsed_seconds ?? 0)}</p>
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        {agents.length === 0 ? (
          <Card className="border-dashed border-2 border-border/50">
            <CardContent className="flex flex-col items-center justify-center py-12 gap-4 text-muted-foreground">
              ...
              <p className="text-sm">{t('missionControl:noAgents', 'No agents yet. Add your first agent to get started.')}</p>
              <Button variant="outline" size="sm" onClick={() => setShowAddAgent(true)} className="gap-1.5">
                <Plus className="h-3.5 w-3.5" />
                {t('missionControl:addAgent', 'Add Agent')}
              </Button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
            {agents.map((agent) => (
              <AgentPanel
                ...
                onUpdateConfig={(config: Record<string, unknown>) => updateAgentConfig(agent.id, config)}
              />
            ))}
          </div>
        )}
              ...
              <CardContent className="p-3">
                {selectedTree ? (
                  ...
                  <div className="flex items-center justify-center h-32 text-sm text-muted-foreground">
                    {t('missionControl:selectAgentForTree', 'Select an agent to view its decision tree')}
                  </div>
                )}
              </CardContent>
            </Card>
