	...
	const fetchPRDetails = async () => {
		...
		try {
			...
			if (prMatch) {
				...
				if (!adoMatch) {
					throw new Error(
						"Invalid PR URL format - expected GitHub or Azure DevOps URL",
					);
				}
				...
				<Tabs defaultValue="files" className="flex-1 overflow-hidden">
					<TabsList className="grid w-full grid-cols-3">
						<TabsTrigger value="overview">
							{t("taskReview:pr.files.overview")}
						</TabsTrigger>
						<TabsTrigger value="files">
							{t("taskReview:pr.files.files")} ({prData.changed_files})
						</TabsTrigger>
						<TabsTrigger value="diff">
							{t("taskReview:pr.files.diff")}
						</TabsTrigger>
					</TabsList>
						...
						<div className="space-y-4">
							<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
								<div className="text-center p-4 border rounded-lg">
									...
									<div className="text-sm text-muted-foreground">
										{t("taskReview:pr.files.additions")}
									</div>
								</div>
								<div className="text-center p-4 border rounded-lg">
									...
									<div className="text-sm text-muted-foreground">
										{t("taskReview:pr.files.deletions")}
									</div>
								</div>
								<div className="text-center p-4 border rounded-lg">
									...
									<div className="text-sm text-muted-foreground">
										{t("taskReview:pr.files.changedFiles")}
									</div>
								</div>
								<div className="text-center p-4 border rounded-lg">
									<div className="text-2xl font-bold">
									...
									<div className="text-sm text-muted-foreground">
										{t("taskReview:pr.files.totalChanges")}
									</div>
								</div>
							</div>
							...
							<div className="space-y-2">
								<h4 className="font-semibold">
									{t("taskReview:pr.files.details")}
								</h4>
								<div className="grid grid-cols-2 gap-2 text-sm">
									<div>
										<span className="text-muted-foreground">
											{t("taskReview:pr.files.author")}:{" "}
										</span>
										<span>{prData.author}</span>
									</div>
									<div>
										<span className="text-muted-foreground">
											{t("taskReview:pr.files.sourceBranch")}:{" "}
										</span>
										<span className="font-mono">{prData.source_branch}</span>
									</div>
									<div>
										<span className="text-muted-foreground">
											{t("taskReview:pr.files.targetBranch")}:{" "}
										</span>
										<span className="font-mono">{prData.target_branch}</span>
									</div>
									<div>
										<span className="text-muted-foreground">
											{t("taskReview:pr.files.state")}:{" "}
										</span>
										<Badge
											variant={
												prData.state === "open" ? "default" : "secondary"
											}
										>
											{prData.state}
										</Badge>
							...
							{prData.body && (
								<div className="space-y-2">
									<h4 className="font-semibold">
										{t("taskReview:pr.files.description")}
									</h4>
					...
					<TabsContent value="diff" className="mt-4">
						<ScrollArea className="h-[500px] rounded-md border">
							<div className="p-4">
								<DiffViewer patch={prData.diff} className="max-h-full" />
							</div>
						</ScrollArea>
					</TabsContent>
				</Tabs>
