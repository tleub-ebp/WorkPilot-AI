...
export const MigrationWizard: React.FC = () => {
	...
	const defaultProjectPath =
		selectedProject?.path || t("defaultProjectPath", { defaultValue: "./" });
	...
	const categories = [
		{ value: "all", label: t("categories.all"), icon: <Layers className="w-4 h-4" /> },
		{ value: "frontend", label: t("categories.frontend"), icon: <Layout className="w-4 h-4" /> },
		{ value: "backend", label: t("categories.backend"), icon: <Shield className="w-4 h-4" /> },
		{ value: "database", label: t("categories.database"), icon: <Database className="w-4 h-4" /> },
		{ value: "language", label: t("categories.language"), icon: <Code className="w-4 h-4" /> },
		{ value: "api", label: t("categories.api"), icon: <GitBranch className="w-4 h-4" /> },
		{ value: "build", label: t("categories.build"), icon: <Wrench className="w-4 h-4" /> },
		{ value: "testing", label: t("categories.testing"), icon: <Shield className="w-4 h-4" /> },
		{ value: "mobile", label: t("categories.mobile"), icon: <Smartphone className="w-4 h-4" /> },
		{ value: "package", label: t("categories.package"), icon: <Package className="w-4 h-4" /> },
	];
	...
	const pollMigrationStatus = async (id: string) => {
		const interval = setInterval(async () => {
			try {
				// TODO: Connect to backend migration API when available
				// biome-ignore lint/suspicious/noExplicitAny: TODO: type this properly
				const status = (await (window as any).electronAPI?.getMigrationStatus?.(
					id,
				)) ?? {
					progress: 100,
					state: "complete",
					...
					<div className="space-y-6">
						<div>
							<Label className="block text-sm font-medium mb-2">
								{t("fields.projectPath")}
							</Label>
							<Input
								type="text"
								value={config.projectPath}
								onChange={(e) =>
									setConfig({ ...config, projectPath: e.target.value })
								}
								...
								{filteredMigrations.map((m) => (
									<Card
										...
										onClick={() => {
											setConfig({
												...config,
												sourceFramework: m.source,
												targetFramework: m.target,
											});
										}}
										className={`p-4 border rounded-lg text-left cursor-pointer transition-colors ${
											config.sourceFramework === m.source &&
											config.targetFramework === m.target
						...
						<div className="space-y-4">
							...
							<label className="flex items-center space-x-3">
								<Checkbox
									...
									onCheckedChange={(checked) =>
										setConfig({ ...config, enableLLM: !!checked })
									}
									className="w-4 h-4 text-blue-600 rounded focus:ring-blue-500"
								/>
								<div>
									<div className="font-medium">
										{t("options.llmEnhancement.title")}
									</div>
							...
							<label className="flex items-center space-x-3">
								<Checkbox
									...
									onCheckedChange={(checked) =>
										setConfig({ ...config, autoFix: !!checked })
									}
									className="w-4 h-4 text-blue-600 rounded focus:ring-blue-500"
								/>
								<div>
									<div className="font-medium">
										{t("options.autoFixLoop.title")}
									</div>
									<div className="text-sm text-gray-500">
