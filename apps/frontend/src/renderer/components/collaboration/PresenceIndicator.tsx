...
import { useCollaborationStore, type ConnectedUser, type UserStatus } from '../../stores/collaboration-store';
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
...
function UserAvatar({ user, isCurrentUser }: { user: ConnectedUser; isCurrentUser: boolean }) {
        ...
        <TooltipContent side="bottom" className="max-w-52">
          <div className="space-y-1">
            <div className="flex items-center gap-1.5">
              ...
              {isCurrentUser && (
                <span className="text-xs text-muted-foreground">{t('presence.you')}</span>
              )}
            </div>
            <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
              <span className={cn('h-2 w-2 rounded-full', STATUS_COLORS[user.status])} />
              {statusLabel}
            </div>
...
export function PresenceIndicator() {
  ...
  const showPresenceIndicator = useCollaborationStore((s) => s.settings.showPresenceIndicator);
