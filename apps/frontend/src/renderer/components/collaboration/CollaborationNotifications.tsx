		...
		for (const event of newEvents) {
			...
			if (notification) {
				toast({
					...
					variant: notification.variant as
						| "default"
						| "destructive"
						| undefined,
				});
	...
	switch (event.eventType) {
		case "user_joined":
			...
			return {
				title:
					"👤 " +
					t("notifications.userJoined", {
						user: data.display_name || data.user_id,
					}),
				description: "",
				variant: "default",
			};
		...
		case "task_unlocked":
			...
			return {
				title:
					"🔓 " +
					t("notifications.taskUnlocked", {
						user: data.unlocked_by,
						task: data.task_id,
					}),
				description: "",
				variant: "default",
		...
		case "agent_completed":
			...
			return {
				title:
					(data.success === "true" ? "✅ " : "❌ ") +
					(data.success === "true"
						? t("notifications.agentCompleted", {
								agent: data.agent_type,
								task: data.task_id,
							})
						: t("notifications.agentFailed", {
								agent: data.agent_type,
								task: data.task_id,
							})),
				description: "",
