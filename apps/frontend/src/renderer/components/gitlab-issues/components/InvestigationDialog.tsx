...
export function InvestigationDialog({
	...
	useEffect(() => {
		if (open && selectedIssue && projectId) {
			...
			window.electronAPI
				.getGitLabIssueNotes(projectId, selectedIssue.iid)
			...
			<DialogContent className="max-w-2xl max-h-[80vh] flex flex-col">
				<DialogHeader>
					...
					<DialogDescription>
						{selectedIssue && (
							<span>
								{t("investigation.issuePrefix")} #{selectedIssue.iid}:{" "}
								{selectedIssue.title}
							</span>
						)}
					</DialogDescription>
							...
							<div className="space-y-2 flex-1 min-h-0 flex flex-col">
								<div className="flex items-center justify-between">
									<h4 className="text-sm font-medium flex items-center gap-2">
										...
										{t("investigation.selectNotes")} ({selectedNoteIds.length}/
										{notes.length})
									</h4>
									<Button
										variant="ghost"
									...
									>
										{selectedNoteIds.length === notes.length
											? t("investigation.deselectAll")
											: t("investigation.selectAll")}
									</Button>
								</div>
								<ScrollArea
				...
				<DialogFooter>
					...
					{investigationStatus.phase === "complete" && (
						<Button onClick={onClose}>{t("investigation.done")}</Button>
					)}
				</DialogFooter>
			</DialogContent>
		</Dialog>
