...
> = {
	...
	spec_generation: {
		icon: Layout,
		labelKey: "phases.spec_generation",
		color: "text-purple-500",
	},
	code_generation: {
		icon: Code,
		labelKey: "phases.code_generation",
		color: "text-green-500",
	},
	design_token_integration: {
		icon: Palette,
		labelKey: "phases.design_token_integration",
		color: "text-amber-500",
	},
	visual_test_generation: {
		icon: TestTube,
		labelKey: "phases.visual_test_generation",
		color: "text-cyan-500",
	},
	figma_sync: {
		icon: Palette,
		labelKey: "phases.figma_sync",
					...
					{phaseOrder.slice(0, -1).map((p, i) => {
						...
						return (
							<div key={p} className="flex items-center flex-1">
								<div
									className={`flex items-center gap-1.5 text-xs font-medium rounded-full px-2 py-1 transition-all ${phaseStatusClass}`}
								>
									{phaseIconElement}
									<span className="hidden sm:inline truncate">
										{t(config.labelKey).replace("...", "")}
									</span>
								</div>
			...
			>
				<TabsList className="grid grid-cols-5 w-full shrink-0">
					...
					<TabsTrigger
						value="code"
						className="text-xs gap-1"
						disabled={!result?.generated_files?.length}
					>
					...
					<TabsTrigger
						value="tests"
						className="text-xs gap-1"
						disabled={!result?.visual_tests?.length}
					>
					...
					<TabsTrigger
						value="tokens"
						className="text-xs gap-1"
						disabled={!result?.design_tokens_used?.length}
					>
								...
								<div className="space-y-4 pr-4">
									...
									{Object.keys(result.design_spec.color_palette).length > 0 && (
										<div>
											...
											<div className="flex flex-wrap gap-2">
												{Object.entries(result.design_spec.color_palette).map(
													([name, value]) => (
														<div
															key={name}
									...
									{Object.keys(result.design_spec.typography).length > 0 && (
										<div>
											...
											<div className="space-y-1">
												{Object.entries(result.design_spec.typography).map(
													([name, props]) => (
														<div
															key={name}
															className="flex items-center gap-3 px-3 py-1.5 rounded-lg border bg-card text-xs"
														>
											...
											{result.generated_files.map((file, i) => (
												...
												>
													...
													<div className="flex items-center gap-1.5 mt-1 pl-5">
														<Badge
															className={`text-[10px] px-1 py-0 ${LANGUAGE_COLORS[file.language] || LANGUAGE_COLORS.text}`}
														>
															{file.language}
														</Badge>
											...
											<div className="flex items-center justify-between px-3 py-2 bg-muted/50 border-b">
												...
												<div className="flex items-center gap-2">
													...
													<Button
														...
														className="h-6 px-2 text-xs"
														onClick={() =>
															handleCopyCode(test.test_code, test.name)
														}
													>
							...
							{result?.design_tokens_used?.length ? (
								<div className="space-y-3 pr-4">
									<div className="flex items-center gap-2 mb-2">
										<Palette className="h-4 w-4 text-amber-500" />
										<span className="text-sm font-semibold">
											{t("tokens.designTokensIntegrated")} (
											{result.design_tokens_used.length})
										</span>
									</div>
									...
									{Object.entries(
										result.design_tokens_used.reduce<
											Record<string, typeof result.design_tokens_used>
										>((acc, token) => {
										...
										<div key={category}>
											...
											<div className="grid grid-cols-2 gap-1.5">
												{tokens.map((token) => (
													<div
														key={token.name}
														className="flex items-center gap-2 px-2.5 py-1.5 rounded-md border bg-card text-xs"
													>
				...
				<div className="flex items-center gap-2 text-xs text-muted-foreground">
					{phase === "complete" && result && (
						<>
							...
							<span>
								{result.generated_files.length} {t("footer.files")} •{" "}
								{result.visual_tests.length} {t("footer.tests")} •{" "}
