								...
								<TooltipContent className="max-w-md">
									<div className="space-y-2">
										<div className="font-semibold">
											{tRoadmap("header.clickToViewDetailedAnalysis")}
										</div>
										<div className="text-sm text-muted-foreground">
											{tRoadmap("header.analyzedCompetitors", {
												count: competitorAnalysis.competitors.length,
												painPoints: competitorAnalysis.competitors.reduce(
													(sum, c) => sum + c.painPoints.length,
													0,
												),
											})}
										</div>
					...
					<Tooltip>
						<TooltipTrigger asChild>
							<Button
								...
								size="icon"
								onClick={onRefresh}
								aria-label={tCommon("accessibility.regenerateRoadmapAriaLabel")}
							>
								<RefreshCw className="h-4 w-4" />
							</Button>
						</TooltipTrigger>
						<TooltipContent>
							{tRoadmap("header.regenerateRoadmap")}
						</TooltipContent>
					</Tooltip>
				</div>
			</div>
							...
							<TooltipContent className="max-w-md">
								<div className="space-y-1">
									<div className="font-semibold mb-2">
										{tRoadmap("header.secondaryPersonas")}
									</div>
