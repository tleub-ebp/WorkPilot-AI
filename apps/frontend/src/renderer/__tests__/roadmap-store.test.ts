...
describe("Roadmap Store", () => {
	...
	describe("reorderFeatures", () => {
		it("should reorder features within a phase", () => {
			...
			useRoadmapStore
				.getState()
				.reorderFeatures("phase-1", ["feature-3", "feature-1", "feature-2"]);

			const state = useRoadmapStore.getState();
			const phase1Features =
				state.roadmap?.features.filter((f) => f.phaseId === "phase-1") || [];
		...
		it("should not affect features in other phases", () => {
			...
			useRoadmapStore
				.getState()
				.reorderFeatures("phase-1", ["feature-2", "feature-1"]);

			const state = useRoadmapStore.getState();
			const phase2Features =
				state.roadmap?.features.filter((f) => f.phaseId === "phase-2") || [];
		...
		it("should update updatedAt timestamp", () => {
			...
			const state = useRoadmapStore.getState();
			expect(state.roadmap?.updatedAt.getTime()).toBeGreaterThan(
		...
		it("should handle non-existent feature IDs gracefully", () => {
			...
			useRoadmapStore
				.getState()
				.reorderFeatures("phase-1", ["feature-2", "nonexistent", "feature-1"]);
		...
		it("should do nothing if roadmap is null", () => {
			...
			useRoadmapStore
				.getState()
				.reorderFeatures("phase-1", ["feature-1", "feature-2"]);
	...
	describe("updateFeaturePhase", () => {
		it("should move feature to a different phase", () => {
			...
			const state = useRoadmapStore.getState();
			const movedFeature = state.roadmap?.features.find(
				(f) => f.id === "feature-1",
			);
		...
		it("should not affect other features", () => {
			...
			expect(
				state.roadmap?.features.find((f) => f.id === "feature-2")?.phaseId,
			).toBe("phase-1");
			expect(
				state.roadmap?.features.find((f) => f.id === "feature-3")?.phaseId,
			).toBe("phase-2");
		});
		...
		it("should handle moving feature to same phase (no change needed)", () => {
			...
			const state = useRoadmapStore.getState();
			expect(
				state.roadmap?.features.find((f) => f.id === "feature-1")?.phaseId,
			).toBe("phase-1");
		});
	});
	...
	describe("updateFeatureStatus", () => {
		it("should update feature status by id", () => {
			const features = [
				createTestFeature({ id: "feature-1", status: "under_review" }),
			];
			const roadmap = createTestRoadmap({ features });
			...
			useRoadmapStore
				.getState()
				.updateFeatureStatus("feature-1", "in_progress");
	...
	describe("updateFeatureLinkedSpec", () => {
		it("should update linked spec and set status to in_progress", () => {
			...
			useRoadmapStore
				.getState()
				.updateFeatureLinkedSpec("feature-1", "spec-abc");
