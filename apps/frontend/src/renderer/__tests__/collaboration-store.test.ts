...
describe('Collaboration Store — User Management', () => {
  ...
  it('should remove user (set offline)', () => {
    ...
    const user = useCollaborationStore.getState().users.find((u) => u.userId === 'user-2');
  ...
  it('should update user status', () => {
    ...
    const user = useCollaborationStore.getState().users.find((u) => u.userId === 'user-1');
...
describe('Collaboration Store — Conflicts', () => {
  ...
  it('should get unresolved conflicts', () => {
    ...
    addConflict({
      ...
      fieldName: 'a', valueA: 1, valueB: 2, resolution: 'manual',
      resolved: false, resolvedValue: null, timestamp: '',
    });
    addConflict({
      conflictId: 'cfl-2', taskId: 't-2', userA: 'u-1', userB: 'u-2',
      fieldName: 'b', valueA: 3, valueB: 4, resolution: 'manual',
      resolved: false, resolvedValue: null, timestamp: '',
    });
    resolveConflict('cfl-1', 'resolved');

    expect(useCollaborationStore.getState().getUnresolvedConflicts()).toHaveLength(1);
  });
...
describe('Collaboration Store — Events', () => {
  ...
  it('should keep last 100 events', () => {
    ...
    expect(useCollaborationStore.getState().events.length).toBeLessThanOrEqual(110);
  });
...
describe('Collaboration Store — Computed & Stats', () => {
  ...
  it('should get online users only', () => {
    ...
    addUser({
      ...
      status: 'online', currentTask: '', connectedAt: '', lastActivity: '',
    });
    addUser({
      userId: 'user-3', displayName: 'Charlie', role: 'developer',
      status: 'online', currentTask: '', connectedAt: '', lastActivity: '',
    });
    removeUser('user-3');
  ...
  it('should calculate stats correctly', () => {
    const { addUser, lockTask, sendMessage, addConflict } = useCollaborationStore.getState();
    addUser({
      userId: 'user-2', displayName: 'Bob', role: 'developer',
      status: 'online', currentTask: '', connectedAt: '', lastActivity: '',
    });
  ...
  it('should handle reply-to workflow', () => {
    ...
    setReplyingTo(msg);
    expect(useCollaborationStore.getState().replyingTo?.messageId).toBe(msg.messageId);
