...
class BufferPersistence {
	...
	register(terminalId: string, xterm: Terminal): SerializeAddon {
		...
		const saveInterval = setInterval(() => {
			this.saveBuffer(terminalId).catch((error) => {
				console.warn(
					`[BufferPersistence] Auto-save failed for ${terminalId}:`,
					error,
				);
			});
		}, SAVE_INTERVAL_MS);
	...
	async saveNow(terminalId: string): Promise<void> {
		...
		try {
			...
			console.warn(
				`[BufferPersistence] Immediate save for ${terminalId} complete`,
			);
		} catch (error) {
			console.error(
				`[BufferPersistence] Failed to immediately save ${terminalId}:`,
				error,
			);
			throw error;
		}
	...
	async saveAll(): Promise<void> {
		console.warn(
			`[BufferPersistence] Saving all buffers (${this.terminals.size} terminals)`,
		);
...
window.addEventListener("beforeunload", () => {
	...
	bufferPersistence.saveAll().catch((error) => {
		console.error(
			"[BufferPersistence] Failed to save all buffers on unload:",
			error,
		);
	});
});
