...
import { getMaxWebGLContexts, isSafari, supportsWebGL2 } from "./webgl-utils";
...
class WebGLContextManager {
	...
	acquire(terminalId: string): boolean {
		...
		if (!xterm) {
			console.warn(
				`[WebGLContextManager] Terminal ${terminalId} not registered`,
			);
			return false;
		}
		...
		try {
			...
			addon.onContextLoss(() => {
				console.warn(
					`[WebGLContextManager] Context lost for terminal ${terminalId}`,
				);
			...
			console.warn(
				`[WebGLContextManager] Failed to acquire context for ${terminalId}:`,
				error,
			);
	...
	release(terminalId: string): void {
		...
		try {
			...
			console.warn(
				`[WebGLContextManager] Error disposing context for ${terminalId}:`,
				error,
			);
...
if (typeof window !== "undefined") {
	(
		window as Window & { __webglContextManager?: WebGLContextManager }
	).__webglContextManager = webglContextManager;
}
