...
class TerminalBufferManager {
	...
	append(id: string, data: string): void {
		...
		const truncated =
			combined.length > this.MAX_BUFFER_SIZE
		...
		if (disposables) {
			for (const disposable of disposables) {
				try {
					...
					console.warn(
						`[TerminalBufferManager] Error disposing resource for ${id}:`,
						e,
					);
				}
			}
...
if (typeof window !== "undefined") {
	(
		window as Window & { __terminalBufferManager?: TerminalBufferManager }
	).__terminalBufferManager = terminalBufferManager;
}
