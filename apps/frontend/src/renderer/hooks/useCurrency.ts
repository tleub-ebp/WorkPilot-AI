 ...
 */
function getCurrencyForLocale(lang: string): { code: string; symbol: string; needsConversion: boolean } {
  switch (lang) {
    case 'fr':
...
export function useCurrency() {
  ...
  const [rate, setRate] = useState<number>(sharedCache?.rate ?? (currency.needsConversion ? FALLBACK_EUR_RATE : 1));
