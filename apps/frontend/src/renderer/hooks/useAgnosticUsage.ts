 ...
 */
export function useAgnosticUsage(
	selectedProvider?: string,
): UseAgnosticUsageReturn {
	const { t } = useTranslation(["common"]);
	const { selectedProvider: contextProvider } = useProviderContext();
	const provider =
		selectedProvider ||
		contextProvider ||
		t("agnosticUsage.providerGeneric", "generic");
	 ...
	 */
	const convertLegacyUsageData = useCallback(
		(legacyData: UsageData): AgnosticUsageData | null => {
			try {
				...
				const snapshot: UsageSnapshot = {
					...
					sessionResetTimestamp:
						legacyData.usage.sessionResetTimestamp?.toString(),
					weeklyResetTimestamp:
						legacyData.usage.weeklyResetTimestamp?.toString(),
			...
			try {
				...
				if (!supportsAuthMethod(providerName, type)) {
					const usageError: UsageError = {
						...
						message: t(
							"agnosticUsage.unsupportedAuthMethod",
							"Provider {{provider}} does not support {{authMethod}} authentication",
							{ provider: providerName, authMethod: type },
						),
				...
				await credentialService.setActiveProvider(
					providerName,
					type,
					profileId,
				);
				...
				if (providerName === provider) {
					const legacyUsage =
						await credentialService.getUsageData(providerName);
					if (legacyUsage) {
						const agnosticUsage = convertLegacyUsageData(legacyUsage);
						if (agnosticUsage) {
							setUsageData(agnosticUsage);
				...
				const usageError: UsageError = {
					...
					message: t(
						"agnosticUsage.setProviderFailed",
	 ...
	 * Refresh usage data
	 */
	const handleRefreshUsageData = useCallback(
		async (providerName?: string) => {
			try {
				...
				if (targetProvider && targetProvider !== "generic") {
					const legacyUsage =
						await credentialService.getUsageData(targetProvider);
					if (legacyUsage) {
						const agnosticUsage = convertLegacyUsageData(legacyUsage);
						if (agnosticUsage) {
							setUsageData(agnosticUsage);
	...
	const handleValidateCredentials = useCallback(async (): Promise<boolean> => {
		try {
			...
			const usageError: UsageError = {
				...
				message: t(
					"agnosticUsage.validationFailed",
	 ...
	 */
	const handleGetColorClass = useCallback(
		(type: "text" | "badge" | "gradient" = "text") => {
			if (!usageData)
				return type === "badge"
					? getBadgeColorClasses(0)
					: getUsageColorClass(0);
	...
	useEffect(() => {
		...
		loadInitialData,
		provider,
		convertLegacyUsageData,
		handleRefreshUsageData,
	]);

	// Computed values
	const needsReauth = usageData?.needsReauthentication || false;
	const isRateLimited = usageData?.isRateLimited || false;
	const limitingPercent = usageData
		? Math.max(
				usageData.metrics.sessionPercent,
				usageData.metrics.periodicPercent,
			)
		: 0;
