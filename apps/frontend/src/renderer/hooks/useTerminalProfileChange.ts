...
export function useTerminalProfileChange(): void {
	...
	const recreateTerminal = useCallback(
		async (
			...
			if (recreatingTerminals.current.has(terminalId)) {
				debugLog(
					"[useTerminalProfileChange] Terminal already being recreated:",
					terminalId,
				);
				return;
			}
			...
			try {
				...
				if (!terminal) {
					debugLog(
						"[useTerminalProfileChange] Terminal not found in store:",
						terminalId,
					);
					return;
				}
				...
				const newTerminal = store.addTerminal(
					terminalState.cwd,
				...
				if (!newTerminal) {
					debugError(
						"[useTerminalProfileChange] Failed to create new terminal",
					);
					return;
				}
				...
				if (terminalState.worktreeConfig) {
					window.electronAPI.setTerminalWorktreeConfig(
						newTerminal.id,
						terminalState.worktreeConfig,
					);
				}

				if (!createResult.success) {
					debugError(
						"[useTerminalProfileChange] Failed to create PTY:",
						createResult.error,
					);
				...
				if (sessionId && sessionMigrated) {
					debugLog(
						"[useTerminalProfileChange] Session migrated, ready for resume:",
						sessionId,
					);
	...
	useEffect(() => {
		const cleanup = window.electronAPI.onTerminalProfileChanged(
			async (event: TerminalProfileChangedEvent) => {
