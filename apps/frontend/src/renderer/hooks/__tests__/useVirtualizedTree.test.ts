...
describe("flattenTree", () => {
	...
	describe("expansion state", () => {
		...
		it("should not mark files as expanded even if in expanded set", () => {
			const fileNode = createTestFileNode({
				path: "/root/file.ts",
				name: "file.ts",
			});
	...
	describe("nested tree flattening", () => {
		it("should include children of expanded directories", () => {
			const dirNode = createTestDirNode({ path: "/root/dir", name: "dir" });
			const childFile = createTestFileNode({
				path: "/root/dir/child.ts",
				name: "child.ts",
			});
		...
		it("should handle deeply nested structures", () => {
			...
			const file = createTestFileNode({
				path: "/root/dir1/dir2/file.ts",
				name: "file.ts",
			});
		...
		it("should handle multiple directories at same level", () => {
			...
			const file1 = createTestFileNode({
				path: "/root/dir1/file1.ts",
				name: "file1.ts",
			});
			const file2 = createTestFileNode({
				path: "/root/dir2/file2.ts",
				name: "file2.ts",
			});
	...
	describe("depth calculation", () => {
		...
		it("should increment depth for nested children", () => {
			...
			const child = createTestFileNode({
				path: "/root/dir/child.ts",
				name: "child.ts",
			});
...
describe("useVirtualizedTree", () => {
	...
	describe("handleToggle", () => {
		...
		it("should toggle directory expansion state", () => {
			...
			expect(
				useFileExplorerStore.getState().expandedFolders.has(`${ROOT_PATH}/dir`),
			).toBe(true);
	...
	describe("memoization", () => {
		it("should return same reference when dependencies unchanged", () => {
			const files = [
				createTestFileNode({ path: `${ROOT_PATH}/file.ts`, name: "file.ts" }),
			];
	...
	describe("complex scenarios", () => {
		it("should handle mixed files and directories", () => {
			const dir1 = createTestDirNode({
				path: `${ROOT_PATH}/dir1`,
				name: "dir1",
			});
			const file1 = createTestFileNode({
				path: `${ROOT_PATH}/file1.ts`,
				name: "file1.ts",
			});
			const dir2 = createTestDirNode({
				path: `${ROOT_PATH}/dir2`,
				name: "dir2",
			});
			const file2 = createTestFileNode({
				path: `${ROOT_PATH}/file2.ts`,
				name: "file2.ts",
			});
		...
		it("should handle partial expansion", () => {
			...
			const child1 = createTestFileNode({
				path: `${ROOT_PATH}/dir1/child1.ts`,
				name: "child1.ts",
			});
			const child2 = createTestFileNode({
				path: `${ROOT_PATH}/dir2/child2.ts`,
				name: "child2.ts",
			});
			...
			expect(
				result.current.flattenedNodes.map((n: FlattenedNode) => n.node.name),
			).toEqual(["dir1", "child1.ts", "dir2"]);
		});
