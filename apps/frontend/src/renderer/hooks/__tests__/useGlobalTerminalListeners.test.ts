...
describe("useGlobalTerminalListeners", () => {
	...
	beforeEach(() => {
		...
		(
			window as unknown as {
				electronAPI: { onTerminalOutput: typeof mockOnTerminalOutput };
			}
		).electronAPI = {
	...
	describe("listener registration", () => {
		it("should register global terminal output listener on mount", async () => {
			...
			const { useGlobalTerminalListeners: freshHook } = await import(
	...
	describe("cleanup", () => {
		...
		it("should allow re-registration after cleanup", async () => {
			...
			const { useGlobalTerminalListeners: freshHook2 } = await import(
				"../useGlobalTerminalListeners"
			);
