...
export interface TerminalAPI {
	...
	) => Promise<IPCResult<import("../../shared/types").ClaudeProfile | null>>;
	onSDKRateLimit: (
	...
	requestUsageUpdate: (
		providerName?: string,
	) => Promise<IPCResult<import("../../shared/types").UsageSnapshot | null>>;
	requestAllProfilesUsage: (
		forceRefresh?: boolean,
	) => Promise<IPCResult<import("../../shared/types").AllProfilesUsage | null>>;
	onUsageUpdated: (
		callback: (usage: import("../../shared/types").UsageSnapshot) => void,
	) => () => void;
...
export const createTerminalAPI = (): TerminalAPI => ({
	...
	onTerminalOAuthCodeNeeded: (
		...
		return () => {
			ipcRenderer.removeListener(
				IPC_CHANNELS.TERMINAL_OAUTH_CODE_NEEDED,
				handler,
			);
		};
	},
	...
	onTerminalOnboardingComplete: (
		...
		return () => {
			ipcRenderer.removeListener(
				IPC_CHANNELS.TERMINAL_ONBOARDING_COMPLETE,
				handler,
			);
		};
	},
	...
	onTerminalProfileChanged: (
		...
		return () => {
			ipcRenderer.removeListener(
				IPC_CHANNELS.TERMINAL_PROFILE_CHANGED,
				handler,
			);
		};
	},
	...
	onUsageUpdated: (
		...
		const handler = (
			_event: Electron.IpcRendererEvent,
			usage: import("../../shared/types").UsageSnapshot,
		): void => {
			callback(usage);
		};
