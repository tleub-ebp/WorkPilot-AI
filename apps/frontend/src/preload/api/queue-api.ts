...
export interface QueueAPI {
	...
	getBestProfileForTask: (
		options?: GetBestProfileOptions,
	) => Promise<IPCResult<BestProfileResult | null>>;
	...
	onQueueProfileSwapped: (
		callback: (event: QueueProfileSwapEvent) => void,
	) => () => void;
	...
	onQueueSessionCaptured: (
		callback: (event: QueueSessionCapturedEvent) => void,
	) => () => void;
	...
	onQueueBlockedNoProfiles: (
		callback: (info: { reason: string; timestamp: string }) => void,
	) => () => void;
}
...
export const createQueueAPI = (): QueueAPI => ({
	...
	onQueueProfileSwapped: (
		...
		const handler = (
			...
			data: QueueProfileSwapEvent,
		) => callback(data);
		ipcRenderer.on(IPC_CHANNELS.QUEUE_PROFILE_SWAPPED, handler);
		return () =>
			ipcRenderer.removeListener(IPC_CHANNELS.QUEUE_PROFILE_SWAPPED, handler);
	},
	...
	onQueueSessionCaptured: (
		...
		const handler = (
			_event: Electron.IpcRendererEvent,
			data: QueueSessionCapturedEvent,
		) => callback(data);
		ipcRenderer.on(IPC_CHANNELS.QUEUE_SESSION_CAPTURED, handler);
		return () =>
			ipcRenderer.removeListener(IPC_CHANNELS.QUEUE_SESSION_CAPTURED, handler);
	},
