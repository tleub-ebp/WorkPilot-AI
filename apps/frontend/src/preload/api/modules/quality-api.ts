...
export interface QualityAPI {
   ...
   * @returns Quality score and issues
   */
  analyzePRQuality(prNumber: number, changedFiles: string[], projectDir?: string): Promise<QualityScore>;
}
...
export const createQualityAPI = (): QualityAPI => ({
  ...
  analyzePRQuality: (prNumber: number, changedFiles: string[], projectDir?: string) =>
    ipcRenderer.invoke('quality:analyze-pr', { prNumber, changedFiles, projectDir }),
});
