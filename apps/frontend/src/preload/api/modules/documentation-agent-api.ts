...
export interface DocumentationAgentAPI {
  generateDocumentation: (request: DocumentationAgentRequest) => Promise<{ success: boolean; error?: string }>;
  cancelDocumentation: () => Promise<{ success: boolean; cancelled: boolean; error?: string }>;
  configureDocumentationAgent: (config: { pythonPath?: string }) => Promise<{ success: boolean; error?: string }>;
  onDocumentationAgentStatus: (callback: (status: string) => void) => () => void;
  onDocumentationAgentStreamChunk: (callback: (chunk: string) => void) => () => void;
  onDocumentationAgentError: (callback: (error: string) => void) => () => void;
  onDocumentationAgentComplete: (callback: (result: DocumentationAgentResult) => void) => () => void;
}
