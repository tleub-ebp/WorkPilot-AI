...
import { invokeIpc, sendIpc, createIpcListener, type IpcListenerCleanup } from './ipc-utils';
...
export interface AiAction {
  id: string;
  actionType: 'file_created' | 'file_modified' | 'file_deleted' | 'command_run' | 'thinking';
  description: string;
  filePath?: string;
  timestamp: string;
...
export interface PairStreamChunk {
  type: 'stream' | 'status' | 'action' | 'question' | 'conflict' | 'done' | 'error';
  content?: string;
  status?: string;
  message?: string;
  actionType?: string;
...
export interface PairProgrammingAPI {
  ...
  startPairSession: (params: StartSessionParams) => Promise<IPCResult<PairSession>>;
  ...
  sendPairMessage: (projectId: string, sessionId: string, message: string) => void;
...
export function createPairProgrammingAPI(): PairProgrammingAPI {
  return {
    startPairSession: (params) =>
      invokeIpc<IPCResult<PairSession>>(IPC_CHANNELS.PAIR_PROGRAMMING_START, params),

    stopPairSession: (projectId, sessionId) =>
      invokeIpc<IPCResult>(IPC_CHANNELS.PAIR_PROGRAMMING_STOP, projectId, sessionId),

    sendPairMessage: (projectId, sessionId, message) =>
      sendIpc(IPC_CHANNELS.PAIR_PROGRAMMING_SEND_MESSAGE, projectId, sessionId, message),

    getPairSession: (projectId) =>
      invokeIpc<IPCResult<PairSession | null>>(IPC_CHANNELS.PAIR_PROGRAMMING_GET_SESSION, projectId),

    onPairStreamChunk: (callback) =>
      createIpcListener<[string, PairStreamChunk]>(IPC_CHANNELS.PAIR_PROGRAMMING_STREAM_CHUNK, callback),

    onPairStatus: (callback) =>
      createIpcListener<[string, string, string]>(IPC_CHANNELS.PAIR_PROGRAMMING_STATUS, callback),

    onPairAiAction: (callback) =>
      createIpcListener<[string, AiAction]>(IPC_CHANNELS.PAIR_PROGRAMMING_AI_ACTION, callback),

    onPairConflict: (callback) =>
      createIpcListener<[string, string, string]>(IPC_CHANNELS.PAIR_PROGRAMMING_CONFLICT, callback),

    onPairError: (callback) =>
      createIpcListener<[string, string]>(IPC_CHANNELS.PAIR_PROGRAMMING_ERROR, callback),

    onPairComplete: (callback) =>
      createIpcListener<[string, string]>(IPC_CHANNELS.PAIR_PROGRAMMING_COMPLETE, callback),
  };
}
