...
export interface AppEmulatorAPI {
	...
	detectAppProject: (
		projectDir: string,
	) => Promise<{ success: boolean; data?: AppEmulatorConfig; error?: string }>;
	 ...
	 */
	getAppEmulatorStatus: () => Promise<{
		success: boolean;
		data?: { running: boolean; url?: string; config?: AppEmulatorConfig };
	}>;
	 ...
	 */
	onAppEmulatorConfig: (
		callback: (config: AppEmulatorConfig) => void,
	) => () => void;
}
...
export function createAppEmulatorAPI(): AppEmulatorAPI {
	return {
		...
		onAppEmulatorStatus: (callback: (status: string) => void) => {
			...
			return () =>
				ipcRenderer.removeListener("app-emulator:status-update", handler);
		},
