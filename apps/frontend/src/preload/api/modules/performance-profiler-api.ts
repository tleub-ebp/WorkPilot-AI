...
export interface PerformanceProfilerAPI {
  startPerformanceProfiling: (request: PerformanceProfilerRequest) => Promise<{ success: boolean; error?: string }>;
  cancelPerformanceProfiling: () => Promise<{ success: boolean; cancelled: boolean; error?: string }>;
  configurePerformanceProfiler: (config: { pythonPath?: string }) => Promise<{ success: boolean; error?: string }>;
  onPerformanceProfilerStatus: (callback: (status: string) => void) => () => void;
  onPerformanceProfilerStreamChunk: (callback: (chunk: string) => void) => () => void;
  onPerformanceProfilerError: (callback: (error: string) => void) => () => void;
  onPerformanceProfilerComplete: (callback: (result: PerformanceProfilerResult) => void) => () => void;
  onPerformanceProfilerImplementationComplete: (callback: (result: unknown) => void) => () => void;
}
...
export function createPerformanceProfilerAPI(): PerformanceProfilerAPI {
  return {
    ...
    onPerformanceProfilerImplementationComplete: (callback) =>
      globalThis.electronAPI.on('performanceProfiler:implementationComplete', callback),
  };
}
