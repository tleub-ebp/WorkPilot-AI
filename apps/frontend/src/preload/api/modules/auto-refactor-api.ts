...
export interface AutoRefactorAPI {
  // Methods
  startAutoRefactor: (request: AutoRefactorRequest) => Promise<{ success: boolean; error?: string }>;
  cancelAutoRefactor: () => Promise<{ success: boolean; cancelled: boolean; error?: string }>;
  configureAutoRefactor: (config: { pythonPath?: string; autoBuildSourcePath?: string }) => Promise<{ success: boolean; error?: string }>;
...
export function createAutoRefactorAPI(): AutoRefactorAPI {
  return {
    ...
    cancelAutoRefactor: () => 
      window.electronAPI.invoke('auto-refactor:cancel'),
    
    configureAutoRefactor: (config: { pythonPath?: string; autoBuildSourcePath?: string }) => 
