...
export interface ArchitectureVisualizerResult {
  ...
  diagrams: Record<string, { title: string; mermaid_code: string; nodes?: unknown[]; edges?: unknown[] }>;
  output_dir: string;
  summary: { total_diagrams: number; total_nodes: number; total_edges: number };
}

export interface ArchitectureVisualizerAPI {
  generateArchitectureDiagrams: (request: ArchitectureVisualizerRequest) => Promise<{ success: boolean; error?: string }>;
  cancelArchitectureVisualization: () => Promise<{ success: boolean; cancelled: boolean; error?: string }>;
  configureArchitectureVisualizer: (config: { pythonPath?: string }) => Promise<{ success: boolean; error?: string }>;
  onArchitectureVisualizerStatus: (callback: (status: string) => void) => () => void;
  onArchitectureVisualizerStreamChunk: (callback: (chunk: string) => void) => () => void;
  onArchitectureVisualizerError: (callback: (error: string) => void) => () => void;
  onArchitectureVisualizerComplete: (callback: (result: ArchitectureVisualizerResult) => void) => () => void;
}
