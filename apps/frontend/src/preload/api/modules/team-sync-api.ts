...
export interface TeamSyncAPI {
  teamSyncGetStatus: (projectDir: string) => Promise<{ success: boolean; data?: TeamSyncStatus; error?: string }>;
  teamSyncPush: (projectDir: string) => Promise<{ success: boolean; data?: unknown; error?: string }>;
  teamSyncPull: (projectDir: string) => Promise<{ success: boolean; data?: unknown; error?: string }>;
  teamSyncListPeers: (projectDir: string) => Promise<{ success: boolean; data?: TeamSyncPeer[]; error?: string }>;
  teamSyncGetPeerEpisodes: (projectDir: string, memberId: string) => Promise<{ success: boolean; data?: TeamSyncEpisode[]; error?: string }>;
  teamSyncConfigure: (projectDir: string, config: TeamSyncConfig) => Promise<{ success: boolean; error?: string }>;
  teamSyncStartServer: (projectDir: string, port?: number) => Promise<{ success: boolean; port?: number; error?: string }>;
  teamSyncStopServer: () => Promise<{ success: boolean }>;
  onTeamSyncServerStatus: (callback: (status: { running: boolean; port: number }) => void) => () => void;
  onTeamSyncProgress: (callback: (progress: { step: string; message: string }) => void) => () => void;
}
...
export function createTeamSyncAPI(): TeamSyncAPI {
  return {
    ...
    teamSyncStopServer: () =>
      invokeIpc(IPC_CHANNELS.TEAM_SYNC_STOP_SERVER),
