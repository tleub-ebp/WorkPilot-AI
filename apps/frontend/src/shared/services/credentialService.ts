...
class CredentialService {
  ...
  private initializeEventListeners(): void {
    ...
    window.electronAPI.on('provider:switched', (data: { provider: string; type: 'oauth' | 'api_key' }) => {
  ...
  async getActiveCredential(): Promise<CredentialConfig | null> {
    ...
    try {
      ...
      console.error('[CredentialService] Failed to get active credential:', error);
      return null;
    }
  }
  ...
  async setActiveProvider(provider: string, type: 'oauth' | 'api_key', profileId?: string): Promise<void> {
    ...
    try {
      await window.electronAPI.invoke('credential:setActive', provider, type, profileId);
    } catch (error) {
      console.error('[CredentialService] Failed to set active provider:', error);
      throw error;
    }
  }
   ...
   */
  async updateUsageData(provider: string, usageData: Partial<UsageData>): Promise<void> {
    if (!window.electronAPI?.invoke) {
      console.warn('[CredentialService] Electron API not available');
      return;
    }
  ...
  async validateCredentials(): Promise<boolean> {
    ...
    try {
      ...
      console.error('[CredentialService] Failed to validate credentials:', error);
      return false;
    }
  }
  ...
  async testProvider(provider: string): Promise<{ success: boolean; message: string; details?: any }> {
    ...
    try {
      return await window.electronAPI.invoke('credential:testProvider', provider);
    } catch (error) {
    ...
    if (listeners) {
      listeners.forEach(callback => {
        try {
          ...
          console.error(`[CredentialService] Error in event listener for ${event}:`, error);
        }
      });
    }
  }
