...
import {
	...
	type BaseUsageMetrics,
	getProviderConfig,
	type ProviderSpecificDetails,
	type UsageError,
 ...
 */
export function convertToAgnosticUsage(
 ...
 */
function getActionTypeForError(
	errorCode: string,
): "reauth" | "permission" | "configuration" | "retry" {
	if (
		errorCode.includes("INSUFFICIENT_PERMISSIONS") ||
		errorCode.includes("admin:org")
	) {
		return "permission";
	}
	if (
		errorCode.includes("NEEDS_REAUTHENTICATION") ||
		errorCode.includes("AUTH_FAILED")
	) {
		return "reauth";
	}
	if (
		errorCode.includes("BACKEND_UNAVAILABLE") ||
