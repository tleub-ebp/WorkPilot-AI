...
export async function getStaticProviders(
	...
	for (const p of allProviders) {
		...
		const hasProfile = profiles.some((prof) => {
			...
			if (p.name === "anthropic") {
				return (
					prof.baseUrl?.includes("anthropic.com") ||
					prof.name?.toLowerCase().includes("claude")
				);
			}
			if (p.name === "openai") {
				return (
					prof.baseUrl?.includes("openai.com") ||
					prof.name?.toLowerCase().includes("openai")
				);
			}
			if (p.name === "google") {
				return (
					prof.baseUrl?.includes("google.com") ||
					prof.name?.toLowerCase().includes("gemini")
				);
			}
			if (p.name === "meta") {
				return (
					prof.baseUrl?.includes("meta.com") ||
					prof.name?.toLowerCase().includes("llama")
				);
			}
			if (p.name === "mistral") {
				return (
					prof.baseUrl?.includes("mistral.ai") ||
					prof.name?.toLowerCase().includes("mistral")
				);
			}
			if (p.name === "deepseek") {
				return (
					prof.baseUrl?.includes("deepseek.com") ||
					prof.name?.toLowerCase().includes("deepseek")
				);
			}
			if (p.name === "windsurf") {
				return (
					prof.baseUrl?.includes("codeium.com") ||
					prof.baseUrl?.includes("windsurf.com") ||
					prof.baseUrl?.includes("windsurf.ai") ||
					prof.name?.toLowerCase().includes("windsurf")
				);
			}
