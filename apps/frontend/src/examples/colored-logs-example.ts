...
frontendInfoLog('[UsageMonitor:PROVIDER_DETECTION] Detected provider: anthropic');
frontendSuccessLog('[UsageMonitor:FETCH] Successfully fetched usage data');
frontendWarningLog('[UsageMonitor:CLI] CLI fallback - attempting to use claude usage command');
frontendErrorLog('[UsageMonitor:API] API request failed: 429 Too Many Requests');

// Example with nested scopes
frontendDebugLog('[UsageMonitor:PROVIDER_DETECTION:OAUTH] Checking OAuth token validity');
frontendInfoLog('[UsageMonitor:PROVIDER_DETECTION:API_KEY] Using API key authentication');
