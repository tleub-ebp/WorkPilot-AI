...
For more information, see the documentation.
    ...
    async quickDev() {
        ...
        console.log('💡 Use /bmad-net-architect or /bmad-performance-analyst to start');
    }
    ...
    async status() {
        ...
        try {
            ...
            if (existsSync(this.agentsPath)) {
                ...
                const hybridAgents = agents.filter(agent => agent.startsWith('bmad-'));
                console.log(`✅ Agents available: ${agents.length} total, ${hybridAgents.length} hybrid`);
                
                if (hybridAgents.length > 0) {
                    console.log('   Hybrid agents:');
                    hybridAgents.forEach(agent => console.log(`     - ${agent.replace('.md', '')}`));
                }
            } else {
