...
function validateRelease(version) {
	...
	try {
		...
		if (
			remoteBranches.includes(`origin/${version}`) ||
			remoteBranches.includes(`fork/${version}`)
		) {
			console.error(`\u274C Remote branch "${version}" already exists!`);
			console.error("   This will cause HTTP 300 errors during updates.");
			console.error(
				`   Please delete the remote branch: git push origin --delete ${version}`,
			);
			process.exit(1);
		}
	} catch (error) {
		...
		console.warn(
			"\u26A0\uFE0F  Could not check remote branches:",
			error.message,
		);
	}
