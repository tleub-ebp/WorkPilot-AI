...
console.log("Installing WorkPilot AI backend dependencies...\n");
		...
		try {
			...
			if (result.status === 0) {
				...
				if (versionMatch) {
					...
					if (major === 3 && minor >= 12) {
						console.log(
							`Found Python 3.12+: ${cmd} -> ${result.stdout.trim()}`,
						);
						return cmd;
					}
				}
...
async function main() {
	...
	if (fs.existsSync(envPath)) {
		...
		console.warn(
			"Please create a .env file manually if your configuration requires it.",
		);
	}
