...
class Validator {
  ...
  run() {
    ...
    try {
      ...
      this.check('git configured', false, 'Git not available or not configured');
    }
    ...
    executableScripts.forEach(script => {
      ...
      if (fs.existsSync(fullPath)) {
        try {
          ...
          this.check(`${script} is executable`, isExecutable, 'Run: chmod +x ' + script);
        } catch (e) {
    ...
    if (this.failed === 0) {
      this.log('✓ All checks passed! Upstream sync is properly configured.', 'success');
      this.log('Run: pnpm merge-upstream', 'info');
    } else {
      this.log(`✗ ${this.failed} check(s) failed. Please review the errors above.`, 'error');
    }
