...
def main():
    ...
    if project == "MeCa Web":
        ...
        print(f"   Type: Repository (incorrect)")
        print()
    ...
    if extracted_project:
        ...
        match = re.search(r'https://dev\.azure\.com/([^/]+)/([^/]+)/_git/(.+)', repo_url)
        if match:
            org = match.group(1)
        ...
        print("-" * 70)
        response = input(f"Définir AZURE_DEVOPS_PROJECT='{extracted_project}' ? (o/n): ").strip().lower()
        
        if response in ['o', 'oui', 'y', 'yes']:
            os.environ["AZURE_DEVOPS_PROJECT"] = extracted_project
            print(f"✓ Variable définie pour cette session: AZURE_DEVOPS_PROJECT='{extracted_project}'")
            print()
            print("⚠️  ATTENTION: Cette modification n'est valable que pour ce terminal.")
